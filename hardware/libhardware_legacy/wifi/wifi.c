...
int ensure_config_file_exists(const char *config_file)
{
    ...
    if ((ret == 0) || (errno == EACCES)) {
		...
		if(!strcmp(SUPP_CONFIG_FILE,config_file))
		{
			tmp = TEMP_FAILURE_RETRY(open(config_file, O_RDONLY));
			if (tmp < 0) {
	        	ALOGE("Cannot open \"%s\": %s", config_file, strerror(errno));
	        	return -1;
			}
			while ((nread = TEMP_FAILURE_RETRY(read(tmp, buf, sizeof(buf)))) != 0) {
		        if (nread < 0) {
		            ALOGE("Error reading \"%s\": %s", config_file, strerror(errno));
		            close(tmp);
		            return -1;
		        }

				if(strstr(buf,"update_config=1")) {
					checkfile++;
				}

				if(strstr(buf,"ctrl_interface=wlan0")) {
					checkfile++;
				}
				ALOGE("checkfile = %d",checkfile);
				if(checkfile < 2) {
					close(tmp);
					unlink(config_file);
					goto creat_file;
