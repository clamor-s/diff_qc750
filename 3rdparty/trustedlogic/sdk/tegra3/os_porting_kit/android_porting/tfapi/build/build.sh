...
if [ -z "$BUILD_VARIANT" ] || [ "$BUILD_VARIANT" == "release" ]; then
   mkdir release
   cp $ANDROID_PRODUCT/system/lib/libsmapi.so release
elif [ "$BUILD_VARIANT" == "debug" ]; then
   mkdir debug
   cp $ANDROID_PRODUCT/system/lib/libsmapi.so debug
else
