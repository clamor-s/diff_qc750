...
echo -c ..\..\..\src\drm_service.c >> armcc_via.txt
echo --thumb >> armcc_via.txt

echo Compile drm_service.c

armcc --via armcc_via.txt
@if ERRORLEVEL 1 goto build_error

echo Compile the resources
..\..\..\..\..\tools\ix86_win32\tf_resc.exe --output drm_service.manifest.o --target elf --manifest ..\..\..\src\drm.manifest.txt

echo Linking the service
echo. > armlink_via.txt
echo --bpabi --fpu=none --pltgot=none --dll -o drm_service.srvx drm_service.manifest.o drm_service.o >> armlink_via.txt
