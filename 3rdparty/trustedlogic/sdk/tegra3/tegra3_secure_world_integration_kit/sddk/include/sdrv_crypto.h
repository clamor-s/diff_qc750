/*
* Copyright (c) 2007-2009 Trusted Logic S.A.
* All Rights Reserved.
*
...
typedef struct SDRV_CRYPTO_KEY_INFO
{
   ...
   uint32_t nFlags;
   uint32_t nReserved1;
#ifdef SUPPORT_TPM_CRYPTO_EXTENSIONS
   /* if nKeyType == PRIVATE and nKeyAlgorithm == RSA and
      (nFlags | TPM_GENERATE_RSA_COUNTER) != 0, then nReserved1 contains
      the KDFa outer counter value after deriving the key. */
#endif /*defined(SUPPORT_TPM_CRYPTO_EXTENSIONS)*/
   uint32_t nReserved2;
...
#ifdef SUPPORT_TPM_CRYPTO_EXTENSIONS
typedef struct SDRV_CRYPTO_RSA_DETERMINISTIC_SEED
{
   uint32_t nHmacAlgoId;
   uint32_t nDigestLen;
   uint8_t *pSeed;
   uint16_t nSeedLen;
   uint8_t *pLabel;
   ...
   uint16_t nExtraLen;
   uint32_t nCounter;
} SDRV_CRYPTO_RSA_DETERMINISTIC_SEED;
#endif /*defined(SUPPORT_TPM_CRYPTO_EXTENSIONS)*/
...
#ifdef SUPPORT_TPM_CRYPTO_EXTENSIONS
#define SDRV_CRYPTO_FLAG_TPM_GENERATE_RSA_COUNTER  0x80000000 /*only used on some RSA private keys*/
#endif /*defined(SUPPORT_TPM_CRYPTO_EXTENSIONS)*/
...
#define SDRV_CRYPTO_ALG_AES_CMAC               0x61000201
#define SDRV_CRYPTO_ALG_DES_CMAC               0x61000202
#define SDRV_CRYPTO_ALG_DES3_CMAC              0x61000204
...
#ifdef SUPPORT_TPM_CRYPTO_EXTENSIONS
#define SDRV_CRYPTO_ALG_GEN_RSA_TPM            0x90000071
#endif /*defined(SUPPORT_TPM_CRYPTO_EXTENSIONS)*/
