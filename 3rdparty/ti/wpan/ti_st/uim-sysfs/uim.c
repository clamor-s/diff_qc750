...
/* This flag is used to set bt address (as user input)
 * for bluetootth chip.
 */
#define USER_BD_ADDR 0
#define RM_GPS_PERMISSION_DATA 1
...
static int del_path (const char *path)
{
    ...
    UIM_ERR("in del_path for path:%s", path);
    dir = opendir(path);

    if (dir == NULL) {
        UIM_ERR("opendir failed on path:%s", path);
        return -1;
    }
    ...
    for (;;) {
        ...
        if (de == NULL) {
            UIM_ERR("readdir failed for path:%s", path);
            break;
        }
        ...
        if((int)strlen(de->d_name) > PATH_MAX - nameLen) {
            UIM_ERR("d_name len:%d is too big", strlen(de->d_name));
            ret = -1;
            break;
        }
        ...
        if (ret != 0) {
            UIM_ERR("lstat failed for path:%s", nameBuffer);
            break;
        }
        ...
        if(S_ISDIR(statBuffer.st_mode)) {
            ...
            if (ret != 0) {
                UIM_ERR("unlink failed for path:%s", nameBuffer);
                break;
            }
        }
    }
    ...
    if(ret == 0) {
        ret = rmdir(path);
        UIM_ERR("rmdir return:%d for path:%s", ret, path);
    }
...
int main(int argc, char *argv[])
{
...
#ifdef RM_GPS_PERMISSION_DATA
	if (0 == lstat("/data/gps/navlfifo", &file_stat))
	{
		//if((AID_ROOT == file_stat.st_uid) || (AID_ROOT == file_stat.st_gid))
		{
			del_path("/data/gps");
		}
	}
	...
	{
		UIM_ERR("----file /data/gps/navlfifo is exit----");
	}
#endif
