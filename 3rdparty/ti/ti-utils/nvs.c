...
static int read_mac_from_sysfs(unsigned char *in_mac)
{
	int fd, pos, ret;
	char mac_sysfs[MAC_ADDR_LENGTH];

	fd = open(WIFI_SYSFS_MAC_PATH, O_RDONLY);
	if (fd < 0) {
		fprintf(stderr, "Failed to open %s\n", WIFI_SYSFS_MAC_PATH);
		return -1;
	}

	pos = read(fd, mac_sysfs, MAC_ADDR_LENGTH - 1);
	if (pos < 0) {
	...
	fprintf(stderr, "Read MAC address %s from sysfs %s\n", mac_sysfs, WIFI_SYSFS_MAC_PATH);

	ret = sscanf(mac_sysfs, "%2x:%2x:%2x:%2x:%2x:%2x",
	...
	if (ret !=6 || ( in_mac[0] == 0xff && in_mac[1] == 0xff && in_mac[2] == 0xff &&
		in_mac[3] == 0xff && in_mac[4] == 0xff && in_mac[5] == 0xff))
	{
		fprintf(stderr, "WARNING: MAC address is not valid!\n");
		goto error;
	}
...
int nvs_set_mac(char *nvsfile, char *mac)
{
	...
	if (mac) {
		...
		(unsigned int *)&in_mac[4], (unsigned int *)&in_mac[5]);
		if (ret != 6) {
			fprintf(stderr, "WARNING: MAC address %s is not valid!\n", mac);
			return -1;
		}
	}
	else if (!read_mac_from_sysfs(in_mac)) {
		fprintf(stderr, "Using MAC from sysfs: %s\n", WIFI_SYSFS_MAC_PATH);
	}
	else {
	...
	printf("Writing mac address %02x:%02x:%02x:%02x:%02x:%02x to file %s\n",
		in_mac[0], in_mac[1], in_mac[2], in_mac[3], in_mac[4], in_mac[5], nvsfile);
