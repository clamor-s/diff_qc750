...
/*
 * plt_autocalibrate2 calibrate nvs file in auto calibration mode assuming wifi
 * sdio driver is already loaded to avoid insmod/rmmod. It fist read mac address
 * from specified file in file system and verify for valid mac address else set
 * random mac address.
 */
static int plt_autocalibrate2(struct nl80211_state *state, struct nl_cb *cb,
			struct nl_msg *msg, int argc, char **argv)
{
	struct wl12xx_common cmn = {
		.auto_fem = 0,
	...
	char *devname, *inifile1;
	int single_dual = 0, res, fems_parsed, rm_nvs = 1;
	argc -= 2;
	argv += 2;
	...
	if (file_exist(cmn.nvs_name) >= 0) {
		fprintf(stderr, "nvs file %s. File already exists. Overwriting...\n", cmn.nvs_name);
		rm_nvs = 0;
	}
	...
	if (res && rm_nvs) {
		...
		fprintf(stderr, "Calibration not complete\n");
		return 0;
	}

	res = nvs_set_mac(cmn.nvs_name, NULL);
	if (res && rm_nvs) {
		goto out_removenvs;
	}
	else if(res) {
...
COMMAND(plt, autocalibrate2, "<dev> <ini file1> <nvs file>", 0, 0, CIB_NONE,
	plt_autocalibrate2, "Do automatic calibration and set mac address from file system\n");
