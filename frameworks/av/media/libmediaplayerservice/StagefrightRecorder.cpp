...
status_t StagefrightRecorder::setupVideoEncoder(
    ...
    // Similar situation applies to effect reccording, for this case,
    // mediasource output buffer count is defined in media frameworks,
    // while the encoder input buffer count is defined in BSP,
    // and varies in different BSP implementation. There could be a
    // a case where mediasource output buffer count is less than encoder
    // input buffer count. This leads effect recording hang.
    if (mCaptureTimeLapse || (mVideoSource == VIDEO_SOURCE_GRALLOC_BUFFER)) {
