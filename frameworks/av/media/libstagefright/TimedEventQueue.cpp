...
TimedEventQueue::event_id TimedEventQueue::postEvent(const sp<Event> &event, int arg1, int arg2, int arg3) {
    // Reserve an earlier timeslot an INT64_MIN to be able to post
    // the StopEvent to the absolute head of the queue.
    return postTimedEvent(event, INT64_MIN + 1, arg1, arg2, arg3);
}
...
TimedEventQueue::event_id TimedEventQueue::postTimedEvent(
        const sp<Event> &event, int64_t realtime_us, int arg1, int arg2, int arg3) {
...
void TimedEventQueue::threadEntry() {
    ...
    for (;;) {
...

        {
            ...
            event = removeEventFromQueue_l(eventID, arg1, arg2, arg3);
        }

        if (event != NULL) {
            // Fire event with the lock NOT held.
            if(event->mUsesArgs)
                event->fire(this, now_us, arg1, arg2, arg3);
            else
...
sp<TimedEventQueue::Event> TimedEventQueue::removeEventFromQueue_l(
        event_id id, int &arg1, int &arg2, int &arg3) {
