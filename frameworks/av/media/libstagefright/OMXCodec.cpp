...
skipcodecdata:
    ...
    if (mFlags & kUseVideoEditorBuffers) {
        OMX_INDEXTYPE index;

        status_t err =
            mOMX->getExtensionIndex(
                    mNode,
                    "OMX.Nvidia.index.config.waitOnFence",
                    &index);

        if (err == OK) {
            OMX_BOOL waitOnFence = OMX_TRUE;
            err = mOMX->setConfig(mNode, index, &waitOnFence, sizeof(waitOnFence));

            if (err != OK) {
                CODEC_LOGE("setConfig('OMX.Nvidia.index.config.waitOnFence') "
                        "returned error 0x%08x", err);
                return err;
            }
        }
...
void OMXCodec::setVideoInputFormat(
    ...
    err = mOMX->getParameter(
            mNode, OMX_IndexParamPortDefinition, &def, sizeof(def));
    CHECK_EQ(err, (status_t)OK);
    meta->setInt32(kKeyInputBufferSize, def.nBufferSize);
...
status_t OMXCodec::read(
    ...
    *buffer = NULL;
    ...
    while (mState != ERROR && !mNoMoreOutputData && mFilledBuffers.empty() &&
        ...
        if (mFlags & kUseVideoEditorBuffers) {
            return OK;
        } else if((err = waitForBufferFilled_l()) != OK) {
            return err;
        }
    }
