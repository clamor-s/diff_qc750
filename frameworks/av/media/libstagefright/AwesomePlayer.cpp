...
struct AwesomeNotifyEvent : public TimedEventQueue::Event {
    AwesomeNotifyEvent(
            AwesomePlayer *player)
        : mPlayer(player) {
    mUsesArgs = true;
...
protected:
    virtual ~AwesomeNotifyEvent() {}

    virtual void fire(TimedEventQueue *queue, int64_t /* now_us */, int msg, int ext1, int ext2) {
        if (mPlayer->mListener != NULL) {
            sp<MediaPlayerBase> listener = mPlayer->mListener.promote();
...
private:
    ...
    AwesomeNotifyEvent(const AwesomeNotifyEvent &);
    AwesomeNotifyEvent &operator=(const AwesomeNotifyEvent &);
};
...
AwesomePlayer::AwesomePlayer()
    ...
    mNotifyEvent = new AwesomeNotifyEvent(this);
...
void AwesomePlayer::notifyListener_l(int msg, int ext1, int ext2) {
    mNotifyQueue.postEvent(mNotifyEvent, msg, ext1, ext2);
}
    ...
    if (getCachedDuration_l(&cachedDurationUs, &eos)) {
        ...
        if ((mFlags & PLAYING) && !eos
            ...
            mIsBufferingStartEventNotified = true;
               ...
               && !(mFlags & PREPARING)
               && !(mFlags & PLAYING)
               && mIsBufferingStartEventNotified) {
            if (mCachedSource != NULL) {
...
status_t AwesomePlayer::getPosition(int64_t *positionUs) {
    if (mSeeking != NO_SEEK) {
        ...
        *positionUs = mVideoTimeUs;
        ...
        if (mOwnerAudioULP || (mEnabledAudioULP && mReachedEOSAudioULP)) {
            uint64_t pos = 0;
