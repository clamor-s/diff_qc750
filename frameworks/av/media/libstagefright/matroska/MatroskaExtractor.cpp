...
status_t MatroskaSource::readBlock() {
    ...
    if (timeUs < 0) {
        LOGV("MKV error: timeUs < 0 and skip to next block");
        mBlockIter.advance();
        return OK;
    }
            ...
            case VIDEO_TRACK:
            {
                ...
                if (!strcmp("V_MPEG4/ISO/AVC", codecID)) {
                    ...
                    if (codecPrivateSize > 0) {
                        ...
                        meta->setCString(kKeyMIMEType, ""); // VOL Header is absent, still adding track
                    }
                } else if (!strcmp("V_VP8", codecID)) {
                    meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_VPX);
                } else {
                    ALOGW("%s is not supported.", codecID);
                    meta->setCString(kKeyMIMEType, ""); // return invalid MIME type
                }
