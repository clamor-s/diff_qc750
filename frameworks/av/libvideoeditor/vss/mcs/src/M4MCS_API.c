...
#ifdef VIDEOEDITOR_ENHANCEMENT_GRALLOC_SUPPORT

            err = pC->m_pVideoDecoder->m_pFctCreate(&pC->pViDecCtxt,
                &pC->pReaderVideoStream->m_basicProperties, pC->m_pReader,
                pC->m_pReaderDataIt, &pC->ReaderVideoAU, decoderUserData, M4OSA_TRUE);
#else

            err = pC->m_pVideoDecoder->m_pFctCreate(&pC->pViDecCtxt,
                &pC->pReaderVideoStream->m_basicProperties, pC->m_pReader,
                pC->m_pReaderDataIt, &pC->ReaderVideoAU, decoderUserData, M4OSA_FALSE);

#endif /* VIDEOEDITOR_ENHANCEMENT_GRALLOC_SUPPORT */
    ...
    * Create video encoder */
        ...
        //    && (M4VIDEOEDITING_kH264 == pC->InputFileProperties.VideoStreamType))
    {
        ...
        M4OSA_TRACE1_2("mcs encoder open profile :%d, level %d",
            ...
            err = pC->pVideoEncoderGlobalFcts->pFctOpen(pC->pViEncCtxt,
                &pC->WriterVideoAU, &EncParams1, M4OSA_TRUE);
#else

            err = pC->pVideoEncoderGlobalFcts->pFctOpen(pC->pViEncCtxt,
                &pC->WriterVideoAU, &EncParams1, M4OSA_FALSE);
    ...
    else
    {
        M4OSA_TRACE1_2("mcs encoder open Adv profile :%d, level %d",
            ...
            err = pC->pVideoEncoderGlobalFcts->pFctOpen(pC->pViEncCtxt,
                &pC->WriterVideoAU, &EncParams, M4OSA_TRUE);
#else

            err = pC->pVideoEncoderGlobalFcts->pFctOpen(pC->pViEncCtxt,
                &pC->WriterVideoAU, &EncParams, M4OSA_FALSE);
...
static M4OSA_ERR M4MCS_intVideoTranscoding( M4MCS_InternalContext *pC )
{
    ...
    * Decode video */
    ...
    pC->dViDecCurrentCts = mtTranscodedTime;
