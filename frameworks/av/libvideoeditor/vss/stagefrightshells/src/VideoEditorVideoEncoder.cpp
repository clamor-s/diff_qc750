...
struct VideoEditorVideoEncoderSource : public MediaSource, public MediaBufferObserver {
    public:
        ...
        Vector<MediaBuffer *>   mDecoderBuffers;
    ...
    private:
        ...
        int32_t           mNbBuffer;
        int32_t           mBufferCountThreshold;
        bool              mIsEOS;
        State             mState;
        sp<MetaData>      mEncFormat;
        Mutex             mLock;
        Condition         mBufferCond;
        Condition         mBufferListUpperBound;
        int64_t           mFirstPTS;
        bool              mFirstPTSSet;
};
...
status_t VideoEditorVideoEncoderSource::stop() {
    ...
    while( mFirstBufferLink ) {
        ...
        MediaBuffer *buffer = tmpLink->buffer;
        if (buffer != NULL) {
            sp<GraphicBuffer> graphicBuf = buffer->graphicBuffer();
            if (graphicBuf != NULL)
                buffer->release();
        }
...
status_t VideoEditorVideoEncoderSource::read(MediaBuffer **buffer,
    ...
    lbuf = mFirstBufferLink->buffer;
    tmpLink = mFirstBufferLink;
    mFirstBufferLink = mFirstBufferLink->nextLink;

    sp<GraphicBuffer> graphicBuf = lbuf->graphicBuffer();
    if (graphicBuf != NULL) {
        int32_t nBufferSize;
        bool success = mEncFormat->findInt32(kKeyInputBufferSize, &nBufferSize);
        if (success != true)
            return UNKNOWN_ERROR;

        M4OSA_ERR lerr = M4NO_ERROR;

        lerr = (lbuf->graphicBuffer())->lock((uint32_t)GRALLOC_USAGE_HW_TEXTURE, NULL);
        if (lerr != M4NO_ERROR) {
            LOGE("Error Occured while locking\n");
            return lerr;
        }

        MediaBuffer *tempBuffer =
            new MediaBuffer(nBufferSize);
        ...
        uint32_t type = KMetadataVideoEditorBufferTypeGrallocSource;
        int32_t decoderCropRectWidth;
        int32_t decoderCropRectHeight;
        lbuf->meta_data()->findInt32(kKeyVideoEditorBufferWidth, &decoderCropRectWidth);
        lbuf->meta_data()->findInt32(kKeyVideoEditorBufferHeight, &decoderCropRectHeight);

        memcpy(data, &type, 4);
        memcpy(data+4, &decoderCropRectWidth, 4);
        memcpy(data+8, &decoderCropRectHeight, 4);
        memcpy(data + 12, (graphicBuf->handle), (nBufferSize-12));

        *buffer = tempBuffer;
        lbuf->meta_data()->findInt64(kKeyTime, &timeStamp);

        if (!mFirstPTSSet) {
            mFirstPTS = timeStamp;
            mFirstPTSSet = 1;
        } else if (mFirstPTS > timeStamp) {
            mFirstPTS = timeStamp;
        }

        int64_t presentPTS = timeStamp - mFirstPTS;
        (*buffer)->meta_data()->setInt64(kKeyTime, presentPTS);
        (*buffer)->setObserver(this);
        (*buffer)->add_ref();
        lbuf->meta_data()->setInt64(kKeyTime, presentPTS);
        mDecoderBuffers.push_back(lbuf);
    } else {
        *buffer = lbuf;
    }
    ...
    if (mNbBuffer < mBufferCountThreshold)
        mBufferListUpperBound.signal();

    ALOGV("VideoEditorVideoEncoderSource::read() END (0x%x)", err);
    return err;
}

void VideoEditorVideoEncoderSource::signalBufferReturned(MediaBuffer *buffer) {
    int64_t timeStamp;
    int64_t decTimeStamp;
    M4OSA_ERR lerr = M4NO_ERROR;
    buffer->meta_data()->findInt64(kKeyTime, &timeStamp);

    for (int32_t i = 0; i < mDecoderBuffers.size(); ++i) {
        MediaBuffer *tmpBuffer = mDecoderBuffers.editItemAt(i);
       lerr = (tmpBuffer->graphicBuffer())->unlock();
        if (lerr != M4NO_ERROR) {
            LOGE("Error Occured while unlocking\n");
            return;
        }
        tmpBuffer->meta_data()->findInt64(kKeyTime, &decTimeStamp);
        if (timeStamp == decTimeStamp) {
            mDecoderBuffers.removeAt(i);
            tmpBuffer->release();
            break;
        }
    }
...
int32_t VideoEditorVideoEncoderSource::storeBuffer(MediaBuffer *buffer) {
    ...
    if( NULL == buffer ) {
        ...
        // If encoder consume buffers at slower rate, Buffer list could
        //grow as decoder keeps adding buffers and can cause out of memory scenario.
        //Hence add limit to growth of buffer list.
        //Wait until encoder consume buffers to bring list below threshold.
        //Threshold 10, is decided assuming it as approximately sufficient threshold
        //Ideally decoder/encoder should add/consume alternate manner.

        while (mNbBuffer > mBufferCountThreshold) {
               mBufferListUpperBound.wait(mLock);
        }
...
M4OSA_ERR VideoEditorVideoEncoder_getDSI(M4ENCODER_Context pContext,
    ...
    memset(inputBuffer->data(), 0, size);
...
M4OSA_ERR VideoEditorVideoEncoder_close(M4ENCODER_Context pContext) {
    ...
    nBuffersCount = pEncoderContext->mEncoderSource->mDecoderBuffers.size();
    if (pEncoderContext->mEncoderSource!= NULL && pEncoderContext->mEncoderSource->mDecoderBuffers.size() != 0) {
        for (int32_t i = 0; i < nBuffersCount; ++i) {
            MediaBuffer *tmpBuffer = pEncoderContext->mEncoderSource->mDecoderBuffers.editItemAt(i);
                tmpBuffer->release();
        }
        for (int32_t i = 0; i < nBuffersCount; ++i)
            pEncoderContext->mEncoderSource->mDecoderBuffers.removeAt(i);
    }
...
M4OSA_ERR VideoEditorVideoEncoder_open(M4ENCODER_Context pContext,
        M4SYS_AccessUnit* pAU, M4OSA_Void* pParams, M4OSA_Bool isTranscoding) {
    M4OSA_ERR err = M4NO_ERROR;
    VideoEditorVideoEncoder_Context* pEncoderContext = M4OSA_NULL;
    ...
    pEncoderContext->mIsTranscoding = isTranscoding;
    ...
    encoderMetadata->setInt32(kKeySliceHeight,
        (int32_t)pEncoderContext->mCodecParams->FrameHeight);
    encoderMetadata->setInt32(kKeyNotRealTime, nonRealtime);
    ...
    if (isTranscoding) {
        codecFlags |= OMXCodec::kStoreMetaDataInVideoBuffers;
        codecFlags |= OMXCodec::kOnlySubmitOneInputBufferAtOneTime;
    }
...
M4OSA_ERR VideoEditorVideoEncoder_processInputBuffer(
    ...
    if ( M4OSA_FALSE == bReachedEOS ) {
        ...
        err = pEncoderContext->mPreProcFunction(
            pEncoderContext->mPreProcContext, M4OSA_NULL, pOutPlane, &tmpBuffer);
        VIDEOEDITOR_CHECK(M4NO_ERROR == err, err);

        if (pEncoderContext->mIsTranscoding) {
            MediaBuffer *tmpBuf = (MediaBuffer *)tmpBuffer;
            if (tmpBuf != NULL) {
                nbBuffer = pEncoderContext->mEncoderSource->storeBuffer(tmpBuf);
            }

                while(tmpBuffer != NULL) {
                    tmpBuffer = NULL;
                    err = pEncoderContext->mPreProcFunction(pEncoderContext->mPreProcContext,
                                 M4OSA_NULL, pOutPlane, &tmpBuffer);
                    VIDEOEDITOR_CHECK(M4NO_ERROR == err, err);
                    if (tmpBuffer == NULL)
                        break;
                    MediaBuffer *tmpBuf1 = (MediaBuffer *)tmpBuffer;
                    if (tmpBuf != NULL)
                    nbBuffer = pEncoderContext->mEncoderSource->storeBuffer(tmpBuf1);
                }
            buffer->release();
            goto cleanUp;
        }
