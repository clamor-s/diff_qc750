...
M4OSA_ERR VideoEditorVideoDecoder_configureFromMetadata(M4OSA_Context pContext,
    ...
    if (!pDecShellContext->mIsTranscoding )  {
...
M4OSA_ERR VideoEditorVideoDecoder_destroy(M4OSA_Context pContext) {
    ...
    // Destroy the graph
    nBuffersCount = pDecShellContext->mMediaBufferPool.size();

    if (nBuffersCount) {
        for(size_t i = 0; i < nBuffersCount; ++i) {
                MediaBuffer *buffer = pDecShellContext->mMediaBufferPool.editItemAt(i);

                if (buffer != NULL)
                    buffer->release();
        }
        for(size_t i = 0; i < nBuffersCount; ++i)
            pDecShellContext->mMediaBufferPool.removeAt(i);
    }
...
M4OSA_ERR VideoEditorVideoDecoder_create(M4OSA_Context *pContext,
        ...
        M4_AccessUnit *pAccessUnit, M4OSA_Void *pUserData, M4OSA_Bool isTranscoding) {
    M4OSA_ERR err = M4NO_ERROR;
    VideoEditorVideoDecoder_Context* pDecShellContext = M4OSA_NULL;
    status_t status = OK;
    bool success = TRUE;
    ...
    if (isTranscoding)
        pDecShellContext = new VideoEditorVideoDecoder_Context;
    else
        SAFE_MALLOC(pDecShellContext, VideoEditorVideoDecoder_Context, 1,"VideoEditorVideoDecoder");
    ...
    pDecShellContext->mNativeWindow = NULL;
    pDecShellContext->mReachedEOS = M4OSA_FALSE;
    pDecShellContext->mIsTranscoding = isTranscoding;

    if (isTranscoding) {
        pDecShellContext->mSurfaceTexture =  new SurfaceTexture(123); // " 123 " is just a TextureID to which this surface texture binds. we can give any const value.
        sp<ANativeWindow> nativeWindow = new SurfaceTextureClient(pDecShellContext->mSurfaceTexture);
        pDecShellContext->mNativeWindow = nativeWindow;
    }
    ...
    if (!isTranscoding) {
        ...
        uint32_t flags;
        flags |= OMXCodec::kUseVideoEditorBuffers;
        pDecShellContext->mVideoDecoder = OMXCodec::Create(
            pDecShellContext->mClient.interface(),
            decoderMetadata, false, pDecShellContext->mReaderSource, NULL, flags, pDecShellContext->mNativeWindow);
    }
...
M4OSA_ERR VideoEditorVideoDecoder_decode(M4OSA_Context context,
    ...
    while (pDecoderBuffer == NULL || pDecShellContext->m_lastDecodedCTS + tolerance < *pTime) {
        ...
        if (pDecShellContext->mIsTranscoding && bJump && (pDecShellContext->m_lastDecodedCTS + tolerance < *pTime)) {
            if (pNextBuffer != NULL)
                pNextBuffer->release();
            pNextBuffer = NULL;
        }
...
static M4OSA_ERR copyBufferToQueue(
    ...
    if (pDecShellContext->mIsTranscoding) {
        MediaBuffer *buf = pDecoderBuffer;
        pDecShellContext->mMediaBufferPool.push_back(buf);
        return lerr;
    }
    ...
    tmpDecBuffer->size = (pDecShellContext->m_pVideoStreamhandler->m_videoWidth *
                            pDecShellContext->m_pVideoStreamhandler->m_videoHeight * 1.5);
...
M4OSA_ERR VideoEditorVideoDecoder_render(M4OSA_Context context,
        ...
        M4OSA_Bool bForceRender, void **buffer) {
    M4OSA_ERR err = M4NO_ERROR;
    VideoEditorVideoDecoder_Context* pDecShellContext =
        (VideoEditorVideoDecoder_Context*) context;
    ...
    if (pDecShellContext->mIsTranscoding) {
        nBuffersCount = pDecShellContext->mMediaBufferPool.size();
        if (nBuffersCount) {
            MediaBuffer *lbuf = pDecShellContext->mMediaBufferPool.editItemAt(i);
            *buffer = lbuf;
            int32_t cropRectWidth = pDecShellContext->m_pVideoStreamhandler->m_videoWidth;
            int32_t cropRectHeight = pDecShellContext->m_pVideoStreamhandler->m_videoHeight;
            lbuf->meta_data()->setInt32(kKeyVideoEditorBufferWidth,cropRectWidth);
            lbuf->meta_data()->setInt32(kKeyVideoEditorBufferHeight,cropRectHeight);
            lbuf->meta_data()->findInt64(kKeyTime, &mediaTime);
            pDecShellContext->mMediaBufferPool.removeAt(i);
            pDecShellContext->m_lastRenderCts = mediaTime;
