...
class ZoomManager {
    ...
    // Task that is posted to update scale from WebView.onSizeChanged. The scale update must be
    // asynchronous during view layout.
    private PostScale mOnSizeChangedScaleTask;
    ...
    public void onSizeChanged(int w, int h, int ow, int oh) {
        ...
        // Post the Runnable as first event of WebView.mPrivateHandler. This way
        // there content size update messages cannot be executed between this function
        // and the application of scale.
        if (mOnSizeChangedScaleTask == null) {
            mOnSizeChangedScaleTask = new PostScale(w != ow &&
                !mWebView.getSettings().getUseFixedViewport(), mInZoomOverview, w < ow);
        } else {
            mOnSizeChangedScaleTask.init(w != ow &&
                !mWebView.getSettings().getUseFixedViewport(), mInZoomOverview, w < ow);
            mWebView.mPrivateHandler.removeCallbacks(mOnSizeChangedScaleTask);
        }

        mWebView.mPrivateHandler.postAtFrontOfQueue(mOnSizeChangedScaleTask);
    }
    ...
    private class PostScale implements Runnable {
        ...
        public PostScale(boolean updateTextWrap,
            ...
            init(updateTextWrap, inZoomOverview, inPortraitMode);
        }

        public void init(boolean updateTextWrap,
                         boolean inZoomOverview,
                         boolean inPortraitMode) {
