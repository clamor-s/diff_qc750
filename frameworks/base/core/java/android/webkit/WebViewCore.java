...
public final class WebViewCore {
    ...
    */
    ...
    /*
     * Called by JNI, from any thread.
     */
    private void sendViewInvalidateLayer(int layerId) {
        if (mWebViewClassic != null) {
            Message.obtain(mWebViewClassic.mPrivateHandler,
                           WebViewClassic.INVALIDATE_LAYER,
                           layerId, 0).sendToTarget();
        }
    }
    ...
    /* Called by the WebViewCore thread */
    /* package */ String getLayerTreeAsText() {
        if (Thread.currentThread().getName() != THREAD_NAME)
            return "";

        return nativeGetLayerTreeAsText(mNativeClass);
    }
    ...
    private native String nativeGetLayerTreeAsText(int nativeClass);
}
