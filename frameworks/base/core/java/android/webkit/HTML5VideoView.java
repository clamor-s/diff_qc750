...
public class HTML5VideoView implements MediaPlayer.OnPreparedListener {
    ...
    public void start() {
        if (mCurrentState == STATE_PREPARED) {
            ...
            if (mTimer == null && !isFullScreenMode()) {
                mTimer = new Timer();
    ...
    public void pause() {
        if (isPlaying()) {
            // Send timeupdate once. The position is cached in the native side, but timeupdate
            // is not sent during fullscreen playback. This send makes sure that native playback
            // position is correct when we exit fullscreen.
            mProxy.dispatchTimeupdate();
            mPlayer.pause();
