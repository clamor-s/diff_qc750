...
public class HTML5VideoInline extends HTML5VideoView{
    ...
    private Timer mBufferingAnimationTimer;

    private static final int BUFFERING_UPDATE_PERIOD = 33;  // ms
    ...
    public void switchProgressView(boolean playerBuffering) {
        super.switchProgressView(playerBuffering);
        if (playerBuffering) {
            startTimer();
        } else {
            stopTimer();
    ...
    private void startTimer() {
        if (mBufferingAnimationTimer == null) {
            mBufferingAnimationTimer = new Timer();
            mBufferingAnimationTimer.schedule(new BufferingAnimationTask(mProxy),
                      BUFFERING_UPDATE_PERIOD, BUFFERING_UPDATE_PERIOD);
        }
    }

    private void stopTimer() {
        if (mBufferingAnimationTimer != null) {
            mBufferingAnimationTimer.purge();
            mBufferingAnimationTimer.cancel();
            mBufferingAnimationTimer = null;
        }
    }
    private final class BufferingAnimationTask extends TimerTask {
        private HTML5VideoViewProxy mProxy;

        public BufferingAnimationTask(HTML5VideoViewProxy proxy) {
            mProxy = proxy;
        }

        @Override
        public void run() {
            mProxy.onBufferingupdate();
        }
    }
}
