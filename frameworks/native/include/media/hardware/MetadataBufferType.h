...
typedef enum {
    ...
    /*
     * KMetadataVideoEditorBufferTypeGrallocSource is used to indicate that
     * the payload of the metadata buffers can be interpreted as
     * decoder crop rect width and height and a NvNativeHandle
     * So in this case,the metadata that the encoder receives
     * will have a byte stream that consists of four parts:
     * --------------------------------------------------------------------------------------
     * buffer = KMetadataVideoEditorBufferTypeGrallocSource + width + height + NvNativeHandle
     * --------------------------------------------------------------------------------------
     */
    KMetadataVideoEditorBufferTypeGrallocSource = 2,
    // Add more here...
