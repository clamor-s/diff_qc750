...
void SurfaceFlinger::initializeGL(EGLDisplay display) {
    ...
    /*
     * fix for bug 1194226: set the maximum GL texture size as a system property
     */
    char tmpbuf[32];
    snprintf(tmpbuf, 32, "%d", mMaxTextureSize);
    property_set("sys.max_texture_size", tmpbuf);
            ...
            for (size_t i=0 ; i<dc ; i++) {
                ...
                if (j < 0) {
                    ...
                    if (disp != NULL) {
                        ...
                        {
                            // Boost frequency on rotation
                            // TODO: We may not need to boost for 3s on T114
                            NvCpuClient cpuClient;
                            cpuClient.requestCpuFreqMin(NVCPU_BOOST_STRENGTH_HIGH,
                                                        ms2ns(3000), systemTime());
...
void SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& hw, const Region& dirty)
{
    ...
    if (hasGlesComposition) {
        if (!DisplayDevice::makeCurrent(mEGLDisplay, hw, mEGLContext)) {
            ALOGW("DisplayDevice::makeCurrent failed. Aborting surface composition");
            return;
        }
