...
jpeg_start_decompress (j_decompress_ptr cinfo)
{
  ...
  if (cinfo->tegra_acceleration == TRUE)
  {
    ...
    jpegTegraDecoderSetParams(cinfo);

    if (cinfo->tegra_acceleration == TRUE)
      jpegTegraDecoderResize(cinfo);

    if (cinfo->tegra_acceleration)
      jpegTegraDecoderSurfaceResize (cinfo);

    if (cinfo->tegra_acceleration == TRUE)
    {
      unsigned int outWidth, outHeight;

      cinfo->tegra_acceleration = jpegTegraDecoderRender(cinfo, &outWidth, &outHeight);
      if (cinfo->tegra_acceleration == TRUE)
      {
        /* output resolution value is slightly different
        * when hardware acceleration is used. The
        * values are updated only when Tegra acceleration is
        * successful
        */
        cinfo->output_width = outWidth;
        cinfo->output_height = outHeight;
        return TRUE;
      }
    }
  }
		     ...
		     JDIMENSION max_lines)
{
  ...
  if (cinfo->tegra_acceleration == TRUE)
  {
    unsigned int scanlinesRead;
    scanlinesRead = jpegTegraDecoderRenderWait (cinfo);
    cinfo->output_scanline = jpegTegraDecoderOutputHeight(cinfo);
    return scanlinesRead;
  }
#endif
