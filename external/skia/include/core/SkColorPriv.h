...
static inline U16CPU SkPixel4444ToPixel16(SkPMColor c) {
    unsigned r = SkGetPackedR4444(c);
    unsigned g = SkGetPackedG4444(c);
    unsigned b = SkGetPackedB4444(c);

    return
        (r << (SK_R16_SHIFT + (SK_R16_BITS - 4))) |
        ((r >> (8 - SK_R16_BITS)) << SK_R16_SHIFT) |
        (g << (SK_G16_SHIFT + (SK_G16_BITS - 4))) |
        ((g >> (8 - SK_G16_BITS)) << SK_G16_SHIFT) |
        (b << (SK_B16_SHIFT + (SK_B16_BITS - 4))) |
        ((b >> (8 - SK_B16_BITS)) << SK_B16_SHIFT) ;
}
...
static inline U16CPU SkCompact_565(uint32_t c) {
    unsigned r = (c >> 24) & 0xF;
    unsigned b = (c >> 16) & 0xF;
    unsigned g = (c >> 8) & 0xF;
    return
