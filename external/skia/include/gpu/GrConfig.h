...
/**
 * GR_GL_AGGRESSIVE_BUFFER_ORPHANING controls whether Gr should aggressively
 * attempt to orphan index and vertex buffer objects during updateData() or
 * lock(). Orphaning a buffer involves telling the GL driver that the buffer
 * object data is not needed anymore and the buffer data update call should
 * prefer allocating a new buffer instead of waiting for the GPU to finish using
 * the buffer so that the data can be overwritten.
 *
 * If GR_GL_AGGRESSIVE_BUFFER_ORPHANING is enabled every call to updateData() or
 * lock() will attempt to orphan the buffer, otherwise it becomes the GL driver's
 * responsibility to orphan the buffers when it can. Turn this off for systems
 * where buffer object allocation and garbage collection is a significant overhead.
 */
#if !defined(GR_GL_AGGRESSIVE_BUFFER_ORPHANING)
    #define GR_GL_AGGRESSIVE_BUFFER_ORPHANING 1
#endif
