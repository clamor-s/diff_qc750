...
static inline SkPMColor Filter_4444_D16(unsigned x, unsigned y,
                                        uint32_t a00, uint32_t a01,
                                        uint32_t a10, uint32_t a11) {
    SkASSERT((unsigned)x <= 0xF);
    SkASSERT((unsigned)y <= 0xF);
...
#define GENERIC_4X_UNROLL(dstPtr, srcPtr, SRC2DST) \
    *dstPtr++ = SRC2DST(*srcPtr++); \
    *dstPtr++ = SRC2DST(*srcPtr++); \
    *dstPtr++ = SRC2DST(*srcPtr++); \
    *dstPtr++ = SRC2DST(*srcPtr++);
...
#define MAKENAME(suffix)        S32_D32 ## suffix
#define DSTSIZE                 32
#define SRCTYPE                 SkPMColor
#define CHECKSTATE(state)       SkASSERT(state.fBitmap->config() == SkBitmap::kARGB_8888_Config); \
                                SkASSERT(state.fAlphaScale == 256)
...
#define UNROLLED_4X_BODY(dstPtr, srcPtr) \
    GENERIC_4X_UNROLL(dstPtr, srcPtr, RETURNDST)
...
#define MAKENAME(suffix)        S16_D32 ## suffix
#define DSTSIZE                 32
#define SRCTYPE                 uint16_t
#define CHECKSTATE(state)       SkASSERT(state.fBitmap->config() == SkBitmap::kARGB_565_Config); \
                                SkASSERT(state.fAlphaScale == 256)
#define RETURNDST(src)          SkPixel16ToPixel32(src)
#define UNROLLED_4X_BODY(dstPtr, srcPtr) \
    GENERIC_4X_UNROLL(dstPtr, srcPtr, RETURNDST)
...
#define MAKENAME(suffix)        S4444_D32 ## suffix
#define DSTSIZE                 32
#define SRCTYPE                 SkPMColor16
#define RETURNDST(src)          SkPixel4444ToPixel32(src)
#define CHECKSTATE(state)       SkASSERT(state.fBitmap->config() == SkBitmap::kARGB_4444_Config); \
...
#undef FILTER_PROC
#define FILTER_PROC(x, y, a, b, c, d, dst)  *(dst) = Filter_4444_D16(x, y, a, b, c, d)

#define MAKENAME(suffix)        S4444_D16 ## suffix
#define DSTSIZE                 16
#define SRCTYPE                 SkPMColor16
#define CHECKSTATE(state)       SkASSERT(state.fBitmap->config() == \
...
bool SkBitmapProcState::chooseProcs(const SkMatrix& inv, const SkPaint& paint) {
    ...
    static const SampleProc16 gSample16[] = {
        ...
        // TODO (sjaved)
        // The reason is because the shadeSpan16 function doesn't have an
        // equivalent for fOpaqueProc, so it can't handle the alpha->opaque
        // transition properly
        S4444_D16_nofilter_DXDY,
        S4444_D16_nofilter_DX,
        S4444_D16_filter_DXDY,
        S4444_D16_filter_DX,
        NULL, NULL, NULL, NULL,
    ...
    if ((fInvType & ~SkMatrix::kTranslate_Mask) == 0 &&
            !fDoFilter &&
            fInvKy == 0) {
        if (SkShader::kRepeat_TileMode == fTileModeX &&
                SkShader::kRepeat_TileMode == fTileModeY) {
            // 32 bit
            if (S32_opaque_D32_nofilter_DX == fSampleProc32) {
                fShaderProc32 =
                    S32_D32_opaque_repeat_transonly_nofilter_shaderproc_DX;
            } else if (S16_opaque_D32_nofilter_DX == fSampleProc32) {
                fShaderProc32 =
                    S16_D32_opaque_repeat_transonly_nofilter_shaderproc_DX;
            } else if (S4444_opaque_D32_nofilter_DX == fSampleProc32) {
                fShaderProc32 =
                    S4444_D32_opaque_repeat_transonly_nofilter_shaderproc_DX;
            }

            // 16 bit
            if (S32_D16_nofilter_DX == fSampleProc16) {
                fShaderProc16 =
                    S32_D16_opaque_repeat_transonly_nofilter_shaderproc_DX;
            } else if (S4444_D16_nofilter_DX == fSampleProc16) {
                fShaderProc16 =
                    S4444_D16_opaque_repeat_transonly_nofilter_shaderproc_DX;
            } else if (S16_D16_nofilter_DX == fSampleProc16) {
                fShaderProc16 =
                    S16_D16_opaque_repeat_transonly_nofilter_shaderproc_DX;
            }
        } else if (SkShader::kClamp_TileMode == fTileModeX &&
                SkShader::kClamp_TileMode == fTileModeY) {
            // 32 bit
            if (S32_opaque_D32_nofilter_DX == fSampleProc32) {
                fShaderProc32 =
                    S32_D32_opaque_clamp_transonly_nofilter_shaderproc_DX;
            } else if (S16_opaque_D32_nofilter_DX == fSampleProc32) {
                fShaderProc32 =
                    S16_D32_opaque_clamp_transonly_nofilter_shaderproc_DX;
            } else if (S4444_opaque_D32_nofilter_DX == fSampleProc32) {
                fShaderProc32 =
                    S4444_D32_opaque_clamp_transonly_nofilter_shaderproc_DX;
            }

            // 16 bit
            if (S32_D16_nofilter_DX == fSampleProc16) {
                fShaderProc16 =
                    S32_D16_opaque_clamp_transonly_nofilter_shaderproc_DX;
            } else if (S4444_D16_nofilter_DX == fSampleProc16) {
                fShaderProc16 =
                    S4444_D16_opaque_clamp_transonly_nofilter_shaderproc_DX;
            } else if (S16_D16_nofilter_DX == fSampleProc16) {
                fShaderProc16 =
                    S16_D16_opaque_clamp_transonly_nofilter_shaderproc_DX;
            }
        }
    }
