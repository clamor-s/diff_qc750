...
void GrGpuGLShaders::flushRadial2(int s) {
    ...
    const int &uniVS = fProgramData->fUniLocations.fStages[s].fRadial2VSUni;
    const int &uniFS = fProgramData->fUniLocations.fStages[s].fRadial2FSUni;
    const GrSamplerState& sampler = this->getDrawState().getSampler(s);
    if ((GrGLProgram::kUnusedUniform != uniVS || GrGLProgram::kUnusedUniform != uniFS) &&
        ...
        if (GrGLProgram::kUnusedUniform != uniVS) {
            GL_CALL(Uniform1fv(uniVS, 6, values));
        }
        if (GrGLProgram::kUnusedUniform != uniFS) {
            GL_CALL(Uniform1fv(uniFS, 6, values));
        }
        fProgramData->fRadial2CenterX1[s] = sampler.getRadial2CenterX1();
        fProgramData->fRadial2Radius0[s]  = sampler.getRadial2Radius0();
        fProgramData->fRadial2PosRoot[s]  = sampler.isRadial2PosRoot();
