...
inline void radial2_param_name_vs(int stage, GrStringBuilder* s) {
    *s = "uRadial2VSParams";
    s->appendS32(stage);
}

inline void radial2_param_name_fs(int stage, GrStringBuilder* s) {
    *s = "uRadial2FSParams";
    s->appendS32(stage);
}
    ...
    if (fProgramDesc.fEdgeAANumEdges > 0) {
        ...
        *coverageVar = "edgeAlpha";
        ...
        if (GrDrawState::kHairLine_EdgeType == fProgramDesc.fVertexEdgeType) {
            segments->fFSCode.appendf("\tfloat edgeAlpha = abs(dot(vec3(gl_FragCoord.x + 0.5, gl_FragCoord.y + 0.5, 1), %s.xyz));\n", fsName);
...
void GrGLProgram::getUniformLocationsAndInitCache(const GrGLContextInfo& gl,
    ...
    for (int s = 0; s < GrDrawState::kNumStages; ++s) {
        ...
        if (fProgramDesc.fStages[s].isEnabled()) {
            ...
            if (kUseUniform == locations.fRadial2VSUni) {
                GrStringBuilder radial2VSParamName;
                radial2_param_name_vs(s, &radial2VSParamName);
                GL_CALL_RET(locations.fRadial2VSUni,
                            GetUniformLocation(progID, radial2VSParamName.c_str()));
                GrAssert(kUnusedUniform != locations.fRadial2VSUni);
            }

            if (kUseUniform == locations.fRadial2FSUni) {
                GrStringBuilder radial2FSParamName;
                radial2_param_name_fs(s, &radial2FSParamName);
                GL_CALL_RET(locations.fRadial2FSUni,
                            GetUniformLocation(progID, radial2FSParamName.c_str()));
                GrAssert(kUnusedUniform != locations.fRadial2FSUni);
            }
...
GrGLShaderVar* genRadialVS(int stageNum,
    ...
    GrGLShaderVar* radial2VSParams = &segments->fVSUnis.push_back();
    radial2VSParams->setType(GrGLShaderVar::kFloat_Type);
    radial2VSParams->setTypeModifier(GrGLShaderVar::kUniform_TypeModifier);
    radial2VSParams->setArrayCount(6);
    radial2_param_name_vs(stageNum, radial2VSParams->accessName());
    radial2VSParams->setEmitPrecision(true);

    locations->fRadial2VSUni = kUseUniform;
    ...
    if (varyingDims == coordDims) {
        ...
        radial2VSParams->appendArrayAccess(2, &radial2p2);
        radial2VSParams->appendArrayAccess(3, &radial2p3);

        // r2Var = 2 * (r2Parm[2] * varCoord.x - r2Param[3])
        const char* r2ParamName = radial2VSParams->getName().c_str();
        segments->fVSCode.appendf("\t%s = 2.0 *(%s * %s.x - %s);\n",
                                  *radial2VaryingVSName, radial2p2.c_str(),
                                  varyingVSName, radial2p3.c_str());
    }

    return radial2VSParams;
}
...
bool genRadial2GradientDegenerateCoordMapping(int stageNum,
    ...
    radial2FSParams->appendArrayAccess(4, &radial2p4);
...
void GrGLProgram::genStageCode(const GrGLContextInfo& gl,
    ...
    GrGLShaderVar* radial2VSParams = NULL;
    GrGLShaderVar* radial2FSParams = NULL;
    ...
    if (isRadialMapping((StageDesc::CoordMapping) desc.fCoordMapping)) {
        radial2VSParams = genRadialVS(stageNum, segments,
                                    locations,
                                    &radial2VaryingVSName,
                                    &radial2VaryingFSName,
                                    varyingVSName,
        ...
        radial2FSParams = &segments->fFSUnis.push_back();
        ...
        radial2_param_name_fs(stageNum, radial2FSParams->accessName());
        locations->fRadial2FSUni = kUseUniform;
    }
    ...
    case StageDesc::kRadial2Gradient_CoordMapping:
        complexCoord = genRadial2GradientCoordMapping(
                           stageNum, segments,
                           radial2VaryingFSName, radial2FSParams,
                           sampleCoords, fsCoordName,
                           varyingDims, coordDims);
