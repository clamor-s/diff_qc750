...
static void sk_init_source(j_decompress_ptr cinfo) {
    ...
    // Support for MPF files
    // 1. Be aware that there might be other images
    // Some bytes of the next image might have been read already
    src->current_offset = src->fNextImageOffset + src->fBytesInLeftoversBuffer;
    src->bytes_in_buffer = src->fBytesInLeftoversBuffer;
    // 2. Don't rewind for image after first image in file
    if (src->current_offset == 0) {
        src->fStream->rewind();
    }
    // 3. Copy bytes leftover from previous read buffer to the new buffer
    if (src->bytes_in_buffer > 0 &&
        src->bytes_in_buffer <= skjpeg_source_mgr::kBufferSize) {
        memcpy(src->fBuffer, src->fLeftoversBuffer, src->bytes_in_buffer);
    }
}
...
static void skmem_init_source(j_decompress_ptr cinfo) {
    SkDebugf("xxxxxxxxxxxxxx skmem_init_source called\n");
    skjpeg_source_mgr*  src = (skjpeg_source_mgr*)cinfo->src;
    ...
    // These skmem_ functions are not used, enable this code when need arises
    // and when it becomes possible to test it
#if 0
    // Support for MPF files
    // 1. Be aware that there might be other images, keep track of previous position
    if (src->fNextImageOffset != 0)
    {
        src->current_offset = src->fNextImageOffset;
    }
    if (src->fBytesInLeftoversBuffer)
    {
        src->bytes_in_buffer = src->fBytesInLeftoversBuffer;
    }
#endif
...
skjpeg_source_mgr::skjpeg_source_mgr(SkStream* stream, SkImageDecoder* decoder,
                                     ...
                                     size_t bytesInLeftoversBuffer /* = 0 */,
                                     char* leftoversBuffer /* = NULL */)
  : fStream(stream), fNextImageOffset(nextImageOffset),
    fBytesInLeftoversBuffer(bytesInLeftoversBuffer) {
    if (leftoversBuffer && fBytesInLeftoversBuffer > 0 &&
        fBytesInLeftoversBuffer <= kBufferSize ) {
        memcpy(fLeftoversBuffer, leftoversBuffer, fBytesInLeftoversBuffer);
    }
