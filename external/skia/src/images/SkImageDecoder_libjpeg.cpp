...
/// Helper class for parsing of Multi-Picture Format files
class SkMpfMetadata {
public:
    /// Constructor sets all entries to zero values
    SkMpfMetadata() {
        fImageCount = 0;
        fCurrentImage = 0;
        fNextImageOffset = 0;
        fLeftoverBytesInBuffer = 0;
        memset(fLeftoversBuffer, 0, SK_ARRAY_COUNT(fLeftoversBuffer));
    }
    /// Number of images in current MPF file (0 if normal JPEG)
    size_t fImageCount;
    /// Index of currently decoded image within the MPF file
    size_t fCurrentImage;
    /// Offsets of images from the start of MPF file
    SkTDArray<size_t> fImageOffsets;
    /// Individual image numbers for each image in the MPF file (0 if no number)
    SkTDArray<size_t> fIndividualNumbers;
    /// Offset of next image to be decoded from the start of the MPF file
    size_t fNextImageOffset;
    /// Number of bytes placed in buffer while reading previous image
    size_t fLeftoverBytesInBuffer;
    /// Bytes placed in buffer while reading previous image
    char fLeftoversBuffer[skjpeg_source_mgr::kBufferSize];
};
...
protected:
    ...
    bool decodeSingleImage(SkStream* stream, SkBitmap* bm, Mode mode);
    void processMpfMetadata(jpeg_decompress_struct& cinfo);
    bool prepareForNextImage(jpeg_decompress_struct& cinfo);
private:
    SkJPEGImageIndex *index;
    int imageWidth;
    int imageHeight;
    SkMpfMetadata fMpfMetadata;
};
...
bool SkJPEGImageDecoder::onDecode(SkStream* stream, SkBitmap* bm, Mode mode) {
    bool status = decodeSingleImage(stream, bm, mode);
    if (status && fMpfMetadata.fImageCount >= 2 &&
        SkImageDecoder::kDecodePixels_Mode == mode) {
        SkBitmap compoundImage;
        // Use references for more compact code
        const size_t& imageCount = fMpfMetadata.fImageCount;
        size_t& currentImage = fMpfMetadata.fCurrentImage;

        // Prepare new bitmap objects and decode remaining images
        SkTDArray<SkBitmap*> bitmaps;
        bitmaps.insert(0, imageCount);
        bitmaps[0] = bm;
        for (currentImage = 1; currentImage < imageCount; currentImage++) {
            bitmaps[currentImage] = SkNEW(SkBitmap);
            status = decodeSingleImage(stream, bitmaps[currentImage], mode);
            if (!status) {
                break;
            }
        }

        // Proceed only if all images were decoded sucessfuly
        if (status) {
            // Determine aggregate image size of the image set.
            // This is primitive approach that doesn't take into account what
            // the MPF image represents (disparity, etc.). It will just tile all
            // images into one big image. For most image types this is fine.
            int totalWidth = 0;
            int maxHeight = 0;
            for (size_t imageIndex = 0; imageIndex < imageCount; imageIndex++) {
                if (fMpfMetadata.fIndividualNumbers[imageIndex] != 0) {
                    totalWidth += bitmaps[imageIndex]->width();
                    if (bitmaps[imageIndex]->height() > maxHeight) {
                        maxHeight = bitmaps[imageIndex]->height();
                    }
                }
            }
            // Prepare the compound image
            compoundImage.setConfig(bm->config(), totalWidth, maxHeight);
            if (!this->allocPixelRef(&compoundImage, NULL)) {
                status = false;
            }
        }

        // Proceed only if resultant bitmap was successfully allocated
        if(status) {
            // Tile all images together
            SkCanvas merger(compoundImage);
            int drawnWidth = 0;
            for (size_t imageIndex = 0; imageIndex < imageCount; imageIndex++) {
                if (fMpfMetadata.fIndividualNumbers[imageIndex] != 0) {
                    merger.drawBitmap(*bitmaps[imageIndex], drawnWidth, 0);
                    drawnWidth += bitmaps[imageIndex]->width();
                }
            }
            // Swap compound image with the provided image
            compoundImage.swap(*bm);
        }

        // Clean up only images allocated by this block, don't touch bm object
        for (size_t imageIndex = 1; imageIndex < imageCount; imageIndex++) {
            SkDELETE(bitmaps[imageIndex]);
        }
    }
    return status;
}

void SkJPEGImageDecoder::processMpfMetadata(jpeg_decompress_struct& cinfo) {
    // If marker has been successfully decoded, copy MPF metadata (if any),
    // but do it in detail only for the first image in MPF file
    if (cinfo.saw_MPF_marker && cinfo.mpf_info &&
        fMpfMetadata.fCurrentImage == 0) {
        fMpfMetadata.fImageCount = cinfo.mpf_info->number_of_images;
        fMpfMetadata.fImageOffsets.reset();
        fMpfMetadata.fImageOffsets.append(fMpfMetadata.fImageCount);
        for (size_t i = 0; i < fMpfMetadata.fImageCount; i++) {
            fMpfMetadata.fImageOffsets[i] =
                cinfo.mpf_info->image_info[i].image_data_offset;
        }
        fMpfMetadata.fIndividualNumbers.reset();
        fMpfMetadata.fIndividualNumbers.append(fMpfMetadata.fImageCount);

        // Detect whether this is a Baseline file
        // (This is not needed with correct Large Thumbnails processing,
        // but speeds up the code as decoding of Large Thumbnails gets skipped)
        // The const value is taken from external/jpeg/jdmarker.c
        /// @todo On next libjpeg change, move the type enum into a .h file
        /// and remove this const line from here.
        const unsigned MPF_TC_BASELINE = 0x030000;
        if (fMpfMetadata.fImageCount > 0 &&
            MPF_TC_BASELINE == cinfo.mpf_info->image_info[0].mp_type_code) {
            // If this is a Baseline file, set image count to 1
            // so no decoding of subsequent images is done.
            // Standard requires the Primary image to be first in the file,
            // hence the correct image will be returned after decoding.
            fMpfMetadata.fImageCount = 1;
        }
    }
    // For all images in MPF file, extract image-specific details
    // The individual numbers array will be empty for non-MPF images
    if (fMpfMetadata.fIndividualNumbers.count() > 0) {
        // Currently only use Individual Image Number (where it exists)
        if (cinfo.mpf_info) {
            fMpfMetadata.fIndividualNumbers[fMpfMetadata.fCurrentImage] =
                cinfo.mpf_info->image_info[0].individual_image_number;
        } else {
            fMpfMetadata.fIndividualNumbers[fMpfMetadata.fCurrentImage] = 0;
        }
    }
}

bool SkJPEGImageDecoder::prepareForNextImage(jpeg_decompress_struct& cinfo) {
    bool prepared = false;
    /* Skip only for multiple images, but don't skip for last image */
    const size_t& currentImage = fMpfMetadata.fCurrentImage;
    if (fMpfMetadata.fImageCount > 0 &&
        currentImage < fMpfMetadata.fImageCount - 1) {
        fMpfMetadata.fNextImageOffset =
            fMpfMetadata.fImageOffsets[currentImage + 1];
        size_t currentStreamOffset =
            cinfo.src->current_offset - cinfo.src->bytes_in_buffer;
        size_t dataToSkip =
            fMpfMetadata.fNextImageOffset - currentStreamOffset;
        cinfo.src->skip_input_data(&cinfo, dataToSkip);
        fMpfMetadata.fLeftoverBytesInBuffer = cinfo.src->bytes_in_buffer;
        memcpy(fMpfMetadata.fLeftoversBuffer,
            (char*) cinfo.src->next_input_byte, cinfo.src->bytes_in_buffer);
...
bool SkJPEGImageDecoder::decodeSingleImage(SkStream* stream, SkBitmap* bm,
                                           Mode mode) {
#ifdef TIME_DECODE
    ...
    skjpeg_source_mgr       sk_stream(stream, this, false,
                                      fMpfMetadata.fNextImageOffset,
                                      fMpfMetadata.fLeftoverBytesInBuffer,
                                      fMpfMetadata.fLeftoversBuffer);
    ...
    */
                ...
                cinfo.out_color_space == JCS_RGB_565)))
    {
        ...
        jpeg_finish_decompress(&cinfo);
//      SkDebugf("-- [shortcut code] bm size %d [%d %d] %d\n", bm->getSize(), bm->width(), bm->height(), bm->config());
        prepareForNextImage(cinfo);
        return true;
    }
#endif
