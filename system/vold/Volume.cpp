...
int Volume::mountVol() {
    ...
    const char* app2SdStorage = getenv("APP2SD_STORAGE");
    ...
    if(!strcmp(enableApp2Sd2, "true")){
        isApp2Sd2Sd = app2SdStorage && !strcmp(getMountpoint(), app2SdStorage);
    }

    SLOGD("mountVol primaryStorage=%d app2SdStorage=%d\n",primaryStorage,isApp2Sd2Sd);
    ...
    for (i = 0; i < n; i++) {
        ...
        if (primaryStorage) {
            ...
            gid = AID_SDCARD_RW;//AID_MEDIA_RW;
        }
        if (Fat::doMount(devicePath, "/mnt/secure/staging", false, false, false,
                AID_SYSTEM, gid, 0702, true) &&
            Ntfs::doMount(devicePath, "/mnt/secure/staging", false, false, false,
                AID_SYSTEM, gid, 0702, true)) {
        ...
        if(isApp2Sd2Sd){
            char filename[255];
            snprintf(filename, sizeof(filename), "%s/Android", SEC_STGDIR);
            if (mkdir(filename, 0777)) {
                SLOGE("fail to create Android (%s)", strerror(errno));
            }
            snprintf(filename, sizeof(filename), "%s/Android/obb", SEC_STGDIR);
            if (mkdir(filename, 0777)) {
                SLOGE("fail to create Android/obb (%s)", strerror(errno));
            }
        }

        // only create android_secure on primary storage
        if ((primaryStorage || isApp2Sd2Sd) && createBindMounts()) {
            if(!isApp2Sd2Sd){
