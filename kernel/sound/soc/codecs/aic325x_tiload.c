			   ...
			   size_t count, loff_t * offset)
{
	...
	dprintk("TiLoad DRIVER : %s\n", __FUNCTION__);
	if (count > 132) {
		printk("Max 132 bytes can be read\n");
		count = 132;
	}
	...
	mutex_lock(&aic325x_private->io_lock);
	if (i2c_master_send(i2c, &reg_addr, 1) != 1) {
		mutex_unlock(&aic325x_private->io_lock);
			    ...
			    size_t count, loff_t * offset)
{
	...
	if (wr_data[0] == 0) {
		...
		aic325x_private->page_no = wr_data[1];
		curpage = aic325x_private->page_no;
		mutex_unlock(&aic325x_private->io_lock);
		return count;
	}
	mutex_lock(&aic325x_private->io_lock);
	if(curpage != aic325x_private->page_no)
	{
		aic325x_change_page(aic325x_codec, curpage);
		aic325x_private->page_no = curpage;
	}
	ret = i2c_master_send(i2c, wr_data, count);
	mutex_unlock(&aic325x_private->io_lock);
...
int aic325x_driver_init(struct snd_soc_codec *codec)
{
	...
	printk("Registered aic325x TiLoad driver, Major number: %d \n",
	       aic325x_major);
	device_create(tiload_class, NULL, dev, NULL, DEVICE_NAME, 0);
	return 0;
}
