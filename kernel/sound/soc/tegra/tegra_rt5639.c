...
static int tegra_rt5639_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (gpio_is_valid(pdata->gpio_hp_det)) {
		tegra_rt5639_hp_jack_gpio.invert =1; 
...
static __devinit int tegra_rt5639_driver_probe(struct platform_device *pdev)
{
	...
	if (!machine) {
		...
		ret = gpio_request(pdata->gpio_ldo1_en, "power_en");
		if (ret) {
			dev_err(card->dev, "cannot get ext_mic_en gpio\n");
			return ret;
		}
		machine->gpio_requested |= GPIO_RST;

		/* Disable ext mic; enable signal is active-low */
		ret=gpio_direction_output(pdata->gpio_ldo1_en, 1);
		printk("enable power for audio codec");
			msleep(200);
	}
	machine->pdata = pdata;
	...
	ret = switch_dev_register(&tegra_rt5639_headset_switch);
	if (ret < 0)
		goto err_fini_utils;
#endif
...
#ifdef CONFIG_SWITCH
	switch_dev_unregister(&tegra_rt5639_headset_switch);
err_fini_utils:
#endif
