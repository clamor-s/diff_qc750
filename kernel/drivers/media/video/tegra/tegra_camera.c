...
{
    dump_address(0x60006000, 0x1000);
    dump_address(0x6000d400, 0x100);
    dump_address(0x6000d700, 0x100);
    dump_address(0x70000000, 0x4000);
}
#endif
...
{
	if(machine_is_nabi2() || machine_is_nabi2_3d() ||  machine_is_n710() || machine_is_itq700() || machine_is_itq701() || machine_is_mm3201() || machine_is_wikipad()){
	...
	else if(machine_is_nabi2_xd() || machine_is_qc750() || machine_is_nabi_2s() 
			|| machine_is_n1010() || machine_is_n750() || machine_is_birch() ||machine_is_ns_14t004() )
	{
		clk_enable(dev->vi_clk);
		clk_enable(dev->vi_sensor_clk);
		clk_enable(dev->csus_clk);
...
#ifdef CONFIG_ARCH_TEGRA_2x_SOC
	dev_info(dev->dev, "%s CONFIG_ARCH_TEGRA_2x_SOC get vcsi\n", __func__);
	dev->reg = regulator_get(&pdev->dev, "vcsi");
#else
	dev_info(dev->dev, "%s CONFIG_ARCH_TEGRA_2x_SOC  get avdd_dsi_csi\n", __func__);
