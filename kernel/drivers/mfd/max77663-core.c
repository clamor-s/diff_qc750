...
#define MAX77663_REG_CFG_GLBL1		0x00
...
#define GLBL_CFG_MBLPD_MASK		(1 << 6)
#define GLBL_CFG_LBDAC_MASK		(1 << 2)
...
int max77663_acok_state(void)
{
	...
	if (!chip)
		return 0;
	ret = max77663_read(chip->dev, 0x15, &val, 1, 0);
...
EXPORT_SYMBOL(max77663_acok_state);
...
static int max77663_sleep(struct max77663_chip *chip, bool on)
{
	...
	int ret_emp = 0;
	u8 val;
	if (chip->pdata->flags & SLP_MONITORS_ENABLE) {
		
		ret = max77663_set_bits(chip->dev, MAX77663_REG_CFG_GLBL1,
					GLBL_CFG_MBLPD_MASK|GLBL_CFG_LBDAC_MASK,
					on ? GLBL_CFG_MBLPD_MASK|GLBL_CFG_LBDAC_MASK : 0, 0);
		if (ret < 0){
			pr_err("%s: 0x%02x set MBLPD fail!\n",__func__,MAX77663_REG_CFG_GLBL1);
		}
	}
	...
	if (chip->pdata->flags & SLP_LPM_ENABLE) {
		...
		pr_err("%s: entry pmu low power mode!\n",__func__);
		if (ret < 0)
			return ret;
	}
