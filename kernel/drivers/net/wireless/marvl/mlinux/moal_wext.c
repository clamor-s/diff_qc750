              ...
              struct iw_point *dwrq, char *extra)
{
    ...
    if (strncmp(buf, "RSSI", strlen("RSSI")) == 0) {
	...
	} else if( strncmp( buf, "SIGNAL_POLL", strlen( "SIGNAL_POLL" )) == 0 ) {
		/*
		 * added by suxin @20111230
		 * to support SIGNAL_POLL command
		 * now, ONLY need rssi and linkspeed fields
		 * result is in the format "RSSI=%d\nLINKSPEED=%d\nNOISE=%d\nFREQUENCY=%u\n"
		 */
		__u32 frequency = 0;
		int current_signal = 0;
		int current_txrate = 0;
		int current_noise = 0;

		if( MLAN_STATUS_SUCCESS == woal_get_bss_info( priv, MOAL_IOCTL_WAIT, &bss_info ))
		{
			if( bss_info.media_connected )
			{
				if( MLAN_STATUS_SUCCESS == woal_get_signal_info( priv, MOAL_IOCTL_WAIT, &signal ))
				{
					current_signal = signal.bcn_rssi_avg + 1;
				}
			}
		}
		...
		if( MLAN_STATUS_SUCCESS == woal_get_data_rate( priv, &rate ))
		{
			current_txrate = (int)( rate.param.rate_cfg.rate * 500000 / 1000000 ) + 1;
		}

		len = sprintf( buf, "RSSI=%d\nLINKSPEED=%d\nNOISE=%d\nFREQUENCY=%u\n",
						current_signal, current_txrate, current_noise, frequency );
