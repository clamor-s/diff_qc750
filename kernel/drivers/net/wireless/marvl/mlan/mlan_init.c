 ...
 */
 //#define KEENHI_WIFI
 #ifdef KEENHI_WIFI
	extern char *mlan_scan_table_mem;
#endif
...
#ifdef KEENHI_WIFI	
	//PRINTM(MERROR, "------------------->>buf_size = %d\n", buf_size);
	if((mlan_scan_table_mem != MNULL) && (buf_size <= 50000))
	{
		//PRINTM(MERROR, "------------------->>use define mem!!\n");	
		pmadapter->pscan_table = MNULL;
		ptemp_scan_table = mlan_scan_table_mem;
	} else {
#endif
    ret =
...
#ifdef KEENHI_WIFI
		if( mlan_scan_table_mem != MNULL)
		{
			//PRINTM(MERROR, "------------->>use define mem no mem free\n");
		} else {
			//PRINTM(MERROR, "------------->>use define mem but no mem malloc, so mem free\n");
