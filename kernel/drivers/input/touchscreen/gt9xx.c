...
/****************************************************************
Function:
auto detect which tp is used

   			PH4(ID1)      PH5(ID2)     PH6(ID3)    PH7(ID4)
秋田微:        1                   1                 0              1
瑞视  :            1                  1                  0              0 //default

****************************************************************/
static int gt9xx_TPfacture_detect(void)
{
	int  touch_detect = gpio_get_value(('h'-'a')*8+4);
	int touch_detect1 =  gpio_get_value(('h'-'a')*8+5);
	int touch_detect2=  gpio_get_value(('h'-'a')*8+6);
	int touch_detect3= gpio_get_value(('h'-'a')*8+7);
	int status =0;
	
	status = (touch_detect<<3) |(touch_detect1 <<2) |(touch_detect2<<1) |(touch_detect3) ;
	
	return (status &0x0f);//清除高4位
}
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    #endif
        GTP_DEBUG("I2C gtp_i2c_read communication timeout, resetting chip...");
        gtp_reset_guitar(client, 10);
    }
    return ret;
}

//larry add for fix a bug :device will die if no tp
s32 gtp_i2c_read_test(struct i2c_client *client, u8 *buf, s32 len)
{
    struct i2c_msg msgs[2];
    s32 ret=-1;
    s32 retries = 0;
...
s32 gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len)
{
    ...
    while(retries < 5)
    {
		...
		//GTP_DEBUG("i2c_transfer(client->adapter, &msg, 1)= ret =%d ",ret);
		//GTP_DEBUG("i2c_transfer(client->adapter, &msg, 1)= retries =%d ",retries);
        if (ret == 1)break;
        retries++;
    }
    if((retries >= 5))
    ...
    #endif
		//GTP_DEBUG("I2C gtp_i2c_write communication timeout, resetting chip flags =%d",msg.flags);
		//GTP_DEBUG("I2C gtp_i2c_write communication timeout, resetting chip addr = %d",msg.addr);
        //GTP_DEBUG("I2C gtp_i2c_write communication timeout, resetting chip len = %d",msg.len);
		//GTP_DEBUG("I2C gtp_i2c_write communication timeout, resetting chip buf = $s",msg.buf);
        //GTP_DEBUG("I2C gtp_i2c_write communication timeout, resetting chip...");		
        gtp_reset_guitar(client, 10);
    }
    return ret;
}
...
{
    GTP_DEBUG_FUNC();
    GTP_DEBUG("gtp_reset_guitar...");
    GTP_DEBUG("begin select I2C slave addr...");
    ...
    GTP_DEBUG("end select I2C slave addr...");
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    u8 sensor_id = 0;
	int TP_switch =0;
    u8 cfg_info_group1_0[] = CTP_CFG_GROUP1_0;	
...
//----------------------------------------------------------------------------------------------------------
//defualt tp factary is 瑞视
	TP_switch =gt9xx_TPfacture_detect();
    GTP_INFO("gt9xx_TPfacture_detect --> TP_switch = %x",TP_switch); 

	if(TP_switch == 0x0d)
		{
		send_cfg_buf[0] = cfg_info_group1_0;
		cfg_info_len[0] =CFG_GROUP_LEN(cfg_info_group1_0);
...
//    s32 ret = 0;
    ...
    while(retry++ < 5)
    {
        ret = gtp_i2c_read_test(client, test, 3);
        if (ret > 0)
        {
            return ret;
        }
