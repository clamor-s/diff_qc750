...
static int kionix_acc_get_acceleration_data(struct kionix_accel_driver *acceld)
{
	struct { union {
...
static void kionix_accel_grp2_report_values(struct kionix_accel_driver *acceld)
{
	int i;
	#ifdef __KIONIX_ACC_DEV__
	for(i = 0; i < SENSOR_DATA_SIZE; i++)
		acceld->accel_data[i] -= kionix_offset.v[i];
	#endif
	if(atomic_read(&acceld->accel_input_event) > 0) {
			input_report_abs(acceld->input_dev, ABS_X, acceld->accel_data[acceld->axis_map_y]);
			input_report_abs(acceld->input_dev, ABS_Y, acceld->accel_data[acceld->axis_map_x]);
			input_report_abs(acceld->input_dev, ABS_Z, acceld->accel_data[acceld->axis_map_z]);
			input_sync(acceld->input_dev);
	}
}
	...
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			if (err < 0) {
				...
				kionix_accel_grp2_report_values(acceld);
				write_unlock(&acceld->rwlock_accel_data);
			}
		}
		else
...
static void kionix_acc_read_avg(kionix_raw_data *avg_p )
{
	...
	for(i = 0; i < AVG_NUM; i++) 
	{      
		err = kionix_acc_get_acceleration_data(flash_priv);
		if (err < 0)
...
static int kionix_acc_dev_init(struct kionix_acc_data *acc)
{
