...
#define BT_WAKEUP_PIN	161
#define BT_SHUTDOWN_PIN	160
...
static long download_firmware(struct kim_data_s *kim_gdata)
{
	...
	while (len > 0 && ptr) {
		...
		case ACTION_SEND_COMMAND:	/* action send */
			//printk("% s , %d",__func__,__LINE__); // test 
			pr_debug("S");
...
long st_kim_start(void *kim_data)
{
	...
	do {
		...
		gpio_set_value(BT_WAKEUP_PIN, GPIO_HIGH);
...
long st_kim_stop(void *kim_data)
{
	...
	mdelay(1);
	gpio_set_value(kim_gdata->nshutdown, GPIO_LOW);
	gpio_set_value(BT_WAKEUP_PIN, GPIO_LOW);
...
static int kim_probe(struct platform_device *pdev)
{
	...
	gpio_request(BT_WAKEUP_PIN, "kim1");
	gpio_direction_output(BT_WAKEUP_PIN, 0);
