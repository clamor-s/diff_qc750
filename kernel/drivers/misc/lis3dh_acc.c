...
#define __LIS3DH_ACC_DEV__

#ifdef __LIS3DH_ACC_DEV__
...
static int list3dh_major = 240;
static int list3dh_minor = 0;
static struct cdev list3dh_cdev;
static struct class *list3dh_class = NULL;
static dev_t list3dh_dev;
static char OffsetFileName[] = "/forever/misc/gsensor/offset.txt";
static int Device_First_Time_Opened_flag=1;
//#define LIS3DH_DEBUG_DATA
#ifdef LIS3DH_DEBUG_DATA
...
static struct lis3dh_acc_data *flash_priv;
...
static void lis3dh_acc_input_work_func(struct work_struct *work)
{
	...
	mutex_lock(&acc->lock);
	err = lis3dh_acc_get_acceleration_data(acc, xyz,1);
	if (err < 0)
...
static void lis3dh_acc_read_avg(raw_data *avg_p )
{
	...
	for(i = 0; i < AVG_NUM; i++) 
	{      
		err = lis3dh_acc_get_acceleration_data(flash_priv, xyz,0);
		if (err < 0)
		dev_err(&flash_priv->client->dev, "get_acceleration_data failed\n");
...
/* calc delta offset */
static int lis3dh_acc_calculate_offset(int gAxis,raw_data avg)
{
	switch(gAxis)
	{
...
void lis3dh_acc_calibrate(int side)
{	
	raw_data avg;
	IN_FUNC_MSG;
	// get acceleration average reading
	lis3dh_acc_read_avg(&avg);
	// calculate and set the offset
	lis3dh_acc_calculate_offset(side, avg); 
}

void lis3dh_acc_set_offset(int val[3])
{
	int i;
	IN_FUNC_MSG;
	for(i = 0; i < SENSOR_DATA_SIZE; ++i)
		offset.v[i] = (s32) val[i];
}
static void lis3dh_acc_write_offset_to_file(void)
{
	char data[18];
	unsigned int orgfs;
	long lfile=-1;

	sprintf(data,"%6d %6d %6d",offset.u.x,offset.u.y,offset.u.z);
...
static void lis3dh_acc_read_offset_from_file(void)
{
	unsigned int orgfs;
	char data[18];
	long lfile=-1;
...
static int lis3dh_open(struct inode *inode, struct file *filp)
{
	IN_FUNC_MSG;
		...
		if(Device_First_Time_Opened_flag)
	{
	      ...
	      lis3dh_acc_read_offset_from_file();
	}
	printk("device_open: open_from_Android\n");
	return 0; 
}

static ssize_t lis3dh_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	IN_FUNC_MSG;
...
static ssize_t lis3dh_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
...
static long lis3dh_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err = 0, ret = 0, i;
	...
	switch(cmd) 
	{
		...
		case SENSOR_CALIBRATION:
			...
			lis3dh_acc_calibrate(intBuf[0]);
			// save file 2011-11-30	
			lis3dh_acc_write_offset_to_file();
...
static int lis3dh_close(struct inode *inode, struct file *filp)
{
	IN_FUNC_MSG;
...
struct file_operations list3dh_fops = 
{
	.owner = THIS_MODULE,
	.read = lis3dh_read,
	.write = lis3dh_write,
	...
	.open = lis3dh_open,
	.release = lis3dh_close,
};

static int lis3dh_acc_dev_init(struct lis3dh_acc_data *acc)
{
	dev_t dev = MKDEV(list3dh_major, 0);
	int alloc_ret = 0;
	int major;
	int cdev_err = 0;
	int input_err = 0;
	...
	alloc_ret = alloc_chrdev_region(&dev, 0, 1, LIS3DH_ACC_DEV_NAME);
	if(alloc_ret) {
		goto error;
	}
	major = MAJOR(dev);
	...
	cdev_init(&list3dh_cdev, &list3dh_fops);
	list3dh_cdev.owner = THIS_MODULE;
	list3dh_cdev.ops = &list3dh_fops;
	cdev_err = cdev_add(&list3dh_cdev, MKDEV(list3dh_major, list3dh_minor), 1);
	if(cdev_err) {
		goto error;
	}

	// register class
	list3dh_class = class_create(THIS_MODULE, LIS3DH_ACC_CLASS_NAME);
	if(IS_ERR(list3dh_class)) {
		goto error;
	}
	list3dh_dev = MKDEV(list3dh_major, list3dh_minor);
	class_dev = device_create(list3dh_class, NULL, list3dh_dev, NULL, LIS3DH_ACC_DEV_NAME);

	printk("============================================================\n");
	printk("list3dh driver loaded\n");
	printk("============================================================\n");	
