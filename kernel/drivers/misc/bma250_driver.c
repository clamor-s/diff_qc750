...
int bma250_soft_reset(void)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_EN_SOFT_RESET__REG,
						    &data, C_One_U8X);
	}
	return comres;
}
...
int bma250_write_reg(unsigned char addr, unsigned char *data, unsigned char len)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr, addr,
						    data, len);
	}
	return comres;
}
...
int bma250_read_reg(unsigned char addr, unsigned char *data, unsigned char len)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr, addr,
						   data, len);
	}
	return comres;
}
...
unsigned char bma250_set_mode(unsigned char Mode)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		if (Mode < C_Three_U8X) {
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
...
unsigned char bma250_set_range(unsigned char Range)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		if (Range < C_Four_U8X) {
			...
			case C_Zero_U8X:
				data1 =
				    bma250_SET_BITSLICE(data1, bma250_RANGE_SEL,
							C_Zero_U8X);
				break;
			case C_One_U8X:
				data1 =
...
int bma250_read_accel_xyz(bma250acc_t * acc)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_ACC_X_LSB__REG, data,
						   6);

		acc->x =
		    bma250_GET_BITSLICE(data[0],
					bma250_ACC_X_LSB) |
...
int bma250_reset_interrupt(void)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_INT_RESET_LATCHED__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_INT_RESET_LATCHED,
					C_One_U8X);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_INT_RESET_LATCHED__REG,
...
int bma250_set_enable_slope_interrupt(unsigned char slope)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_SLOPE_X_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_SLOPE_X_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_SLOPE_X_INT__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_SLOPE_Y_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_SLOPE_Y_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_SLOPE_Y_INT__REG,
		...
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_SLOPE_Z_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_SLOPE_Z_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_SLOPE_Z_INT__REG,
		...
		case 3:
			...
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_SLOPE_XYZ_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_SLOPE_XYZ_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_SLOPE_XYZ_INT__REG,
...
int bma250_get_enable_slope_interrupt(unsigned char param,
...
int bma250_set_enable_tap_interrupt(unsigned char tapinterrupt)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_DOUBLE_TAP_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_DOUBLE_TAP_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_DOUBLE_TAP_INT__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_SINGLE_TAP_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_SINGLE_TAP_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_SINGLE_TAP_INT__REG,
		...
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_ORIENT_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_ORIENT_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_ORIENT_INT__REG,
		...
		case 3:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_FLAT_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_FLAT_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_FLAT_INT__REG,
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_DOUBLE_TAP_INT);
			*status = data;
			break;
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_SINGLE_TAP_INT);
			*status = data;
			break;
		case 2:
			comres =
...
int bma250_set_enable_high_g_interrupt(unsigned char highinterrupt)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_HIGHG_X_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_HIGHG_X_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_HIGHG_X_INT__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_HIGHG_Y_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_HIGHG_Y_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_HIGHG_Y_INT__REG,
		...
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_HIGHG_Z_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_HIGHG_Z_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_HIGHG_Z_INT__REG,
		...
		case 3:
			...
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_HIGHG_XYZ_INT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_HIGHG_XYZ_INT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_HIGHG_XYZ_INT__REG,
...
int bma250_get_enable_high_g_interrupt(unsigned char param,
				       unsigned char *status)
{
	int comres = C_Zero_U8X;
	unsigned char data;
		...
		case 3:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_HIGHG_XYZ_INT);
			*status = data;
			break;
		default:
			break;
...
int bma250_set_int1_pad_sel(unsigned char int1sel)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT1_PAD_LOWG__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT1_PAD_LOWG,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT1_PAD_LOWG__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT1_PAD_HIGHG__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT1_PAD_HIGHG,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT1_PAD_HIGHG__REG,
		...
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT1_PAD_SLOPE__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT1_PAD_SLOPE,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT1_PAD_SLOPE__REG,
		...
		case 3:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT1_PAD_DB_TAP__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT1_PAD_DB_TAP,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT1_PAD_DB_TAP__REG,
		...
		case 4:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT1_PAD_SNG_TAP__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_EN_INT1_PAD_SNG_TAP,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT1_PAD_SNG_TAP__REG,
		...
		case 5:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT1_PAD_ORIENT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT1_PAD_ORIENT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT1_PAD_ORIENT__REG,
		...
		case 6:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT1_PAD_FLAT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT1_PAD_FLAT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT1_PAD_FLAT__REG,
...
int bma250_get_int1_pad_sel(unsigned char param, unsigned char *status)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_INT1_PAD_SEL_REG,
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_INT1_PAD_LOWG);
			*status = data;
			break;
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_INT1_PAD_HIGHG);
			*status = data;
			break;
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_INT1_PAD_SLOPE);
			*status = data;
			break;
		case 3:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data,
						bma250_EN_INT1_PAD_DB_TAP);
			*status = data;
			break;
		case 4:
			comres =
		...
		case 6:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_INT1_PAD_FLAT);
			*status = data;
			break;
		default:
			break;
...
int bma250_set_int_data_sel(unsigned char intsel)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT1_PAD_NEWDATA__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_EN_INT1_PAD_NEWDATA,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT1_PAD_NEWDATA__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT2_PAD_NEWDATA__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_EN_INT2_PAD_NEWDATA,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT2_PAD_NEWDATA__REG,
...
int bma250_set_int2_pad_sel(unsigned char int2sel)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT2_PAD_LOWG__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT2_PAD_LOWG,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT2_PAD_LOWG__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT2_PAD_HIGHG__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT2_PAD_HIGHG,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT2_PAD_HIGHG__REG,
		...
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT2_PAD_SLOPE__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT2_PAD_SLOPE,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT2_PAD_SLOPE__REG,
		...
		case 3:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT2_PAD_DB_TAP__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT2_PAD_DB_TAP,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT2_PAD_DB_TAP__REG,
		...
		case 4:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT2_PAD_SNG_TAP__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_EN_INT2_PAD_SNG_TAP,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT2_PAD_SNG_TAP__REG,
		...
		case 5:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT2_PAD_ORIENT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT2_PAD_ORIENT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT2_PAD_ORIENT__REG,
		...
		case 6:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_INT2_PAD_FLAT__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_INT2_PAD_FLAT,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_INT2_PAD_FLAT__REG,
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_INT2_PAD_LOWG);
			*status = data;
			break;
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_INT2_PAD_HIGHG);
			*status = data;
			break;
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_INT2_PAD_SLOPE);
			*status = data;
			break;
		case 3:
			comres =
		...
		case 6:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   ...
							   &data, C_One_U8X);
			data =
			    bma250_GET_BITSLICE(data, bma250_EN_INT2_PAD_FLAT);
			*status = data;
			break;
		default:
			break;
...
int bma250_set_int_src(unsigned char intsrc)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_UNFILT_INT_SRC_LOWG__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_UNFILT_INT_SRC_LOWG,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_UNFILT_INT_SRC_LOWG__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_UNFILT_INT_SRC_HIGHG__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_UNFILT_INT_SRC_HIGHG,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_UNFILT_INT_SRC_HIGHG__REG,
		...
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_UNFILT_INT_SRC_SLOPE__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_UNFILT_INT_SRC_SLOPE,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_UNFILT_INT_SRC_SLOPE__REG,
		...
		case 3:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_UNFILT_INT_SRC_TAP__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_UNFILT_INT_SRC_TAP,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_UNFILT_INT_SRC_TAP__REG,
		...
		case 4:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_UNFILT_INT_SRC_DATA__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_UNFILT_INT_SRC_DATA,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_UNFILT_INT_SRC_DATA__REG,
...
int bma250_set_int_set(unsigned char intset, unsigned char state)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_INT1_PAD_ACTIVE_LEVEL__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_INT1_PAD_ACTIVE_LEVEL,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_INT1_PAD_ACTIVE_LEVEL__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_INT1_PAD_OUTPUT_TYPE__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_INT1_PAD_OUTPUT_TYPE,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_INT1_PAD_OUTPUT_TYPE__REG,
		...
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_INT2_PAD_ACTIVE_LEVEL__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_INT2_PAD_ACTIVE_LEVEL,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_INT2_PAD_ACTIVE_LEVEL__REG,
		...
		case 3:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_INT2_PAD_OUTPUT_TYPE__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data,
						bma250_INT2_PAD_OUTPUT_TYPE,
						state);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_INT2_PAD_OUTPUT_TYPE__REG,
...
int bma250_set_tap_quiet_duration(unsigned char duration)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_TAP_QUIET_DURN__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_TAP_QUIET_DURN, duration);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_TAP_QUIET_DURN__REG,
						    &data, C_One_U8X);
...
int bma250_set_orient_hyst(unsigned char orienthyst)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_ORIENT_HYST__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_ORIENT_HYST, orienthyst);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_ORIENT_HYST__REG,
						    &data, C_One_U8X);
...
int bma250_set_flat_hold_time(unsigned char holdtime)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_FLAT_HOLD_TIME__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_FLAT_HOLD_TIME, holdtime);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_FLAT_HOLD_TIME__REG,
						    &data, C_One_U8X);
...
int bma250_set_ee_w(unsigned char eew)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_UNLOCK_EE_WRITE_SETTING__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_UNLOCK_EE_WRITE_SETTING,
					eew);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_UNLOCK_EE_WRITE_SETTING__REG,
...
int bma250_get_ee_w(unsigned char *eew)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_EE_WRITE_SETTING_S__REG,
						   eew, C_One_U8X);
...
int bma250_set_ee_prog_trig(unsigned char eeprog)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_START_EE_WRITE_SETTING__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_START_EE_WRITE_SETTING,
					eeprog);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_START_EE_WRITE_SETTING__REG,
...
int bma250_set_i2c_wdt_timer(unsigned char timedly)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_I2C_WATCHDOG_PERIOD__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_I2C_WATCHDOG_PERIOD,
					timedly);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_I2C_WATCHDOG_PERIOD__REG,
...
int bma250_set_hp_en(unsigned char param, unsigned char hpval)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case 0:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_SLOW_COMP_X__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_SLOW_COMP_X,
						hpval);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_SLOW_COMP_X__REG,
		...
		case 1:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_SLOW_COMP_Y__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_SLOW_COMP_Y,
						hpval);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_SLOW_COMP_Y__REG,
		...
		case 2:
			comres =
			    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
							   bma250_EN_SLOW_COMP_Z__REG,
							   &data, C_One_U8X);
			data =
			    bma250_SET_BITSLICE(data, bma250_EN_SLOW_COMP_Z,
						hpval);
			comres =
			    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
							    bma250_EN_SLOW_COMP_Z__REG,
...
int bma250_set_cal_trigger(unsigned char caltrigger)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_EN_FAST_COMP__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_EN_FAST_COMP, caltrigger);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_EN_FAST_COMP__REG,
						    &data, C_One_U8X);
...
int bma250_set_offset_reset(void)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_RESET_OFFSET_REGS__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_RESET_OFFSET_REGS,
					offsetreset);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_RESET_OFFSET_REGS__REG,
...
int bma250_set_offset_cutoff(unsigned char offsetcutoff)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   bma250_COMP_CUTOFF__REG,
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_COMP_CUTOFF, offsetcutoff);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_COMP_CUTOFF__REG,
						    &data, C_One_U8X);
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   ...
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_COMP_TARGET_OFFSET_X,
					offsettarget);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_COMP_TARGET_OFFSET_X__REG,
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   ...
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_COMP_TARGET_OFFSET_Y,
					offsettarget);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_COMP_TARGET_OFFSET_Y__REG,
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		comres =
		    p_bma250->bma250_BUS_READ_FUNC(p_bma250->dev_addr,
						   ...
						   &data, C_One_U8X);
		data =
		    bma250_SET_BITSLICE(data, bma250_COMP_TARGET_OFFSET_Z,
					offsettarget);
		comres =
		    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->dev_addr,
						    bma250_COMP_TARGET_OFFSET_Z__REG,
...
int bma250_set_Int_Enable(unsigned char InterruptType, unsigned char value)
{
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		case C_Zero_U8X:
			/* Low G Interrupt  */
			data2 =
			    bma250_SET_BITSLICE(data2, bma250_EN_LOWG_INT,
						value);
			break;
		case C_One_U8X:
			/* High G X Interrupt */
			/* EasyCASE - */
			data2 =
			    bma250_SET_BITSLICE(data2, bma250_EN_HIGHG_X_INT,
						value);
			break;
		case C_Two_U8X:
			/* High G Y Interrupt */
			/* EasyCASE - */
			data2 =
			    bma250_SET_BITSLICE(data2, bma250_EN_HIGHG_Y_INT,
						value);
			break;
		case C_Three_U8X:
			/* High G Z Interrupt */
			/* EasyCASE - */
			data2 =
			    bma250_SET_BITSLICE(data2, bma250_EN_HIGHG_Z_INT,
						value);
			break;
		case C_Four_U8X:
			/* New Data Interrupt  */
			/* EasyCASE - */
			data2 =
			    bma250_SET_BITSLICE(data2, bma250_EN_NEW_DATA_INT,
						value);
			break;
		case C_Five_U8X:
			/* Slope X Interrupt */
			/* EasyCASE - */
			data1 =
			    bma250_SET_BITSLICE(data1, bma250_EN_SLOPE_X_INT,
						value);
			break;
		case C_Six_U8X:
			/* Slope Y Interrupt */
			/* EasyCASE - */
			data1 =
			    bma250_SET_BITSLICE(data1, bma250_EN_SLOPE_Y_INT,
						value);
			break;
		case C_Seven_U8X:
			/* Slope Z Interrupt */
			/* EasyCASE - */
			data1 =
			    bma250_SET_BITSLICE(data1, bma250_EN_SLOPE_Z_INT,
						value);
			break;
		case C_Eight_U8X:
			...
			/* EasyCASE - */
			data1 =
			    bma250_SET_BITSLICE(data1, bma250_EN_SINGLE_TAP_INT,
						value);
			break;
		case C_Nine_U8X:
			...
			/* EasyCASE - */
			data1 =
			    bma250_SET_BITSLICE(data1, bma250_EN_DOUBLE_TAP_INT,
						value);
			break;
		case C_Ten_U8X:
			/* Orient Interrupt  */
			/* EasyCASE - */
			data1 =
			    bma250_SET_BITSLICE(data1, bma250_EN_ORIENT_INT,
						value);
			break;
		case C_Eleven_U8X:
			/* Flat Interrupt */
			/* EasyCASE - */
			data1 =
			    bma250_SET_BITSLICE(data1, bma250_EN_FLAT_INT,
						value);
			break;
		default:
			break;
	...
	if (p_bma250 == C_Zero_U8X) {
		...
		if (p_bma250->delay_msec == C_Zero_U8X) {
			...
			if (ee_busy) {
				...
				comres +=
				    p_bma250->bma250_BUS_WRITE_FUNC(p_bma250->
								    dev_addr,
								    addr, &data,
								    C_One_U8X);
...
static char bma250_i2c_write(unsigned char reg_addr, unsigned char *data,
			     unsigned char len);
static char bma250_i2c_read(unsigned char reg_addr, unsigned char *data,
			    unsigned char len);
...
static inline char bma250_i2c_write(unsigned char reg_addr, unsigned char *data,
				    unsigned char len)
{
	s32 dummy;
#ifndef BMA250_SMBUS
...
#ifdef BMA250_SMBUS
		dummy =
		    i2c_smbus_write_byte_data(bma250_client, reg_addr, *data);
#else
		buffer[0] = reg_addr;
		buffer[1] = *data;
...
static inline char bma250_i2c_read(unsigned char reg_addr, unsigned char *data,
				   unsigned char len)
{
	s32 dummy;
	if (bma250_client == NULL)	/*  No global client pointer?       */
...
static ssize_t bma250_read(struct file *file, char __user * buf, size_t count,
			   loff_t * offset)
{
	bma250acc_t acc;
	int ret;
...
static ssize_t bma250_write(struct file *file, const char __user * buf,
...
static int bma250_ioctl(struct inode *inode, struct file *file,
			unsigned int cmd, unsigned long arg)
{
	...
	case BMA250_READ_ACCEL_XYZ:
		...
		if (copy_to_user((bma250acc_t *) arg, (bma250acc_t *) data, 6)
		    != 0) {
#ifdef BMA250_DEBUG
			...
			const struct i2c_device_id *id)
{
	...
	data->IRQ = client->irq;
	err =
	    request_irq(data->IRQ, bma250_irq_handler, IRQF_TRIGGER_RISING,
			"bma250", &data->bma250);
	if (err) {
