...
struct early_suspend wikipad_otg_early_suspender;
static struct wake_lock mWakeLock;
extern int keenhi_is_xpad_otg(void);
static int keenhi_xpad_disable;

static void wikipad_otg_disable(struct tegra_otg_data *tegra, int disable)
{
	unsigned long val;
	struct otg_transceiver *otg = &tegra->otg;
	...
	if(disable)
	{
		if(keenhi_is_xpad_otg() && OTG_STATE_A_HOST == otg->state)
		{
			clk_enable(tegra->clk);
			val = otg_readl(tegra, USB_PHY_WAKEUP);
			clk_disable(tegra->clk);
			printk("%s-------tegra->int_status = 0x%lx, val = 0x%lx, otg->state = %d\n",__func__,tegra->int_status,val,otg->state);
			mutex_lock(&tegra->irq_work_mutex);
			tegra_stop_host(tegra);
			tegra_otg_notify_event(otg, USB_EVENT_NONE);
			mutex_unlock(&tegra->irq_work_mutex);
			wake_lock_timeout(&mWakeLock, msecs_to_jiffies(10000));
			keenhi_xpad_disable = 1;
			printk(">>remove otg<< tegra->int_status = 0x%lx\n",tegra->int_status);
		}
	}
	else
	{
		if(/*(OTG_STATE_A_HOST == otg->state) && */(keenhi_xpad_disable == 1))
		{
			clk_enable(tegra->clk);
			val = otg_readl(tegra, USB_PHY_WAKEUP);
			clk_disable(tegra->clk);
			...
			keenhi_xpad_disable = 0;
			printk("%s-------tegra->int_status = 0x%lx, val = 0x%lx, otg->state = %d\n",__func__,tegra->int_status,val,otg->state);
		}
	}
}

static void wikipad_otg_early_suspend(struct early_suspend *h)
{
	struct tegra_otg_data *tegra;

	tegra = container_of(h, struct tegra_otg_data, early_suspend);
	wikipad_otg_disable(tegra,1);
}

static void wikipad_otg_late_resume(struct early_suspend *h)
{
	struct tegra_otg_data *tegra;

	tegra = container_of(h, struct tegra_otg_data, early_suspend);
	wikipad_otg_disable(tegra,0);
}
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
		tegra->early_suspend.suspend = wikipad_otg_early_suspend;
		tegra->early_suspend.resume = wikipad_otg_late_resume;
		tegra->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN+1;
		register_early_suspend(&tegra->early_suspend);
		wake_lock_init(&mWakeLock, WAKE_LOCK_SUSPEND, "tegra-otg");
#endif
	return 0;
