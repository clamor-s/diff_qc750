...
static struct tegra_rt5631_platform_data kai_audio_pdata = {
	...
	.gpio_ext_mic_det = -1,
	.gpio_ext_audio_det = -1,
	.gpio_hp_mute		= -1,
	.gpio_int_mic_en	= TEGRA_GPIO_INT_MIC_EN,
	.gpio_ext_mic_en	= TEGRA_GPIO_EXT_MIC_EN,
	.gpio_rt5631_reset = TEGRA_GPIO_RT5631_RST,
};
...
static int __init kai_touch_init_Focaltech(void)
{
	...
	tegra_gpio_enable(TEGRA_GPIO_PN5); //reset
	...
	gpio_request(TEGRA_GPIO_PN5, "tp_enable2");
	gpio_direction_output(TEGRA_GPIO_PN5, 1);
	gpio_set_value(TEGRA_GPIO_PN5,1);
...
static const struct i2c_board_info ventana_i2c_bus1_NT11003_info[] = {
	{
	 I2C_BOARD_INFO("nt1103-ts", 0x1),           //0x70),     	
...
{
	printk("=====touch====\n");
	gpio_request(TEGRA_GPIO_PZ3, "tp_rst");
	tegra_gpio_enable(TEGRA_GPIO_PZ3);
	gpio_direction_output(TEGRA_GPIO_PZ3, 1);
	
	gpio_request(TEGRA_GPIO_PK2, "tp_int");
...
static struct plat_vehicle_port vehicle_platform_data= {
		.handbrake_gpio = TEGRA_GPIO_PJ0,
		.dock_gpio = TEGRA_GPIO_PO4,
};

static struct platform_device vehicle_device = {
        .name = CM9000_CAR_DEV_NAME,
        .id = -1,
        .dev = {
		.platform_data = &vehicle_platform_data,
	},
};
static void tegra_car_init(){

	printk("tegra_car_init");
	
	tegra_gpio_enable(TEGRA_GPIO_PO4);
	tegra_gpio_enable(TEGRA_GPIO_PJ0);
	tegra_gpio_enable(TEGRA_GPIO_PJ2);

	platform_device_register(&vehicle_device);
}
