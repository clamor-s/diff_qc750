...
#define GC0308_POWER_DWN_PIN TEGRA_GPIO_PS2
#define GC0308_POWER1_EN TEGRA_GPIO_PS0
#define GC0308_POWER2_EN TEGRA_GPIO_PR7
...
static int kai_camera_init(void)
{
	...
	ret = gpio_request(GC0308_POWER_DWN_PIN, "cam2_power_dwn");
	if (ret < 0) {
	...
	tegra_gpio_enable(GC0308_POWER1_EN);
	ret = gpio_request(GC0308_POWER1_EN, "cam2_pwer1");
	if (ret < 0) {
		pr_err("%s: gpio_request failed for gpio %s\n",
			__func__, "CAM2_RST_GPIO");
	}
	gpio_direction_output(GC0308_POWER1_EN, 1);
	mdelay(5);
	
	tegra_gpio_enable(GC0308_POWER2_EN);
	ret = gpio_request(GC0308_POWER2_EN, "cam2_pwer2");
	if (ret < 0) {
		pr_err("%s: gpio_request failed for gpio %s\n",
			__func__, "CAM2_RST_GPIO");
	}
	gpio_direction_output(GC0308_POWER2_EN, 1);
	mdelay(5);
#else
...
static const struct i2c_board_info ventana_i2c1_board_info[] = {
	{
		...
		.irq = -1,//(INT_GPIO_BASE + TEGRA_GPIO_PV1),
	},
};
...
{
	kai_camera_init();
#if defined(CONFIG_VIDEO_OV2710)
