...
void AkmAotController::setEnabled(SensorType sensorType, bool isEnabled)
{
	...
	if (0 > ioctl(cmd, &value)) {
		LOGE_IF(ENXIO != errno, "%s -sensorType=%d ioctl failed (%s) .", __func__,sensorType,strerror(errno));
	} else {
		switch (sensorType) {
		case AKM_SENSOR_TYPE_MAGNETOMETER:
			m_magEn = value;
...
bool AkmAotController::openDevice_internal()
{
	...
	aot_fd = open(AKM_AOT_DEVICE_PATH, O_RDONLY|O_NONBLOCK);
	if (0 > aot_fd) {
