...
static const struct sensor_t sSensorList[] = {
	 ...
	 { "Kionix 3-axis Accelerometer",
          ...
          SENSOR_TYPE_ACCELEROMETER, RANGE_A_K, RESOLUTION_A_K, 0.23f, 20000, { } },
	
  
         { "AK8975 3-axis Magnetic field sensor",
          "Asahi Kasei Microdevices",
          1, SENSORS_MAGNETIC_FIELD_HANDLE,
          SENSOR_TYPE_MAGNETIC_FIELD, 2000.0f, CONVERT_M_K, 6.8f, 30000, { } },

        { "AK8975 Orientation sensor",
          "Asahi Kasei Microdevices",
          1, SENSORS_ORIENTATION_HANDLE,
          SENSOR_TYPE_ORIENTATION, 360.0f, CONVERT_O_K, 7.8f, 30000, { } },
...
int sensors_poll_context_t::activate(int handle, int enabled) {
    LOGD("sensors_poll_context_t->activate(handle: %d, enabled: %d)", handle, enabled);
    int index = handleToDriver(handle);
    if (index < 0) return index;
    int err =  mSensors[index]->enable(handle, enabled);
    if ((0 == err) && (akm == index))
        err = mSensors[acc]->enable(handle, enabled);
    if (enabled && !err) {
...
{
    //LOGD("sensors_poll_context_t::pollEvents(count: %d)", count);
    ...
    do {
     ...
     //           LOGD("sensors_poll_context_t::pollEvents(): mPollFds[%d].fd = %d; mPollFds[%d].revents = %d; PendingEvent = %d", i,mPollFds[i].fd,i, (mPollFds[i].revents & POLLIN ? 99:0), (sensor->hasPendingEvents()?99:0));

                int nb = sensor->readEvents(data, count);
  //              LOGD("sensors_poll_context_t::pollEvents(): sensor idex: %d; senor->readEvents(coutn=%d) return %d;", i, count, nb);
