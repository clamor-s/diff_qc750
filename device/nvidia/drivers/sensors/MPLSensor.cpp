...
/* mpu fifo rate is in increments of 5ms (5000000ns)*/
#define FIFO_RATE_STEP_NS 5000000LLU
                               ...
                               int index)
{
    ...
    if (res != INV_SUCCESS) {
        ALOGD("compass_handler inv_get_float_array(INV_MAGNETOMETER) returned %d",
             res);
    }
...
int MPLSensor::update_delay()
{
    ...
    if (mEnabled) {
        ...
        int rate = (wanted / FIFO_RATE_STEP_NS) - ((wanted %
                                    FIFO_RATE_STEP_NS == 0) ? 1 : 0);
            ...
            if (mUseTimerirq) {
                ...
                if (inv_get_dl_config()->inv_mpu_cfg->requested_sensors
                    ...
                    ioctl(mIrqFds.valueFor(TIMERIRQ_FD), TIMERIRQ_START,
                          (unsigned long) mPollPeriod);
                    ALOGV_IF(EXTRA_VERBOSE, "updated timerirq period to %d",
                            (int) mPollPeriod);
                } else {
                    mPollPeriod = inv_get_sample_step_size_ms();
                    ioctl(mIrqFds.valueFor(TIMERIRQ_FD), TIMERIRQ_START,
                          (unsigned long) mPollPeriod);
                    ALOGV_IF(EXTRA_VERBOSE, "updated timerirq period to %d",
                            (int) mPollPeriod);
...
bool MPLSensor::isDeviceOrientationStable()
{
    ...
    if (mPollPeriod < DEFAULT_POLL_PERIOD) {
        ...
        diff[0] = mAccelWindow[mAccelWindowIndex][0]
                  - mAccelWindow[prevIndex][0];
        diff[1] = mAccelWindow[mAccelWindowIndex][1]
                  - mAccelWindow[prevIndex][1];
        diff[2] = mAccelWindow[mAccelWindowIndex][2]
    ...
    if (mUseTimerirq) {
        if (stable) {
            if (IDLE_POLL_PERIOD != mPollPeriod) {
                mPollPeriod = IDLE_POLL_PERIOD;
                ioctl(mIrqFds.valueFor(TIMERIRQ_FD), TIMERIRQ_STOP, 0);
                clearIrqData(irq_set);
                ioctl(mIrqFds.valueFor(TIMERIRQ_FD), TIMERIRQ_START, mPollPeriod);
                ALOGV("Changed Polling period to %dms", mPollPeriod);
            }
        } else {
            if (mPollPeriod != inv_get_sample_step_size_ms()) {
                resetAccelWindow();
                mPollPeriod = inv_get_sample_step_size_ms();
                ioctl(mIrqFds.valueFor(TIMERIRQ_FD), TIMERIRQ_STOP, 0);
                clearIrqData(irq_set);
        ...
        if (stable) {
            ...
            if (mPollPeriod != mPollPeriodSave) {
                resetAccelWindow();
                windowReset = true;
                mPollPeriod = mPollPeriodSave;
                update = true;
            }
        }

        if (update) {
            int rate = (mPollPeriod / (FIFO_RATE_STEP_NS / 1000000LLU)) - ((mPollPeriod %
                                        (FIFO_RATE_STEP_NS / 1000000LLU) == 0) ? 1 : 0);
            if (rate == 0) // disallow fifo rate 0
                rate = 1;
            adjustFifoRate(rate);

            if (rate != mCurFifoRate) {
                ALOGV("set fifo rate - divider : %d, delay : %d ms ",
                    rate, mPollPeriod);
                inv_error_t res = inv_dmp_stop();
                ALOGE_IF(res != INV_SUCCESS, "error stopping the DMP");
                res = inv_set_fifo_rate(rate);
