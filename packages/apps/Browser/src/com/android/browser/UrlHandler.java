...
public class UrlHandler {
    ...
    boolean shouldOverrideUrlLoading(Tab tab, WebView view, String url) {
        ...
        // The "data:" schemes are internal to the browser; don't want these to be dispatched to
        // other apps. Otherwise we might end up passing images through intent system.
    ...
    boolean startActivityForUrl(Tab tab, String url) {
      ...
      } catch (RuntimeException resolveException) {
          // Handle cases where package manager dies, for example if the url is huge.
          Log.w("Browser", "Exception while running package manager for URI: " + url.substring(0, 100) +
                  "... : " +  resolveException.getMessage());
          return false;
      }
