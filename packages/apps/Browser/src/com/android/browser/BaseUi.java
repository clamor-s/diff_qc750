...
public abstract class BaseUi implements UI {
    ...
    private View mDummyFixedTitlebar;
    private View mDummyMovingTitlebar;
    ...
    public BaseUi(Activity browser, UiController controller) {
        ...
        mDummyFixedTitlebar = (View) frameLayout.findViewById(R.id.dummy_fixed_titlebar);
        mDummyMovingTitlebar = (View) frameLayout.findViewById(R.id.dummy_moving_titlebar);
    ...
    public void removeSubWindow(View subviewContainer) {
        ...
        mUiController.endActionMode();
        mTitleBar.setForceFixedTitleBar(false);
    }
    ...
    public void attachSubWindow(View container) {
        ...
        mContentView.addView(container, COVER_SCREEN_PARAMS);
        mTitleBar.setForceFixedTitleBar(true);
    }
    ...
    /** Adds or updates UI titlebar
     * @param view the titlebar
     * @param isFixed true if titlebar should be stationary relative to content.
     * This method may be called from View.dispatch* methods.
     * @hide
     */
    public void setTitleBar(View titlebar, boolean isFixed) {
        // Updates the dummy titlebar views because otherwise
        // we cannot reparent the titlebar during
        // the View.dispatch* methods.
        // FIXME: this is very hacky. The proper solution would be
        // to remove the need to reparent by manually controlling
        // the title bar position during scrolling.

        ViewGroup parent = (ViewGroup) titlebar.getParent();
        if (parent != null) {
            // If titlebar is child of correct parent, do nothing.
            if (isFixed && titlebar.getParent() == mFixedTitlebarContainer)
                return;

            if (!isFixed && titlebar.getParent() == mContentView)
                return;

            // Else remove the titlebar because its parent will change.
            parent.removeView(titlebar);
        }

        ViewGroup.LayoutParams movingTitlebarLayout =
            new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                    LayoutParams.WRAP_CONTENT);

        if (isFixed) {
            if (mDummyFixedTitlebar.getParent() != null)
                mFixedTitlebarContainer.removeView(mDummyFixedTitlebar);

            mFixedTitlebarContainer.addView(titlebar);

            if (mDummyMovingTitlebar.getParent() == null)
                mContentView.addView(mDummyMovingTitlebar, movingTitlebarLayout);
        } else {
            if (mDummyMovingTitlebar.getParent() != null)
                mContentView.removeView(mDummyMovingTitlebar);

            mContentView.addView(titlebar, movingTitlebarLayout);

            if (mDummyFixedTitlebar.getParent() == null)
                mFixedTitlebarContainer.addView(mDummyFixedTitlebar);
        }
    }
