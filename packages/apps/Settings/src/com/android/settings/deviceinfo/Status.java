...
public class Status extends PreferenceActivity {
    ...
    private static final String KEY_KEENHI_BOARD_NUMBER = "keenhi_board_number";
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (serial != null && !serial.equals("")) {
			if(Build.DEVICE.equals("wikipad")) {
				serial = rename_serial(serial);
			}
        ...
        String boardNum =SystemProperties.get("ro.keenhi.boardid", null);
            if(boardNum != null && !boardNum.equals("")){
            	setSummaryText(KEY_KEENHI_BOARD_NUMBER, boardNum);
            }else{
            	removePreferenceFromScreen(KEY_KEENHI_BOARD_NUMBER);
            }
    }

	private String rename_serial(String serial) {
		// TODO Auto-generated method stub
		String serial2 = serial.toUpperCase();
		int num = serial2.length();
		String tmp = "";
		if(num <= 4)
			return serial2;
		for(int i = 0; i < num; i+=4) {
			tmp += serial2.substring(0, 4);
			serial2 = serial2.substring(4);
			if(serial2.length() >= 4)
				tmp += "-";
		}
		tmp += serial2;
