...
public class CameraActivity extends ActivityBase
    ...
    public void init() {
        ...
        int removeCounts = LightCycleHelper.hasLightCycleCapture(this)?0:1;
        removeCounts += LightCycleHelper.hasPanoramaCapture(this)?0:1;
        int[] drawids = new int[DRAW_IDS.length - removeCounts];
        int ix = 0;
        for (int i = 0; i < mDrawables.length; i++) {
            if ((i == LIGHTCYCLE_MODULE_INDEX && !LightCycleHelper.hasLightCycleCapture(this))||
            		(i == PANORAMA_MODULE_INDEX && !LightCycleHelper.hasPanoramaCapture(this))) {
