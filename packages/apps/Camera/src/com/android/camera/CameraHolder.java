...
public class CameraHolder {
    ...
    private CameraHolder() {
        ...
        if (mMockCameraInfo != null) {
            ...
            // Hold the first back and front cameras if there are any
            // and ignore anything else
            for (int i = 0; i < NumberOfCameras; i++) {
                android.hardware.Camera.getCameraInfo(i, TempInfo);
                if (mBackCameraId == -1 &&
                        TempInfo.facing == CameraInfo.CAMERA_FACING_BACK) {
                    mBackCameraId = i;
                    v.add(i);
                }
                if (mFrontCameraId == -1 &&
                        TempInfo.facing == CameraInfo.CAMERA_FACING_FRONT) {
                    mFrontCameraId = i;
                    v.add(i);
                }
            }
            mNumberOfCameras = v.size();

            mInfo = new CameraInfo[mNumberOfCameras];
            for (int i = 0; i < mNumberOfCameras; i++) {
                mInfo[i] = new CameraInfo();
                android.hardware.Camera.getCameraInfo(v.get(i), mInfo[i]);
            }
        }
