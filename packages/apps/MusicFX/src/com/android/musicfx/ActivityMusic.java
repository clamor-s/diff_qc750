...
public class ActivityMusic extends Activity implements OnSeekBarChangeListener {
    ...
    private boolean mA12UserEQOn = true;
    private CompoundButton mToggleSwitch;

    private StringBuilder mFormatBuilder = new StringBuilder();
    private Formatter mFormatter = new Formatter(mFormatBuilder, Locale.getDefault());
    public final static String ACTION_EQ_CHANGED= "com.keenhi.EQ_CHANGED_ACTION";
    /**
    ...
    private void equalizerSetPreset(final int preset) {
        ...
        equalizerUpdateDisplay();
		String value;
		value = SystemProperties.get("ro.user.eq.a12");
		if(value.equals("true")){
		 
		 if(preset == 11 && mA12UserEQOn){
		 	Intent intent = new Intent(ACTION_EQ_CHANGED);
			mContext.sendBroadcast(intent);
			Log.e(TAG, "Send Broadcast preset = "+preset);
			mA12UserEQOn = false;
		 }
	 }
    }
