...
typedef struct _NvCapTestCtx
{
    ...
    unsigned int uiProfiling; // 1 => Enable latency profiling, 0 => disable
...
static int UpdateLatencyProfiling(NVCAP_CTX ctx, NvCapTestCtx* pTestCtx)
{
    ...
    stProfiling.uiEnable = pTestCtx->uiProfiling;
    ALOGE("UpdateLatencyProfiling(): %s latency profiling\n",
            pTestCtx->uiProfiling ? "Enable" : "Disable");
...
static void InitCtx()
{
    ...
    g_InitialCtx.uiWidth = 720;
    g_InitialCtx.uiHeight = 480;
    g_InitialCtx.uiFPSNum = 30;
    g_InitialCtx.uiFPSDen = 1;
    g_InitialCtx.uiVideoBitRate = 7*1024*1024; // 7 Mbps
    g_InitialCtx.uiTransmissionRate = 14*1024*1024; // 14Mbps
    g_InitialCtx.iSkipFactor = -1;
    g_InitialCtx.uiStallMS = 0;
    g_InitialCtx.uiNumIter = 1;
    g_InitialCtx.uiDump = 1;
    g_InitialCtx.uiManual = 0;
    g_InitialCtx.uiProfiling = 0;
    ...
    for (i = 0; i < g_InitialCtx.uiNumIter; i++)
    {
        ...
        while (!bDone)
        {
            ...
            switch (option)
            {
                ...
                case 'l':
                    // Enable latency profiling
                    printf("Enable latency profiling? (1/0): ");
                    scanf("%d", &g_FinalCtx.uiProfiling);
