...
static NvError NvAviParserHal_ChooseStreams(
               NvAviParserHalCore* pNvAviParserHalCoreHandle,
               NvU32 *tempAudStream,
               NvU32 *tempVidStream);
/******************************************************************************
...
status_t NvAviParserHal_Create(char* Url, void **pAviParserHalHandle) {
    ...
    NvU32 tempAudStream = 0xDeadBeef;
    NvU32 tempVidStream = 0xDeadBeef;
    ...
    if (pContext->Parser.AviHeader.dwStreams > 2) {
        NvAviParserHal_ChooseStreams(pNvAviParserHalCoreHandle, &tempAudStream, &tempVidStream);
        i = 0;
        if (tempVidStream != 0xDeadBeef) {
            ALOGV("--- %s[%d] ---, Creating video stream with index = %d",__FUNCTION__,__LINE__,tempVidStream);
            ret = NvAviParserHal_CreateStream(pNvAviParserHalCoreHandle, i, tempVidStream);
            if (ret != OK) {
                ALOGE("===== %s[%d] Error =====",__FUNCTION__, __LINE__);
                goto cleanup;
            }
        ...
        if (tempAudStream != 0xDeadBeef) {
            ALOGV("--- %s[%d] ---, Creating audio stream with index = %d",__FUNCTION__,__LINE__,tempAudStream);
            ret = NvAviParserHal_CreateStream(pNvAviParserHalCoreHandle, i, tempAudStream);
            if (ret != OK) {
                ALOGE("===== %s[%d] Error =====",__FUNCTION__, __LINE__);
                goto cleanup;
            }
...
static NvError NvAviParserHal_ChooseStreams(
    ...
    NvError Status = NvSuccess;
    NvU32 i = 0;
    NvAviParser *pParser = &pNvAviParserHalCoreHandle->pCoreContext->Parser;
    NvU32 numStreams = pParser->AviHeader.dwStreams;
    int32_t maxAudBitrate = 0;
    int32_t maxVidBitrate = 0;

    ALOGV("+++ %s +++",__FUNCTION__);
    if (pParser->AviHeader.dwStreams > AVI_MAX_TRACKS) {
        numStreams = AVI_MAX_TRACKS;
    }

    // Select max bitrate
    for (i=0; i<numStreams; i++) {
        if ((NVMM_ISSTREAMVIDEO(pParser->AllTrackInfo[i].ContentType)) &&
            (pParser->AllTrackInfo[i].ContentType != NvMMStreamType_UnsupportedVideo)) {
            if (maxVidBitrate < pParser->AllTrackInfo[i].BitRate) {
                *tempVidStream = i;
                maxVidBitrate = pParser->AllTrackInfo[i].BitRate;
                ALOGV("--- %s[%d] ---, Found video track = %d",__FUNCTION__,__LINE__,i);
            }
        } else if (NVMM_ISSTREAMAUDIO(pParser->AllTrackInfo[i].ContentType)) {
            if (maxAudBitrate < pParser->AllTrackInfo[i].BitRate) {
                *tempAudStream = i;
                maxAudBitrate = pParser->AllTrackInfo[i].BitRate;
                ALOGV("--- %s[%d] ---, Found audio track = %d, pParser->AllTrackInfo[i].ContentType = %d",__FUNCTION__,__LINE__,i,pParser->AllTrackInfo[i].ContentType);
            }
        }
    }
