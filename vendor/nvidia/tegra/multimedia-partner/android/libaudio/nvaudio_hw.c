                              ...
                              const XML_Char **attr)
{
    ...
    if (!strcmp(elem, "alsa_device")) {
        ...
        const XML_Char *node = NULL, *command = NULL;
        ...
        if (!state->dev_cfg) {
            ALOGV("at command for invalid dev_cfg");
            return;
        }

        if (state->path_status == PATH_UNINIT) {
            ALOGV("at command without path element");
            return;
        }
        ...
        ALOGV("Parsing at command %s for node %s\n", command, node);
...
static void config_parse_end(void *data, const XML_Char *name)
{
    ...
    if (!strcmp(name, "playback") ||
        ...
        if (state->path_status == PATH_INIT)
            set_route_by_array(state->dev->mixer, state->init_route_list);
...
static void apply_volume(int16_t* buffer, size_t frames,
                         uint32_t channels, uint32_t *vol)
{
    uint32_t i;
                      ...
                      uint32_t old_devices)
{
    ...
    ALOGV("%s : devices 0x%x -> 0x%x", __FUNCTION__, old_devices, new_devices);

    /* Enable new route */
    if (dev->devices & new_devices) {
        if (!dev->ref_count)
            set_route_by_array(dev->mixer, dev->on_route_list);
        ...
        while (n) {
            ...
            if (!dev_cfg->on && (dev_cfg->devices & new_devices)) {
                set_route_by_array(dev->mixer, dev_cfg->on_route_list);
                if (dev_cfg->at_cmd)
                    send_at_cmd(dev_cfg->at_cmd);
                dev_cfg->on = true;
            } else if (dev_cfg->on && !(dev_cfg->devices & new_devices)) {
                set_route_by_array(dev->mixer, dev_cfg->off_route_list);
                dev_cfg->on = false;
            }
            n = list_get_next_node(n);
        }
    ...
    if (!(dev->devices & new_devices) && (dev->devices & old_devices)) {
        ...
        if (!dev->ref_count) {
            ...
            set_route_by_array(dev->mixer, dev->off_route_list);
        }
    }
}
...
static void trigger_dma_callback(void *args, void *dma_addr, int on)
{
    ...
    if (dev && dev->pcm) {
        if (on) {
            ...
            pcm_write(dev->pcm, (void *)&zero, sizeof(zero));
            pcm_start(dev->pcm);
        } else {
            pcm_stop(dev->pcm);
            if (ctl)
...
static uint32_t get_num_of_pcm_streams(list_handle stream_list)
{
                             ...
                             struct alsa_dev *dev)
{
    ...
    if (dev->pcm) {
        ...
        if (dev->id == DEV_ID_AUX) {
           if ((out->format == AUDIO_FORMAT_IEC61937) &&
                 !dev->is_pass_thru) {
    ...
    if ((nvoice_supported) &&
       (out->hw_dev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
        dev->config.rate = NVOICE_SAMPLE_RATE;
        if (out->avp_stream) {
            reset_resampler = 1;
        } else {
            if (out->resampler[id] ) {
    ...
    if (dev->id != DEV_ID_WFD) {
        ...
        if (!pcm_is_ready(dev->pcm)) {
            ...
            if (out->format == AUDIO_FORMAT_PCM_16_BIT) {
                ...
                err = mixer_ctl_get_iec958(ctl,(void*)&iec958,0);
                if (err < 0) {
                    ALOGE("Channel Status bit get has failed\n");
                }
                iec958.status[0] &= ~IEC958_AES0_NONAUDIO;
                err = mixer_ctl_set_iec958(ctl,(void*)&iec958,0);
                if (err < 0) {
    ...
    if (out->avp_stream) {
        ...
        if (reset_resampler) {
            node_handle n = list_get_head(dev->active_stream_list);
            ...
            if (dev->id != DEV_ID_WFD) {
                ...
                if ((dev->id != DEV_ID_AUX) ||
                    ...
                    for (i = 0; i < dev->config.period_count; i++)
                        pcm_write(dev->pcm, (void*)dev->mix_buffer,
                        dev->mix_buff_size);
                    pthread_mutex_lock(&out->lock);
                    pthread_mutex_lock(&dev->lock);
        ...
        uint32_t new_devices, uint32_t old_devices)
{
    ...
    if ((alsa_call_pb_dev_prev->id == alsa_call_pb_dev->id)
        ...
        if ((alsa_call_pb_dev->mode != AUDIO_MODE_IN_CALL)
           ...
           alsa_call_cap_dev->mode = AUDIO_MODE_IN_CALL;
           set_route(alsa_call_pb_dev, call_devices, 0);
           set_route(alsa_call_cap_dev, call_devices_cap, 0);
        } else {
            set_route(alsa_call_pb_dev, call_devices, call_devices_prev);
            set_route(alsa_call_cap_dev, call_devices_cap,
                               call_devices_cap_prev);
        }
            ...
            set_route(alsa_call_pb_dev_prev, 0, call_devices_prev);
            set_route(alsa_call_cap_dev_prev, 0, call_devices_cap_prev);
...
{
    node_handle n;
    struct nvaudio_stream_out *out_avp = NULL, *out_pcm = NULL;
    ...
    if (out_avp || out_pcm) {
        ...
        pthread_mutex_unlock(&dev->lock);
        if (out_avp)
            avp_stream_write(out_avp->avp_stream, NULL, 0);
        if (out_pcm)
            avp_stream_write(dev->avp_stream, NULL, 0);
        pthread_mutex_lock(&dev->lock);
    }
                                      ...
                                      const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        if (new_devices && ((out->devices != new_devices) ||
            ...
            while (n) {
                ...
                if (dev->id == DEV_ID_AUX) {
                    if ((new_devices & AUDIO_DEVICE_OUT_AUX_DIGITAL) &&
                        ...
                        sprintf(prop_val, "%d", dev->rec_dec_capable);
                        property_set(TEGRA_HW_AC3DEC_PROPERTY, prop_val);
                        property_set(TEGRA_HW_DTSDEC_PROPERTY, prop_val);
                    }
                }
                ...
                if ((dev->devices & new_devices) &&
                    ...
                    if (!out->standby)
                        close_alsa_dev(stream, dev);
                    set_route(dev, 0, out->devices);
                } else
                    set_route(dev, new_devices, out->devices);
                pthread_mutex_unlock(&dev->lock);
            }
...
static uint32_t nvaudio_out_get_latency(const struct audio_stream_out *stream)
{
    ...
    while (n) {
        ...
        if (out->avp_stream || dev->avp_stream)
            temp = ((nvaudio_out_get_buffer_size(&stream->common) >> 2)
    ...
    ALOGV("%s: latency %d", __FUNCTION__, latency);
    return latency;
}
                                 ...
                                 const void* buffer, size_t bytes)
{
    ...
    node_handle n;
    int ret = 0;
    uint32_t src_ch_map, dest_ch_map, dwnmix_stereo_ch_map;
    ...
    while (n) {
        ...
        do {
            ...
            if (out->channel_count > dev->config.channels) { /*downmix*/
                if (dev->config.channels == 2) {/*downmix to stereo*/
                    dwnmix_stereo_ch_map =
                        tegra_stereo_dwn_mix_ch_map[out->channel_count - 1];

                    if (src_ch_map != dwnmix_stereo_ch_map)
                        shuffle_channels((int16_t*)buffer, bytes,
                            out->channel_count, src_ch_map,
                            dwnmix_stereo_ch_map);

                    downmix_to_stereo((int16_t*)buffer, out->channel_count,
                        bytes / in_frame_size);
                } else {/*downmix*/
                    downmix((int16_t*)buffer, src_ch_map, out->channel_count,
                        (int16_t*)buffer, dest_ch_map, dev->config.channels,
                        (bytes / in_frame_size));
                }
                bytes = (bytes*alsa_dev_frame_size(dev))/(in_frame_size);
            } else if (out->channel_count < dev->config.channels) { /*upmix*/
                upmix((int16_t*)buffer, src_ch_map, out->channel_count,
                    (int16_t*)out->buffer[id], dest_ch_map,
                    dev->config.channels, bytes / in_frame_size);
                buffer = (void*)out->buffer[id];
                bytes = (bytes*alsa_dev_frame_size(dev))/(in_frame_size);
            } else if ((src_ch_map != dest_ch_map) &&
                   (out->channel_count == dev->config.channels)) { /*shuffle*/
                shuffle_channels((int16_t*)buffer, bytes, out->channel_count,
                    src_ch_map, dest_ch_map);
            }

            if ((out->volume[0] != INT_Q15(1) || out->volume[1] != INT_Q15(1))
             && (out->format == AUDIO_FORMAT_PCM_16_BIT))
                apply_volume((int16_t*)buffer, bytes/alsa_dev_frame_size(dev),
                dev->config.channels, out->volume);

             dev_stream_cnt =
                 get_num_of_pcm_streams(dev->active_stream_list);
             if ((dev_stream_cnt > 1) &&
                ...
                if((nvoice_supported) &&
                  (out->hw_dev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
                    if(dev->config.channels == 2)
                        stereo_to_mono((void*)dev->mix_buffer,(void*)dev->mix_buffer,
                            dev->config.period_size);
                    nvoice_processrx((void*)dev->mix_buffer,dev->config.period_size );
                    if(dev->config.channels == 2)
                        mono_to_stereo((void*)dev->mix_buffer,(void*)dev->mix_buffer,
                            dev->config.period_size);
                    ...
                    if (out->avp_stream || dev->avp_stream) {
                        avp_stream_handle_t stream = out->avp_stream ?
                        out->avp_stream : dev->avp_stream;
                        ret = avp_stream_write(stream, (void*)dev->mix_buffer,
                            bytes);
                    } else if (dev->id != DEV_ID_WFD) {
                        ret = pcm_write(dev->pcm, (void*)dev->mix_buffer,
                            bytes);
                        if (ret == -ESTRPIPE) {
                            ...
                            ret = pcm_write(dev->pcm, (void*)buffer, bytes);
                        }
                    }
                    else
                        ret = wfd_write(dev, (void*)dev->mix_buffer, bytes);

                    pthread_cond_signal(&dev->cond);
                    pthread_mutex_lock(&out->lock);
                    pthread_mutex_lock(&dev->lock);
                    if (ret != 0)
                        ALOGE("Failed to write to alsa_device %s err %d",
...
#ifdef NVOICE
                if((nvoice_supported) && (!out->avp_stream) &&
                  (out->hw_dev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
                    if(dev->config.channels == 2)
                        stereo_to_mono((void *)buffer,(void *)buffer,dev->config.period_size);
                    nvoice_processrx((void *)buffer, dev->config.period_size);
                    if(dev->config.channels == 2)
                        mono_to_stereo((void *)buffer,(void *)buffer,dev->config.period_size);
                ...
                if (out->avp_stream || dev->avp_stream) {
                    avp_stream_handle_t stream = out->avp_stream ?
                                     out->avp_stream : dev->avp_stream;
                    ret = avp_stream_write(stream, buffer, bytes);
                } else if (dev->id != DEV_ID_WFD) {
                    ret = pcm_write(dev->pcm, (void*)buffer, bytes);
                    if (ret == -ESTRPIPE) {
                        ALOGE("ESTRPIPE : suspend state\n");
           ...
           if ((dev->config.channels == 1) &&
               (out->channel_count == 2)) {
               stereo_to_mono((int16_t*)resampling_buffer,
               (int16_t*)resampling_buffer,
               out_frames);
           } else if((dev->config.channels == 2) &&
               (out->channel_count == 1)) {
               mono_to_stereo((int16_t*)resampling_buffer,
               (int16_t*)resampling_buffer,
               out_frames);
           } else {
               if (out->channel_count > dev->config.channels) { /*downmix*/
                   if (dev->config.channels == 2) {/*downmix to stereo*/
                       dwnmix_stereo_ch_map =
                       tegra_stereo_dwn_mix_ch_map[out->channel_count -
                       1];

                       if (src_ch_map != dwnmix_stereo_ch_map)
                       shuffle_channels((int16_t*)resampling_buffer,
                       out_frames*in_frame_size, out->channel_count,
                       src_ch_map, dwnmix_stereo_ch_map);

                       downmix_to_stereo((int16_t*)resampling_buffer,
                       out->channel_count, out_frames);
                   } else {
                       downmix((int16_t*)resampling_buffer,
                       src_ch_map,
                       out->channel_count, (int16_t*)resampling_buffer,
                       dest_ch_map, dev->config.channels, out_frames);
                   }
               } else if (out->channel_count
                   < dev->config.channels) {
                   /*upmix*/
                   upmix((int16_t*)resampling_buffer,
                   src_ch_map,
                   out->channel_count, NULL/*inplace computation*/,
                   dest_ch_map, dev->config.channels, out_frames);
               } else if ((src_ch_map != dest_ch_map) &&
                   (out->channel_count == dev->config.channels)) {
                   /*shuffle*/
                   shuffle_channels((int16_t*)resampling_buffer,
                   out_frames * out_frame_size, out->channel_count,
                   src_ch_map, dest_ch_map);
               }
           }

           if ((out->volume[0] != INT_Q15(1) ||
               out->volume[1] != INT_Q15(1)) &&
               (out->format == AUDIO_FORMAT_PCM_16_BIT))
               apply_volume((int16_t*)resampling_buffer, out_frames,
               dev->config.channels, out->volume);
              ...
              frames_to_copy);*/
               ...
               if ((dev_stream_cnt > 1) &&
                ...
                if((nvoice_supported) &&
                  (out->hw_dev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
                    ...
                    if(dev->config.channels == 2)
                        ...
                        if (dev->avp_stream) {
                            ret = avp_stream_write(dev->avp_stream,
                               (void*)dev->mix_buffer,
                               dev->config.period_size * out_frame_size);
                        } else if (dev->id != DEV_ID_WFD) {
                            ret = pcm_write(dev->pcm, (void*)dev->mix_buffer,
                            dev->config.period_size * out_frame_size);
                            if (ret == -ESTRPIPE) {
                            ...
                            ret = wfd_write(dev,
                            (void*)dev->mix_buffer, dev->config.period_size *
                                out_frame_size);
                        ...
                        pthread_mutex_lock(&dev->lock);
                        if (ret != 0) {
                            ALOGE("Failed to write to alsa_device"
                ...
                if((nvoice_supported) &&
                  (out->hw_dev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
                    if(dev->config.channels == 2)
                        stereo_to_mono((void *)out->buffer[id],(void *)out->buffer[id],
                            dev->config.period_size);
                    nvoice_processrx((void *)out->buffer[id], dev->config.period_size);
                    if(dev->config.channels == 2)
                        mono_to_stereo((void *)out->buffer[id],(void *)out->buffer[id],
                            dev->config.period_size);
                    ...
                    if (dev->avp_stream) {
                        ...
                        ret = pcm_write(dev->pcm, (void*)out->buffer[id],
                        dev->config.period_size * out_frame_size);
                        if (ret == -ESTRPIPE) {
                            ALOGE("ESTRPIPE : suspend state\n");
                            pcm_close(dev->pcm);
                    ...
                    pthread_mutex_lock(&dev->lock);
                    if (ret != 0) {
                        ALOGE("Failed to write to alsa_device %s with err %d",
                        alsa_dev_names[id].name, ret);
                        break;
                    }
                }
...
static size_t nvaudio_in_get_buffer_size(const struct audio_stream *stream)
{
    ...
    buffer_size = (dev->config.period_size * in->rate) / dev->config.rate;
        ...
        if (new_devices && (in->devices != new_devices)) {
            ...
            while (n) {
                ...
                if ((dev->devices & new_devices) &&
                    ...
                    if (hw_dev->mode != AUDIO_MODE_IN_CALL)
                        set_route(dev, 0, in->devices);
                } else if (hw_dev->mode != AUDIO_MODE_IN_CALL)
                    set_route(dev, new_devices, in->devices);
                       ...
                       size_t bytes)
{
    ...
    if (!in->resampler && (in->channel_count == dev->config.channels)) {
        ...
        if((nvoice_supported) &&
          (in->hw_dev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
            nvoice_processtx((void*)buffer,dev->config.period_size);
        ...
        do {
            ...
            if (!in->buffer_offset) {
                ...
                if((nvoice_supported) &&
                  (in->hw_dev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
                    nvoice_processtx((void*)in->buffer,dev->config.period_size);
                                          ...
                                          struct audio_stream_out **stream_out)
{
    ...
    while (n) {
        ...
        pthread_mutex_lock(&dev->lock);
        /* If another stream is already open don't overwrite route from here */
        if (!list_get_node_count(hw_dev->stream_out_list))
            set_route(dev, devices, 0);
    ...
    while (n) {
        ...
        /* For first output stream igonre samplerate set by AF */
        if (!list_get_node_count(hw_dev->stream_out_list) ||
            !(config->sample_rate))
                                            ...
                                            struct audio_stream_out *stream)
{
    ...
    while (n_j) {
        ...
        pthread_mutex_lock(&dev->lock);
        if (list_get_node_count(hw_dev->stream_out_list) == 1)
            set_route(dev, 0, out->devices);
        pthread_mutex_unlock(&dev->lock);
                                        ...
                                        float volume)
{
    ...
    sprintf(vol,"%2d\r\n", vol_level);
    strcpy(vol_cmd, "at+clvl=");
    strcat(vol_cmd, vol);
    cmd.node = "dev/ttyACM2";
    cmd.command = vol_cmd;
    send_at_cmd(&cmd);
...
static int nvaudio_dev_set_mode(struct audio_hw_device *dev, int mode)
{
...
#ifdef NVOICE
    if ((nvoice_supported) && ((mode == AUDIO_MODE_IN_COMMUNICATION) ||
            (hw_dev->mode == AUDIO_MODE_IN_COMMUNICATION)))

    {
        ...
        while (n) {
            ...
            while (n_j) {
                ...
                avp_stream_handle_t stream = out->avp_stream ?
                                             out->avp_stream : dev->avp_stream;
                if ((dev->ulp_supported) && (out->avp_stream)) {
                    avp_stream_pause(stream,1);
                    avp_stream_stop(stream);
                }
                else if (dev->pcm) {
                    if(dev->ulp_supported)
    ...
    if ((mode == AUDIO_MODE_IN_CALL) &&
        ...
        /* Disable routes for out stream, after call a setparam call will
...
{
    struct nvaudio_hw_device *hw_dev = (struct nvaudio_hw_device *)dev;
    node_handle n = list_get_head(hw_dev->stream_in_list);
    ...
    while (n) {
        ...
        n = list_get_next_node(n);
        pthread_mutex_lock(&in->lock);
        if (in->dev->devices & AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET) {
            ALOGE("Muting of BT SCO mic is not supported");
            pthread_mutex_unlock(&in->lock);
            pthread_mutex_unlock(&hw_dev->lock);
            return -ENOSYS;
        }
                                         ...
                                         const struct audio_config *config)
{
    ...
    while (n) {
        ...
        if (dev->devices & AUDIO_DEVICE_IN_DEFAULT)
            buffer_size = (dev->config.period_size * config->sample_rate) /
                           dev->config.rate;
    }
    buffer_size = (buffer_size / 16) * 16;
                                   ...
                                   struct audio_stream_in *in)
{
    ...
    pthread_mutex_lock(&hw_dev->lock);
    set_route(adev, 0, in_stream->devices);
    ...
    for (i = 0; i < STREAM_MAX; i++) {
        ...
        while (n) {
            ...
            while (n_j) {
                ...
                if (dev_cfg->at_cmd) {
                    if (dev_cfg->at_cmd->node)
                        free(dev_cfg->at_cmd->node);
                    if (dev_cfg->at_cmd->command)
                        free(dev_cfg->at_cmd->command);
                    free(dev_cfg->at_cmd);
                }
                n_j = list_get_next_node(n_j);
            }
    ...
    for (i = 0; i < STREAM_MAX; i++) {
        ...
        while (n) {
            ...
            if (dev->ulp_supported) {
                ...
                if (hw_dev->avp_audio)
                    avp_audio_set_ulp(hw_dev->avp_audio,
                                      is_ulp_supported(hw_dev));
            }
        }
    }
