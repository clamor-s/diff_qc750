 ...
 *************************************************************************************/
NvAsfResult AsfOnAsfParserFilePropertiesObject(
            void *data,
            CAsfFilePropertiesObject* pFilePropertiesObject);
    ...
    if(pNvAsfParserHalCoreHandle) {
        if(pNvAsfParserHalCoreHandle->pCoreContext) {
            if(asf) {
                if(asf->NvAsfParser) {
                    if(asf->NvAsfParser->m_ScratchBuff) {
                        NvOsFree(asf->NvAsfParser->m_ScratchBuff);
                        asf->NvAsfParser->m_ScratchBuff = NULL;
                    }
                }
            }
            if(pNvAsfParserHalCoreHandle->pStreams) {
                ...
                NvOsFree(pNvAsfParserHalCoreHandle->pStreams);
                pNvAsfParserHalCoreHandle->pStreams = NULL;
            }

            NvAsfParserHal_DestroyCore(pNvAsfParserHalCoreHandle);

            if(pNvAsfParserHalCoreHandle->pCoreContext->filename) {
                NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->filename);
                pNvAsfParserHalCoreHandle->pCoreContext->filename = NULL;
            }

            NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext);
            pNvAsfParserHalCoreHandle->pCoreContext = NULL;
        }
...
status_t NvAsfParserHal_Destroy(
    ...
    if(pNvAsfParserHalCoreHandle->pCoreContext) {
        if(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfParser->m_ScratchBuff) {
            NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfParser->m_ScratchBuff);
            pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfParser->m_ScratchBuff = NULL;
        }
        if(pNvAsfParserHalCoreHandle->pStreams) {
...
status_t NvAsfParserHal_GetTrackMetaData(
    ...
    int MaxInputSize = 0;
    NvAsfResult err = RESULT_OK;
    int32_t coreStreamIndex = pNvAsfParserHalCoreHandle->pStreams[streamIndex]->coreStreamIndex;
...
static status_t NvAsfParserHal_OpenCore(
    ...
    asf->m_oCallbacks.OnAsfParserFilePropertiesObject = AsfOnAsfParserFilePropertiesObject;
...
static status_t NvAsfParserHal_DestroyCore(
    ...
    if(pNvAsfParserHalCoreHandle->pCoreContext) {
        if(pNvAsfParserHalCoreHandle->pCoreContext->TempAudBuff) {
            NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->TempAudBuff);
            pNvAsfParserHalCoreHandle->pCoreContext->TempAudBuff = NULL;
        }
        if(pNvAsfParserHalCoreHandle->pCoreContext->TempVidBuff) {
            NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->TempVidBuff);
            pNvAsfParserHalCoreHandle->pCoreContext->TempVidBuff = NULL;
        }
        if(pNvAsfParserHalCoreHandle->pCoreContext->asf) {
           if(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfParser) {
               pNvAsfParserHalCoreHandle->pNvAsfCoreParserApi.pDestroyNvAsfParser
                   (pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfParser);
           }
           if(pNvAsfParserHalCoreHandle->pCoreContext->asf->pWmPicture) {
               NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->asf->pWmPicture);
           }
           if(pNvAsfParserHalCoreHandle->pCoreContext->asf->pMarkerDataInfo) {
               NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->asf->pMarkerDataInfo);
           }
           if(pNvAsfParserHalCoreHandle->pCoreContext->asf->AsfMetadata) {
               NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->asf->AsfMetadata);
           }
           if(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfVideoBuffer) {
               NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfVideoBuffer);
           }
           if(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfAudioBuffer) {
               NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfAudioBuffer);
           }
           if(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfIndexObject) {
               NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->asf->NvAsfIndexObject);
           }
           if(pNvAsfParserHalCoreHandle->pCoreContext->asf->cphandle) {
               pNvAsfParserHalCoreHandle->pCoreContext->asf->pPipe->cpipe.Close(
                   pNvAsfParserHalCoreHandle->pCoreContext->asf->cphandle);
           }

           NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->asf);
           pNvAsfParserHalCoreHandle->pCoreContext->asf = NULL;
       }
       if(pNvAsfParserHalCoreHandle->pCoreContext->coreHandle) {
           NvOsFree(pNvAsfParserHalCoreHandle->pCoreContext->coreHandle);
           pNvAsfParserHalCoreHandle->pCoreContext->coreHandle = NULL;
       }
    }
...
status_t NvAsfParserHal_SetPosition(
    ...
    for(i = 0; i<asf->NoOfStreams; i++) {
        if(NVMM_ISSTREAMVIDEO(asf->StreamInfo[i].StreamType)) {
            ...
            if((er == RESULT_END_OF_FILE) && (direction == 1) &&
               (NewSeekTimeStamp < (asf->NvAsfParser->m_oFilePropertiesObject.uPlayDuration
                    - asf->m_oPreRoll * MS_TO_100NS_MULTIPLY))) {
                /* not found I frame in fwd direction
                   now search in backward direction */
                    er = pNvAsfParserHalCoreHandle->pNvAsfCoreParserApi.pNvAsfVideoNewSeekTime(pContext,
                    &NewSeekTimeStamp,&NewSeekTime,-1);
            }
...
NvAsfResult AsfOnAsfParserFilePropertiesObject(
    ...
    CAsf* asf = (CAsf*)data;
    asf->m_oPreRoll = pFilePropertiesObject->uPreroll;
    // TODO:assign searchwindow as 2 percent of total packets(Need to determine actual percentage)
    asf->no_of_pkts_search_window = (pFilePropertiesObject->uDataPacketsCount * 2)/100;

    ALOGV(
        "    guidFileID: %s\n"
        "    uFileSize: %llu\n"
        "    uCreationDate: %llu\n"
        "    uDataPacketsCount: %llu\n"
        "    uPlayDuration: %llu\n"
        "    uSendDuration: %llu\n"
        "    uPreroll: %llu\n"
        "    oFlags {\n"
        ...
        "        uReserved: %u\n"
        "    }\n"
        "    uMinimumDataPacketSize: %u\n"
        "    uMaximumDataPacketSize: %u\n"
        "    uMaximumBitrate: %u\n"
        "}\n",
        CGuidString(pFilePropertiesObject->guidFileID),
        pFilePropertiesObject->uFileSize,
        pFilePropertiesObject->uCreationDate,
        pFilePropertiesObject->uDataPacketsCount,
        pFilePropertiesObject->uPlayDuration,
        pFilePropertiesObject->uSendDuration,
        pFilePropertiesObject->uPreroll,
        pFilePropertiesObject->oFlags.uBroadcastFlag,
        pFilePropertiesObject->oFlags.uSeekableFlag,
        pFilePropertiesObject->oFlags.uReserved,
        pFilePropertiesObject->uMinimumDataPacketSize,
        pFilePropertiesObject->uMaximumDataPacketSize,
        pFilePropertiesObject->uMaximumBitrate
        );
    return RESULT_OK;
}
