...
void EGLStreamConsumer::readFromSource() {
    ...
    while (mStarted && err == OK) {
        ...
        NvRmMemHandle phMem;
        NvError aErr = NvRmMemHandleFromId(newFrame->info.memId, &phMem);
        if(NvSuccess != aErr)
        {
            ALOGV("EGLStreamConsumer - NvRmMemHandleFromId failed: %d  [ BIG ERROR] \n", aErr);
            //return the Frame..
...
int EGLStreamConsumer::AllocateFrameFully(Aframe *pFrame)
{
    ...
    pBuffer->BufferID  =  NvRmMemGetId(pBuffer->Payload.Surfaces.Surfaces[0].hMem);
