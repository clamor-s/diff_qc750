                      ...
                      RTPPacket *packet, RTPStream *stream)
{
    ...
    int FRAG, RA, SL, LP, PT, DT, R;
    int RACount, AUPLen, PTSDelta, DTSDelta;
...
/*  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ */
    ...
    RACount   = (header >> 16) & 0xFF;
    AUPLen    = (header & 0xFFFF);
    buf  += 4;
    size -= 4;

    PTSDelta = NvMMNToHL(*(unsigned int*)buf);
    buf  += 4;
    size -= 4;

    DTSDelta = NvMMNToHL(*(unsigned int*)buf);
    buf  += 4;
    size -= 4;
...
/*  +----+----+----+----+----+----+----+----+ */
    ...
    RA   = (auControlHeader >> 5) & 0x1;
    SL   = (auControlHeader >> 4) & 0x1;
    LP   = (auControlHeader >> 3) & 0x1;
    PT   = (auControlHeader >> 2) & 0x1;
    DT   = (auControlHeader >> 1) & 0x1;
    R    = (auControlHeader) & 0x1;
