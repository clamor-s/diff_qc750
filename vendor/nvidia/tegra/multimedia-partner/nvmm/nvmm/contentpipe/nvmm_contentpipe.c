...
static CPresult Nvmm_InitializeCP(CPhandle hContent, CPuint64 MinCacheSize, CPuint64 MaxCacheSize, CPuint32 SpareAreaSize, CPuint64* pActualSize)
{
    ...
    while (!done)
    {
        e = NvRmMemHandleCreate(pFileCache->hRmDevice, &pFileCache->hMem, (NvU32)pFileCache->CacheSize);
        if (e == NvSuccess)
        {
            e = NvRmMemAlloc(pFileCache->hMem,
                            NULL,
                            0,
                            4,
    ...
    if (pFileCache->bIsStreaming && pFileCache->nProtocolVersion >= 2)
    {
        ...
        queryret = pFileCache->pProtocol->QueryConfig(pFileCache->hFile,
