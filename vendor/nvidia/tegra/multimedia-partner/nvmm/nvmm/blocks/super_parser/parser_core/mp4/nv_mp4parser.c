    ...
    void *pData)
{
    ...
    if (AtomType == MP4_FOURCC('d', 'r', 'e', 'f'))
    {
        ...
        while(NumEntries > 0)
        {
            ...
            Size = NV_BE_TO_INT_32(&ReadBuffer[0]);
    ...
    NvU32 Size)
{
    ...
    NvU8 identified = 0;
    NvU32 AtomType;
    NvU32 AtomSize, BufferFullness = 0 , AvgBitRate = 0, MaxBitRate = 0;
    NvU32 i, j, BtrtSize = 0;
    NvU32 FileBytesRead = 0;
    ...
    NvBool Direction)
{
    ...
    if (Direction == NV_TRUE)
    {
        ScanWindow = pNvMp4Parser->FramingInfo[pNvMp4Parser->VideoIndex].TotalNoFrames;
    }
    else if (Direction == NV_FALSE)
    {
        ScanWindow = 0;
    }
