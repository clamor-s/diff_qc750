...
typedef struct
{
    ...
    /** NON TRANSPORT CODE DO NOT USE THE BELOW */

    /** internal storage for pMem, cpu and avp side */
    void *pMemCpu;
    void *pMemAvp;

    /** internal storage for hMem, cpu and avp side */
    NvRmMemHandle hMemCpu;
    NvRmMemHandle hMemAvp;

    /** allocator side mem handle id */
    NvU32 id;
} NvMMMemReference;
...
typedef enum
{
    ...
    /** Aes Metadata for Widevine
     *  NvMMAesWvMetadata describes the metadata
     */
...
typedef struct
{
    ...
    union
    {
        ...
        NvMMAesWvMetadata           AesWvMetadata;
