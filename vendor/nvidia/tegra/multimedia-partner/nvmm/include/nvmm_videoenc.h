...
typedef enum
{
    NvMMVideoEncAppType_Camcorder = 0,
...
typedef enum
{
    NvMMVideoEncQualityLevel_Low = 0,
...
/**
 * @brief Defines the Rate Control Mode for h264 encode configuration
 * that can be set by SetAttribute() call with NvMMAttributeVideoEnc_H264StreamProperty
 * @see GetAttribute
 */
typedef enum
{
    NvMMVideoEncRateCtrlMode_CBR = 0,
    NvMMVideoEncRateCtrlMode_VBR,
    NvMMVideoEncRateCtrlMode_VBR2
...
typedef struct
{
    ...
    /// Intra Frame generation Interval
...
typedef struct
{
    ...
    NvU32   StreamMode;
    // bRTPMode override StreamMode to NAL stream mode, and replace
    // start code with marker field.
    NvU32   bRTPMode;
