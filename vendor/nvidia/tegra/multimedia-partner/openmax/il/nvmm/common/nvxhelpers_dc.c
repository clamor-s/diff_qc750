                                   ...
                                   NvU32 nLayout)
{
    ...
    //  Create the overlay
    eReturnVal = nvdcGetWindow(hnvdc, DC_HEAD, TEGRA_DC_WINDOW);
    if (eReturnVal)
    {
        return eReturnVal;
    }
...
OMX_ERRORTYPE NvxReleaseOverlayDC(NvxOverlay *pOverlay)
{
    ...
    nvdcPutWindow(pOverlay->hnvdc, DC_HEAD, TEGRA_DC_WINDOW);
