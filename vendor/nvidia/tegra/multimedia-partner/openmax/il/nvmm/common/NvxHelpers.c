    ...
    NvBool UseAlignment)
{
    ...
    NvU32 ComponentSize, SurfaceAlignment, ChromaWidth = 0, ChromaHeight = 0,
          ChromaFactor = 1;
    ...
    *pImageSize = 0;
    ...
    if (NvMMVideoDecColorFormat_YUV420SemiPlanar == Format)
        pSurfaceDesc->Surfaces[1].ColorFormat = NvColorFormat_U8_V8;
    else
        pSurfaceDesc->Surfaces[1].ColorFormat = NvColorFormat_U8;
    pSurfaceDesc->Surfaces[1].Layout = Layout;
    pSurfaceDesc->Surfaces[1].Offset = 0;
#ifdef NV_DEF_USE_PITCH_MODE
    ...
    SurfaceAlignment = NvRmSurfaceComputeAlignment(hRmDev, &pSurfaceDesc->Surfaces[1]);
#endif
    ComponentSize = NvRmSurfaceComputeSize(&pSurfaceDesc->Surfaces[1]);
    err = NvxAllocateMemoryBuffer(hRmDev, &pSurfaceDesc->Surfaces[1].hMem,
                                  ComponentSize, SurfaceAlignment,
                                  &pSurfaceDesc->PhysicalAddress[1],NvOsMemAttribute_Uncached);
    if (err != NvSuccess)
    {
    ...
    *pImageSize += ComponentSize;
    ...
    if (NvMMVideoDecColorFormat_YUV420SemiPlanar != Format)
    {
        ...
        pSurfaceDesc->Surfaces[2].Layout = Layout;
        pSurfaceDesc->Surfaces[2].Offset = 0;
        pSurfaceDesc->Surfaces[2].Pitch = pSurfaceDesc->Surfaces[1].Pitch;
        err = NvxAllocateMemoryBuffer(hRmDev, &pSurfaceDesc->Surfaces[2].hMem,
                                  ComponentSize, SurfaceAlignment,
                                  &pSurfaceDesc->PhysicalAddress[2], NvOsMemAttribute_Uncached);
        if (err != NvSuccess)
        {
            NV_DEBUG_PRINTF(("Error in memory allocation for V\n"));
            goto nvx_error_bail;
        }
        *pImageSize += ComponentSize;
    }
    ...
    pSurfaceDesc->Surfaces[0].Layout = Layout;
    pSurfaceDesc->Surfaces[0].Offset = 0;
#ifdef NV_DEF_USE_PITCH_MODE
    NvRmSurfaceComputePitchTest(NULL, 0, &pSurfaceDesc->Surfaces[0]);
    SurfaceAlignment = NVRM_3D_SURFACE_YUV_PLANAR_ALIGN;
#else
    // Adding WAR for VDE HW limitation where separate register for setting  chroma-pitch is not available
    // Hence calculating Y-Pitch based on chroma format instead of calling NvRmSurfaceComputePitch
    pSurfaceDesc->Surfaces[0].Pitch = pSurfaceDesc->Surfaces[1].Pitch * ChromaFactor;
    ...
    err = NvxAllocateMemoryBuffer(hRmDev, &pSurfaceDesc->Surfaces[0].hMem,
                                  ComponentSize, SurfaceAlignment,
                                  &pSurfaceDesc->PhysicalAddress[0],NvOsMemAttribute_Uncached);
    if (err != NvSuccess)
    {
    ...
    *ppSurfHandle = pSurface;
    ...
    switch (Format)
    {
        ...
        default:
        {
            ...
            pSurf->Width           = Width;
            pSurf->Height          = Height;
            pSurf->ColorFormat     = Format;
            pSurf->Layout          = NvRmSurfaceLayout_Pitch;

            NvRmSurfaceComputePitch(hRmDev, 0, pSurf);
...
static NvError NvxSwapSurfaceUV(NvRmSurface *pSurf)
{
    ...
    RmSurf.Width = pSurf->Width + 4;
    RmSurf.Height = pSurf->Height;
    RmSurf.ColorFormat = NvColorFormat_L8;
    RmSurf.Layout = pSurf->Layout;
    RmSurf.Pitch = RmSurf.Width;
    SurfaceAlignment = NvRmSurfaceComputeAlignment(s_hRmDevice, &RmSurf);
    ComponentSize = NvRmSurfaceComputeSize(&RmSurf);
    err = NvxAllocateMemoryBuffer(s_hRmDevice, &RmSurf.hMem, ComponentSize, SurfaceAlignment, &PhyAddr,NvOsMemAttribute_Uncached);
    if (err != NvSuccess)
    {
        NV_DEBUG_PRINTF(("Error in memory allocation for Y\n"));
        goto cleanup;
...
NvError NvxCopyMMBufferToOMXBuf(NvMMBuffer *pMMBuf, OMX_BUFFERHEADERTYPE *pOMXBuf)
{
    NvMMSurfaceDescriptor *pSurfaceDesc = NULL;
    ...
    return NvxCopyNvxSurfaceToOMXBuf(pSurfaceDesc, pOMXBuf);
}
