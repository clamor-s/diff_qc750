        ...
        pPortDef = (OMX_PARAM_PORTDEFINITIONTYPE *)pComponentParameterStructure;
        {
            ...
            if((NvU32)s_nvx_port_output == pPortDef->nPortIndex)
            {
                ...
                switch (pPortDef->format.video.eColorFormat) {
                    ...
                    case NVGR_PIXEL_FORMAT_YUV420 | NVGR_PIXEL_FORMAT_TILED:
                    case NVGR_PIXEL_FORMAT_YUV420_NV12:
                    case NVGR_PIXEL_FORMAT_YUV420_NV12 | NVGR_PIXEL_FORMAT_TILED:
                            pPortDef->nBufferSize = (pPortDef->format.video.nFrameWidth*pPortDef->format.video.nFrameHeight*3)>>1;
                    break;
    ...
    case OMX_IndexParamVideoPortFormat:
        {
            ...
            if (portFormat->nPortIndex  >= pComponent->nPorts)
                return OMX_ErrorBadPortIndex;
            pPort = &pComponent->pPorts[portFormat->nPortIndex];

            if (portFormat->nSize != sizeof (*portFormat))
                return OMX_ErrorBadParameter;

            if (portFormat->nPortIndex == s_nvx_port_input)
            {
                if (portFormat->nIndex > 0)
                    return OMX_ErrorNoMore;

                portFormat->eColorFormat = OMX_COLOR_FormatUnused;
                portFormat->eCompressionFormat = pPort->oPortDef.format.video.eCompressionFormat;
            }
            else if (portFormat->nPortIndex == s_nvx_port_output)
            {
                portFormat->eCompressionFormat = OMX_VIDEO_CodingUnused;
                switch (portFormat->nIndex)
                {
                    case 0:
    ...
    case NVX_IndexParamEnableAndroidBuffers:
        {
            OMX_COLOR_FORMATTYPE *format =
                &pComponent->pPorts[s_nvx_port_output].oPortDef.format.video.eColorFormat;
            OMX_COLOR_FORMATTYPE tmp = *format;

            if (pNvxVideoDecoder->nSurfaceLayout == NvRmSurfaceLayout_Tiled)
            ...
            pNvxVideoDecoder->oBase.oPorts[s_nvx_port_output].bUsesANBs = (*format != OMX_COLOR_FormatYUV420Planar);
        }
        break;
    ...
    case OMX_IndexParamVideoPortFormat:
        {
            ...
            if (pPortFormat->nPortIndex  >= pComponent->nPorts)
                return OMX_ErrorBadPortIndex;
            ...
            if (pPortFormat->nPortIndex == s_nvx_port_input)
            {
                if (pPortFormat->eColorFormat != OMX_COLOR_FormatUnused)
                    return OMX_ErrorUnsupportedSetting;
            }
            else if (pPortFormat->nPortIndex == s_nvx_port_output)
            {
                switch (pPortFormat->eColorFormat)
                {
                    case OMX_COLOR_FormatYUV420Planar:
                    case OMX_COLOR_FormatYUV420SemiPlanar:
                        pPort->oPortDef.format.video.eColorFormat = pPortFormat->eColorFormat;
                        break;
                    default:
                        return OMX_ErrorUnsupportedSetting;
                }
...
OMX_ERRORTYPE NvxLiteH264DecoderInit(OMX_IN  OMX_HANDLETYPE hComponent)
{
    ...
    NvOsDebugPrintf("%s : Opening TVMR H264 block \n", __func__);
