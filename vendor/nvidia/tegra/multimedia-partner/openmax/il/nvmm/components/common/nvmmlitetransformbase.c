...
static OMX_ERRORTYPE NvxNvMMLiteProcessInputBuffer(SNvxNvMMLiteTransformData *pData, int nNumStream, OMX_BOOL *pbNextWouldFail)
{
        ...
        else if (pPort->bUsesAndroidMetaDataBuffers)
        {
            ...
            switch (nMetadataBufferFormat)
            {
                ...
                case kMetadataBufferTypeEglStreamSource:
                {
                    ...
                    eError= NvRmMemHandleFromId(pEmbeddedOMXBuffer->pPlatformPrivate,
                ...
                case KMetadataVideoEditorBufferTypeGrallocSource:
                {
                    ...
                    buffer_handle_t *pEmbeddedGrallocSrc;
                    ...
                    pEmbeddedGrallocSrc = (buffer_handle_t *)(pOMXBuf->pBuffer+ 12);
                    handle = (NvNativeHandle *)pEmbeddedGrallocSrc ;
                    ...
                    for (i = 0; i < handle->SurfCount; i++)
                        NvOsMemcpy(&pSurfaceDescriptor->Surfaces[i], &handle->Surf[i], sizeof(NvRmSurface));
                ...
                if (p_extraDataType->eType != OMX_ExtraDataNone)
                {
                    ...
                    do
                    {
                        ...
                        if ((p_extraDataType == NULL) || (p_extraDataType->eType == OMX_ExtraDataNone))
                            break;
                        // Should use sizeof(OMX_OTHER_EXTRADATATYPE), but client which fills pData is stale. Need to fix that first
                        pData += sizeof(OMX_U32) + sizeof(OMX_VERSIONTYPE) + sizeof(OMX_U32) + sizeof(OMX_EXTRADATATYPE) + sizeof(OMX_U32);

                        metadata = (DRM_EXTRADATA_ENCRYPTION_METADATA *) pData;
                        // Should use sizeof(DRM_EXTRADATA_ENCRYPTION_METADATA), but client which fills pData is stale. Need to fix that first
                        pData += sizeof(OMX_U32) + sizeof(OMX_U32) + sizeof(OMX_U64) + sizeof(OMX_U64) + sizeof(OMX_U8);
                        pData = (OMX_U8*)(((OMX_U32)pData+3)&(~3));
    ...
    if (pData->oBlockType == NvMMLiteBlockType_AudioMixer &&
...
OMX_ERRORTYPE NvxNvMMLiteTransformFlush(SNvxNvMMLiteTransformData *pData, OMX_U32 nPort)
{
    ...
    NVMMLITEVERBOSE(("%s: %s[%d] flush/seek stated \n", pData->sName, __FUNCTION__, __LINE__));
...
static OMX_ERRORTYPE NvxNvMMLiteTransformDestroyPort(SNvxNvMMLiteTransformData *pData, int nStreamNum)
{
    ...
    if(pData->pOutputQ[pData->nNumOutputPorts - 1])
        NvMMQueueDestroy(&(pData->pOutputQ[pData->nNumOutputPorts - 1]));
    pData->pOutputQ[pData->nNumOutputPorts - 1] = NULL;
                                                ...
                                                OMX_U32 streamIndex)
{
    ...
    for (i = 0; i < pVideoStreamInfo->NumOfSurfaces; i++)
    {
        ...
        status = NvxVideoSurfaceAlloc(pData->hRmDevice,
            ...
            &ImageSize, UseAlignment, NvOsMemAttribute_Uncached);
                                  ...
                                  void *eventInfo)
{
    ...
    if (pPort->bUsesANBs)
    {
        ...
        switch (NewStreamInfo->ColorFormat) {
            case NvMMVideoDecColorFormat_YUV420:
                {
                    int format = pPortDef->format.video.eColorFormat & ~NVGR_PIXEL_FORMAT_TILED;
                    int layout = NewStreamInfo->Layout == NvRmSurfaceLayout_Tiled ? NVGR_PIXEL_FORMAT_TILED : 0;
                    switch(format)
                    {
        ...
        if (pData->bVideoTransform)
        {
            ...
            else
            {
                        ...
                        pPrivateData->eType == NVX_BUFFERTYPE_ANDROID_BUFFER_HANDLE_T)
                {
                    ...
                    {
                        ExportANBLite(pData->pParentComp, PayloadBuffer,
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                pPrivateData->eType = NVX_BUFFERTYPE_NORMAL;
                                pPrivateData->StereoInfo = (OMX_U32) stereoType;
                                NvxCopyMMBufferToOMXBuf(PayloadBuffer, pPortOut->pCurrentBufferHdr);
                            }
                        }
                    }
                }
