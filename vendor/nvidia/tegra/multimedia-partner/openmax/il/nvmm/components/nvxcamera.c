...
// Number of output still buffers of camera should be
// less than or equal to DZ Output buffers i.e.
// CAMERA_MAX_OUTPUT_BUFFERS_STILL for still passthrough mode.
#define CAMERA_MAX_OUTPUT_BUFFERS_STILL        5
                                        ...
                                        OMX_INOUT OMX_PTR pComponentConfigStructure)
{
    ...
    switch (nIndex)
    {
        ...
        case NVX_IndexConfigFocuserCapabilities:
        {
            ...
            pCap->nRange = focuserCaps.actuatorRange;
            pCap->nSettleTime = focuserCaps.settleTime;
            break;
        }
    ...
    switch (nIndex)
    {
...
#if defined(ANDROID)
        case NVX_IndexConfigFaceTracker:
        {
            NvMMCameraFaceTrackerMode newMode, curMode;
            NvError err = NvSuccess;
            NvMMCameraFaceTrackerRequest req;
            NVX_CONFIG_FACETRACKER *pFDParams =
                (NVX_CONFIG_FACETRACKER *)pComponentConfigStructure;

            newMode = ((NVX_CONFIG_FACETRACKER *)pComponentConfigStructure)->enable;
            err = cb->GetAttribute(cb,
                                   NvMMCameraAttribute_FaceTracker,
                                   sizeof(NvMMCameraFaceTrackerMode),
                                   &curMode);
            ...
            if (newMode != curMode)
            {
                ...
                req.minFaceSize = pFDParams->minFaceSize;
                req.maxFaceSize = pFDParams->maxFaceSize;
                req.maxFaces = pFDParams->maxFaces;
                cb->Extension(cb,
                              NvMMCameraExtension_TrackFace,
                              sizeof(NvMMCameraFaceTrackerRequest),
                              &newMode, 0, NULL);
            }
        ...
        case OMX_IndexConfigCommonFrameStabilisation:
        {
            ...
            if (pfs->bStab)
            {
                ...
                {
                    sMode = NvCameraIspStabilizationMode_Image;
                }
        ...
        case NVX_IndexConfigAllocateRemainingBuffers:
        {
            NvMMRemainingBuffers pRemainingBuffers;
            NvError Err = NvSuccess;
            NvU32 nPortIndex;
            NVX_CONFIG_REMAININGBUFFERTYPE * configBufferType =
                (NVX_CONFIG_REMAININGBUFFERTYPE *) pComponentConfigStructure;
            pRemainingBuffers.bEnable = configBufferType->enableAllocateBuffers;
            pRemainingBuffers.numBuffers = configBufferType->numBuffers;
            nPortIndex = configBufferType->nPortIndex;

            if (nPortIndex == s_nvx_port_still_capture)
                pRemainingBuffers.StreamIndex = s_nvx_port_dz_output_still;
            else if (nPortIndex == s_nvx_port_video_capture)
                pRemainingBuffers.StreamIndex = s_nvx_port_dz_output_video;
            else
                return OMX_ErrorBadPortIndex;

            Err = zb->SetAttribute(zb,
                    NvMMDigitalZoomAttributeType_AllocateRemainingBuffers,
                    0, sizeof(NvMMRemainingBuffers), &pRemainingBuffers);
            if (Err != NvSuccess)
                return OMX_ErrorBadParameter;

            Err = cb->SetAttribute(cb, NvMMCameraAttribute_AllocateRemainingBuffers,
                                0, sizeof(NvMMRemainingBuffers), &pRemainingBuffers);
            if (Err != NvSuccess)
                return OMX_ErrorBadParameter;
        }
...
static NvError s_SetCrop(SNvxCameraData *pNvxCamera, int nPortId)
{
    OMX_CONFIG_RECTTYPE *pRect;
    NvRect oNvmmRect;
    NvMMDigitalZoomAttributeType eAttrRect;
    NvMMDigitalZoomAttributeType eAttrEnable;
    NvBool bCropping;
    ...
    NvMMDigitalZoomFrameCopyColorFormat *pNvColorFormat)
{
    ...
    else if (NvxImageColorFormat == NVX_IMAGE_COLOR_FormatY)
    {
        pNvColorFormat->NumSurfaces = 1;
        pNvColorFormat->SurfaceColorFormat[0] = NvColorFormat_Y8;
    }
    else
        return OMX_ErrorUnsupportedSetting;
