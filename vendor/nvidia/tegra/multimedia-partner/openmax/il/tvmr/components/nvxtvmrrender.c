    ...
    *pbMoreWork = bAllPortsReady;
    ...
    if (pPortIn->pCurrentBufferHdr->nFlags & OMX_BUFFERFLAG_EOS)
    {
        ...
        if (pNvxVideoTvmrRenderer->bYuvOverlay)
        {
            ...
            pVideoSurface =  TVMRVideoSurfaceCreate(pNvxVideoTvmrRenderer->pDevice, TVMRSurfaceType_YV12, 800, 480, OMX_TRUE);
            NvOsMemset(pVideoSurface->surfaces[0]->mapping, 0, pVideoSurface->surfaces[0]->pitch * pVideoSurface->height);
            NvOsMemset(pVideoSurface->surfaces[1]->mapping, 128, (pVideoSurface->surfaces[1]->pitch * pVideoSurface->height >> 1));
            NvOsMemset(pVideoSurface->surfaces[2]->mapping, 128, (pVideoSurface->surfaces[2]->pitch * pVideoSurface->height >> 1));
...
#ifdef TVMR_FLIP_QUEUE_DISPLAY_WITH_TIMESTAMP
                                    , NULL
#endif
                                   );
