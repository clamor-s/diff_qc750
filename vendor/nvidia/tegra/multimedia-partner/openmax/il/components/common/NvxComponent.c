                                     ...
                                     OMX_VERSIONTYPE nVerB)
{
    ...
    if (nVerB.s.nVersionMinor != nVerB.s.nVersionMinor)
    {
    ...
    else
    {
            ...
            pNvComp->eState == OMX_StateIdle)
        {
            for (uPort = 0; uPort < pNvComp->nPorts; ++uPort) {
                ...
                if (bUnpopulated)
                {
                   pNvComp->pCallbacks->EventHandler(pNvComp->hBaseComponent, pNvComp->pCallbackAppData, OMX_EventError, OMX_ErrorPortUnpopulated, uPort, 0);
                   break;
                }
            }
        }
...
struct NvxExtEntry NvxExtensionIndexTable[] = {
    ...
    { NVX_INDEX_CONFIG_FACETRACKER, NVX_IndexConfigFaceTracker },
    ...
    { NVX_INDEX_CONFIG_ALLOCATEREMAININGBUFFERS, NVX_IndexConfigAllocateRemainingBuffers},
