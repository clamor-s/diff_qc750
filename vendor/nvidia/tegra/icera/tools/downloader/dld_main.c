...
int DLD_CDECL main(int argc, char *argv[])
{
    ...
    if (error == 0)
    {
        ...
        if (error != 8)
        {
            ...
            {
                error = convert_status(Updater(option_archname, option_dev_name, option_speed,
                                               option_delay, option_block_sz, option_flash, option_verbose,
                                               option_modechange, option_pcid));
            }
        }
#ifdef _WIN32
