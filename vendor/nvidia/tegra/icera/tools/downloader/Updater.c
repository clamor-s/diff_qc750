...
static const tArchFileProperty arch_type[] = DRV_ARCH_TYPE_TABLE;
    ...
    if (size > 0)
    {
        ...
        if (arch_desc)
        {
            do
            {
                ...
                if  ((arch_header->tag != ARCH_TAG_8040) && (arch_header->tag != ARCH_TAG_8060))
                {
...
static char *FindPkgvFile(char *filelist)
{
    ...
    if (pkgv_end)
    {
        ...
        file = malloc(pkgv_end - start + sizeof(PKGV_EXTENSION));
...
static void GetCurrentPkgv(UPD_Handle handle, char *pkgv)
{
    if (AT_SUCCESS(at_SendCmd(HDATA(handle).com_hdl, "AT%%IPKGV?\r\n")))
    {
        char *response = strstr(at_GetResponse(HDATA(handle).com_hdl),"%IPKGV: ");
...
static int RestoreSettings(UPD_Handle handle)
{
    ...
    error = SwitchMode(handle, HDATA(handle).option_modechange);
                                    ...
                                    if (backup)
                                    {
                                        ...
                                        while (line && (line < (content + size)))
                                        {
                                            ...
                                            if (strlen(line) > 0)
                                            {
                                                ...
                                                if ((GetWait(line) <= 0) && (GetMode(line) == MODE_INVALID))
                                                {
                                                    free(tmpfilename);
    ...
    if (files)
    {
        ...
        if (filesdup)
        {
            ...
            while (filepath && (filepath < (filesdup + size)))
            {
                ...
                if ((GetWait(filepath) <= 0) && (GetMode(filepath) == MODE_INVALID))
                {
...
static int DelayedSwitchMode(UPD_Handle handle, char *command, int size, int mode)
{
    ...
    if (file)
    {
        ...
        if (stat(file, &stat_info) == 0)
        {
            ...
            if (stat_info.st_size > MAX_LOGFILE_SIZE)
            {
                ...
                new_file_name = alloca(strlen(file) + 4 + 1);
...
ICERA_API int SwitchMode(UPD_Handle handle, unsigned int mode)
{
    ...
    if (mode != current_mode)
    {
        ...
        do
        {
            ...
            #endif /* _WIN32 */
            {
                status = DelayedSwitchMode(handle, command, sizeof(command), mode);
...
LRESULT CALLBACK WinProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (msg == WM_DEVICECHANGE)
    {
        ...
        if (HDATA((UPD_Handle)idx).pnp_callback)
            result = HDATA((UPD_Handle)idx).pnp_callback(wParam, (unsigned char *)lParam, (UPD_Handle)idx);
        if (result == TRUE)
