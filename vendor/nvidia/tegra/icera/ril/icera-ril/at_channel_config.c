...
#define LOG_TAG "RIL_ATCC"  /**< log tag for this file */
...
static void atcc_set_ril_request_list( at_channel_config_t* config )
{
    if ( config->chan_id < (sizeof(s_chan_list) / sizeof(int*)) )
    {
        config->ril_req_id_list = s_chan_list[config->chan_id];
    }
    else
    {
            ...
            *line = '\0';
            ...
            if ( strlen(line) )
            {
                if ( strstr( line, "DLC" ) )
                {
                    ...
                    {
                        atcc_set_ril_request_list( config );        /* initialize RIL Request list */
