...
static void CloseFile(void)
{
    ...
    {
        ALOGI("%s: %s: closing file (%s)", obex_log_prefix, __FUNCTION__, obex_filesystem.objstate.name);
...
static void CloseDir(void)
{
    ...
    {
        ALOGI("%s: %s: closing dir", obex_log_prefix, __FUNCTION__);
...
static obex_ResCode HandleNameOrDestNameHeader(char *name, obex_GenericHeader *header)
{
    ...
    do
    {
        ...
        if ((strlen(obex_filesystem.inbox) + name_character_length + strlen(DRV_FS_SEPARATOR))
            > MAX_FILENAME_LENGTH)
        {
        ...
        if (header->id == OBEX_OBJHDID_NAME)
        {
            obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_NAME_HEADER, (uint32)name_character_length);
        }
        else if (header->id == OBEX_OBJHDID_DESTNAME)
        {
            obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_DEST_NAME_HEADER, (uint32)name_character_length);
        }
        else
        {
            OBEX_FAIL_EXTRA("Unknown header id", 1, header->id);
        }
    } while (0);
...
static obex_ResCode HandleTypeHeader(obex_PacketDesc *packet, obex_GenericHeader *header)
{
    ...
    do
    {
        ...
        obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_TYPE_MD5_HEADER);
    } while (0);
...
static obex_ResCode HandleLengthHeader(obex_PacketDesc *packet, obex_GenericHeader *header)
{
    ...
    do
    {
        ...
        obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_LENGTH_HEADER, (uint32)obex_filesystem.actionstate.length);
    } while (0);
...
static obex_ResCode HandleActionIdHeader(obex_PacketDesc *packet, obex_GenericHeader *header)
{
    ...
    do
    {
        ...
        obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_ACTIONID_HEADER, (uint32)obex_filesystem.actionstate.actionid);
    } while (0);
...
static obex_ResCode HandleBodyEndOfBodyHeaders(obex_PacketDesc *packet, obex_GenericHeader *header)
{
    ...
    do
    {
        ...
        if (obex_filesystem.objstate.handle==INVALID_HANDLE)
        {
             ...
             *  supported */
            ALOGI("%s: %s: opening %s for creating.\n",
                 obex_log_prefix,
                 __FUNCTION__,
            ...
            if (handle < 0)
            {
                ...
                obex_LogValue(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_FAILED_TO_OPEN, handle);
                ret = OBEX_ERR;
                break;
            }
            ...
            /* if s start offset specified through a APP_PARAMS header,
            let us move the write pointer */
            if (obex_filesystem.chunkinfo.start_offset != 0)
            {
                /* retrieve file size */
                ALOGI("%s: %s: get %s size", obex_log_prefix, __FUNCTION__, obex_filesystem.objstate.name);
                file_size = obex_filesystem.funcs.lgetsize(handle);
                if (file_size == -1)
                {
                    /* c.f. TC TestErrorOnPutChunkWithFailureInGetSize */
                    obex_LogMarker(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_FAILED_TO_LGETSIZE);
                    ret = OBEX_ERR;
                    break;
                }
                ALOGI("%s: %s: %s size: %d", obex_log_prefix, __FUNCTION__, obex_filesystem.objstate.name, file_size);
                ...
                ALOGI("%s: %s: seek in %s", obex_log_prefix, __FUNCTION__,  obex_filesystem.objstate.name);
                if (obex_filesystem.funcs.lseek(handle, obex_filesystem.chunkinfo.start_offset, SEEK_SET) == -1)
                {
                    ...
                    obex_LogMarker(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_FAILED_TO_SEEK_IN_FILE);
                    ret = OBEX_ERR;
                    break;
                }
            }
        ...
        if (header->value.byte_sequence.byte_length>0)
        {
            ...
            if ((uint32)write_ret != header->value.byte_sequence.byte_length)
            {
                ...
                obex_LogMarker(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_FAILED_TO_WRITE_TO_FILE);
                ret = OBEX_ERR;
                break;
            }
        }

        if (OBEX_OBJHDID_BODY == header->id)
        {
            obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_BODY_HEADER, (uint32)header->value.byte_sequence.byte_length);
        }
        else
        {
            obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_END_OF_BODY_HEADER, (uint32)header->value.byte_sequence.byte_length);
        }
    } while (0);
...
static obex_ResCode HandleAppParamsHeaders(obex_PacketDesc *packet, obex_GenericHeader *header)
{
    ...
    do
    {
        ...
        obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_APP_PARAMS_HEADER_START_OFFSET, obex_filesystem.chunkinfo.start_offset);
        /* retrieve length */
        ...
        obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_APP_PARAMS_HEADER_LENGTH, obex_filesystem.chunkinfo.length);
...
static obex_ResCode HandlePutRequest(obex_PacketDesc *packet)
{
    ...
    do
    {
        ...
        if (OBEX_OPS_PUT == packet->code)
        {
            obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_PUT);
        }
        else if (OBEX_OPS_PUT_FINAL == packet->code)
        {
            obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_PUT_FINAL);
        }
        else
        {
            OBEX_FAIL_EXTRA("Unsupported opcode", 1, packet->code);
        }
        ...
        if (packet->code == OBEX_OPS_PUT_FINAL)
        {
            ...
            else if (strcmp(obex_filesystem.objstate.name,obex_file_system_initial_state.objstate.name) != 0)
            {
                ...
                {
                    obex_LogMarker(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_PUT_REMOVE_FAILED);
                    ret = OBEX_ERR;
                    break;
                }
            ...
            /* send OK_FBS back */
            ALOGD("%s: %s: send OBEX_RESP_OK_FBS", obex_log_prefix, __FUNCTION__);
            ret = obex_CoreSendSimplePacket(OBEX_RESP_OK_FBS,NULL,0);
            break;
        }

        ALOGD("%s: %s: send OBEX_RESP_CONT_FBS", obex_log_prefix, __FUNCTION__);
...
static obex_ResCode HandleGetFile(void)
{
    ...
    do
    {
        ...
        if ( bytes_read != bytes_in_next_packet)
        {
            ...
            obex_LogMarker(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_FAILED_TO_READ_FROM_FILE);
            ret = OBEX_ERR;
            break;
        }
        ...
        /* send packet */
        ALOGD("%s: %s: send packet with code 0x%x and header 0x%x",
             obex_log_prefix,
             __FUNCTION__,
             packet_code,
             header_code);
...
static obex_ResCode HandleGetMD5(void)
{
    ...
    */
    do
    {
        ...
        /* send packet */
        ALOGD("%s: %s: send packet", obex_log_prefix, __FUNCTION__);
        ret = obex_CoreSendPacket(&response_packet);
        if (ret!=OBEX_OK)
        {
            obex_LogValue(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_PACKET_SEND_FAILURE, ret);
...
static obex_ResCode HandleFirstGetResponse(void)
{
    ...
    */
    do
    {
        ...
        if(ans == -1)
        {
            obex_LogMarker(OBEX_VERBOSE_WARNING, OBEX_LOG_FILE_ERR_OBJ_STAT_FAILURE);
            ret = OBEX_ERR;
            break;
        }
        else
        {
            ...
            if(OBEX_ISDIR(stat))
            {
                ...
                if(dir==NULL)
                {
                    obex_LogMarker(OBEX_VERBOSE_ERROR, OBEX_LOG_DIR_ERR_FAILED_TO_OPEN);
                    ret = OBEX_ERR;
                    break;
                }
            ...
            else
            {
                ...
                if (handle < 0)
                {
                    ...
                    obex_LogMarker(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_FAILED_TO_OPEN);
                    ret = OBEX_ERR;
                    break;
                }
                ...
                ALOGI("%s: %s: size %d", obex_log_prefix, __FUNCTION__, file_size);
                ...
                if ((obex_filesystem.chunkinfo.start_offset != 0) || (obex_filesystem.chunkinfo.length != 0))
                {
                    ...
                    ALOGI("%s: %s: seek in file %s", obex_log_prefix, __FUNCTION__, obex_filesystem.objstate.name);
                    if (obex_filesystem.funcs.lseek(handle, obex_filesystem.chunkinfo.start_offset, SEEK_SET) == -1)
                    {
...
static obex_ResCode HandleGetRequest(obex_PacketDesc *packet)
{
    ...
    do
    {
        ...
        if (OBEX_OPS_GET == packet->code)
        {
            obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_GET);
        }
        else
        {
            obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_GET_FINAL);
        }
    ...
    {
        obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_RESET_GLOBAL_STATE);
        ResetFileSystemGlobalState();
    }
...
static obex_ResCode HandleActionResize(void)
{
    ...
    do
    {
        ...
        ALOGI("%s: %s: opening %s for writing.\n", obex_log_prefix, __FUNCTION__, obex_filesystem.objstate.name);
#ifdef _WIN32
    ...
    do
    {
        ...
        if (OBEX_ACTION_COPY == obex_filesystem.actionstate.actionid)
        {
            ...
            if (res != 0)
            {
                ...
                obex_LogValue(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_FAILED_TO_COPY_FILE, res);
                ret = OBEX_ERR;
                break;
            }
        }
        else if (OBEX_ACTION_MOVE_RENAME == obex_filesystem.actionstate.actionid)
        {
            ...
            if (res != 0)
            {
                ...
                obex_LogMarker(OBEX_VERBOSE_ERROR, OBEX_LOG_FILE_ERR_FAILED_TO_RENAME_FILE);
                ret = OBEX_ERR;
                break;
            }
        }
        ...
        {
            OBEX_FAIL_EXTRA("Unknown ACTION ID", 1, obex_filesystem.actionstate.actionid);
        }
...
static obex_ResCode HandleActionRequest(obex_PacketDesc *packet)
{
    ...
    do
    {
        ...
        {
            obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_FILE_ACTION_FINAL);
        }
        else
        {
            OBEX_FAIL_EXTRA("Unsupported opcode", 1, packet->code);
...
obex_ResCode obex_FileRegister(const obex_FileFuncs *funcs, const obex_DirFuncs *d_funcs)
{
    ...
    if ((NULL==funcs->open)
        ...
        || (NULL==funcs->stat))
    {
        OBEX_FAIL("One of the callbacks from the OBEX file functional interface is NULL");
    }
    ...
    {
        OBEX_FAIL("One of the callbacks from the OBEX dir functional interface is NULL");
    }
