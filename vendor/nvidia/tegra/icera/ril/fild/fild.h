...
/* Default root dir for boot server and obex server */
#define DEFAULT_ICERA_ROOT_DIR "/data/rfs"

/* Default dir for coredump storage */
#define DEFAULT_COREDUMP_DIR DEFAULT_ICERA_ROOT_DIR"/data/debug"
...
#define DEFAULT_MAX_NUM_OF_CORE_DUMP "3"

/* Default max num of crash_dump files */
#define DEFAULT_MAX_NUM_OF_CRASH_DUMP "10"
...
#define PLATFORM_CONFIG_FILE ICERA_DATA_FACT"/platformConfig.xml"   /** Platform config xml file */
...
/** 0MB reserved by default at the top of private memory
 *  window (noninit...) */
#define DEFAULT_SHM_RESERVED  "0"
...
#define LATEST_SUPPORTED_BT2_VERSION_COMPATIBILITY VERSION_IFT_IN_MDM
...
typedef struct
{
    ...
    int is_uart;                     /** If channel is_uart then channel require specific settings at init (open) */
...
/**
 * Scan folder and look forward file with given
 * format and remove older ones if storage limit is reached.
 *
 * File format:
 * <prefix><data>.<extension>
 *
 * @param dir        directory to scan
 * @param prefix     prefix of file to count
 * @param extension  extension of file to count
 * @param format     full format of file
 * @param limit      max num of file to store
 */
