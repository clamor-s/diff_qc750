...
static obex_ServerOperationStruct *FindOperation(obex_Opcode opcode)
{
    ...
    OBEX_ASSERT_EXTRA( OBEX_OPS_NONE == obex_server.operations[OBEX_SERVER_MAX_OPCODES-1].opcode,
                      1,
                      obex_server.operations[OBEX_SERVER_MAX_OPCODES-1].opcode);

    obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_SERVER_FIND_CB_FOR_OPCODE, opcode);
    ...
    if(return_value)
    {
        obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_SERVER_FIND_CB_FOR_OPCODE_FOUND);
    }
    else
    {
        obex_LogMarker(OBEX_VERBOSE_DEBUG, OBEX_LOG_SERVER_FIND_CB_FOR_OPCODE_NOT_FOUND);
    }
...
static obex_ResCode HandleError(obex_ResCode res)
{
    ...
    obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_SERVER_ERR_REPORT, response_code);
    ...
    do
    {
        ...
        if ((operation=FindOperation(packet.code)) != NULL)
        {
            ...
            default:
                ...
                obex_LogValue(OBEX_VERBOSE_DEBUG, OBEX_LOG_SERVER_ERR_OPERATION_FAILED, packet.code);
                obex_server.state.processing_request = OBEX_OPS_NONE;
                HandleError(res);
                break;
            }
...
obex_ResCode obex_ServerRegisterOperationCallback(obex_Opcode opcode, obex_ServerOperationCallback callback)
{
    ...
    /* sanity checks - note: it is OK to ASSERT in this init-time code */
    OBEX_ASSERT_EXTRA(opcode != OBEX_OPS_NONE, 1, opcode);
    OBEX_ASSERT(callback != NULL);
