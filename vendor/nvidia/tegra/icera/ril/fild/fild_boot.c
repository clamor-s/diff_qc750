...
/* Coredump files */
#define CORE_DUMP_NAME_PREFIX "coredump_"
#define CORE_DUMP_NAME_EXTENSION ".bin"
#define CORE_DUMP_NAME_FORMAT CORE_DUMP_NAME_PREFIX""FILE_DATE_FORMAT""CORE_DUMP_NAME_EXTENSION

/* Crashdump files */
#define CRASH_DUMP_NAME_PREFIX "crash_dump_"
#define CRASH_DUMP_NAME_EXTENSION ".zip"
#define CRASH_DUMP_NAME_FORMAT CRASH_DUMP_NAME_PREFIX""FILE_DATE_FORMAT""CRASH_DUMP_NAME_EXTENSION
...
{
    int buf_len = 0, fd, len;
    char filename[MAX_FILENAME_LEN];
    RespOpenItemHeader header;

    /* Platform config is always OK at this moment... */
    len = strlen(fild_boot.plat_config)+1;

    if(bt2_version >= VERSION_FILES_BUFFER)
    {
        ...
        if(fild_CheckFileExistence(filename) != -1)
        {
            ...
            if(len)
            {
                ...
                *buffer = realloc(*buffer, buf_len + sizeof(header) + len);
                ...
                header.len = len;
                memcpy(*buffer + buf_len, &header, sizeof(header));
    ...
    {
        /* Old scheme: simply send plat config.*/
        *buffer = malloc(len);
        memcpy(*buffer, fild_boot.plat_config, len);
        buf_len = len;
    }

    /** Always align buffer (and add padding) whatever the scheme
     *  used for RESPOPEN_EXT_VER */
    len = buf_len;
    AlignBlockSize((unsigned int *)&buf_len, len);
    if(buf_len != len)
    {
        ...
        *buffer = realloc(*buffer,buf_len);
        ...
        /* Set 0 for padding... */
        memset(*buffer+len, 0, (buf_len - len));
    }
    ...
    do
    {
        ...
        if(ok == fild_boot.req_size_in_bytes)
        {
            ...
            default:
                ...
                {
                    LOGI("%s: invalid ACK on last block considered as BLOCK ACCEPT...", __FUNCTION__);
                    status = ACCEPTED;
                }
                break;
            }
 ...
 * @param coredump_dir      directory to store received coredump
 * @param block_size        block size in bytes.
 * @param max_coredump      max num of coredump files to store
 *                          in file system.
 *
                           ...
                           int max_coredump)
{
    ...
    do
    {
        ...
        {
            /** TODO:
               - check if feature is really enabled in file system, in AP environment...
               - authenticate target... */

            /** Check if we need to remove a previous coredump file:
               No more than max_coredump stored in file system... */
            fild_CheckDatedFilesDir(coredump_dir,
                                    CORE_DUMP_NAME_PREFIX,
                                    CORE_DUMP_NAME_EXTENSION,
                                    CORE_DUMP_NAME_FORMAT,
                                    max_coredump-1);

            /** Open new coredump file */
            fild_SetTimeStr(time_str);
            len = strlen(coredump_dir) +
                strlen("/") +
                strlen(CORE_DUMP_NAME_PREFIX) +
                strlen(time_str) +
                strlen(CORE_DUMP_NAME_EXTENSION) +
                1;
            dump_name = malloc(len);
            snprintf(dump_name,
                     len,
                    "%s/"CORE_DUMP_NAME_PREFIX"%s"CORE_DUMP_NAME_EXTENSION,
                    coredump_dir, time_str);
            dump_fd = fild_Open(dump_name, O_CREAT | O_WRONLY, DEFAULT_CREAT_FILE_MODE);
            if(dump_fd == -1)
            {
                /* File system is not OK for coredump reception: force coredump disabled */
                fild_boot.coredump_enabled = 0;
                ALOGE("%s: fail to open %s [%s].\n", __FUNCTION__, dump_name, fild_LastError());
            }
        }
        ...
        if(numread == fild_boot.req_size_in_bytes)
        {
            ...
            case UART_BT2_ACQ_REQ:
                ...
                if((fild_boot.chip_type != ARCH_TAG_8060) &&
                   (fild_boot.channel[FILD_PRIMARY_BOOT_CHANNEL].is_uart))
                {
                ...
                if (fild_boot.channel[FILD_PRIMARY_BOOT_CHANNEL].is_uart)
                {
                    /** UART scheme for HIF means USB usage for MDM (and BT3)
                     *  transmission: BB just reset, unload USB stack to reset it
                     *  also on the AP side. */
                    fild_UnloadUsbStack();
                }
            ...
            case UART_APP_ACQ_REQ:
                ...
                if((index == FILD_PRIMARY_BOOT_CHANNEL) && fild_boot.use_bt3)
                {
                     ...
                     *  enumerate USB: reload USB stack. */
                    fild_ReloadUsbStack();
            ...
            case UART_DEBUG_REQ:
                ...
                ok = ReceiveCoredump(handle,
                                     fild_boot.coredump_dir,
                                     fild_boot.dbg_block_size,
                                     fild_boot.max_coredump);
                if(fild_boot.coredump_enabled)
                {
                    if(ok)
                    {
...
static void SerialBoot(int channel)
{
    ...
    do
    {
        ...
        else
        {
			...
			ok = 1;
            if(fild_boot.channel[channel].is_uart)
            {
                        ...
                        fildHif *hif)
{
    ...
    fild_boot.plat_config = malloc(MAX_FILENAME_LEN);
    strncpy(fild_boot.plat_config, boot_cfg->plat_config, MAX_FILENAME_LEN);
    ...
    if(boot_cfg->coredump_enabled)
    {
        ...
        fild_boot.coredump_dir = malloc(PROPERTY_VALUE_MAX);
        strncpy(fild_boot.coredump_dir, boot_cfg->coredump_dir, PROPERTY_VALUE_MAX);
    ...
    case FILD_HSI:
        ...
        fild_boot.max_crashdump  = boot_cfg->max_crashdump;
        fild_boot.max_coredump   = boot_cfg->max_coredump;
        ...
        if(fild_boot.hif == FILD_UART)
        {
            ...
            fild_boot.channel[FILD_PRIMARY_BOOT_CHANNEL].is_uart = 1;
            ...
            fild_boot.channel[FILD_SECONDARY_BOOT_CHANNEL].is_uart = 1;
