...
// FIXME: figure out how to reclaim this memory
...
static NvU32 nvaos_GetUncachedOffset( void )
{
    return nvaos_GetMaxMemorySize();
}
...
#define AOS_REMAIN_ALLOC_BOTTOM (AOS_RESERVE_FOR_RAMDISK_IMAGE_BOTTOM + RAMDISK_MAX_SIZE)
...
Allocator *s_Allocators[] = { &s_InitAllocator, &s_RemainAllocator, NULL };
...
NvAosChip s_Chip         = NvAosChip_Ap20;
...
nvaosReadPmcScratch( void )
{
    ...
    if( s_Chip == NvAosChip_Ap20 )
    {
        odmData = nvaosAp20GetOdmData();
    }
    else if( s_Chip == NvAosChip_T30 )
    {
        odmData = nvaosT30GetOdmData();
    }
    return odmData;
}
...
nvaosGetMemoryMap( MemoryMap *map )
{
    switch( s_Chip ) {
    case NvAosChip_Ap20:
        map->ext_start = APXX_EXT_MEM_START;
        map->ext_end = APXX_EXT_MEM_END;
        map->mmio_start = APXX_MMIO_START;
        map->mmio_end = APXX_MMIO_END;
        break;
    case NvAosChip_T30:
...
NvU32 nvaosGetOscFreq( NvU32 *dividend, NvU32 *divisor )
{
    NvU32 freq;

    freq = NV_DRF_VAL( CLK_RST_CONTROLLER, OSC_CTRL, OSC_FREQ,
...
void nvaosTimerInit( void )
{
    ...
    switch( s_Chip ) {
    case NvAosChip_Ap20: id = 0x20; break;
    case NvAosChip_T30: id = 0x30; break;
    default: id = 0x20; break;
    }
     ...
     */
    s_usec = 0;
    if (s_Chip >= NvAosChip_T30)
    {
        (void)nvaosT30GetOscFreq(&dividend, &divisor);
    }
    else
    {
        (void)nvaosGetOscFreq(&dividend, &divisor);
    }
    ...
    reg = NV_FLD_SET_DRF_NUM(TIMERUS, CNTR_FREEZE, DBG_FREEZE_CPU0, 1, reg);
...
static void
nvaos_InitPmcScratch(NvAosChip Chip)
{
    //  Initialize the BCT customer option and PMC scratch registers
    if( Chip == NvAosChip_Ap20 )
    {
        nvaosAp20InitPmcScratch();
    }
    else if( Chip == NvAosChip_T30 )
    {
        nvaosT30InitPmcScratch();
    }
}
...
void nvaosConfigureCache(void)
{
    ...
    /* We enable the SCU early only for AP20 because it is required for PCIE.
     * For all other A9-based chips, delay SCU initialization until it is
     * decided that we need it for SMP to save power and reduce latency.
     */
    reg = AOS_REGR( MISC_PA_BASE + APB_MISC_GP_HIDREV_0 );
    if (NV_DRF_VAL(APB_MISC, GP_HIDREV, CHIPID, reg) == 0x20)
    {
        enableScu();
    }
...
void
nvaosL2CacheInit(void)
{
    nvaosPl310Init();
}

void
nvaosL2CacheEnable(void)
{
    nvaosPl310Enable();
}

extern Allocator *s_Allocators[];
...
void   nvaosEnableMMU(NvU32 TtbAddr)
{
    ...
    // Select Access Flag Enable so that AP[0] is used as Access bit.
    MRC(p15, 0, i, c1, c0, 0);
    i |= SCTLR_AFE;
    MCR(p15, 0, i, c1, c0, 0);
...
//                          memory aperture
// page 0x108 - page 0x1ffff: Reminder of 512MB RAM used by program and data.
//
// LD script should load the program starting from page 0x108
    ...
    // Map all of MMIO space, except for the alread-mapped last megabyte
...
void nvaosFinalSetup(void)
{
    ...
    nvaosSetupTZMemory();
    NvRmPrivHeapSecuredInit(s_SecuredAllocator.size, (NvRmPhysAddr)s_SecuredAllocator.base);
...
//
// Reverse page mapping order
// For SMMU test
//
#define PTE2PAGEADDR(pte) (void *)((pte)&~(NVAOS_PAGE_SIZE-1))
NvError
NvOsReverseMemMap(NvOsPhysAddr phys, NvU32 NumPages)
{
    int uncached;

    for (uncached = 0; uncached <= 1; uncached++)
    {
        NvU32 PTEbase = (phys + uncached * nvaos_GetUncachedOffset()
                            - BottomOfDram) >> 12;
        NvU32 *pL = &(SecondLevelPageTable[AosPte_Dram][PTEbase]);
        NvU32 *pH = pL + (NumPages - 1);
        ...
        while (pL < pH)
        {
            ...
            {// Swap contents as well
                NvU8 buffer[NVAOS_PAGE_SIZE];
                NvOsMemcpy(buffer, PTE2PAGEADDR(*pL), NVAOS_PAGE_SIZE);
                NvOsMemcpy(PTE2PAGEADDR(*pL),PTE2PAGEADDR(*pH),NVAOS_PAGE_SIZE);
                NvOsMemcpy(PTE2PAGEADDR(*pH), buffer, NVAOS_PAGE_SIZE);
                NvOsDataCacheWritebackRange(PTE2PAGEADDR(*pL), NVAOS_PAGE_SIZE);
                NvOsDataCacheWritebackRange(PTE2PAGEADDR(*pH), NVAOS_PAGE_SIZE);
            }
            *pL++ = *pH;
            *pH-- = pteSaver;
        }
        ...
        NvOsDataCacheWritebackRange(
            &(SecondLevelPageTable[AosPte_Dram][PTEbase]),
            sizeof(NvU32) * NumPages);
    }
    nvaosFlushCache();
...
{
    NvU32 PageAlingedStart;

    NV_ASSERT(descriptor != NULL);
    PageAlingedStart = ((NvU32)descriptor + NVAOS_PAGE_SIZE - 1) &
                                            ~(NVAOS_PAGE_SIZE - 1);
    /* Cached virtual == Physical on AOS */
    return PageAlingedStart + offset;
}
