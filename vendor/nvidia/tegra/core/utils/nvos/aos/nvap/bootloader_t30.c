...
#define ENTERPRISE_BOARD_ID 0xC05
#define E1197_BOARD_ID 0x0B61
#define E1565_BOARD_ID 0x0F41
#define TAI_BOARD_ID   0xC27
...
static void NvBlAvpResetCpu(NvBool reset)
{
    ...
    /* Read the board Id */
    #ifdef BOARD_IS_NABI2
	BoardId =E1565_BOARD_ID;
   #else
   	BoardId = NvBlAvpI2cReadBoardId(EEPROM_I2C_SLAVE_ADDRESS);
   #endif
	
    if (BoardId == E1565_BOARD_ID)
    {
        // *((volatile long *)0x6000d208) = 0x2 ; // GPIO_K.01 GPIO mode
        Reg = NV_GPIO_REGR(GPIO3_K_PA_BASE, CNF);
        Reg = NV_FLD_SET_DRF_NUM(GPIO, CNF, BIT_1, 1, Reg);
        NV_GPIO_REGW(GPIO3_K_PA_BASE, CNF, Reg);

        // *((volatile long *)0x6000d218) = 0x2 ; // GPIO_K.01 output
        Reg = NV_GPIO_REGR(GPIO3_K_PA_BASE, OE);
        Reg = NV_FLD_SET_DRF_NUM(GPIO, OE, BIT_1, 1, Reg);
        NV_GPIO_REGW(GPIO3_K_PA_BASE, OE, Reg);

        // *((volatile long *)0x6000d228) = 0x0 ; // GPIO_K.01 low
        Reg = NV_GPIO_REGR(GPIO3_K_PA_BASE, OUT);
        Reg = NV_FLD_SET_DRF_NUM(GPIO, OUT, BIT_1, 0, Reg);
        NV_GPIO_REGW(GPIO3_K_PA_BASE, OUT, Reg);
    }
...
#ifdef BOARD_IS_NABI2
#define I2C_POLL_TIMEOUT_MSEC   100
#else
...
#ifdef SET_I2C_EXPANDER
static void NvBlAvpI2cPoll(void)
{
    NvU32 status = 0;
    NvU32 timeout = 0;
...
static void NvBlSetI2cExpander(void)
{    // Program the i2c expander
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CMD_ADDR0, 0x42);
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CMD_DATA1, 0x03);
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CNFG, 0xA04);
    NvBlAvpI2cPoll();

    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CMD_ADDR0, 0x42);
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CMD_DATA1, 0x01);
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CNFG, 0xA04);
    NvBlAvpI2cPoll();
}
#endif
...
static NvU32 NvBlAvpI2cReadEeprom(NvU32 Addr, NvU32 Reg)
{
    NvU32 Ret = 0;
#ifdef SET_I2C_EXPANDER
    static NvBool SetI2cExpander = NV_TRUE;

    if (SetI2cExpander)
    {
        // Program the i2c expander before reading Board Id
        NvBlSetI2cExpander();
        SetI2cExpander = NV_FALSE;
    }
#endif

    //Read the Board Id
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CMD_ADDR0, (Addr & 0xFE));
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CMD_ADDR1, (Addr | 0x1));
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CMD_DATA1, Reg);
    NV_PWRI2C_REGW(PWRI2C_PA_BASE, I2C_CNFG, 0x290);
    NvBlAvpI2cStatusPoll();
    Ret = NV_PWRI2C_REGR(PWRI2C_PA_BASE, I2C_CMD_DATA2);
...
static void NvBlAvpEnableCpuPowerRail(void)
{
    ...
    if (BoardId == E1565_BOARD_ID)
    {
#define KAI_PMU_I2C_SLAVE_ADDRESS   0x78
        NvBlAvpI2cWrite(KAI_PMU_I2C_SLAVE_ADDRESS, 0x2016);
        NvBlAvpI2cWrite(KAI_PMU_I2C_SLAVE_ADDRESS, 0xc74f);
        NvBlAvpI2cWrite(KAI_PMU_I2C_SLAVE_ADDRESS, 0x701d);
		
        //BoardFab = NvBlAvpI2cReadEeprom(EEPROM_I2C_SLAVE_ADDRESS,
        //                                EEPROM_FAB_ADDRESS);
        
        if (BoardFab == BOARD_FAB_A00)
        {
            NvBlAvpI2cWrite(KAI_PMU_I2C_SLAVE_ADDRESS, 0x7420);
        }
        NvBlAvpI2cWrite(KAI_PMU_I2C_SLAVE_ADDRESS, 0x013a);
        if (BoardFab >= BOARD_FAB_A01)
        {
            NvBlAvpI2cWrite(KAI_PMU_I2C_SLAVE_ADDRESS, 0x4f50);
        }
#undef  KAI_PMU_I2C_SLAVE_ADDRESS
    }
    else if ((BoardId == ENTERPRISE_BOARD_ID) || (BoardId == E1197_BOARD_ID))
    {
#define ENTERPRISE_PMU_I2C_SLAVE_ADDRESS1   0x24
#define ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2   0x90
        // SMPS1
        NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS1, 0x2155);
        NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x154);

        // SMPS4
        NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS1, 0x1A44);
        NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x142);

        // LDO5
        NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x139B);
        NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x19A);

        BoardFab = NvBlAvpI2cReadEeprom(EEPROM_I2C_SLAVE_ADDRESS,
                                        EEPROM_FAB_ADDRESS);

        if (BoardFab < BOARD_FAB_A03)
        {
            // LDO4
            NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x158B);
            NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x18A);

            // LDO1
            NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x39F);
            NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x19E);
        }
        else
        {
            // LDO7
            NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x15A7);
            NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x1A6);

            // LDO4
            NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x38B);
            NvBlAvpI2cWrite(ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2, 0x18A);
        }
#undef  ENTERPRISE_PMU_I2C_SLAVE_ADDRESS2
#undef  ENTERPRISE_PMU_I2C_SLAVE_ADDRESS1
    }
    else if (BoardId == TAI_BOARD_ID)
    {
#define TAI_PMU_I2C_SLAVE_ADDRESS1   0x24
#define TAI_PMU_I2C_SLAVE_ADDRESS2   0x90
        // SMPS1
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS1, 0x2155);
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS2, 0x154);

        // SMPS4
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS1, 0x1A44);
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS2, 0x142);

        // LDO5
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS2, 0x139B);
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS2, 0x19A);

        // LDO7
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS2, 0x15A7);
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS2, 0x1A6);

        // LDO4
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS2, 0x38B);
        NvBlAvpI2cWrite(TAI_PMU_I2C_SLAVE_ADDRESS2, 0x18A);
#undef  TAI_PMU_I2C_SLAVE_ADDRESS2
#undef  TAI_PMU_I2C_SLAVE_ADDRESS1
    }
   else
    {
#if PMU_IGNORE_PWRREQ
#define CARDHU_PMU_I2C_SLAVE_ADDRESS    0x5A
        NvBlAvpI2cWrite(CARDHU_PMU_I2C_SLAVE_ADDRESS, 0x2328);
        NvBlAvpI2cWrite(CARDHU_PMU_I2C_SLAVE_ADDRESS, 0x127);
...
NvBool NvBlQueryGetNv3pServerFlag_T30()
{
    // Pointer to the start of IRAM which is where the Boot Information Table
    // will reside (if it exists at all).
...
{ 
	...
	Reg = NV_FLD_SET_DRF_NUM(APBDEV_PMC, SCRATCH0, FORCE_RECOVERY_FLAG, 1, Reg); 
...
__asm void NvBlStartUpAvp_T30( void )
{
    ...
    LDR     r0, =ColdBoot_T30               //; R0 = reset vector for CPU
    BL      NvBlStartCpu_T30
