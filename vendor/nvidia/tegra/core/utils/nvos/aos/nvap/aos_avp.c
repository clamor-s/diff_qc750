...
static NvUPtr nvaos_GetUncachedOffset( void )
{
    NvU32 offset = 0;

    switch( s_Chip ) {
    case NvAosChip_Ap20:
        offset = AP20_CACHED_TO_UNCACHED_OFFSET;
        break;
    case NvAosChip_T30:
    default:
        // do nothing
...
NvU32 measureOscFreq(NvU32* dividend, NvU32* divisor)
{
    NvU32 cnt;
    NvU32 reg;
    NvU32 result;
...
ppiTimerInit( void )
{
    ...
    if (s_Chip >= NvAosChip_T30)
        ...
        (void)measureOscFreq(&dividend, &divisor);
...
/* enable the avp cache */
static void APXXCacheEnable( void )
{
    unsigned char *prepCacheStart;
    unsigned char *prepCacheEnd;
    volatile unsigned char *prepSpot;
    NvU32 reg;

    volatile NvU32 *pCacheWindow  = (volatile NvU32 *)(AVP_MMU_TLB_PA_BASE +
        AVP_MMU_TLB_CACHE_WINDOW_0);
    volatile NvU32 *pCacheOptions = (volatile NvU32 *)(AVP_MMU_TLB_PA_BASE +
        AVP_MMU_TLB_CACHE_OPTIONS_0);
    volatile NvU32 *pCacheControl = (volatile NvU32 *)(AVP_CACHE_PA_BASE +
        AVP_CACHE_CONTROL_0);

    /* start at 16KB offset */
    prepCacheStart = (unsigned char *)(0x00000000 + (16 * 1024));
    prepCacheEnd = prepCacheStart + 0x2000;

    // turns cache on.
    *pCacheWindow  = 0x0;
    *pCacheOptions = 0x0fc0;    // enable cache for code+data
    //*pCacheOptions = 0x07c0;  // enable cache just for code.
    reg = *pCacheControl;
    reg |=
        (1 << 0) |    // enable the cache
        (1 << 1) |    // enable cacheram access
        (1 << 2);     // enable tagram access
    *pCacheControl = reg;

    // run through all the lines, which guarantees every line has valid
    // data, and there aren't any lines in an invalid state.
    // going by 16s ensures we hit every cache line
    for (prepSpot = prepCacheStart; prepSpot < prepCacheEnd; prepSpot += 16)
    {
        (void)*prepSpot;
    }
}
...
static void
APXXInstrCacheFlushInvalidate( void )
{
    NvU32 state;
    ...
    // Flush and Invalidate the cache here
    *pCacheOptions |= 0x06;

    // Wait till invalidate and flush is complete.
    while(*pCacheControl & 0x8000);
...
NvOsAVPSetIdle(NvU32 sourceAddress, NvU32 bufferAddress, NvU32 bufSize)
{
    ...
    g_iramSize = bufSize;
    g_syncAddr = bufferAddress + bufSize + nvaos_GetUncachedOffset();
}
...
static NvError
APXXPhysicalMemMap( NvOsPhysAddr phys, size_t size,
    NvOsMemAttribute attrib, NvU32 flags, void **ptr )
{
    if( attrib == NvOsMemAttribute_Uncached )
    {
        if( (phys - nvaos_GetMemoryStart() ) <
                nvaos_GetMemoryEnd() - nvaos_GetMemoryStart() )
        {
            phys = phys + nvaos_GetUncachedOffset();
        }
    }
    ...
    NvOsMemAttribute attrib, NvU32 flags, void **ptr )
{
    ...
    switch( s_Chip ) {
    case NvAosChip_Ap20:
        e = APXXPhysicalMemMap(phys, size, attrib, flags, ptr);
        break;
    case NvAosChip_T30:
        e = T30PhysicalMemMap(phys, size, attrib, flags, ptr);
        break;
    default:
