...
#ifdef SET_KERNEL_PINMUX
#if NVAOS_SHELL && !NVML_BYPASS_PRINT
    // initialization of board pinmux
    argc = NvOdmPinmuxInit();
#endif
#endif
    /* timer init */
    nvaosTimerInit();
...
#else
    NvOsDebugPrintf("Pinmux changes to be applied in bootloader way\n");
#endif
    NvOsDebugPrintf("Bootloader Start at:%d ms\n", s_BlStart);
