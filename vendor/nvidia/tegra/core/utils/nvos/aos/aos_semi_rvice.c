...
void rvice_Setup( void )
{
}

void rvice_BreakPoint( void )
{
    swiSemi(0x18, 0x20025);
}

void rvice_Vprintf( const char *format, va_list ap )
{
    vprintf( format, ap );
}

NvS32 rvice_Vsnprintf( char *str, size_t size, const char *format, va_list ap )
{
    int n;
...
NvError rvice_Vfprintf( NvOsFileHandle stream, const char *format, va_list ap )
{
    int n;
    int length = 0;
    int ExpandedLength;
    ...
    if (ExpandedLength < (sizeof(SmallBuf)-1) )
    {
...
void rvice_DebugString( const char *msg )
{
    swiSemi(0x04, (int) msg);
}

NvError rvice_Fopen( const char *path, NvU32 flags, NvOsFileHandle *file )
{
    FILE *f;
    char *mode;
...
void rvice_Fclose( NvOsFileHandle stream )
{
    (void)fclose((FILE *)stream);
}

NvError rvice_Fwrite( NvOsFileHandle stream, const void *ptr,
    size_t size )
{
    int bytes;
...
NvError rvice_Fread( NvOsFileHandle stream, void *ptr, size_t size,
    size_t *bytes )
{
    int b;
    long pos;
...
NvError rvice_Fseek( NvOsFileHandle file, NvS64 offset, NvOsSeekEnum whence )
{
    int w;
    int err;
...
NvError rvice_Ftell( NvOsFileHandle file, NvU64 *position )
{
    long offset;
...
NvError rvice_Fflush( NvOsFileHandle stream )
{
    int err;
...
NvError rvice_Fsync( NvOsFileHandle stream )
{
    return NvSuccess;
}

NvError rvice_Fremove( const char *filename )
{
...
NvError rvice_Flock( NvOsFileHandle stream, NvOsFlockType type )
{
    return NvError_NotImplemented;
}

NvError rvice_Ftruncate( NvOsFileHandle stream, NvU64 length )
{
    return NvError_NotImplemented;
}

NvError rvice_Opendir( const char *path, NvOsDirHandle *dir )
{
    return NvError_NotImplemented;
}

NvError rvice_Readdir( NvOsDirHandle dir, char *name, size_t size )
{
    return NvError_NotImplemented;
}

void rvice_Closedir(NvOsDirHandle dir)
{
}

NvError rvice_Mkdir( char *dirname )
{
    return NvError_NotImplemented;
}

NvError rvice_GetConfigU32( const char *name, NvU32 *value )
{
    char *EnvVar;
...
NvError rvice_GetConfigString( const char *name, char *value, NvU32 size )
{
    char *EnvVar;
