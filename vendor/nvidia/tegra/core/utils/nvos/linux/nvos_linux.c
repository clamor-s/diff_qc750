...
#if !defined(ANDROID)
/**
 * NvOs_CreateKey - creates a key_t for use with System V apis.
 *
 * @param name The name of the object.
 * The rationale for this is that there is no guaranteed valid file name for
 * a linux system.
 */
static key_t NvOs_CreateKey(const char *name)
{
    NvU32 ret;
    NvU32 hash;
    char *n = (char *)name;
    ...
    ret = (NvU32)( (NvU32)'n' << 24 | (NvU32)'v' << 16 | (NvU16)hash );
    return (key_t)ret;
}
...
static char* PrefixProp(const char* prop)
{
    NvU32 propsize = NvOsStrlen(prop);
    char* ret = NvOsAllocInternal(propsize + sizeof(TEGRA_PROP_PREFIX));
    if (!ret) return NULL;
    NvOsMemcpy(ret, TEGRA_PROP_PREFIX, sizeof(TEGRA_PROP_PREFIX));
    NvOsMemcpy(ret + sizeof(TEGRA_PROP_PREFIX) - 1, prop, propsize);
    ret[sizeof(TEGRA_PROP_PREFIX)-1+propsize] = 0;
    return ret;
}
static char* SysPrefixProp(const char* prop)
{
    NvU32 propsize = NvOsStrlen(prop);
    char* ret = NvOsAllocInternal(propsize + sizeof(SYS_PROP_PREFIX));
    if (!ret) return NULL;
    NvOsMemcpy(ret, SYS_PROP_PREFIX, sizeof(SYS_PROP_PREFIX));
    NvOsMemcpy(ret + sizeof(SYS_PROP_PREFIX) - 1, prop, propsize);
    ret[sizeof(SYS_PROP_PREFIX)-1+propsize] = 0;
    return ret;
}
#endif
...
NvError NvOsSetConfigStringInternal(const char *name, const char *value)
{
    ...
    if (sizeof(TEGRA_PROP_PREFIX) + NvOsStrlen(name) > PROP_NAME_MAX)
        return NvError_InvalidSize;

    full = PrefixProp(name);
    if (!full)
        return NvError_InsufficientMemory;

    ret = property_set(full, value);

    NvOsFreeInternal(full);
...
NvError NvOsSetSysConfigStringInternal(const char *name, const char *value)
{
    ...
    if (sizeof(SYS_PROP_PREFIX) + NvOsStrlen(name) > PROP_NAME_MAX)
        return NvError_InvalidSize;

    full = SysPrefixProp(name);
    if (!full)
        return NvError_InsufficientMemory;
...
NvError NvOsGetConfigStringInternal(const char *name, char *value, NvU32 size)
{
    ...
    str = getenv(name);
#else
    char* full = PrefixProp(name);
    char buf[PROPERTY_VALUE_MAX];

    if (!full)
        return NvError_InsufficientMemory;

    if (property_get(full, buf, NULL) > 0)
        str = buf;
    ...
    NvOsSharedMemHandle *descriptor)
{
    ...
    /* get some shared memory from the kernel */
    k = NvOs_CreateKey( key );
    shmid = shmget( k, size, IPC_CREAT | IPC_EXCL | 0666 );
    if( shmid == -1 )
    {
        if (errno != EEXIST)
            goto fail;
    ...
    *descriptor = (NvOsSharedMemHandle)shmid;
    ret = NvSuccess;
    goto cleanup;

fail:
    if( shmid != -1 ) (void)shmctl( shmid, IPC_RMID, 0 );

cleanup:
    return ret;
#else // #if !defined(ANDROID)
    // since Android doesn't support shmget, we use /dev/ashmem instead
    int fd;
    void *ptr;
    NvError ret = NvError_InsufficientMemory;
    int err;
    ...
    // map the region for resetting the value
...
{
#if !defined(ANDROID)
    int shmid = (int)descriptor;
    void *addr;

    addr = shmat( shmid, 0, 0 );
    if ( (NvU32)addr == (NvU32)-1 )
    {
        return NvError_MemoryMapFailed;
    }

    *ptr = ((NvU8 *)addr) + offset;
    return NvSuccess;
#else // #if !defined(ANDROID)
    int fd;
    void *region;
...
void NvOsSharedMemUnmap(void *ptr, size_t size)
{
    ...
    if(!ptr) {                                  // check ptr for both release/debug codes
        NV_ASSERT(ptr);                         // raise an assertion to catch memory leak for debug
        return;
    }
    err = munmap(ptr, size);
    NV_ASSERT(err == 0);
...
void NvOsSharedMemFree(NvOsSharedMemHandle descriptor)
{
    ...
    //Processes auto-detach when they finish.
    if ( shmctl(shmid, IPC_STAT, &shminfo) == 0 )
    {
        if (shminfo.shm_nattch <= 1)
        {
            NV_ASSERT(shmctl( shmid, IPC_RMID, 0 ) == 0);
        }
    }
#else // #if !defined(ANDROID)
    int fd;
...
/* ATOMIC OPERATIONS
 * Linux headers contain functions for cmpxchg & xchg, but the way these
 * are included in different kernels/distros varies so much that it was
 * easier to do cpu-based inline assembly implementations here.
 */
NvS32
...
#define USER_DISP_FIELD_NAME_MAX        5
NvError NvOsBootArgGet( NvU32 key, void *arg, NvU32 size )
{
    NvError err = NvError_NotSupported;
#if !NVCPU_IS_X86
#if 0
    NvS32 h_disp_controller;
    NvS32 h_disp_dev_index;
    NvS32 h_disp_benabled;
    NvU32 disp_read_length;
    char disp_temp_buf[USER_DISP_FIELD_NAME_MAX];
#endif
    ...
    {
        NvBootArgsDisplay *d;
        d = (NvBootArgsDisplay *)arg;
        d->bEnabled = NV_TRUE;
        d->Controller = 0;
        d->DisplayDeviceIndex= 0;
    }

#if 0
        NV_ASSERT( size == sizeof(NvBootArgsDisplay));
        if (size != sizeof(NvBootArgsDisplay))
            return NvError_BadValue;
        ...
        h_disp_controller =
            open ("/proc/bootarg/disp/Controller",
                    S_IRUSR|S_IRGRP|S_IROTH);
        if (h_disp_controller<0) {
            return NvError_NotSupported;
        }
        NvOsMemset(disp_temp_buf, 0, USER_DISP_FIELD_NAME_MAX);
        disp_read_length =
            read(h_disp_controller, disp_temp_buf, sizeof(NvU32));
        if (disp_read_length==0) {
            close(h_disp_controller);
            return NvError_BadValue;
        }
        else d->Controller=(NvU32)atoi(disp_temp_buf);
        close(h_disp_controller);

        h_disp_dev_index =
            open ("/proc/bootarg/disp/DisplayDeviceIndex",
                    S_IRUSR|S_IRGRP|S_IROTH);
        if (h_disp_dev_index<0) {
            return NvError_NotSupported;
        }
        NvOsMemset(disp_temp_buf, 0, USER_DISP_FIELD_NAME_MAX);
        disp_read_length =
            read(h_disp_dev_index, disp_temp_buf, sizeof(NvU32));
        if (disp_read_length==0) {
            close(h_disp_dev_index);
            return NvError_BadValue;
        }
        else d->DisplayDeviceIndex=(NvU32)atoi(disp_temp_buf);
        close(h_disp_dev_index);

        h_disp_benabled =
            open ("/proc/bootarg/disp/bEnabled",
                    S_IRUSR|S_IRGRP|S_IROTH);
        if (h_disp_benabled<0) {
            return NvError_NotSupported;
        }
        NvOsMemset(disp_temp_buf, 0, USER_DISP_FIELD_NAME_MAX);
        disp_read_length =
            read(h_disp_benabled, disp_temp_buf, sizeof(NvU32));
        if (disp_read_length==0) {
            close(h_disp_benabled);
            return NvError_BadValue;
        }
        else {
            NvU32 temp;
            temp=(NvU32)atoi(disp_temp_buf);
            if (temp==1) d->bEnabled=NV_TRUE;
            else d->bEnabled=NV_FALSE;
        }
        close(h_disp_benabled);
#endif
...
#if defined(ANDROID)
    fd = open(THROUGHPUT_DEVICE, O_RDWR, 0);
    if (fd == -1)
        NV_DEBUG_PRINTF(( "can\'t open " THROUGHPUT_DEVICE ": %s\n", strerror(errno) ));
    else
    {
        if (NvOsModifyFpsTarget(fd, target) < 0)
        {
            close(fd);
            fd = -1;
        }
...
#if defined(ANDROID)
        if ((err = ioctl(fd, TEGRA_THROUGHPUT_IOCTL_TARGET_FPS, target)) < 0)
        {
