        ...
        int valueSize, int flags)
{
    ...
    switch (stateId)
    {
        ...
        {
            // Read the string data from the file and convert it back to binary (this binary
            // data is actually a Stereo profile structure)
            char *strBufPtr = NULL;
            NvU32 strBufSize;
...
#if !defined(ANDROID)
            // Allocate a buffer to hold the string data. This data is a hex dump of the
            // binary profile data, and is therefore twice the size of the binary buffer
            strBufSize = valueSize * 2;
            strBufPtr = (char *) NvOsAlloc(strBufSize);
            if (strBufPtr == NULL)
            {
            ...
            retVal = NvOsConfigFileReadState(NVOS_STEREO_PROFILES_FILE_PATH, name, strBufPtr, strBufSize);
            if (retVal != NvSuccess)
            {
                NV_DEBUG_PRINTF(("NvOsConfigGetStateInternal(): Read failed for stereoprof %s",
                    name));
                NvOsFree(strBufPtr);
                break;
            }

            NV_DEBUG_PRINTF(("NvOsConfigGetStateInternal(): %s: binary sz = %d, string sz = %d",
                name, valueSize, strBufSize));

            // Convert the string to binary. The string is a hex dump of the binary data,
            // so we do a hex to ASCII conversion below.
            for (i = 0; i < valueSize; i++)
            {
                char upperNibble, lowerNibble;
                upperNibble = NvOsHexToChar(strBufPtr[i*2]);
                lowerNibble = NvOsHexToChar(strBufPtr[(i*2) + 1]);
                value[i] = ((upperNibble << 4) | lowerNibble);
                //NV_DEBUG_PRINTF(("NvOsConfigGetStateInternal(): %c, %c, %d, %d, %d",
                //    strBufPtr[i*2], strBufPtr[(i*2) + 1], upperNibble, lowerNibble, value[i]));
            }

            NvOsFree(strBufPtr);
#else
            // Read 3DV settings from NvCPLSvc
            NvStereoProfs* nvStereoProfs = (NvStereoProfs *)value;
            if (valueSize < (int)sizeof(NvStereoProfs)) {
                return NvError_InvalidSize;
            }

            NV_DEBUG_PRINTF(("Trying the new way to get settings for %s", name));
            int rtnValue = NvCplGetAppProfileSetting3DVStruct(name,
                                                       (unsigned char *)nvStereoProfs,
                                                       sizeof(NvStereoProfs));
            // zero = success (found settings)
            if (rtnValue != 0) {
                // app profiles do not exist
                NV_DEBUG_PRINTF(("No settings for %s ???", name));
                retVal = NvError_ConfigVarNotFound;
            }
            else {
                retVal = NvSuccess;
            }
