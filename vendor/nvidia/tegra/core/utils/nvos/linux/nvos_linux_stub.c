...
{
    if (s_DevConsoleFd >= 0)
        write(s_DevConsoleFd, str, strlen(str));
}
    ...
    NvU32 Timeout)
{
    ...
    do {
        ...
        if (Timeout == NV_WAIT_INFINITE)
            timeout = -1;
        ret = ioctl((int)Sem, TEGRA_SEMA_IOCTL_WAIT, &timeout);
...
{
    NvError e;
    e = NvOsLinStubSemaphoreWaitTimeout(Sem, NV_WAIT_INFINITE);
    if (e != NvSuccess)
...
static void NvOsLinStubSemaphoreSignal(NvOsSemaphoreHandle s)
{
    ...
    ioctl((int)s, TEGRA_SEMA_IOCTL_SIGNAL);
}
...
static void NvOsLinStubSemaphoreDestroy(NvOsSemaphoreHandle s)
{
    ...
    ret = close((int)s);
    if (ret < 0)
        NvOsDebugPrintf("nvos: error = %d while closing fd = %d", errno, (int)s);
}
