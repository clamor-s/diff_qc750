...
static NvU64 NvOs_gettimeofday(void)
{
    ...
    gettimeofday(&tv, 0);
    time = ((NvU64)tv.tv_sec * 1000 * 1000) + tv.tv_usec;
    if (time < s_oldtime)
...
/* Note:
 * The NvOs shared memory function design doesn't map well to unix
 * shared memory functions, in that unix create/free pairs don't have a
 * reference counter. When one process frees the memory's name, then no
 * new processes can open it, even if it is still in use elsewhere.
 * The linux implementation uses the SYSV shared memory functions and plays
 * some (not entirely safe) tricks with the attachment counter to simulate
 * a create/free counter. These versions of the functions are not available
 * on QNX. We're left crossing our fingers that the users stick to a
 * usage model that doesn't cause us any problems.
 */
    ...
    NvOsSharedMemHandle *descriptor)
{
    ...
    NvOsStrncpy(shmptr->key, key, NVOS_KEY_MAX);

    /* Open a shared memory object */
    shmptr->shmid = shm_open(key, O_RDWR | O_CREAT | O_EXCL, 0666);
    if (shmptr->shmid != -1)
    {
        shmptr->owned = 1;
        if (ftruncate(shmptr->shmid, size) == -1) {
            close(shmptr->shmid);
            shm_unlink(key);
            goto fail;
        }
    ...
    else
    {
        if (errno != EEXIST)
            goto fail;
        shmptr->shmid = shm_open(key, O_RDWR, 0666);
        if (shmptr->shmid == -1)
            goto fail;
        shmptr->owned = 0;
    }
...
NvError NvOsSemaphoreWaitTimeoutInternal(NvOsSemaphoreHandle s, NvU32 msec)
{
    ...
    rv = sem_timedwait(&sema->sem, &wait_time);
    return rv ? NvError_Timeout : NvSuccess;
}
...
{
    arm_v6_dsb();
    // cache_sync(); simple write to 0x730 /* TODO: check with QNX how to do this */
}
