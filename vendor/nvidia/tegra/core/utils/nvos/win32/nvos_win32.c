...
#if NVOS_SECURE_STRING_FUNCS || !NVOS_IS_WINDOWS_CE
#include <errno.h>
#endif
...
/** NV_GETPROJECTNAME - the flag to get the SKU type
 */
#define NV_GETPROJECTNAME 259

/** NV_GETPLATFORMTYPE - the flag to get the SKU type
 */
#define NV_GETPLATFORMTYPE 257
...
#if NVOS_IS_WINDOWS_CE
typedef HANDLE  ( __cdecl *FindFirstStoreFn )(PSTOREINFO);
typedef HANDLE  ( __cdecl *OpenStoreFn )(LPCTSTR);
typedef BOOL    ( __cdecl *GetStoreInfoFn )(HANDLE, PSTOREINFO);
typedef HANDLE  ( __cdecl *FindFirstPartitionFn )(HANDLE, PPARTINFO);
typedef BOOL    ( __cdecl *FindNextPartitionFn )(HANDLE, PPARTINFO);
typedef BOOL    ( __cdecl *FindNextStoreFn )(HANDLE, PSTOREINFO);

static FindFirstStoreFn     gs_FindFirstStore;
static OpenStoreFn          gs_OpenStore;
static GetStoreInfoFn       gs_GetStoreInfo;
static FindFirstPartitionFn gs_FindFirstPartition;
static FindNextPartitionFn  gs_FindNextPartition;
static FindNextStoreFn      gs_FindNextStore;
#endif
...
#if NVOS_IS_WINDOWS_CE
# define NVOS_REGISTRY_PATH TEXT("Software\\NVIDIA Corporation\\NvOs")

NvError NvOsThreadPrioritySetInternal( NvOsThreadHandle handle,
...
#if NVOS_IS_WINDOWS_CE
    /* share_delete doesn't work on wince */
...
NvOsOpendirInternal(const char *path, NvOsDirHandle *dir)
{
    ...
    // check to see if it is really a directory
    {
        ...
        if (!wpath)
            return NvError_InsufficientMemory;
        b = GetFileAttributesEx(wpath, GetFileExInfoStandard, &wfattr);
        if (wpath != buf)
        {
            NvOsFreeInternal(wpath);
            wpath=NULL;
...
#else
        b = GetFileAttributesEx(path, GetFileExInfoStandard, &wfattr);
#endif
        if (!b)
            return NvError_FileOperationFailed;
...
{
#if NVOS_IS_WINDOWS_CE == 0
    return NvError_NotSupported;
#else
    /* TODO: [ahatala 2010-03-04] implement */
    return NvError_NotSupported;
#endif
}
...
NvOsGetConfigU32Internal(const char *name, NvU32 *value)
{
    ...
    long err;
    HKEY key;
    wchar_t buf[MAX_WIDE_STR_ON_STACK], *wname = 0;
    DWORD data;
    DWORD len = 4;

    err = RegOpenKeyEx(HKEY_LOCAL_MACHINE, NVOS_REGISTRY_PATH, 0, 0, &key);
    ...
    err = RegQueryValueEx(key, wname, NULL, NULL, (LPBYTE)&data, &len);
    if (err != ERROR_SUCCESS)
    {
        ret = NvError_ConfigVarNotFound;
        goto clean;
    }
...
NvOsGetConfigStringInternal(const char *name, char *value, NvU32 size)
{
    ...
    HKEY key;
    wchar_t buf[MAX_WIDE_STR_ON_STACK], *wname = 0;
    wchar_t data[NVOS_KEY_MAX+1];
    DWORD len = NVOS_KEY_MAX;
    ...
    err = RegQueryValueExW(key, wname, NULL, NULL, (LPBYTE)data, &len);
    if (err != ERROR_SUCCESS)
    {
        ret = NvError_ConfigVarNotFound;
        goto clean;
    ...
    NvUStrlConvertCodePage(value, (size_t)size, NvOsCodePage_Utf8,
                           data, (size_t)len, NvOsCodePage_Utf16);
    ...
    size_t size, void **ptr)
{
    ...
    addr = MapViewOfFile(descriptor, perm, 0, offset, size);
    if (addr == NULL)
    {
        *ptr = 0;
...
#if NVOS_IS_WINDOWS_CE
    addr = GetProcAddressA((HMODULE)library, symbol);
#else
    addr = GetProcAddress((HMODULE)library, symbol);
#endif
...
#if NVOS_IS_WINDOWS_CE
    t = CreateThread(NULL, 0, thread_wrapper, ta, 0, NULL);
    if ( t && NvOsThreadPriorityType_Normal != priority )
    {
        err = NvOsThreadPrioritySetInternal( t, priority );
        if ( NvSuccess != err )
        {
            CloseHandle( t );   // FIXME: Is this sufficient???
            goto fail;          // err != sucess
        }
...
NvError NvOsGetOsInformation(NvOsOsInfo *pOsInfo)
{
    ...
    if (pOsInfo != NULL)
    {
        ...
        /* Needs to be set before we get the OS version */
        OsVersionInfo.dwOSVersionInfoSize = sizeof(OSVERSIONINFO);

        /* get the version of the OS we are running on */
        if (GetVersionEx(&OsVersionInfo))
        {
            pOsInfo->MajorVersion = (NvU16)OsVersionInfo.dwMajorVersion;
            pOsInfo->MinorVersion = (NvU16)OsVersionInfo.dwMinorVersion;

            /* AKU version */
            pOsInfo->SubVersion = OsVersionInfo.dwBuildNumber;

            /* Get the SKU Type */
            if (OsVersionInfo.dwPlatformId == VER_PLATFORM_WIN32_CE)
            {
                pOsInfo->OsType = NvOsOs_Windows;

                /* We do not support CE5 so we assume to be on WM6 */
                if (pOsInfo->MajorVersion == 5)
                {
                    /* Get the SKU type */
                    Ret = SystemParametersInfo(NV_GETPLATFORMTYPE, sizeof(StrW), StrW, 0);
                    if (!Ret)
                    {
                    ...
                    Err = NvOsWStringToString(Str, sizeof(Str), StrW);
                    ...
                    if (!(NvOsStrcmp(Str, "PocketPC")))
                        pOsInfo->Sku = NvOsSku_Mobile_PocketPC;
                    else if (!(NvOsStrcmp(Str, "SmartPhone")))
                        pOsInfo->Sku = NvOsSku_Mobile_SmartFon;
                    else
                        pOsInfo->Sku = NvOsSku_Unknown;
                }
                else if (pOsInfo->MajorVersion == 6)
                {
                    /* CE6 */
                    pOsInfo->Sku = NvOsSku_CeBase;
                }
                else if (pOsInfo->MajorVersion == 7)
                {
                    /* CE7/WM7? */
                    /* Get the SKU type */
                    Ret = SystemParametersInfo(NV_GETPROJECTNAME, sizeof(StrW), StrW, 0);
                    if (!Ret)
                    {
                        pOsInfo->OsType = NvOsOs_Unknown;
                        pOsInfo->Sku = NvOsSku_Unknown;
                    ...
                    if (!(NvOsStrcmp(Str, "CEBase")))
                        pOsInfo->Sku = NvOsSku_CeBase;
                    else if (!(NvOsStrcmp(Str, "PocketPC")))
                        pOsInfo->Sku = NvOsSku_Mobile_PocketPC;
                    else if (!(NvOsStrcmp(Str, "SmartPhone")))
                        pOsInfo->Sku = NvOsSku_Mobile_SmartFon;
                    else
...
typedef struct NvOsResourceRec
{
    NvOsResource    ResourceType;
    TCHAR           DeviceName[NVOS_DEV_NAME_MAX];
    TCHAR           MountPoint[NVOS_PATH_MAX];
} NvOSResourceStruct;

#define NVOS_RESOURCE_MAX   64

static  NvOSResourceStruct gs_ResourceStruct[NVOS_RESOURCE_MAX];

NvError
NvOsListResources(
    NvOsResource ResourceType,
    NvU32 *nResources,
    NvOsResourceHandle *phResourceList)
{
#if NVOS_IS_WINDOWS_CE
    STOREINFO si;
    PARTINFO pi;
    HANDLE hSearchStore, hStore, hSearchPart;
    NvU32 ResourceCount = 0;
    HINSTANCE hCoreLib;

    NV_ASSERT(nResources);

    /* This clever code segment is here to look up an API which might not be
     * implemented in every flavor of WinCE.  That makes this code more
     * portable across different WinCE OS implementations.
     */
    hCoreLib = GetModuleHandle(TEXT("Coredll"));
    NV_ASSERT( hCoreLib != NULL );
    // If the handle is valid, try to get the function address.
    if (hCoreLib != NULL)
    {
        gs_FindFirstStore = (FindFirstStoreFn) GetProcAddress( hCoreLib, TEXT("FindFirstStore") );
        gs_OpenStore = (OpenStoreFn) GetProcAddress( hCoreLib, TEXT("OpenStore") );
        gs_GetStoreInfo = (GetStoreInfoFn) GetProcAddress( hCoreLib, TEXT("GetStoreInfo") );
        gs_FindFirstPartition = (FindFirstPartitionFn) GetProcAddress( hCoreLib, TEXT("FindFirstPartition") );
        gs_FindNextPartition = (FindNextPartitionFn) GetProcAddress( hCoreLib, TEXT("FindNextStore") );
        gs_FindNextStore = (FindNextStoreFn) GetProcAddress( hCoreLib, TEXT("FindNextStore") );
    }

    if ( !gs_FindFirstStore     || 
         !gs_OpenStore          ||
         !gs_GetStoreInfo       ||
         !gs_FindFirstPartition ||
         !gs_FindNextPartition  ||
         !gs_FindNextStore )
        return NvError_NotImplemented;

    if (ResourceType == NvOsResource_Storage)
    {
        // Walk through the stores
        si.cbSize = sizeof(STOREINFO);
        hSearchStore = gs_FindFirstStore(&si);    
        if (hSearchStore != INVALID_HANDLE_VALUE) {
           do
           {
              hStore = gs_OpenStore(si.szDeviceName);
              if (hStore != INVALID_HANDLE_VALUE)
              {
                 // Get a pointer to STOREINFO
                 gs_GetStoreInfo(hStore, &si);
                 pi.cbSize = sizeof(PARTINFO);
                 // Get a handle for first partition 
                 hSearchPart =  gs_FindFirstPartition(hStore, &pi);
                 if (hSearchPart != INVALID_HANDLE_VALUE)
                 {
                    do
                    {
                        if (phResourceList != NULL)
                        {
                            NV_ASSERT(ResourceCount < NVOS_RESOURCE_MAX);

                            gs_ResourceStruct[ResourceCount].ResourceType = NvOsResource_Storage;
                            // Use memcpy, since Windows uses wide character strings
                            NvOsMemcpy(
                                gs_ResourceStruct[ResourceCount].DeviceName,
                                si.szDeviceName,
                                sizeof(si.szDeviceName));
                            NvOsMemcpy(
                                gs_ResourceStruct[ResourceCount].MountPoint,
                                pi.szVolumeName,
                                sizeof(pi.szVolumeName));
                            phResourceList[ResourceCount] = &gs_ResourceStruct[ResourceCount];
                        }
                        ResourceCount++;
                    }
                    while (gs_FindNextPartition(hSearchPart, &pi));
                    FindClose(hSearchPart);
                 }
              }
              CloseHandle(hStore);
           }
           while (gs_FindNextStore(hSearchStore, &si));
           FindClose(hSearchStore);
        }
        *nResources = ResourceCount;
    }
    return NvSuccess;
#else
    return NvError_NotSupported;
...
#if NVOS_IS_WINDOWS_CE
    BOOL b=0;
    NvOsResourceStorage StorageRec;

    NV_ASSERT(hResource);
    NV_ASSERT(Info);

    if (hResource->ResourceType == NvOsResource_Storage)
    {
        b = GetDiskFreeSpaceEx(
            hResource->MountPoint,
            (PULARGE_INTEGER)&StorageRec.FreeBytesAvailable,
            (PULARGE_INTEGER)&StorageRec.TotalBytes,
            (PULARGE_INTEGER)&StorageRec.TotalFreeBytes);

        if (b)
        {
            // Save it
            NvOsMemcpy(
                StorageRec.DeviceName,
                hResource->DeviceName,
                sizeof(hResource->DeviceName));
            NvOsMemcpy(
                StorageRec.MountPoint,
                hResource->MountPoint,
                NVOS_PATH_MAX);
            NvOsMemcpy(Info, &StorageRec, NV_MIN(sizeof(NvOsResourceStorage), InfoSize));
            return NvSuccess;
        }
        return NvError_FileOperationFailed;
    }
