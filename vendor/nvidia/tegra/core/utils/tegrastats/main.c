...
#define EMC_CLK_AVG "/sys/kernel/debug/tegra_actmon/emc/avg_activity"
#define AVP_CLK_AVG "/sys/kernel/debug/tegra_actmon/avp/avg_activity"
...
int main (int argc, char *argv[])
{
    ...
    for (;;)
    {
        ...
        switch (chipId)
        {
            case T30_CHIPID:
                // clk average
                READ_VALUE(bToLog, logFile, EMC_CLK_AVG, &emc_clk_avg);
                READ_VALUE(bToLog, logFile, AVP_CLK_AVG, &avp_clk_avg);
                break;
            case AP20_CHIPID:
            default:
...

        {
                ...
                switch (chipId)
                {
                    case T30_CHIPID:
                        PRINTE(bToLog, logFile, "RAM %d/%dMB (lfb %dx%s) IRAM "
                                "%d/%dkB(lfb %s) cpu [%s,%s]@%d EMC %d%%@%d AVP %d%%@%d "
                                "VDE %d EDP limit %d",
                            ...
                            100 * emc_clk_avg/(emcClk / 1000),
                            emcClk/(FREQUENCY_CONVERT*FREQUENCY_CONVERT),
                            100 * avp_clk_avg/(avpClk / 1000),
            ...
            else
            {
                switch (chipId)
                {
                    case T30_CHIPID:
                        PRINTE(bToLog, logFile, "RAM %d/%dMB (lfb %dx%s) IRAM "
                                "%d/%dkB(lfb %s) cpu [%s,%s,%s,%s]@%d EMC %d%%@%d AVP"
                                " %d%%@%d VDE %d EDP limit %d",
                            kB2MB(totalRAMkB-freeRAMkB-buffersRAMkB-cachedRAMkB),
                            kB2MB(totalRAMkB), numLargestRAMBlock, lfbRAM,
