...
/*
 * TODO: How to reliably get the data cache line size of the target
 * platform? /sys/devices/system/cpu has it but there must be an NV API for
 * it somewhere. For now, we just short-circuit to the bootloader headers.
 */
#include "nvbl_cache.h"
#define NVU_CACHE_LINE_SIZE     NV_BL_CACHE_LINE_SIZE
                             ...
                             NvBool storeHashes)
{
    ...
    newDataPtr          = (NvU32*)(((NvUPtr)newDataPtr +
                                    (NvUPtr)(NVU_CACHE_LINE_SIZE - 1)) &
                                   ~(NvUPtr)(NVU_CACHE_LINE_SIZE - 1));
    hash->keys          = (void**)(void*)newDataPtr;
    newDataPtr          += capacity;
    ...
    if (storeValues)
    {
        hash->values    = (void**)(void*)newDataPtr;
        newDataPtr      += capacity;
    } else
    {
        hash->values    = NULL;
...
/*
 * Hash function for a 32-bit scalar value such as a pointer.
 */
