...
void print_help()
{
    ...
    printf("\nwhere,\n"
        "-n	<i2c_number>		1 (default) for P852 and 4 for P1852 and Harmony\n"
        "-g	<alert_gpio_no>		28 (default) for P852 and 179 for P1852\n"
...
int main(int argc, char *argv[]){
    ...
    char buf[2], gpioToTest[32] = "/sys/class/gpio/gpio28", gpioEdge[32];
    char command[64] = "echo falling > ", i2cDevice[16] = "/dev/i2c-1";
    ...
    while ((opt = getopt(argc, argv, "n:g:r:l:em:h")) != -1) {
        switch (opt) {
            case 'n':
                strcpy(&i2cDevice[strlen(i2cDevice)-1], optarg);
                break;
            case 'g':
                strcpy(&gpioToTest[strlen(gpioToTest)-2], optarg);
                break;
            case 'r':
