...
void NvTestGetHwStrings(const char **pChip, const char **pSim)
{
    ...
    if (!chip)
    {
        ...
        {
            sim = "csim";
            if (!NvOsStrncmp(buf,"ap15_",5))
                chip = "ap15";
            else if (!NvOsStrncmp(buf,"ap16_",5))
                chip = "ap15";
            else if (!NvOsStrncmp(buf,"ap20_",5) ||
                     !NvOsStrncmp(buf,"t20_",4))
                chip = "t20";
            else if (!NvOsStrncmp(buf,"t30_",4))
                chip = "t30";
