...
static NvError InitInterrupInterface(NvU32 ChipId)
{
    ...
    NvIntrGetApbDmaInterface(g_pNvIntr, ChipId);
    return NvSuccess;
}
...
static NvBool NvIntrHandleIsr(NvU32 irq)
{
    ...
    //GPIO is not modeled in linsim
    if(!NvRmIsSimulation() && (g_pNvIntr->NvGpioIrq)(hRmDeviceIntr, g_pNvIntr, &irq))
...
{
    NvU32 mainIrq = hOsInterrupt->irq;
    if(!g_pNvIntr)
    ...
    NvBool InterruptEnable)
{
    ...
    for(i = 0; i < IrqListSize; i++)
    {
        ...
        if(InterruptEnable)
            NvIntrSet(hInt, InterruptEnable);
