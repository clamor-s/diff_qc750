 ...
 *
 * @param hAboot Aboot handle returned by NvAbootOpen().
 * @param hSegment Memory handle to memory allocated to warm bootloader.
 * @param SegAddress Destination address to store the signed bootloader at.
 * @retval NvSuccess If successful, or the appropriate error code.
 */
NvError NvAbootInitializeCodeSegment(
    NvAbootHandle hAboot,
...
#define NV_ABOOT_BREAK() \
    do {                                                                    \
        NvU32 _x = (((NvU32)__FILE__) << 16) | ((NvU32)__LINE__ & 0xffff);  \
