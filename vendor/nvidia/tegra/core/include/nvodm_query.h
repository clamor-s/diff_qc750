...
#if (!NVOS_IS_WINDOWS)
#define NVODM_CPU_BOOT_PERF_SET (1)
#else
...
#define NVODM_CPU_BOOT_PERF_SET (1)
#endif
typedef struct  KhOdmGpioPinInfo_t {
...
typedef struct
{
    ...
    /// Indicates the tap delay to adjust the track delay on the PCB/Boards from
    /// SOC to connector.
    NvU32 TapDelay;
...
NvU32 NvOdmQueryOsMemSize(NvOdmMemoryType MemType,
...
/*
 * Returns NV_TRUE if Sku info is supported
 * for the platform
 */
 ...
 * for NOR read or default depending upon NorTiming.
 */
void NvOdmQueryGetSnorTimings(NvU32 Freq, SnorTimings *timings, enum NorTiming);
#endif
