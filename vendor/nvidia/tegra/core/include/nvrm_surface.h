...
typedef struct NvRmSurfaceRec
{
    ...
    /// The pitch of the surface in bytes -- used only if \c Layout is pitch or
    /// tiled. It is acceptable for \c Pitch to be zero for a 1D surface.
        NvU32 Pitch;
 ...
 * \c ColorFormat, \c Layout, and \c Pitch (if \c Layout is Pitch or Tiled)
 * fields in the surface have already been filled in.
 *
 * @param pSurf A pointer to structure describing the surface.
 *
 ...
 * Does not perform any color space conversion.
 *
 ...
 *
 * For any surface with less than 8 bpp, x*bpp and w*bpp must both be multiples
 * of 8. urther, the given region to read must be within the bounds of the
 * surface -- clipping is not suppported.
 *
 * @param pSurf A pointer to structure describing the surface.
 ...
 * enumeration is built on the top of the \c NvColorFormat format and describes
 * the chroma sub-sampling ratios.
 */
typedef enum
{
