 ...
 *
 * On x86 host systems the configuration data storage is the
 ...
 *
 * @par Other Linux Target Systems
 *
 ...
 *
 * This function is used to aquire one or more ::NvOsResourceHandle
 * for a designated resource type. The returned
 * handle list is used to retrieve specific details about the
 * resource by calling NvOsResourceInfo().
 *
 * This function may also be used to obtain just the number of
 * resources (\a nResources) if \a ResourceList is specified as NULL
 * by the caller.
 *
 * If \a ResourceList is not NULL, this function returns the number
 * of resources (\a nResources) and a pointer to the first resource
 * in the array (\a ResourceList).
 *
 * @see NvOsResourceInfo
 *
 * @param ResourceType The resource type for which to retrieve a handle.
 * @param nResources A pointer to the number of resources in the list.
 * @param ResourceList A pointer to the first resource handle in the list.
 *      If this parameter is NULL, only \a nResources is returned.
 */
NvError
NvOsListResources(
 ...
 * structure that indicates the amount of free space on a
 * particular storage media.
 *
 * @see NvOsListResources, NvOsResourceStorage
 *
 * @param hResource The handle for the resource.
 * @param InfoSize The size of the resource structure (\a Info).
 * @param Info A pointer to a specific resource information structure.
 *
 * @retval NvSuccess Ff resource information is valid.
...
/// Bits [31:16] of \a compatVersion field below. Bump this whenever we have a
/// change that breaks backward compatibility.
#define    CURR_STEREOPROF_COMPAT_VAL  0

/**
 * Structure related to stereo profiles.
 * @note Be sure to add new items at the end to preserve binary compatibility.
 */
typedef struct {
    /// [31:16] = compatibility bits, [15:0] = version
    NvU32 compatVersion;
    /// Game profile label.
    char gameprof_label[MAX_ARRAY_SIZE];
    /// Application label.
    char app_label[MAX_ARRAY_SIZE];
    /// Applicatoin name.
    char app_name[MAX_ARRAY_SIZE];
    /// OS name.
    char os_name[MAX_ARRAY_SIZE];

    /// Version code in the application's manifest file. Only stereoize the
    /// current application if its \a versionCode is >= what is in the profile.
    NvS32 versionCode;

    /// Stereo method value that maps to the \c NvGlStereoMethod enum.
    NvS32 stereo_method_val;

    /// Stereo convergence value.
    NvS32 stereo_conv_val;
    /// Stereo separation value
    NvS32 stereo_sep_val;
    /// Stereo cutoff value.
    NvS32 stereo_cutoff_val;
    /// Stereo Cutoff direction value.
    NvS32 stereo_cutoff_direction_val;

    /// Stereo perf scaling factor.
    NvS32 stereo_perf_scale_factor;
    /// Stereo perf scaling type.
    /// Bitfield that maps to the \c NvGlDownResMethod enum.
    NvS32 stereo_perf_scale_type;

    /// Stereo perf scaling factor for 19 x 12 tablets.
    NvS32 stereo_perf_scale_factor_1912;
    /// Stereo perf scaling type for 19 x 12 tablets.
    /// Bitfield that maps to the \c NvGlDownResMethod enum.
    NvS32 stereo_perf_scale_type_1912;

    /// Resolution override for non-stereo apps.
    NvS32 resoverride_scale_factor;
    /// Bitfield that maps to the \c NvGlDownResMethod enum.
    NvS32 resoverride_scale_type;

    /// Downscaled width for stereo. If 0, use stereo_perf_scale
    NvS32 stereo_perf_width;

    /// Downscaled width for mono. If 0, use resoverride_scale
    NvS32 resoverride_perf_width;
...
/**
 * Sets a frame rate target that matches the CPU
 * frequency scaling to the measured performance.
 *
