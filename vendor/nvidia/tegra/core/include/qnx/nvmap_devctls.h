 ...
 */
#define NVMAP_IOC_CREATE  __DIOTF(_DCMD_MISC, 0, struct nvmap_create_handle)
#define NVMAP_IOC_CLAIM   __DIOTF(_DCMD_MISC, 1, struct nvmap_create_handle)
#define NVMAP_IOC_FROM_ID __DIOTF(_DCMD_MISC, 2, struct nvmap_create_handle)

/* Actually allocates memory for the specified handle */
#define NVMAP_IOC_ALLOC    __DIOT(_DCMD_MISC, 3, struct nvmap_alloc_handle)

/* Frees a memory handle, unpinning any pinned pages and unmapping any mappings
 */
#define NVMAP_IOC_FREE       __DIOT(_DCMD_MISC, 4, unsigned)
...
#define NVMAP_IOC_MMAP       __DIOTF(_DCMD_MISC, 5, struct nvmap_map_caller)

/* Reads/writes data (possibly strided) from a user-provided buffer into the
 * hmem at the specified offset */
#define NVMAP_IOC_WRITE      __DIOT(_DCMD_MISC, 6, struct nvmap_rw_handle)
#define NVMAP_IOC_READ       __DIOTF(_DCMD_MISC, 7, struct nvmap_rw_handle)

#define NVMAP_IOC_PARAM __DIOTF(_DCMD_MISC, 8, struct nvmap_handle_param)
...
#define NVMAP_IOC_PIN_MULT   __DIOTF(_DCMD_MISC, 10, struct nvmap_pin_handle)
#define NVMAP_IOC_UNPIN_MULT __DIOT(_DCMD_MISC, 11, struct nvmap_pin_handle)

#define NVMAP_IOC_CACHE      __DIOTF(_DCMD_MISC, 12, struct nvmap_cache_op)

#define NVMAP_IOC_GET_CLIENT __DIOF(_DCMD_MISC, 14, unsigned long)

/* Returns a global ID usable to allow a remote process to create a handle
 * reference to the same handle */
#define NVMAP_IOC_GET_ID  __DIOTF(_DCMD_MISC, 13, struct nvmap_create_handle)
#define NVMAP_IOC_MUNMAP     __DIOT(_DCMD_MISC, 14, unsigned)
#endif
