...
static NvBool
NvSecureServiceChipCompare(
    const NvRmDeviceHandle hRmDevice,
    const NvU32 Chip,
    const NvU32 MajorRev,
    const NvU32 MinorRev);

static NvBool NvSecureServiceIsA03pChip(const NvRmDeviceHandle hRmDevice);
...
NvBool NvSecureServiceIsSskUpdateAllowed(const NvRmDeviceHandle hRmDevice)
{
    if (NvSecureServiceChipCompare(hRmDevice, 0x20, 0x1, 0x3))
    {
        // It is AO3 chip
        // Check whether it is AO3P or not
        // SSK update is not supported on AO3 board
        if (!NvSecureServiceIsA03pChip(hRmDevice))
            return NV_FALSE;
    }
...
NvBool NvSecureServiceIsA03pChip(const NvRmDeviceHandle hRmDevice)
{
    NvU32 ClkVal = 0;
    NvU32 SpareFuse18 = 0;
    NvU32 SpareFuse19 = 0;
