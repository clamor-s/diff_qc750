...
NvError AesCoreAllocateRmMemory(AesHwContext *pAesHwCtxt)
{
    ...
    if (ChipId == 0x20)
        AesKeyTableOffset = NV3P_AES_KEYTABLE_OFFSET_AP20;
    else if (ChipId == 0x30)
...
NvError AesCoreGetCapabilities(NvRmDeviceHandle hRmDevice, AesHwCapabilities **caps)
{
    ...
    {
            {1, 2, 0, NvRmModulePlatform_Silicon, &s_Caps1},
            {1, 3, 0, NvRmModulePlatform_Silicon, &s_Caps2}
    };

    static NvRmModuleCapability s_engineB_caps[] = {
            {1, 1, 0, NvRmModulePlatform_Silicon, &s_Caps1},
            {1, 2, 0, NvRmModulePlatform_Silicon, &s_Caps2}
    };
    ...
    s_Caps1.IsHwKeySchedGenSupported = NV_TRUE;
    s_Caps1.MinBufferAlignment = 16;
    s_Caps1.MinKeyTableAlignment = 4;
    s_Caps1.NumSlotsSupported = AesHwKeySlot_NumExt;
    ...
    if (caps[AesHwEngine_A] == &s_Caps1)
    {
        if (s_Caps1.pAesInterf == NULL)
        {
            s_Caps1.pAesInterf = NvOsAlloc(sizeof(AesHwInterface));
            if (s_Caps1.pAesInterf == NULL)
            {
                goto fail;
            }
            NvAesIntfAp20GetHwInterface(s_Caps1.pAesInterf);
        }
    }
    if (caps[AesHwEngine_A] == &s_Caps2)
    {
        if (s_Caps2.pAesInterf == NULL)
        {
            ...
            NvAesIntfT30GetHwInterface(s_Caps2.pAesInterf);
        }
    }
