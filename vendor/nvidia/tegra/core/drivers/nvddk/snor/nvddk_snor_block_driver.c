    ...
    NvDdkBlockDevIoctl_AllocatePartitionOutputArgs *pAllocPartOut)
{
    ...
    SnorNumSectors = pAllocPartIn->NumLogicalSectors *
        (hSnorBlockDev->hDev->BlkDevInfo.BytesPerSector / SNOR_SECTOR_SIZE);
...
#define SNOR_SPANSION_ERASE_SIZE   (128*1024)
#define SNOR_BLOCK_NOT_ERASED      (0x00)
#define SNOR_BLOCK_ERASED          (0xFF)
static NvU8 s_SnorBlockEraseStatus[SNOR_BCT_BOUNDARY/SNOR_SPANSION_ERASE_SIZE];
static  NvError
SnorBlockDevIoctl(
    NvDdkBlockDevHandle hBlockDev,
    NvU32 Opcode,
        ...
        case NvDdkBlockDevIoctlType_ErasePhysicalBlock:
            {
                ...
                NV_ASSERT(InputSize ==
                        sizeof(NvDdkBlockDevIoctl_ErasePhysicalBlockInputArgs));
                NV_ASSERT(InputArgs);
                /* FIXME : Move the following translation into a chip
                 * specific callback
                 */
                StartDeviceBlock = (pErasePhysicalBlock->BlockNum / 4);
                StartOffset =  StartDeviceBlock * SNOR_SPANSION_ERASE_SIZE;
                /* Do not erase blocks beyond SNOR_BCT_BOUNDARY as those
                 * partitions are assumed to be formatted.
                 */
                if (StartDeviceBlock >= (SNOR_BCT_BOUNDARY/SNOR_SPANSION_ERASE_SIZE))
                    break;
                if (SNOR_BLOCK_NOT_ERASED == s_SnorBlockEraseStatus[StartDeviceBlock])
                {
                    // FIXME : Assume length does not span across blocks
                    e = SnorEraseSectors(hSnorBlockDev,StartOffset, SNOR_SPANSION_ERASE_SIZE);
