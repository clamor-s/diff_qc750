    ...
    NvDdkDispAudioFrequency AudioClk )
{
    ...
    val = NV_DRF_NUM( HDMI, NV_PDISP_AUDIO_CNTRL0, ERROR_TOLERANCE, 6 )
        ...
        | NV_DRF_DEF( HDMI, NV_PDISP_AUDIO_CNTRL0, SOURCE_SELECT, AUTO );
...
fail:
    ...
    NvRmPhysicalMemUnmap( hdmi->hdmi_regs, hdmi->hdmi_regs_size );
    hdmi->hdmi_regs = 0;

    if( hdmi->kfuse_regs )
    {
        NvRmPhysicalMemUnmap( hdmi->kfuse_regs, hdmi->kfuse_regs_size );
        hdmi->kfuse_regs = 0;
    }
...
DcHdmiObsBus( DcController *cont )
{
    ...
    NvRmPhysicalMemUnmap( regs, size );
fail:
...
DcHdcpOp( NvU32 controller, NvDdkDispHdcpContext *context, NvBool enable )
{
    ...
    if( !enable )
    {
        ...
        // If we get this far, we must have untristated the pins
        // originally...
        NV_ASSERT_SUCCESS(
            NvRmSetOdmModuleTristate(cont->hRm, NvOdmIoModule_Hdcp, 0,
                NV_TRUE )
        );
#endif
    }
    else
    {
        ...
        else if( context->State == NvDdkDispHdcpState_Unauthenticated )
        {
            ...
            DcWaitFrame( cont );
#ifndef SET_KERNEL_PINMUX
            /* if the pins fail to come out of tristate, return so that the
             * assert on the recursive tear-down code on failure doesn't
             * trigger.
             */
            e = NvRmSetOdmModuleTristate(cont->hRm, NvOdmIoModule_Hdcp, 0,
                NV_FALSE );
            if( e != NvSuccess )
            {
                NV_ASSERT( !"tristate failure" );
                return NV_FALSE;
            }
#endif
...
DcHdmiOdmOpen( void )
{
    ...
    if( (e != NvSuccess) || (!bypass) )
    {
        ...
        ret_val = NvOdmDispGetDeviceByGuid(NV_HDMI_PARAM_ODM_ID, &h_hdmi_Odm);
