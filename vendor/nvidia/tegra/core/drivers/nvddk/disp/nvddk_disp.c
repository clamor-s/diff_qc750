    ...
    NvU32 flags)
{
    ...
    if( h->refcount )
    {
        NvOsMutexLock( h->mutex );
        h->refcount++;
        *hDisp = h;
        NvOsMutexUnlock( h->mutex );
        return NvSuccess;
    }
...
NvDdkDispClose( NvDdkDispHandle hDisp, NvU32 flags )
{
    ...
    if( bDestroy )
    {
        ...
        NvOsMemset( hDisp, 0, sizeof(NvDdkDisp) );
    }
}
...
NvDdkDispSmartDimmer( void *v )
{
    ...
    while( !t->bShutdown )
    {
        ...
        if( brightness && (brightness != prevbrightness) )
        {
            ...
            b = t->hal->HwEnd( hDisplay->CurrentController->Index, 0 );
            NV_ASSERT( b );
            if( panel->hPanel )
            {
    ...
    NvU32 size, NvU32 align )
{
    ...
    err = NvRmMemHandleCreate(hDevice, hMem, size);
    if( err != NvSuccess )
    {
        goto fail;
    }

    err = NvRmMemAllocTagged(*hMem, NULL, 0, align, NvOsMemAttribute_Uncached,
...
NvDdkDispPrivGetCaps( NvDdkDispHandle hDisp )
{
    ...
    static NvDdkDispCapabilities s_cap_ap10;
    static NvDdkDispCapabilities s_cap_ap15_s;
    static NvDdkDispCapabilities s_cap_ap15;
    static NvDdkDispCapabilities s_cap_ap16a03;
    static NvDdkDispCapabilities s_cap_ap16_s;
    static NvDdkDispCapabilities s_cap_ap16;
    static NvDdkDispCapabilities s_cap_ap20;
    static NvDdkDispCapabilities s_cap_t30_s;
    static NvDdkDispCapabilities s_cap_t30_e;
    static NvDdkDispCapabilities s_cap_t30;
    NvDdkDispCapabilities *cap;
    static NvRmModuleCapability s_caps[] =
        { { 1, 0, 0, NvRmModulePlatform_Silicon, &s_cap_ap10 },
          { 1, 1, 0, NvRmModulePlatform_Silicon, &s_cap_ap15 },
          { 1, 2, 0, NvRmModulePlatform_Silicon, &s_cap_ap16 },
          { 1, 2, 3, NvRmModulePlatform_Silicon, &s_cap_ap16a03 },
          { 1, 3, 0, NvRmModulePlatform_Silicon, &s_cap_ap20 },
          { 1, 1, 0, NvRmModulePlatform_Simulation, &s_cap_ap15_s },
          { 1, 2, 0, NvRmModulePlatform_Simulation, &s_cap_ap16_s },
          { 1, 4, 0, NvRmModulePlatform_Simulation, &s_cap_t30_s },
          { 1, 4, 0, NvRmModulePlatform_Emulation, &s_cap_t30_e },
    ...
    /* zero out ap10 stuff */
    NvOsMemset( &s_cap_ap10, 0, sizeof(s_cap_ap10) );
     ...
     * 1.0: ap10
     * 1.1: ap15
     * 1.2: ap16 (A03 has the premultiplied alpha eco)
     * 1.3: ap20
     * 1.4: t30
     */

    // FIXME: treat 1.2 hardware like 1.1
    s_cap_ap16 = s_cap_ap15;
    // FIXME: treat 1.2 eco X hardware like 1.1
    s_cap_ap16a03 = s_cap_ap15;
    ...
    s_cap_ap16.WindowCaps = s_dc_win_cap;
    s_cap_ap20.WindowCaps = s_dc_win_cap2;
    s_cap_ap16a03.WindowCaps = s_dc_win_cap1;
    ...
    /* simulation caps */
    s_cap_ap15_s = s_cap_ap15;
    s_cap_ap16_s = s_cap_ap16a03;
    s_cap_t30_s = s_cap_t30;
    NVDDK_DISP_CAP_CLEAR( &s_cap_ap15_s, NvDdkDispCapBit_Bug_Double_Syncpt_Incr );
    NVDDK_DISP_CAP_CLEAR( &s_cap_ap16_s, NvDdkDispCapBit_Bug_Double_Syncpt_Incr );
    NVDDK_DISP_CAP_CLEAR( &s_cap_t30_s, NvDdkDispCapBit_Bug_Double_Syncpt_Incr );

    /* emulation caps */
    s_cap_t30_e = s_cap_t30;
    NVDDK_DISP_CAP_SET( &s_cap_t30_e, NvDdkDispCapBit_Dsi_ForcePclkDiv1 );
    NVDDK_DISP_CAP_SET( &s_cap_t30_e, NvDdkDispCapBit_ForceDefaultExternalDisplayMode );
...
NvDdkDispPrivSetupPanels( NvDdkDispHandle hDisp )
{
     ...
     */
    if( hDisp->bootargs.bEnabled == NV_FALSE )
    {
        disp = &hDisp->displays[0];
...
NvDdkDispPrivAttachableControllers( NvDdkDispHandle hDisp )
{
    ...
    for( i = 0; i < NVDDK_DISP_MAX_DISPLAYS; i++ )
    {
...
#ifndef SET_KERNEL_PINMUX
        switch (disp->attr.Type) {
        case NvDdkDispDisplayType_TV:
            NvOdmQueryPinMux(NvOdmIoModule_Tvo, &pOdmConfigs, &NumOdmConfigs);
            if (NumOdmConfigs && pOdmConfigs[0])
                disp->AttachableControllers = 0xFF;
            break;
        case NvDdkDispDisplayType_CRT:
            NvOdmQueryPinMux(NvOdmIoModule_Crt, &pOdmConfigs, &NumOdmConfigs);
            if (NumOdmConfigs || !pOdmConfigs[0])
                disp->AttachableControllers = 0xFF;
            break;
        ...
        case NvDdkDispDisplayType_HDMI:
            NvOdmQueryPinMux(NvOdmIoModule_Hdmi, &pOdmConfigs, &NumOdmConfigs);
            if (NumOdmConfigs || !pOdmConfigs[0])
                disp->AttachableControllers = 0xFF;
            break;
        default:
        {
            ...
            // workaround for dual tft panel testing (quickturn framegrabber)
            if( disp->attr.Usage == NvDdkDispDisplayUsage_Secondary )
            {
                disp->AttachableControllers = 0xFF;
                break;
            }

            // FIXME: Correlate the Display Pin map with the ODM pin mux
            // configuration. When 2 pin muxes are defined, an LCD may only
            // function on one of them
            NvOdmQueryPinMux(NvOdmIoModule_Display, &pOdmConfigs,
                &NumOdmConfigs);
            for (j=0; j<NumOdmConfigs; j++)
            {
                disp->AttachableControllers |= ((pOdmConfigs[j]) ? (1<<j) : 0);
            }
            break;
        }
        }
...
static void
NvDdkDispPrivBootArgs( NvDdkDispHandle hDisp )
{
    NvError err;

    err = NvOsBootArgGet( NvBootArgKey_Display, &hDisp->bootargs,
        sizeof(hDisp->bootargs) );
    if( err != NvSuccess )
    {
        NvOsMemset( &hDisp->bootargs, 0, sizeof(hDisp->bootargs) );
    }
}
    ...
    NvBool EnableTristate)
{
    ...
    switch (hDisplay->attr.Type) {
    case NvDdkDispDisplayType_TV:
        IoModule = NvOdmIoModule_Tvo;
        Instance = 0;
        break;
    case NvDdkDispDisplayType_CRT:
        IoModule = NvOdmIoModule_Crt;
        Instance = 0;
        break;
    case NvDdkDispDisplayType_DSI:
        IoModule = NvOdmIoModule_Dsi;
        Instance = 0;
        break;
    case NvDdkDispDisplayType_HDMI:
        IoModule = NvOdmIoModule_Hdmi;
        Instance = 0;
        break;
    default:
        IoModule = NvOdmIoModule_Display;
        Instance = hDisplay->CurrentController->Index;
        break;
    }

#ifndef SET_KERNEL_PINMUX
    // Make sure there's a pinmux config before attempting to tristate.
    NvOdmQueryPinMux(IoModule, &pOdmConfigs, &NumOdmConfigs);
    if (NumOdmConfigs)
    {
        NV_ASSERT_SUCCESS(
            NvRmSetOdmModuleTristate(hDisplay->hDisp->hDevice,
            IoModule, Instance, EnableTristate) );
    }

    NvOdmQueryPinMux(NvOdmIoModule_BacklightPwm, &pOdmConfigs, &NumOdmConfigs);
    for (Instance = 0; Instance < NumOdmConfigs; Instance++)
    {
        if (pOdmConfigs[Instance])
        {
            NV_ASSERT_SUCCESS(
                NvRmSetOdmModuleTristate(hDisplay->hDisp->hDevice,
...
NvDdkDispPrivModeCommit( NvDdkDispControllerHandle hController )
{
    ...
    if( hController->state != ControllerState_Active )
    {
        ...
        if( !hController->HwInitialized )
        {
            ...
            NvBootArgsDisplay *arg;

            hRm = hController->hDisp->hDevice;
            NV_ASSERT( hRm );

            /* initalize the hardware, check for a re-opened hw controller */
            arg = &hDisp->bootargs;
            if( arg->bEnabled && arg->Controller == hController->Index )
            {
                bReopen = NV_TRUE;
            }
    ...
    NvOdmDispDevicePowerLevel level )
{
    ...
    #define ENABLE( en ) \
        do { \
            ...
            else if( hDisplay->panel->Type == NvDdkDispDisplayType_DSI ) \
            { \
                hal->HwDsiEnable( hController->Index, panel->hPanel, \
    ...
    for( i = 0; i < hController->nAttached; i++ )
    {
        ...
        if( hDisplay->power == NvOdmDispDevicePowerLevel_On &&
            ...
            hController->bModeDirty )
        {
            if( hDisplay->panel->Type == NvDdkDispDisplayType_DSI )
            {
        ...
        case NvOdmDispDevicePowerLevel_PrePower:
            ...
            if( hDisp->bReopen &&
                hDisp->bootargs.DisplayDeviceIndex == panel->DeviceIndex )
            {
                bReopen = NV_TRUE;
            }
    ...
    {
        /* clear bootargs */
        NvOsMemset( &hDisp->bootargs, 0, sizeof(NvBootArgsDisplay) );
