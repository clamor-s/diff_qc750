    ...
    NvRmPhysAddr OtgPhysAddr)
{
    ...
    err = NvRmMemHandleCreate(pUsbFuncCtxt->hRmDevice,
            &pUsbFuncCtxt->hUsbBuffMem, pUsbFuncCtxt->VirtualBufferSize);
    ...
    err = NvRmMemAlloc(pUsbFuncCtxt->hUsbBuffMem,
            NULL, 0, align, NvOsMemAttribute_WriteCombined);

    if (err != NvSuccess)
    {
        NV_DEBUG_PRINTF(("UsbfError>AllocateBuffers::Couldn't allocate memory!\n"));
        goto fail;
    }
