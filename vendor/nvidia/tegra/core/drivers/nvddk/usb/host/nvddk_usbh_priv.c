...
#define CHIP_ID_NUM_AP20 0x20
#define CHIP_ID_NUM_T30 0x30

void UsbhPrivGetCapabilities(NvDdkUsbhHandle pUsbHcdCtxt)
{
    static NvDdkUsbhCapabilities s_UsbhCap[] =
    {
        {CHIP_ID_NUM_AP20,NV_TRUE},
        {CHIP_ID_NUM_T30,NV_FALSE},
    };
    NvDdkUsbhCapabilities *pUsbhCap = NULL;
    NvRmModuleCapability s_UsbhModCap[] =
    {
        {1, 5, 0, NvRmModulePlatform_Silicon, &s_UsbhCap[0]}, // AP20, USB1
        {1, 6, 0, NvRmModulePlatform_Silicon, &s_UsbhCap[0]}, // AP20, USB2
        {1, 7, 0, NvRmModulePlatform_Silicon, &s_UsbhCap[0]}, // AP20, USB3
        {2, 0, 0, NvRmModulePlatform_Silicon, &s_UsbhCap[1]}, // T30, USB1
        {2, 1, 0, NvRmModulePlatform_Silicon, &s_UsbhCap[1]}, // T30, USB2
        {2, 2, 0, NvRmModulePlatform_Silicon, &s_UsbhCap[1]}, // T30, USB3
    };
    ...
    if (pUsbHcdCtxt->Caps.ChipId == CHIP_ID_NUM_AP20)
        UsbhPrivAp20StoreRegOffsets(pUsbHcdCtxt);
    if (pUsbHcdCtxt->Caps.ChipId == CHIP_ID_NUM_T30)
        UsbhPrivT30StoreRegOffsets(pUsbHcdCtxt);

    if (pUsbHcdCtxt->Caps.IsPortSpeedFromPortSC)
        pUsbHcdCtxt->UsbhGetPortSpeed = &UsbhPrivAp20GetPortSpeed;
    else
        pUsbHcdCtxt->UsbhGetPortSpeed = &UsbhPrivT30GetPortSpeed;
...
static NvError UsbhPrivAckInterrupts(NvDdkUsbhHandle pUsbHcdCtxt)
{
    ...
    if (NewUsbInts & USB_DRF_DEF(USBSTS, PCI, PORT_CHANGE))
    {
        if (USB_REG_READ_VAL(PORTSC1, CSC))
        {
            if (USB_REG_READ_VAL(PORTSC1, CCS))
            {
                ...
                NV_DEBUG_PRINTF(("USBHDDK::UsbhPrivAckInterrupts::Cable Connected\n"));
            }
            else
            {
                pUsbHcdCtxt->UsbhCableDisConnected = NV_TRUE;
                pUsbHcdCtxt->UsbhCableConnected = NV_FALSE;
                NV_DEBUG_PRINTF(("USBHDDK::UsbhPrivAckInterrupts::Cable Disconnected\n"));
            }
        }
    ...
    {
        NV_DEBUG_PRINTF(("USBHDDK::UsbhPrivAckInterruptsUSB INT or ERRORINT::%d\n", NewUsbInts));
...
NvError UsbhPrivAllocateBuffers(NvDdkUsbhHandle pUsbHcdCtxt)
{
    ...
    err = NvRmMemHandleCreate(pUsbHcdCtxt->hRmDevice,
    ...
    //err = NvRmMemAlloc(pUsbHcdCtxt->hUsbBuffMemHandle, NULL, 0, Align,
    //         NvOsMemAttribute_Uncached);

    err = NvRmMemAlloc(pUsbHcdCtxt->hUsbBuffMemHandle,
                       s_Heaps, NV_ARRAY_SIZE(s_Heaps),
                       Align, NvOsMemAttribute_WriteCombined);
    if (err != NvSuccess)
    {
        NV_DEBUG_PRINTF(("UsbhError>AllocateBuffers::Couldn't allocate memory!\n"));
        goto fail;
    }
...
NvError UsbhPrivInitializeController(NvDdkUsbhHandle pUsbHcdCtxt)
{
    ...
    if (pUsbHcdCtxt->Caps.ChipId == CHIP_ID_NUM_AP20)
    {
        ...
        do
        {
            ...
            if (!TimeOut)
            {
                NV_DEBUG_PRINTF(("UsbhPrivInitController:: mode change failed val = %d\n", regValue));
                return NvError_Timeout;
            }
            NvOsWaitUS(1);
            TimeOut--;
        } while (regValue != USB_DRF_DEF_VAL(USBMODE, CM, HOST_MODE));
    }
    return NvSuccess;
}
