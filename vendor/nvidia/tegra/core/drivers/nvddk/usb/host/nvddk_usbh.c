    ...
    NvU32 Instance)
{
    ...
    pUsbProperty = NvOdmQueryGetUsbProperty(NvOdmIoModule_Usb, pUsbhHandle->Instance);
    ...
    if (Instance == 0)
    {
        if (pUsbProperty->IdPinDetectionType == NvOdmUsbIdPinType_Gpio)
        {
            NV_CHECK_ERROR_CLEANUP(NvRmGpioAcquirePinHandle(
                        pUsbhHandle->hUsbHcdGpioHandle,
                        pUsbhGpioPinInfo[NvOdmGpioPin_UsbCableId].Port,
                        pUsbhGpioPinInfo[NvOdmGpioPin_UsbCableId].Pin,
                        &pUsbhHandle->CableIdGpioPin));

            // Configure Cable ID Pin as Input Pin
            NV_CHECK_ERROR_CLEANUP(NvRmGpioConfigPins(pUsbhHandle->hUsbHcdGpioHandle,
                        &pUsbhHandle->CableIdGpioPin, 1, NvRmGpioPinMode_InputData));
        }
    }
...
fail:
    ...
    NvRmPhysicalMemUnmap(pUsbhHandle->UsbOtgVirAdr, pUsbhHandle->UsbOtgBankSize);
...
NvError NvDdkUsbhStart(NvDdkUsbhHandle hUsbh, NvBool IsInitialize)
{
    ...
    if (!hUsbh->ControllerStarted)
    {
        ...
        if (IsInitialize)
        {
            ...
            if (e != NvSuccess)
            {
                NV_DEBUG_PRINTF(("NvDdkUsbhStart:: UsbhPrivInitContrller Failed\n"));
            }
        }
    }
    if (e == NvSuccess)
...
void NvDdkUsbhClose(NvDdkUsbhHandle hUsbh, NvBool IsInitialize)
{
    ...
    if (IsInitialize)
    {
        ...
        {
            NV_DEBUG_PRINTF(("NvDdkUsbhClose:: Error while stopping the controller\n"));
        }
    ...
    // UnMap the virtual Address
    NvRmPhysicalMemUnmap(hUsbh->UsbOtgVirAdr, hUsbh->UsbOtgBankSize);
...
NvError NvDdkUsbhAddPipe(NvDdkUsbhHandle hUsbh, NvDdkUsbhPipeInfo *pAddPipe)
{
    ...
    switch (USB_PIPETYPE(pipe))
    {
        ...
        case NvDdkUsbhPipeType_Bulk:
            ...
            {
                NV_DEBUG_PRINTF(("USBHDDK::AddPipe:pNewHcdQh = 0x%x\n", pNewHcdQh));
                ...
                NV_DEBUG_PRINTF(("USBHDDK::AddPipe::EpPipeHandler = 0x%x::NewHcdQh = 0x%x\n",
    ...
    NvDdkUsbhTransfer* pTransfer)
{
    ...
    while (1)
    {
        ...
        if ((UsbEvents & NvDdkUsbhEvent_TransferDone) && (pTransfer == pCompletedTransfer))
        {
            break;
        }
    }
