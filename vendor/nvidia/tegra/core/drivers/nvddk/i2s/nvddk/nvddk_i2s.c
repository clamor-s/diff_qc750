...
static void InitI2sInterface(void)
{
    ...
    s_I2sHwInterface.TdmModeSetSlotSizeFxn = NvDdkPrivTdmModeSetSlotSizeFxn;
    s_I2sHwInterface.SetDspDataFlowFxn = NvDdkPrivI2sSetDspDataFlow;
#endif //!NV_IS_AVP
    ...
    SocI2sCapability *pI2sSocCaps)
{
    ...
    NvRmModuleCapability I2sCapsList[] =
    {
        { 1, 0, 0, NvRmModulePlatform_Silicon,
            &s_SocI2sCapsList[0] },  // version 1.0 in AP15
        { 1, 1, 0, NvRmModulePlatform_Silicon,
            &s_SocI2sCapsList[1] },  // version 1.1 I2S in AP20
...
// Tristate the dap pads for the i2s instance
#ifndef SET_KERNEL_PINMUX
static NvError SetI2sPadTriState(
        NvDdkI2sHandle hI2s,
        NvBool  bValue)
{
    NvError Error = NvSuccess;
    ...
    {
        Error = NvRmSetOdmModuleTristate(hI2s->hDevice, NvOdmIoModule_Dap,
            hI2s->InstanceId, bValue);
    }
    return Error;
}
#endif
                        ...
                        NvDdkI2sDspDataProperty* pDspProp)
{
    if (!hI2s->pI2sInfo->SocI2sCaps.IsAudioCIFModeSupported)
    {
        ...
        NvDdkPrivSetDataCommFormat(&hI2s->I2sAc97HwRegs,
                                    I2sMode);

        // Setting the mask bit
        NvDdkPrivI2sSetDspMaskBits(&hI2s->I2sAc97HwRegs,
                                    pDspProp->TxMaskBits,
                                    pDspProp->RxMaskBits);

        // Set the FsyncWidth
        NvDdkPrivI2sSetDspFsyncWidth(&hI2s->I2sAc97HwRegs,
                                    pDspProp->FsyncWidth);
        // set dsp mode Highz control
        NvDdkPrivI2sSetDspEdgeMode(&hI2s->I2sAc97HwRegs,
                                    pDspProp->HighzCtrl);

        // Change this to a proper way later
...
#ifndef SET_KERNEL_PINMUX
    // set pad to normal - remove this line later
    // as this is being called in the Resume/Suspend
    SetI2sPadTriState(pI2sChannel, NV_FALSE);
#endif
...
//                        pI2sChannel->I2sAc97HwRegs.TxFifoAddress);
    ...
    NvDdkPrivI2sGetStandbyRegisters(&(pI2sChannel->I2sAc97HwRegs),
                                 &(pI2sChannel->I2sStandbyRegs));
        ...
        NvU32 *pFifoCount)
{
    ...
    if (IsWrite)
    {
        IsSuccess = NvDdkPrivI2sGetFifoCount(&hI2s->I2sAc97HwRegs,
                            I2sAc97Direction_Transmit, pFifoCount);
    }
    else
    {
        IsSuccess = NvDdkPrivI2sGetFifoCount(&hI2s->I2sAc97HwRegs,
                            I2sAc97Direction_Receive, pFifoCount);
    }
    ...
    void *pI2SInterfaceProperty)
{
    ...
    if (IsUpdate)
    {
        ...
        if (!Error && hI2s->I2sUseCount)
        {
            NvDdkPrivI2sGetStandbyRegisters(&(hI2s->I2sAc97HwRegs),
                                 &(hI2s->I2sStandbyRegs));
        }
        ...
        const void *pI2SNewDataProp)
{
    ...
    switch (I2sMode)
    {
        ...
        case NvDdkI2SMode_Network:
            ...
            NvDdkPrivSetDataCommFormat(&hI2s->I2sAc97HwRegs, I2sMode);
...
NvError NvDdkI2sSuspend(NvDdkI2sHandle hI2s)
{
    ...
    // Set Trireset i2s pad - figure out
    //SetI2sPadTriState(hI2s, NV_TRUE);
...
NvError NvDdkI2sResume(NvDdkI2sHandle hI2s)
{
    ...
    if (!Error)
    {
        ...
        // Restored the saved values
        NvDdkPrivI2sSetStandbyRegisters(&(hI2s->I2sAc97HwRegs),
                         &(hI2s->I2sStandbyRegs));

        // Set i2s pad to normal - figure out later
        // SetI2sPadTriState(hI2s, NV_FALSE);
