 ...
 */
static void HwRegisterInitialize(NvU32 CommChannelId, UartHwRegisters *pUartHwRegs)
{
 ...
 */
static void HwSetBreakControlSignal(
    UartHwRegisters *pUartHwRegs,
    NvBool IsStart)
{
    NvU32 LcrValue;
    ...
    UartDataDirection FifoType)
{
    ...
    {
    // Do the rx fifo reset by reading the fifo. This is require for
    // detecting the break continuously.
        do
        {
            LsrValue = UART_REG_READ32(pUartHwRegs->pRegsVirtBaseAdd, LSR);
            if (!(LsrValue & NV_DRF_DEF(UART, LSR, RDR, DATA_IN_FIFO)))
                break;
            RxData = UART_REG_READ32(pUartHwRegs->pRegsVirtBaseAdd, THR_DLAB_0);
        } while(1);
    }
 ...
 */
static void HwSetDmaMode(UartHwRegisters *pUartHwRegs, NvBool IsEnable)
{
    NvU32 FcrValue;
 ...
 */
static NvBool HwIsTransmitFifoEmpty(UartHwRegisters *pUartHwRegs)
{
    NvU32 LsrValue;
    LsrValue = UART_REG_READ32(pUartHwRegs->pRegsVirtBaseAdd, LSR);
 ...
 */
static NvBool HwIsDataAvailableInFifo(UartHwRegisters *pUartHwRegs)
{
    NvU32 LsrValue = UART_REG_READ32(pUartHwRegs->pRegsVirtBaseAdd, LSR);
    if (LsrValue & NV_DRF_DEF(UART, LSR, RDR, DATA_IN_FIFO))
        return NV_TRUE;
...
static void HwClearAllInt(UartHwRegisters *pUartHwRegs)
{
    UART_REG_WRITE32(pUartHwRegs->pRegsVirtBaseAdd, IER_DLAB_0, 0);
    pUartHwRegs->IerRegister = 0;
}

static void HwSetReceiveInterrupt(
    UartHwRegisters *pUartHwRegs,
    NvBool IsEnable,
...
static void HwSetTransmitInterrupt(
    UartHwRegisters *pUartHwRegs,
    NvBool IsEnable)
{
    NvU32 IerValue;
...
static void HwSetModemControlInterrupt(
    UartHwRegisters *pUartHwRegs,
    NvBool IsEnable)
{
    NvU32 IerValue;
 ...
 */
static UartHwInterruptSource
HwGetInterruptReason(
    UartHwRegisters *pUartHwRegs)
{
    NvU32 IirValue;
    NvU32 P0;
 ...
 * register structure.
 */
static void HwUpdateModemSignal(UartHwRegisters *pUartHwRegs)
{
    NvU32 MsrValue;
...
static NvBool HwIsCtsActive(UartHwRegisters *pUartHwRegs)
{
...
static void HwReconfigureRegisters(UartHwRegisters *pUartHwRegs)
{
    NvU32 LcrValue;
 ...
 */
static void HwInitFifo(UartHwRegisters *pUartHwRegs)
{
    pUartHwRegs->FcrRegister = NV_DRF_DEF(UART, IIR_FCR, FCR_EN_FIFO, ENABLE);
    HwResetFifo(pUartHwRegs, UartDataDirection_Both);
    HwSetFifoTriggerLevel(pUartHwRegs, UartDataDirection_Both, 4);
    HwUpdateModemSignal(pUartHwRegs);
    HwSetModemSignal(pUartHwRegs, NvDdkUartSignalName_Rts, NV_FALSE);
 }
...
NvDdkPrivUartInitUartHwInterface(
    ...
    NvU32 Minor)
{
    pUartInterface->HwRegisterInitializeFxn         = HwRegisterInitialize;
    pUartInterface->HwInitFifoFxn                   = HwInitFifo;
    pUartInterface->HwGetReqdClockSourceFreqFxn     = HwGetReqdClockSourceFreq;
    pUartInterface->HwSetBaudRateFxn                = HwSetBaudRate;
    pUartInterface->HwSetParityBitFxn               = HwSetParityBit;
    pUartInterface->HwSetDataLengthFxn              = HwSetDataLength;
    pUartInterface->HwSetStopBitFxn                 = HwSetStopBit;
    pUartInterface->HwSetBreakControlSignalFxn      = HwSetBreakControlSignal;
    pUartInterface->HwResetFifoFxn                  = HwResetFifo;
    pUartInterface->HwSetFifoTriggerLevelFxn        = HwSetFifoTriggerLevel;
    pUartInterface->HwSetDmaModeFxn                 = HwSetDmaMode;
    ...
    pUartInterface->HwIsTransmitFifoEmptyFxn        = HwIsTransmitFifoEmpty;
    pUartInterface->HwIsBreakSignalDetectedFxn      = HwIsBreakSignalDetected;
    pUartInterface->HwIsDataAvailableInFifoFxn      = HwIsDataAvailableInFifo;
    pUartInterface->HwReadFromReceiveFifoFxn        = HwReadFromReceiveFifo;
    pUartInterface->HwClearAllIntFxn                = HwClearAllInt;
    pUartInterface->HwSetReceiveInterruptFxn        = HwSetReceiveInterrupt;
    pUartInterface->HwSetTransmitInterruptFxn       = HwSetTransmitInterrupt;
    pUartInterface->HwSetModemControlInterruptFxn   = HwSetModemControlInterrupt;
    pUartInterface->HwGetInterruptReasonFxn         = HwGetInterruptReason;
    pUartInterface->HwSetIrdaCodingFxn              = HwSetIrdaCoding;
    pUartInterface->HwSetModemSignalFxn             = HwSetModemSignal;
    pUartInterface->HwIsCtsActiveFxn                = HwIsCtsActive;
    pUartInterface->HwUpdateModemSignalFxn          = HwUpdateModemSignal;
    pUartInterface->HwSetHWFlowControlFxn           = HwSetHWFlowControl;
    pUartInterface->HwReconfigureRegistersFxn       = HwReconfigureRegisters;
}
