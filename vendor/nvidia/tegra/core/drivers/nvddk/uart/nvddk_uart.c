    ...
    SocUartCapability *pUartSocCaps)
{
    ...
    static SocUartCapability s_SocUartCapsList[4];
    NvRmModuleCapability UartCapsList[] =
    {
        { 1, 0, 0, NvRmModulePlatform_Silicon,
            &s_SocUartCapsList[0] }, // Major.Minor = 1.0
        { 1, 1, 0, NvRmModulePlatform_Silicon,
            &s_SocUartCapsList[1] }, // Major.Minor = 1.1
        { 1, 2, 0, NvRmModulePlatform_Silicon,
            &s_SocUartCapsList[2] }, // Major.Minor = 1.2
        { 1, 3, 0, NvRmModulePlatform_Silicon,
            &s_SocUartCapsList[3] }, // Major.Minor = 1.3
    };
    ...
    s_SocUartCapsList[0].Major = 1;
    s_SocUartCapsList[0].Minor = 0;

    s_SocUartCapsList[1].IsEndOfDataIntSupported = NV_TRUE;
    s_SocUartCapsList[1].FifoDepth = 16;
    s_SocUartCapsList[1].Major = 1;
    s_SocUartCapsList[1].Minor = 1;

    s_SocUartCapsList[2].IsEndOfDataIntSupported = NV_TRUE;
    s_SocUartCapsList[2].FifoDepth = 32;
    s_SocUartCapsList[2].Major = 1;
    s_SocUartCapsList[2].Minor = 2;

    s_SocUartCapsList[3].IsEndOfDataIntSupported = NV_TRUE;
    s_SocUartCapsList[3].FifoDepth = 32;
    s_SocUartCapsList[3].Major = 1;
    s_SocUartCapsList[3].Minor = 3;
    ...
    s_SocUartCapsList[3].IsRtsHwFlowControlSupported = NV_FALSE;

    // Get the capability from modules files.
    NV_ASSERT_SUCCESS(NvRmModuleGetCapabilities(hRmDevice, ModuleId, UartCapsList,
                            NV_ARRAY_SIZE(UartCapsList), (void **)&pUartCaps));
    ...
    pUartSocCaps->Major = pUartCaps->Major;
    pUartSocCaps->Minor = pUartCaps->Minor;
    return NvSuccess;
}
    ...
    NvU32 BufferSize)
{
    ...
    *phMemHandle = NULL;
    ...
    if (!Error)
        Error = NvRmMemAlloc(hNewMemHandle, NULL, 0, 0x4, NvOsMemAttribute_WriteCombined);
    ...
    UartHwInterruptSource InterruptReason)
{
    ...
    if (hUart->IsRxApbDmaBased)
    {
        ...
        hUart->HwIntf.HwSetReceiveInterruptFxn(&hUart->UartHwRegs, NV_FALSE, NV_TRUE);
    ...
    else
    {
        ...
        {
            Error = hUart->HwIntf.HwReadFromReceiveFifoFxn(&hUart->UartHwRegs,
        ...
        if (hUart->LocalRxBuff.DataAvailable >= hUart->LocalRxBuff.BufferLevelHigh)
        {
            ...
            {
                LockRegisterAccess(hUart);
                hUart->HwIntf.HwSetModemSignalFxn(&hUart->UartHwRegs,
...
static void HandleUartTxInterrupt(NvDdkUartHandle hUart)
{
    ...
    {
        LockRegisterAccess(hUart);
        hUart->HwIntf.HwSetTransmitInterruptFxn(&hUart->UartHwRegs, NV_FALSE);
        UnlockRegisterAccess(hUart);
    ...
    BytesWritten = hUart->HwIntf.HwWriteInTransmitFifoFxn(&hUart->UartHwRegs,
...
static void HandleUartModemInterrupt(NvDdkUartHandle hUart)
{
    ...
    LockRegisterAccess(hUart);
    hUart->HwIntf.HwUpdateModemSignalFxn(&hUart->UartHwRegs);
...
static void HandleUartInterrupt(void *args)
{
    ...
    while (1)
    {
        ...
        InterruptReason = hUart->HwIntf.HwGetInterruptReasonFxn(&hUart->UartHwRegs);
        ...
        switch (InterruptReason)
        {
            case UartHwInterruptSource_ReceiveError:
                hUart->HwIntf.HwSetReceiveInterruptFxn(&hUart->UartHwRegs, NV_FALSE, hUart->IsRxApbDmaBased);
...
static void DestroyUartChannelHandle(NvDdkUartHandle hUart)
{
    ...
    hUart->HwIntf.HwClearAllIntFxn(&hUart->UartHwRegs);
    ...
    NvDdkUartHandle *phUart)
{
    ...
    *phUart = NULL;
    ...
    NvDdkPrivUartInitUartHwInterface(&hNewUart->HwIntf,
                                        hNewUart->SocUartCaps.Major,
                                        hNewUart->SocUartCaps.Minor);
    if ((hNewUart->SocUartCaps.Major == 1) &&
                    (hNewUart->SocUartCaps.Minor == 3))
    {
        NvDdkPrivUartInitUartHwInterface_1_3(&hNewUart->HwIntf,
                                        hNewUart->SocUartCaps.Major,
                                        hNewUart->SocUartCaps.Minor);
    }

    hNewUart->HwIntf.HwRegisterInitializeFxn(ChannelId, &hNewUart->UartHwRegs);
   ...
   hNewUart->hOdmUart = NvOdmUartOpen(ChannelId);
    ...
    if (!Error)
    {
        ...
        hNewUart->HwIntf.HwInitFifoFxn(&hNewUart->UartHwRegs);
        hNewUart->HwIntf.HwSetDmaModeFxn(&hNewUart->UartHwRegs, hNewUart->IsDmaSupport);
    }
...
static void UartStopWrite(NvDdkUartHandle hUart, NvBool IsResetTxFifoIfNotBusy)
{
    ...
    else
    {
        if (IsResetTxFifoIfNotBusy)
            hUart->HwIntf.HwResetFifoFxn(&hUart->UartHwRegs,
...
static void WaitTillTxFifoEmpty(NvDdkUartHandle hUart)
{
    ...
    if (pUartCurrConfig->UartBaudRate)
    {
        ...
        while(1)
        {
            IsTxFifoEmpty = hUart->HwIntf.HwIsTransmitFifoEmptyFxn(&hUart->UartHwRegs);
            if (IsTxFifoEmpty)
                break;
        ...
        {
            LockRegisterAccess(hUart);
            hUart->HwIntf.HwResetFifoFxn(&hUart->UartHwRegs, UartDataDirection_Transmit);
            UnlockRegisterAccess(hUart);
        }
    }
    ...
    const NvDdkUartConfiguarations* const pUartNewConfig)
{
    ...
    if (pUartNewConfig->UartBaudRate != pUartCurrConfig->UartBaudRate)
    {
        ...
        hUart->HwIntf.HwGetReqdClockSourceFreqFxn(pUartNewConfig->UartBaudRate,
        ...
        if (!Error)
        {
            ...
            Error = hUart->HwIntf.HwSetBaudRateFxn(&hUart->UartHwRegs,
    ...
    {
        Error = hUart->HwIntf.HwSetParityBitFxn(&hUart->UartHwRegs,
    ...
    {
        Error = hUart->HwIntf.HwSetDataLengthFxn(&hUart->UartHwRegs,
    ...
    {
        Error = hUart->HwIntf.HwSetStopBitFxn(&hUart->UartHwRegs,
    ...
    {
        hUart->HwIntf.HwSetIrdaCodingFxn(&hUart->UartHwRegs, pUartNewConfig->IsEnableIrdaModulation);
    ...
    NvU32 BufferSize)
{
    ...
    LockRegisterAccess(hUart);
    if (hUart->IsRtsFlowEnable)
        hUart->HwIntf.HwSetModemSignalFxn(&hUart->UartHwRegs, NvDdkUartSignalName_Rts, NV_FALSE);

    // Disable receive interrupt.
    hUart->HwIntf.HwSetReceiveInterruptFxn(&hUart->UartHwRegs, NV_TRUE, hUart->IsRxApbDmaBased);
    UnlockRegisterAccess(hUart);
    ...
    // Reset the Rx fifo.
    hUart->HwIntf.HwResetFifoFxn(&hUart->UartHwRegs, UartDataDirection_Receive);
    ...
    // buffer is empty.
    if (hUart->IsRtsFlowEnable)
        hUart->HwIntf.HwSetModemSignalFxn(&hUart->UartHwRegs, NvDdkUartSignalName_Rts,
                                                            NV_TRUE);
    ...
    NvU32 *pAvailableBytes)
{
    ...
    BytesRead = 0;
    TransferdCount = 0;
    IsBreakDetectd = hUart->HwIntf.HwIsBreakSignalDetectedFxn(&hUart->UartHwRegs);
    if (hUart->IsRxApbDmaBased)
    {
        if (hUart->HwIntf.HwIsDataAvailableInFifoFxn(&hUart->UartHwRegs))
        {
            ...
            hUart->ReadReq.TransferStatus =
                    hUart->HwIntf.HwReadFromReceiveFifoFxn(&hUart->UartHwRegs,
...
//                                TransferdCount, hUart->LastReadIndex);
        ...
        hUart->HwIntf.HwSetReceiveInterruptFxn(&hUart->UartHwRegs, NV_TRUE, NV_TRUE);
    }
    ...
    NvU32 WaitTimeoutMs)
{
    ...
    if (hUart->IsSuspendedState)
        ...
        if (hUart->IsCtsFlowEnable)
            IsCtsPrevActive = hUart->HwIntf.HwIsCtsActiveFxn(&hUart->UartHwRegs);
        if (!IsCtsPrevActive)
    ...
    if (hUart->WriteReq.BytesRemaining > hUart->MaxTxBytesPolling)
    {
        ...
        else
        {
            ...
            hUart->WriteReq.hOnCompleteSema = hUart->hTxSynchSema;
            LockRegisterAccess(hUart);
            hUart->HwIntf.HwSetFifoTriggerLevelFxn(&hUart->UartHwRegs, UartDataDirection_Transmit, 8);
            hUart->HwIntf.HwSetTransmitInterruptFxn(&hUart->UartHwRegs, NV_TRUE);
            UnlockRegisterAccess(hUart);
            Error = NvOsSemaphoreWaitTimeout(hUart->WriteReq.hOnCompleteSema, WaitTimeoutMs);
            if ((hUart->WriteReq.IsStopReq) || (Error == NvError_Timeout))
            {
    ...
    if ((!Error) && (hUart->WriteReq.BytesRemaining) && (!hUart->WriteReq.IsStopReq))
    {
        ...
        {
            BytesWritten = hUart->HwIntf.HwPollingWriteInTransmitFifoFxn(&hUart->UartHwRegs,
                                hUart->WriteReq.pCurrTransBuf, hUart->WriteReq.BytesRemaining);
            if (BytesWritten)
            {
                hUart->WriteReq.pCurrTransBuf += BytesWritten;
            ...
            if (hUart->IsCtsFlowEnable)
            {
                IsCtsActive = hUart->HwIntf.HwIsCtsActiveFxn(&hUart->UartHwRegs);
                if (!IsCtsActive)
                {
                    // Wait for fifo get empty
                    hUart->WriteReq.hOnCompleteSema = hUart->hTxSynchSema;
                    LockRegisterAccess(hUart);
                    hUart->HwIntf.HwSetFifoTriggerLevelFxn(&hUart->UartHwRegs, UartDataDirection_Transmit, 4);
                    hUart->HwIntf.HwSetTransmitInterruptFxn(&hUart->UartHwRegs, NV_TRUE);
                    UnlockRegisterAccess(hUart);
    ...
    DEBUG_PRINT_TX_ERR(((Error == NvError_Timeout) &&(hUart->IsCtsFlowEnable)),
        ("NvDdkUartWrite(): CTS State starting %d and end %d",IsCtsPrevActive,
                        hUart->HwIntf.HwIsCtsActiveFxn(&hUart->UartHwRegs)));
...
NvError NvDdkUartSetBreakSignal(NvDdkUartHandle hUart, NvBool IsStart)
{
    ...
    LockRegisterAccess(hUart);
    if (IsStart)
        hUart->HwIntf.HwSetBreakControlSignalFxn(&hUart->UartHwRegs, NV_TRUE);
    else
        hUart->HwIntf.HwSetBreakControlSignalFxn(&hUart->UartHwRegs, NV_FALSE);
    UnlockRegisterAccess(hUart);
    return NvSuccess;
}
    ...
    NvDdkUartFlowControl FlowControl)
{
    ...
    if (FlowControl == NvDdkUartFlowControl_Handshake)
    {
        ...
        {
            hUart->HwIntf.HwSetHWFlowControlFxn(&hUart->UartHwRegs, NV_TRUE);
    ...
    if (SignalName & NvDdkUartSignalName_Cts)
    {
        ...
        {
            hUart->HwIntf.HwSetHWFlowControlFxn(&hUart->UartHwRegs, NV_FALSE);
            hUart->IsCtsFlowEnable = NV_FALSE;
        }
    }
    hUart->HwIntf.HwSetModemSignalFxn(&hUart->UartHwRegs, SignalName, IsHigh);
    ...
    void *args)
{
    ...
    if (Callback != NULL)
    {
        ...
        if (hUart->NumberOfInterfaceLine == 8)
            hUart->HwIntf.HwSetModemControlInterruptFxn(&hUart->UartHwRegs, NV_TRUE);
    }
    else
    {
        ...
        hUart->HwIntf.HwSetModemControlInterruptFxn(&hUart->UartHwRegs, NV_FALSE);
    }
    UnlockRegisterAccess(hUart);
    return NvSuccess;
}
...
NvError NvDdkUartSuspend(NvDdkUartHandle hUart)
{
    ...
    if (hUart->UartHwRegs.IsDtrActive)
        hUart->HwIntf.HwSetModemSignalFxn(&hUart->UartHwRegs, NvDdkUartSignalName_Dtr, NV_FALSE);
...
NvError NvDdkUartResume(NvDdkUartHandle hUart)
{
    ...
    if (!Error)
    {
        hUart->HwIntf.HwReconfigureRegistersFxn(&hUart->UartHwRegs);
        ...
        if (hUart->OldRtsState)
            hUart->HwIntf.HwSetModemSignalFxn(&hUart->UartHwRegs, NvDdkUartSignalName_Rts, NV_TRUE);
        if (hUart->OldDtrState)
            hUart->HwIntf.HwSetModemSignalFxn(&hUart->UartHwRegs, NvDdkUartSignalName_Dtr, NV_TRUE);
    }
