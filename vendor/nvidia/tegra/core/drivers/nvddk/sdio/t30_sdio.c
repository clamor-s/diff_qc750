...
static void T30SdioSetTapDelay(NvDdkSdioDeviceHandle hSdio, NvU32 TapDelay)
{
    NvU32 val = 0;
    ...
    val = NV_FLD_SET_DRF_NUM(SDMMC, VENDOR_CLOCK_CNTRL, TAP_VAL, TapDelay, val);
    ...
    NvDdkSdioHostCapabilities *pHostCaps)
{
    ...
    if (NV_DRF_VAL(SDMMC, CAPABILITIES_HIGHER, DDR50, capabilities))
        pHostCaps->IsDDRmodeSupported = NV_TRUE;
}
...
void T30SdioOpenHWInterface(NvDdkSdioDeviceHandle hSdio)
{
    ...
    hSdio->NvDdkSdioSetTapDelay = (void *)T30SdioSetTapDelay;
