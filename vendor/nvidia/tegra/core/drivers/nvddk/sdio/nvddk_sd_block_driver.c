...
#define SD_ERROR_PRINT(x) NvOsDebugPrintf x
#else
#define SD_ERROR_PRINT(x)
#endif
...
#define MMC_CARD_TYPE_HIGH_SPEED_26MHZ 0x1
#define MMC_CARD_TYPE_HIGH_SPEED_52MHZ 0x2
#define MMC_CARD_TYPE_DDR_1_8_V 0x4
#define MMC_CARD_TYPE_DDR_1_2_V 0x8
...
MmcSetBusWidth(SdHandle hSd, NvU32 BusWidth)
{
    ...
    {
        NV_CHECK_ERROR_CLEANUP(NvDdkSdioSetHostBusWidth(hSd->hSdDdk, NvDdkSdioDataWidth_8Bit));
    }
    else
    {
        NV_CHECK_ERROR_CLEANUP(NvDdkSdioSetHostBusWidth(hSd->hSdDdk, NvDdkSdioDataWidth_4Bit));
    }
...
static NvError MmcSwitchToHighSpeed( SdHandle hSd )
{
    NvError e = NvError_BadParameter;
    NvDdkSdioCommand *pCmd = &hSd->pCmd;
    NvU32 SdStatus = 0;

    if (!hSd->IsHighSpeed || !hSd->SdHostCaps.IsHighSpeedSupported)
...
static NvError ReadExtCSD(SdHandle hSd, NvU32 Instance)
{
    ...
    if (hSd->CardType & (MMC_CARD_TYPE_HIGH_SPEED_52MHZ |
            MMC_CARD_TYPE_HIGH_SPEED_26MHZ))
    {
        hSd->IsHighSpeed = NV_TRUE;
    }

    // Check if the card support DDR mode
    if (hSd->CardType & (MMC_CARD_TYPE_DDR_1_8_V | MMC_CARD_TYPE_DDR_1_2_V))
    {
    ...
    NvU32 Instance)
{
    ...
    hSd->EraseGrpSize = ((hSd->Response[1] & 0x3F80) >> 7);
    ...
    NvU32 Instance)
{
    ...
    if (MmcSwitchToHighSpeed(hSd))
    {
        ClockRateInKHz = SD_TX_CLOCK_KHZ;
        hSd->IsHighSpeed = NV_FALSE;
    }
    else
    {
        ClockRateInKHz = SD_SDHC_TX_CLOCK_KHZ;
    ...
    if ((hSd->Uhsmode == NvDdkSdioUhsMode_DDR50) &&
        hSd->SdHostCaps.IsDDRmodeSupported)
    {
        // send CMD6 to set the Buswidth to Eight bit ddr mode
        if (hSd->DataBusWidth == 4)
        {
            NV_CHECK_ERROR_CLEANUP(MmcSetBusWidth(hSd, MMC_DDR_MODE_4_BIT_BUSWIDTH));
        }
        else if (MmcSetBusWidth(hSd, MMC_DDR_MODE_8_BIT_BUSWIDTH))
        {
            // If 8 bit ddr bus width switch fails, try 4 bit ddr bus width switch.
            NV_CHECK_ERROR_CLEANUP(MmcSetBusWidth(hSd, MMC_DDR_MODE_4_BIT_BUSWIDTH));
            SD_ERROR_PRINT(("8-bit ddr buswidth switch failed. Using 4-bit buswidth\n"));
        }
        // Configure Uhs mode for the controller now.
        NV_CHECK_ERROR_CLEANUP(NvDdkSdioSetUhsmode(hSd->hSdDdk, hSd->Uhsmode));
    }
    else
    {
        // send CMD6 to set the Buswidth to Eight bit mode
        if (hSd->DataBusWidth == 4)
        {
    ...
    NvDdkBlockDevInfo* pBlockDevInfo)
{
    ...
    pBlockDevInfo->SectorsPerBlock = 512;
    ...
    NvU32 NumberOfBlocks)
{
    ...
    while (ResidueBlks)
    {
        ...
        NV_CHECK_ERROR_CLEANUP(MmcSelectRegion(hSd, &BlockNum, &RegNumBlks, NV_TRUE));
        ...
        while (CurrentBlocks)
        {
            ...
            if (SdioStatus & NvDdkSdioError_DataTimeout)
            {
                ...
                if (hSd->Response[0] & MMC_READ_WRITE_ERROR_MASK)
                {
                    ...
                    //For reselecting the previous region
                    BlockNum = StartBlock;
                    break;
                }
                else
...
static void SdClose(NvDdkBlockDevHandle hBlockDev)
{
    ...
    else
    {
        ...
        if (!((BootDevice == NvBootDevType_Emmc) &&
...
SdFormatDevice(SdBlockDevHandle hSdBlkDev)
{
    ...
    if (!hSdBlkDev->IsPowered)
    {
        NvDdkSdioResume(hSdBlkDev->hDev->hSdDdk, NV_TRUE);
        hSdBlkDev->IsPowered = NV_TRUE;
    }
    ...
    void *OutputArgs)
{
    ...
    switch (Opcode)
    {
        ...
        case NvDdkBlockDevIoctlType_EraseLogicalSectors:
        {
            ...
            NV_ASSERT(InputArgs);
            if ((hSdBlockDev->hDev->IsSecureEraseSupported) &&
                (ip->IsSecureErase == NV_TRUE))
        ...
        case NvDdkBlockDevIoctlType_ErasePhysicalBlock:
        {
            ...
            if ((hSdBlockDev->hDev->IsSecureEraseSupported) &&
                (pErasePhysicalBlock->IsSecureErase == NV_TRUE))
                    hSdBlockDev->hDev->DoSecureErase = NV_TRUE;
            else
                hSdBlockDev->hDev->DoSecureErase = NV_FALSE;
