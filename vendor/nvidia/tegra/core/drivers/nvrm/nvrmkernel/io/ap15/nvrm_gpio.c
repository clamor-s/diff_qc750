    ...
    NvRmGpioHandle* phGpio)
{
    ...
    gpioShadowPhysical = NV_REGR(hRm,
        NVRM_MODULE_ID( NvRmModuleID_Pmif, 0 ), APBDEV_PMC_SCRATCH19_0);

    /* !!!FIXME!!! There is no need to shadow GPIOs on newer chips. */
...
void NvRmGpioClose(NvRmGpioHandle hGpio)
{
    ...
    if (hGpio->RefCount == 0)
    {
        ...
        gpioShadowPhysical = NV_REGR(hGpio->hRm,
            NVRM_MODULE_ID( NvRmModuleID_Pmif, 0 ), APBDEV_PMC_SCRATCH19_0);
