    ...
    NvU32 pinCount)
{
    ...
    /* FIXME: only supports T20 for now */
    for (i=0; i<pinCount; i++) {
        if (handle_to_nr(pin[i]) >= 0)
            Irq[i] = handle_to_nr(pin[i]) + 160;
        else
            Irq[i] = -1;
    }
    ...
    NvBool export)
{
    ...
    for (i=0; i<pinCount; i++)
    {
        ...
        write(fd, name, NvOsStrlen(name)+1);
    ...
    NvRmGpioPinMode Mode)
{
    ...
    for (i=0; i<pinCount; i++)
    {
        ...
        if (Mode==NvRmGpioPinMode_Output && pin->value)
            write(fd, "high", NvOsStrlen("high"));
        else
            write(fd, config_array[Mode][0],
                  NvOsStrlen(config_array[Mode][0])+1);
        ...
        if (fd < 0)
            return NvError_AccessDenied;
        write(fd, config_array[Mode][1],
              NvOsStrlen(config_array[Mode][1])+1);
        close(fd);
    }
    return NvSuccess;
}
    ...
    NvU32 pinCount)
{
    ...
    for (i=0; i<pinCount; i++)
    {
        ...
        write(pin_data[gpio_nr].fd, value[pin_data[gpio_nr].value], 2);
    }
}
