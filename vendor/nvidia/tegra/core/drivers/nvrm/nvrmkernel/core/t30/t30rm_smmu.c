...
/* Switch for artificial reverse-order mapping for SMMU stress test */
#ifndef NVRM_T30_REVERSESMMU
#define NVRM_T30_REVERSESMMU NV_FALSE
#endif
static NvBool s_reverseSMMU = NVRM_T30_REVERSESMMU;
...
// functions used for testing, locally prototype
void NvRmPrivT30SMMUFillRandom(NvRmDeviceHandle hDevice);
void NvRmPrivT30SMMUSetReverse(NvBool reverse);
    ...
    NvRmPhysAddr *ioVAp)
{
    ...
    *ioVAp = FirstPageAddr;
    ...
    basePAddr = NvOsPageAddress(hPageHandle, 0);
    for (nPage = 0; nPage < NumberOfPages; nPage++)
    {
        NvU32 nPage2 = s_reverseSMMU ? (NumberOfPages - nPage - 1) : nPage;
        s_PTEbase[PTEOffset + nPage] =
            SMMU_PTE_ATTR |
            ((basePAddr + nPage2 * SMMU_PAGE_SIZE) >> SMMU_PAGE_SHIFT);
        ((NvU32 *)(basePAddr + nPage2 * SMMU_PAGE_SIZE))[0] =
                        FirstPageAddr + nPage2 * SMMU_PAGE_SIZE;
        ((NvU32 *)(basePAddr + nPage2 * SMMU_PAGE_SIZE))[1] =
                        basePAddr + nPage2 * SMMU_PAGE_SIZE;
        NvOsDataCacheWritebackRange(
                        (void *)(basePAddr + nPage2 * SMMU_PAGE_SIZE),
                                 sizeof(NvU32) *2);
    }

    if (s_reverseSMMU)
        NvOsReverseMemMap(basePAddr, NumberOfPages);
    ...
    NvU32 NumberOfPages)
{
    ...
    basePAddr = s_PTEbase[PTEOffset + (NumberOfPages - 1)] << SMMU_PAGE_SHIFT;
    for (nPage = 0; nPage < NumberOfPages; nPage++)
    {
        s_PTEbase[PTEOffset + nPage] =
            SMMU_PTE_ATTR |
                   ...
                   NvU32 flags, void **cpuVAp)
{
    ...
    if (s_reverseSMMU)
        ioVAAligned += ALIGNDN(size - 1, SMMU_PAGE_SIZE);
...
/*************************************************************************
 Special functions for testing SMMU's inter-AS interference
 *************************************************************************/
NvU32 NvRmPrivT30SMMUrandN(NvU32 mod)
{
    static NvU32 seed = 0;
    NvU32 carry;
...
static void
NvRmPrivT30SMMUFillRandomASID(NvRmDeviceHandle hDevice, NvU32 asid)
{
    NvU32 nPDE, nPTE;
    NvU32 *PDEbase = s_PAA[asid];
    NvU32 *PTEbase = PDEbase + SMMU_PDIR_SIZE;

    // Fill PDEs with random "big page" addresses
    for (nPDE = 0; nPDE < SMMU_PDIR_SIZE; nPDE++)
        PDEbase[nPDE] = SMMU_PDE_ATTR |
                    (NvRmPrivT30SMMUrandN(SMMU_PDIR_SIZE) << SMMU_PDE_PA_SHIFT);

    // Fill SMMU-window PTEs with random page addersses
    // and make corresponding PDEs point to PTBLs
    for (nPTE = 0; nPTE < s_NumPTEs; nPTE++)
    {
        NvU32 *pte = &PTEbase[nPTE];

        *pte = SMMU_PTE_ATTR | (NvRmPrivT30SMMUrandN(256 * 1024) << 2);
        if (!(nPTE % SMMU_PTBL_SIZE))
             PDEbase[s_PDEOffset + nPTE / SMMU_PTBL_SIZE] =
                 SMMU_PDE_ATTR_PT | ((NvU32)pte >> SMMU_PTBL_SHIFT);
    }

    NvOsDataCacheWritebackRange(PDEbase, s_allocSize);

    setPDEbase(hDevice, PDEbase, asid);
    flushSMMURegs(hDevice, 1);
}

void
NvRmPrivT30SMMUFillRandom(NvRmDeviceHandle hDevice)
{
    NvU32 asid;

    for (asid = 0; asid < NV_MC_SMMU_NUM_ASIDS; asid++)
        if (asid != s_ASID)
            NvRmPrivT30SMMUFillRandomASID(hDevice, asid);
}

void
NvRmPrivT30SMMUSetReverse(NvBool reverse)
{
    s_reverseSMMU = reverse;
}
