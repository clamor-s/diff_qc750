        ...
        case NvRmPrivModuleID_ExternalMemoryController:
            {
                ...
                reg = NV_FLD_SET_DRF_NUM(CLK_RST_CONTROLLER, CLK_SOURCE_EMC,
                    EMC_2X_CLK_ENB, 1, reg);
                reg = NV_FLD_SET_DRF_NUM(CLK_RST_CONTROLLER, CLK_SOURCE_EMC,
                    EMC_1X_CLK_ENB, 1, reg);
                NV_REGW(hDevice,
                    NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
...
NvRmPrivContentProtectionFuses( NvRmDeviceHandle hRm )
{
    ...
    reg = NV_REGR( hRm, NVRM_MODULE_ID( NvRmModuleID_Fuse, 0 ),
        FUSE_FUSEBYPASS_0);
    reg = NV_FLD_SET_DRF_DEF( FUSE, FUSEBYPASS, FUSEBYPASS_VAL, ENABLED, reg );
    NV_REGW( hRm, NVRM_MODULE_ID( NvRmModuleID_Fuse, 0 ), FUSE_FUSEBYPASS_0,
        reg );

    reg = NV_REGR( hRm, NVRM_MODULE_ID( NvRmModuleID_Fuse, 0 ),
        FUSE_WRITE_ACCESS_SW_0);
    reg = NV_FLD_SET_DRF_DEF( FUSE, WRITE_ACCESS_SW, WRITE_ACCESS_SW_CTRL,
            READWRITE, reg);
    NV_REGW( hRm, NVRM_MODULE_ID( NvRmModuleID_Fuse, 0 ),
        FUSE_WRITE_ACCESS_SW_0, reg );

    reg = NV_REGR( hRm, NVRM_MODULE_ID( NvRmModuleID_Fuse, 0 ),
        FUSE_RESERVED_PRODUCTION_0);
    reg = NV_FLD_SET_DRF_NUM( FUSE, RESERVED_PRODUCTION,
            RESERVED_PRODUCTION, 0x3, reg );
    NV_REGW( hRm, NVRM_MODULE_ID( NvRmModuleID_Fuse, 0),
        FUSE_RESERVED_PRODUCTION_0, reg );
