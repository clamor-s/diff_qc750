...
#include "ap20/arapb_misc.h" // chipid, has to be the same for all chips
...
NvRmPrivGetRelocationTable( NvRmDeviceHandle hDevice )
{
    ...
    case 0x30:
        return NvRmPrivT30GetRelocationTable( hDevice );
    default:
...
NvRmPrivReadChipId( NvRmDeviceHandle rm )
{
    ...
    if (id->Id == 0x20)
    {
        /* ap20 FUSE register bank */
        if (NvRmPhysicalMemMap(0x7000f800, 0x400, NVOS_MEM_READ_WRITE,
            NvOsMemAttribute_Uncached, (void **)&VirtAddr) == NvSuccess)
        {
            /* FUSE_SPARE_BIT_18_0 and FUSE_SPARE_BIT_19_0 of ap20 */
            if (NV_READ32(VirtAddr + 0x200 + 18 * 4) ||
                NV_READ32(VirtAddr + 0x200 + 19 * 4))
                id->Private = "p";
            NvRmPhysicalMemUnmap(VirtAddr, 0x400);
        }
    }
#endif
}
