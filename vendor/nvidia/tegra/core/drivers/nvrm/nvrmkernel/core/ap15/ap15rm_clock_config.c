    ...
    NvRmFreqKHz* pPllRefKHz)
{
    ...
    {
        case 0x20:
            NvRmPrivAp15OscPllRefFreqInit(hRmDevice, pOscKHz, pPllRefKHz);
            return;
        case 0x30:
            NvRmPrivT30OscPllRefFreqInit(hRmDevice, pOscKHz, pPllRefKHz);
            return;
        default:
...
OscDoublerConfigure(NvRmDeviceHandle hRmDevice, NvRmFreqKHz OscKHz)
{
    switch (hRmDevice->ChipId.Id)
    {
        ...
        case 0x30:
            return NvRmPrivT30OscDoublerConfigure(hRmDevice, OscKHz);
        default:
            NV_ASSERT(!"Unsupported chip ID");
    ...
    NvU32 flags)
{
    ...
    else if (CONFIG_PLLP_BASE_AS_408MHZ ||
    ...
    if (flags & NvRmClockConfig_SubConfig)
    {
        ...
        if ((hRmDevice->ChipId.Id == 0x15) || (hRmDevice->ChipId.Id == 0x16))
            Ap15EnableTvDacClock(hRmDevice, ModuleClockState_Enable);
                 ...
                 (hRmDevice->ChipId.Id == 0x30))    // backward compatible
        ...
        pTvDacState->SourceClock =
            (hRmDevice->ChipId.Id == 0x30) ? (i >> 1) : i;
        pTvDacState->Divider =
...
static void PllEControl(NvRmDeviceHandle hRmDevice, NvBool Enable)
{
    switch (hRmDevice->ChipId.Id)
    {
        case 0x20:
            ...
            return;
        case 0x30:
            NvRmPrivT30PllEControl(hRmDevice, Enable);
            return;
        default:
            NV_ASSERT(!"Unsupported chip ID");
            return;
