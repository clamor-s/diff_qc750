...
static const NvU32 ObsInfoTableSize = NV_ARRAY_SIZE(ObsInfoTable);
        ...
        NvU32 llc_client_id)
{
    ...
    //NvOsDebugPrintf(" AP1x ID 0 = %d and ID 1 = %d \n", client_id_0, client_id_1);
...
// The OBS_PART_SEL is mapped to the specified modID by obsInfoTable which is
// public in this file.
        ...
        NvU32 *value)
{
    ...
    for(offset = 0; offset < length; offset++)
    {
        ...
        NV_REGW(rm, NVRM_MODULE_ID( NvRmModuleID_Misc, 0 ),
            APB_MISC_GP_OBSCTRL_0, temp);
        value1 = NV_REGR(rm, NVRM_MODULE_ID( NvRmModuleID_Misc, 0 ),
            APB_MISC_GP_OBSCTRL_0);
        timeout = 100;
        do {
            value2 = value1;
            value1 = NV_REGR(rm, NVRM_MODULE_ID( NvRmModuleID_Misc, 0 ),
                APB_MISC_GP_OBSDATA_0);
            timeout --;
...
{
    NvU32   reg, mask;
    NvRmModuleID id = NVRM_MODULE_ID( NvRmPrivModuleID_MemoryController, 0 );
    reg = NV_REGR(hRm, id, MC_LOWLATENCY_CONFIG_0);
    ...
    /* 1) TIMEOUT value for VDE is 256 cycles, 3D, 2D timeouts are disabled,
     * all others 512 cycles.
     */
    NV_REGW(hRm, id, MC_TIMEOUT_CTRL_0,    0x00000028);
    NV_REGW(hRm, id, MC_TIMEOUT_CMC_0,     0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_DC_0,      0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_DCB_0,     0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_EPP_0,     0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_G2_0,      0x0);
    NV_REGW(hRm, id, MC_TIMEOUT_HC_0,      0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_ISP_0,     0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_MPCORE_0,  0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_MPEA_0,    0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_MPEB_0,    0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_MPEC_0,    0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_NV_0,      0x0);
    NV_REGW(hRm, id, MC_TIMEOUT_PPCS_0,    0x88888888);
    NV_REGW(hRm, id, MC_TIMEOUT_VDE_0,     0x44444444);
    NV_REGW(hRm, id, MC_TIMEOUT1_VDE_0,    0x44444444);
    NV_REGW(hRm, id, MC_TIMEOUT_VI_0,      0x88888888);

    /* 2) Command Queue values should be 2,2,6 for better performance. */
    NV_REGW(hRm,
        NVRM_MODULE_ID( NvRmPrivModuleID_ExternalMemoryController, 0 ),
        EMC_CMDQ_0, 0x00002206);
    ...
    NV_REGW(hRm, id, MC_EMEM_ARB_CFG0_0,    0x03081010);
}
