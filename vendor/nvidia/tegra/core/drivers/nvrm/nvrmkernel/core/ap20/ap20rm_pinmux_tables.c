...
static const NvU32 g_Ap20Mux_Snor[] = {
    ...
    // config 6. Small density (<64KB) of muxed 16b DPRAM
    CONFIG(A,A,ATC,GMI),CONFIG(A,A,ATD,GMI),CONFIG(B,A,ATE,GMI),
    CONFIG(B,C,GMD,GMI),CONFIGEND(),
    MODULEDONE(),
...
static const NvU32 g_Ap20Mux_Vi[] = {
    ...
    CONFIG(A,B,DTD,VI),CONFIG(A,B,DTE,VI),CONFIG(A,B,DTC,VI),CONFIGEND(),
    MODULEDONE(),
    SUBROUTINESDONE(),
};
...
NvRmAp20GetStraps(
    ...
    NvU32* pStrapValue)
{
    NvU32 reg = NV_REGR( hDevice,
        NVRM_MODULE_ID( NvRmModuleID_Misc, 0 ), APB_MISC_PP_STRAPPING_OPT_A_0);
