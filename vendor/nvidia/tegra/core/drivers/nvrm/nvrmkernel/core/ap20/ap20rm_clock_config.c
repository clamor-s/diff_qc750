...
#define NULL_DEV_SELECTN (3)
    ...
    NvU32 cfg2, cfg5;
    NvU32 id = NVRM_MODULE_ID(NvRmPrivModuleID_ExternalMemoryController, 0);

    cfg2 = NV_REGR(hRmDevice, id, EMC_CFG_2_0);
    cfg5 = NV_REGR(hRmDevice, id, EMC_FBIO_CFG5_0);
    ...
    switch (NV_DRF_VAL(EMC, FBIO_CFG5, DRAM_TYPE, cfg5))
    {
        case EMC_FBIO_CFG5_0_DRAM_TYPE_LPDDR2:
            ...
            NV_REGW(hRmDevice, id, EMC_MRW_0,
...
#if NVRM_EMC_CLKCHANGE_PD
            // Dummy mode control command to activate PD state machine
            NV_REGW(hRmDevice, id, EMC_MRS_0,
    ...
    NV_REGW(hRmDevice, id, EMC_CFG_2_0, cfg2);
    return NV_TRUE;
}
    ...
    NvU32 value)
{
    ...
    NV_ASSERT(
        NV_DRF_VAL(CLK_RST_CONTROLLER, CLK_SOURCE_EMC, USE_PLLM_UD, clk)
    );
...
NvRmPrivAp20FastClockConfig(NvRmDeviceHandle hRmDevice)
{
    ...
    NvRmFreqKHz SclkKHz, PllP2KHz, PllM1KHz, PllC1KHz;
#else
...
#ifndef CPU_CLK_LOW_INITIAL_RATE
    if (CpuKHz != FreqKHz)
    {
