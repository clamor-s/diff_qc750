    ...
    NvOsMemAttribute  Coherency)
{
    ...
    if (!NumHeaps)
    {
        ...
        else
        {
            DefaultHeaps[NumHeaps++] = NvRmHeap_ExternalCarveOut;
            DefaultHeaps[NumHeaps++] = NvRmHeap_External;
            if (!ERRATA_398959(hMem->hRmDevice->ChipId) && s_HeapGartAlloc)
...
NvRmPrivHeap *NvRmPrivHeapGartInit(NvRmDeviceHandle hRmDevice)
{
    ...
    if ((pCap->MajorVersion == 1) && (pCap->MinorVersion == 0))
    {
        ...
        s_HeapGartMap = NvOsPhysicalMemMap;
    }
    else if ((pCap->MajorVersion == 1) && (pCap->MinorVersion == 1))
    {
    ...
    else if ((pCap->MajorVersion == 2) && (pCap->MinorVersion == 0))
    {
