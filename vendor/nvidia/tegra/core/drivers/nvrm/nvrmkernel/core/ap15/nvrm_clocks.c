    ...
    ModuleClockState ClockState)
{
    switch (hRmDevice->ChipId.Id)
    {
        case 0x20:
            ...
            break;
        case 0x30:
            T30EnableModuleClock(hRmDevice, ModuleId, ClockState);
            break;
        default:
...
NvRmPrivClocksInit(NvRmDeviceHandle hRmDevice)
{
    ...
    else if (hRmDevice->ChipId.Id == 0x30)
    {
        s_moduleClockTable = g_T30ModuleClockTable;
        s_moduleClockTableSize = g_T30ModuleClockTableSize;
        NvRmPrivT30PllReferenceTableInit(&s_PllReferencesTable,
                &s_PllReferencesTableSize);
        s_ClockSourceTable = NvRmPrivT30ClockSourceTableInit();
        fpgaModuleFreq = FPGA_MODULE_KHZ_AP20;
    }
    else
        NV_ASSERT(!"Unsupported chip ID");
...
NvRmPrivBoostClocks(NvRmDeviceHandle hRmDevice)
{
    ...
    {
        NvRmPrivT30FastClockConfig(hRmDevice);
    }
    ...
    NvU32 flags)
{
    ...
    else if ((hDevice->ChipId.Id < 0x30) &&
                (PrefFreqList && (ModuleName == NvRmModuleID_Vde)))
    {   // VDE clock not supported
        NV_ASSERT(!"VDE clock configuration is not allowed here");
        return NvError_NotSupported;
    }
