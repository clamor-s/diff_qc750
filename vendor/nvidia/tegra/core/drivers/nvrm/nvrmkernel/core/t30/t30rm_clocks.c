...
void
NvRmPrivT30GetBondOut( NvRmDeviceHandle hDevice,
                       const NvU32     **pTable,
                       NvU32           *bondOut )
{
...
// Top level T30 clock enable register control macro
    ...
    ModuleClockState ClockState)
{
    ...
    switch ( Module ) {
        ...
        case NvRmModuleID_ExtPeriphClk:
            ...
            break;
        default:
            NV_ASSERT(!"Unknown NvRmModuleID passed to T30EnableModuleClock().");
    }
...
void T30ModuleReset(NvRmDeviceHandle hDevice, NvRmModuleID ModuleId, NvBool hold)
{
    // Extract module and instance from composite module id.
    NvU32 Module   = NVRM_MODULE_ID_MODULE( ModuleId );
    NvU32 Instance = NVRM_MODULE_ID_INSTANCE( ModuleId );
    ...
    case NvRmModuleID_2D:
        ...
        // WAR for bug 364497, se also NvRmPrivT30Reset2D()
...
NvRmPrivT30Reset2D(NvRmDeviceHandle hRmDevice)
{
     ...
     */
    T30EnableModuleClock(hRmDevice, NvRmModuleID_Vi,
    ...
    T30EnableModuleClock(hRmDevice, NvRmModuleID_2D, ModuleClockState_Enable);
    NvOsWaitUS(NVRM_RESET_DELAY);
    ...
    NV_REGW(hRmDevice, NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
        offset, reg);
    T30EnableModuleClock(hRmDevice, NvRmModuleID_2D,
        ModuleClockState_Disable);
}
...
void
NvRmPrivT30BasicReset( NvRmDeviceHandle rm )
{
    ...
    if (NvRmIsSimulation())
    {
         ...
         */
         reg = NV_REGR( rm, NVRM_MODULE_ID( NvRmPrivModuleID_Ahb_Arb_Ctrl, 0 ),
                       AHB_ARBITRATION_XBAR_CTRL_0 );
        reg = NV_FLD_SET_DRF_DEF( AHB_ARBITRATION, XBAR_CTRL, MEM_INIT_DONE,
                                  DONE, reg );
        NV_REGW( rm, NVRM_MODULE_ID( NvRmPrivModuleID_Ahb_Arb_Ctrl, 0 ),
...
void
NvRmPrivT30FastClockConfig(NvRmDeviceHandle hRmDevice)
{
#if NVOS_IS_QNX
