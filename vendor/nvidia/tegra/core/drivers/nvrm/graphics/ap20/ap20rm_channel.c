...
static void NvRmPrivCleanup2d( NvRmDeviceHandle hDevice );
...
static void NvRmPrivResetWaitBase( NvRmDeviceHandle hDevice, NvU32 id );
static void NvRmPrivResetModuleWaitBase( NvRmDeviceHandle hDevice );
...
#define NVRM_CHANNEL_3D     1
#define NVRM_CHANNEL_2D     2
#define NVRM_CHANNEL_ISP    3
#define NVRM_CHANNEL_VI     4
#define NVRM_CHANNEL_MPE    5
...
static void NvRmChPrivConfigure2dSpeed(NvRmDeviceHandle hDevice);
...
static NvBool g_b2dInitialized;
...
/* BUG 380396 - only one MPE module may be active at a time  */
static NvBool g_MpeActive;
/* Caps bit 0 - MPE bug 380396 */
#define HAS_MPE_BUG_380396  0x1
static NvU32 g_MpeAp1xCaps = 0x1;
static NvU32 g_MpeAp2xCaps = 0x0;
static NvU32 *g_MpeCaps;

static NvBool g_ViActive;

// 2D WAR in channel
static NvRmMemHandle g_h2dCommands;
static NvRmMemHandle g_h2dSurf;
static NvRmMemHandle g_hRestoreClassIdCommands = NULL;
...
NvRmPrivChannelInit( NvRmDeviceHandle rm )
{
    ...
    NvRmModuleCapability MpeCapList[] =
        {
            {1, 0, 0, NvRmModulePlatform_Silicon, (void *)&g_MpeAp1xCaps},
            {1, 1, 0, NvRmModulePlatform_Silicon, (void *)&g_MpeAp1xCaps},
            {1, 2, 0, NvRmModulePlatform_Silicon, (void *)&g_MpeAp2xCaps},
        };
    ...
    NvRmPrivResetModuleWaitBase( rm );

    NvRmGetRmcFile( rm, &s_rmc );

    // MPE may be bonded-out
    err = NvRmModuleGetCapabilities( rm, NVRM_MODULE_ID(NvRmModuleID_Mpe, 0),
        MpeCapList, NV_ARRAY_SIZE(MpeCapList), (void **)&g_MpeCaps );
    if( err != NvSuccess )
    {
        g_MpeCaps = 0;
    }
...
NvRmPrivChannelDeinit( NvRmDeviceHandle rm )
{
    ...
    if ( g_b2dInitialized )
    {
       NvRmPrivCleanup2d( rm );
    }
    NvRmMemHandleFree( g_hRestoreClassIdCommands );
    g_hRestoreClassIdCommands = NULL;
...
NvRmPrivAllocateChannelResources( NvRmChannelHandle hChannel )
{
     ...
     */
    err = NvRmMemHandleCreate( hChannel->hDevice, &hw->pushbuffer,
        NVRM_PUSHBUFFER_SIZE + 4);
    if( err != NvSuccess )
    {
        goto fail;
    }

    /* uncached, contiguous memory for the pushbuffer */
    err = NvRmMemAllocTagged( hw->pushbuffer, NULL, 0, 32,
        NvOsMemAttribute_Uncached,
    ...
    const NvRmModuleID *pModuleIDs)
{
    ...
    NvBool use3D  = NV_FALSE;
    NvBool use2D  = NV_FALSE;
    NvBool useVI  = NV_FALSE;
    NvBool useMPE = NV_FALSE;
    NvBool useDisplay = NV_FALSE;
    NvBool useCpu = NV_FALSE;
    ...
    for (i = 0; i < NumModules; i++)
    {
        ...
        if ( mod == NvRmModuleID_3D)
            use3D = NV_TRUE;
        else if ( mod == NvRmModuleID_2D)
            use2D = NV_TRUE;
        else if (( mod == NvRmModuleID_Vi) ||
                 ( mod == NvRmModuleID_Csi) ||
                 ( mod == NvRmModuleID_Isp))
            useVI = NV_TRUE;
        else if ( mod == NvRmModuleID_Mpe)
            useMPE = NV_TRUE;
        else if ( mod == NvRmModuleID_Display)
            useDisplay = NV_TRUE;
        else if ( mod == NvRmModuleID_Epp)
        {
            // epp is usually allocated with 2d.
        }
        else if( mod == NvRmModuleID_Cpu )
        {
            useCpu = NV_TRUE;
        }
    ...
    if (use3D)
    {
        NV_ASSERT(!useVI && !useMPE);
        channel = NVRM_CHANNEL_3D;
    }
    else if (use2D)
    {
        NV_ASSERT(!useMPE && !useVI);
        channel = NVRM_CHANNEL_2D;
    }
    else if (useVI)
    {
        NV_ASSERT(!useMPE);
        if( g_ViActive )
        {
            // FIMXE: this good enough?
            *phChannel = NULL;
            NvOsMutexUnlock(  g_channelMutex );
            return NvError_InvalidState;
        }
        g_ViActive = NV_TRUE;
        channel = NVRM_CHANNEL_VI;
    }
    else if (useMPE)
    {
        if( g_MpeCaps && ((*g_MpeCaps) & HAS_MPE_BUG_380396) )
        {
            // BUG 380396 -- only one mpe context may be active at a time
            if( g_MpeActive )
            {
                // FIMXE: this good enough?
                *phChannel = NULL;
        ...
        g_MpeActive = NV_TRUE;
        channel = NVRM_CHANNEL_MPE;
    }
    else if (useDisplay || useCpu)
    {
...
NvRmChannelClose(NvRmChannelHandle hChannel)
{
    ...
    if ( hw->refcount )
    {
        ...
        if( !hw->refcount )
        {
            // BUG 380396 -- check for mpe channel, clear active mpe flag
            if( hw->num == NVRM_CHANNEL_MPE )
            {
                if( g_MpeCaps && ((*g_MpeCaps) & HAS_MPE_BUG_380396) )
                {
                    g_MpeActive = NV_FALSE;
                }
            }
            if ( hw->num == NVRM_CHANNEL_VI )
            {
                g_ViActive = NV_FALSE;
            }
...
#include "ap20/arg2sb.h"
#include "ap20/class_ids.h"
#include "nvrm_surface.h"

static NvBool
NvRmPriv2dModuleEnable( NvRmDeviceHandle hRm,
    struct NvRmAutoClockManagerEntryRec *entry, NvBool enable )
{
    static NvRmSurface s_surf;
    NvError e;
    NvU32 val;
    NvRmChannelHandle hChannel;
    NvRmCommandBuffer buf;
    NvRmPowerEvent event;
    NvBool bSubmit = NV_FALSE;
    const NvU32 NUM_2D_TINYBLT_WAR_COMMANDS   = 35; // hand computed
    ...
    NV_CHECK_ERROR_CLEANUP(
        NvRmPowerGetEvent( hRm, entry->PowerClientID, &event )
    );

    if( event == NvRmPowerEvent_WakeLP0 ||
        event == NvRmPowerEvent_WakeLP1 )
    {
        // FIXME: need to submit is g_b2dInitialized is false, but it
        // crashes the simulators
        bSubmit = NV_TRUE;
    }

    /* need to submit a small stretchblt to work around a hardware bug.
     * always use sync point 0 and the base channel.
     */
    if( !g_b2dInitialized || bSubmit )
    {
        NvU32 surfSize, cmdSize;
        NvU32 align;
        NvU32 reg;
        NvU32 idx;
        NvU32 mutexid;
        ...
        s_surf.Width = 1;
        s_surf.Height = 1;
        s_surf.ColorFormat = NvColorFormat_R5G6B5;
        s_surf.Layout = NvRmSurfaceLayout_Pitch;
        s_surf.Pitch = 16;
        surfSize = 16;
        align = 32;

        idx = 0;
        cmdSize = NUM_2D_TINYBLT_WAR_COMMANDS * 4;
        if( !g_b2dInitialized )
        {
            // Only allocate memory once (until free in ChannelDeinit)
            if ( NULL == g_h2dSurf )
            {
                NV_CHECK_ERROR_CLEANUP(
                    NvRmMemHandleCreate( hRm, &s_surf.hMem, surfSize )
                );
                NV_CHECK_ERROR_CLEANUP(
                    NvRmMemAllocTagged( s_surf.hMem, NULL, 0, align,
                        NvOsMemAttribute_Uncached,
                        NVRM_MEM_TAG_RM_MISC)
                );
                g_h2dSurf = s_surf.hMem;
            }
            else
                NvRmMemUnpin( s_surf.hMem );
                    // unpin if already pre-allocated

            if ( NULL == g_h2dCommands )
            {
                NV_CHECK_ERROR_CLEANUP(
                    NvRmMemHandleCreate( hRm, &g_h2dCommands, cmdSize )
                );
                NV_CHECK_ERROR_CLEANUP(
                    NvRmMemAllocTagged( g_h2dCommands, NULL, 0, align,
                        NvOsMemAttribute_Uncached, NVRM_MEM_TAG_RM_MISC)
                );
            }
        }
        else
        ...
        // FIXME: avoid the unpin/pin path...
        addr = NvRmMemPin( s_surf.hMem );

        #define NV2D_MASK(x,y) (1 << (G2SB_##x##_0 - G2SB_##y##_0))

        #define WRITE_CMD( cmd ) \
            do { \
                NvRmMemWr32( g_h2dCommands, idx, (cmd) ); \
                idx += 4; \
            } while( 0 )

        mutexid = NvRmChannelGetModuleMutex( NvRmModuleID_2D, 1 );

        reg = NVRM_CH_OPCODE_SET_CLASS(NV_GRAPHICS_2D_SB_CLASS_ID,0,0);
        WRITE_CMD( reg );

        reg = NVRM_CH_OPCODE_ACQUIRE_MUTEX( mutexid );
        WRITE_CMD( reg );

        reg =  NVRM_CH_OPCODE_MASK(G2SB_G2TRIGGER_0,
                (NV2D_MASK(G2TRIGGER, G2TRIGGER) |
                 NV2D_MASK(G2CMDSEL, G2TRIGGER)));
        WRITE_CMD( reg );

        reg = G2SB_G2DSTSIZE_0;
        WRITE_CMD( reg );
        reg = G2SB_G2CMDSEL_0_SBOR2D_SB;
        WRITE_CMD( reg );

        reg = NVRM_CH_OPCODE_MASK(G2SB_G2SBFORMAT_0,
                (NV2D_MASK(G2SBFORMAT, G2SBFORMAT) |
                 NV2D_MASK(G2CONTROLSB, G2SBFORMAT) |
                 NV2D_MASK(G2CONTROLMAIN, G2SBFORMAT)));
        WRITE_CMD( reg );

        reg = NV_DRF_DEF( G2SB, G2SBFORMAT, DIFMT, B5G6R5 )
            | NV_DRF_DEF( G2SB, G2SBFORMAT, SIFMT, B5G6R5 );
        WRITE_CMD( reg );

        reg = NV_DRF_DEF( G2SB, G2CONTROLSB, VFEN, DISABLE )
            | NV_DRF_DEF( G2SB, G2CONTROLSB, HFTYPE, DISABLE );
        WRITE_CMD( reg );

        reg = NV_DRF_DEF( G2SB, G2CONTROLMAIN, DSTCD, BPP16)
            | NV_DRF_NUM( G2SB, G2CONTROLMAIN, SRCDIR, 1 )
            | NV_DRF_NUM( G2SB, G2CONTROLMAIN, DSTDIR, 1 );
        WRITE_CMD( reg );

        reg = NVRM_CH_OPCODE_MASK(G2SB_G2DSTBA_0,
                (NV2D_MASK(G2DSTBA, G2DSTBA) |
                 NV2D_MASK(G2DSTST, G2DSTBA)));
        WRITE_CMD( reg );

        WRITE_CMD( addr );
        WRITE_CMD( s_surf.Pitch );

        reg = NVRM_CH_OPCODE_MASK(G2SB_G2SRCBA_0,
                (NV2D_MASK(G2SRCBA, G2SRCBA) |
                 NV2D_MASK(G2SRCST, G2SRCBA)));
        WRITE_CMD( reg );

        WRITE_CMD( addr );
        WRITE_CMD( s_surf.Pitch );

        reg =  NVRM_CH_OPCODE_MASK(G2SB_G2TILEMODE_0,
                (NV2D_MASK(G2TILEMODE, G2TILEMODE) |
                 NV2D_MASK(G2SRCBA_SB_SURFBASE, G2TILEMODE) |
                 NV2D_MASK(G2DSTBA_SB_SURFBASE, G2TILEMODE)));
        WRITE_CMD( reg );
        ...
        reg = NVRM_CH_OPCODE_MASK(G2SB_G2VDDA_0,
                (NV2D_MASK(G2VDDA, G2VDDA) |
                 NV2D_MASK(G2VDDAINI, G2VDDA)));
        WRITE_CMD( reg );

        WRITE_CMD( 0x1000 );
        WRITE_CMD( 0 );

        reg = NVRM_CH_OPCODE_MASK(G2SB_G2HDDA_0,
                (NV2D_MASK(G2HDDA, G2HDDA) |
                 NV2D_MASK(G2HDDAINILS, G2HDDA)));
        WRITE_CMD( reg );

        WRITE_CMD( 0x1000 );
        WRITE_CMD( 0 );

        reg = NVRM_CH_OPCODE_MASK(G2SB_G2CSCFIRST_0,
                (NV2D_MASK(G2CSCFIRST, G2CSCFIRST) |
                 NV2D_MASK(G2CSCSECOND, G2CSCFIRST) |
                 NV2D_MASK(G2CSCTHIRD, G2CSCFIRST)));
        WRITE_CMD( reg );

        #define SB_CSC_CONST_ONE_RGB   0x00080080
        #define SB_CSC_CONST_TWO_RGB   0x80000000
        #define SB_CSC_CONST_THREE_RGB 0x00000000

        WRITE_CMD( SB_CSC_CONST_ONE_RGB );
        WRITE_CMD( SB_CSC_CONST_TWO_RGB );
        WRITE_CMD( SB_CSC_CONST_THREE_RGB );

        #undef SB_CSC_CONST_ONE_RGB
        #undef SB_CSC_CONST_TWO_RGB
        #undef SB_CSC_CONST_THREE_RGB

        reg = NVRM_CH_OPCODE_MASK(G2SB_G2SRCSIZE_0,
                (NV2D_MASK(G2SRCSIZE, G2SRCSIZE) |
                NV2D_MASK(G2DSTSIZE, G2SRCSIZE)));
        WRITE_CMD( reg );

        reg = NV_DRF_NUM( G2SB, G2SRCSIZE, SRCWIDTH, 1 )
            | NV_DRF_NUM( G2SB, G2SRCSIZE, SRCHEIGHT, 0 );
        WRITE_CMD( reg );

        reg = NV_DRF_NUM( G2SB, G2DSTSIZE, DSTWIDTH, 1 )
            | NV_DRF_NUM( G2SB, G2DSTSIZE, DSTHEIGHT, 0 );
        WRITE_CMD( reg );

        reg = NVRM_CH_OPCODE_RELEASE_MUTEX( mutexid );
        WRITE_CMD( reg );

        reg = NVRM_CH_OPCODE_NONINCR( NVRM_INCR_SYNCPT_0, 1 );
        WRITE_CMD( reg );

        reg = NV_DRF_DEF( NVRM, INCR_SYNCPT, COND, OP_DONE )
            | NV_DRF_NUM( NVRM, INCR_SYNCPT, INDX,
                            NVRM_SYNCPOINT_2D_TINYBLT_WAR );
        WRITE_CMD( reg );

        if( !g_b2dInitialized )
        {
            /* make sure the base channel is ok */
            hChannel = &g_channelState.Channels[g_channelState.BaseChannel];
            NV_CHECK_ERROR_CLEANUP(
                NvRmPrivAllocateChannelResources( hChannel )
            );
        }
        #undef WRITE_CMD
        #undef NV2D_MASK

        g_b2dInitialized = NV_TRUE;
    }

    if( bSubmit )
    {
        hChannel = &g_channelState.Channels[g_channelState.BaseChannel];
        buf.hMem = g_h2dCommands;
        buf.Offset = 0;
        buf.Words = NUM_2D_TINYBLT_WAR_COMMANDS;

        NvRmPrivIncrSyncPointShadow( hRm, NVRM_SYNCPOINT_2D_TINYBLT_WAR, 1, NULL, &val );
        NvOsMutexLock(hChannel->HwState->submit_mutex);
        NvRmPrivChannelSubmitCommandBufs( hChannel, &buf, 1,
            NULL, NULL, 0,
            NVRM_SYNCPOINT_2D_TINYBLT_WAR, val, NV_FALSE );
...
fail:
    NvRmPrivCleanup2d( hRm );
    return NV_FALSE;
}

static void
NvRmPrivCleanup2d( NvRmDeviceHandle hDevice )
{
    NvRmMemHandleFree( g_h2dCommands );
    NvRmMemUnpin( g_h2dSurf );
    NvRmMemHandleFree( g_h2dSurf );
    g_h2dCommands = NULL;
    g_h2dSurf = NULL;
    g_b2dInitialized = NV_FALSE;
}

static NvBool
NvRmPriv3dModuleEnable( NvRmDeviceHandle hRm,
    struct NvRmAutoClockManagerEntryRec *entry, NvBool enable )
{
    NvError e;
    NvU32 SyncPointID;
    ...
    if( enable )
    {
        ...
        freq = NvRmFreqMaximum;
        NV_CHECK_ERROR_CLEANUP(
            NvRmPowerModuleClockConfig( hRm, entry->ModuleID,
                entry->PowerClientID, NvRmFreqUnspecified, NvRmFreqMaximum,
                &freq, 1, &freq, 0 )
        );
    }
    else
    {
        /* save the 3d context */
        NV_CHECK_ERROR_CLEANUP(
            NvRmPrivPrepareForPowerdown( entry->hChannel, entry->ModuleID,
                &SyncPointID, &SyncPointValue )
        );

        signaled = NvRmChPrivSyncPointSignalSemaphore( hRm, SyncPointID,
            SyncPointValue, s_Acm.ModuleSem );
        if( !signaled )
        {
            NvOsSemaphoreWait( s_Acm.ModuleSem );
        }

        /* need to poke this to restart the engine */
    ...
    NvRmAutoClockManagerEntry *entry )
{
    ...
    /* special case 3d */
    if( NVRM_MODULE_ID_MODULE(entry->ModuleID) == NvRmModuleID_3D )
    {
        /* cancel the memory system frequency bump */
        NV_CHECK_ERROR_CLEANUP(
            NvRmPowerBusyHint( hRm, NvRmDfsClockId_Emc,
                entry->PowerClientID, 0, 0 )
        );
    }
...
NvRmChPrivPowerdown( NvRmDeviceHandle hRm, NvBool force, NvU32 *pSleeptime )
{
    ...
    if (s_Acm.ModulesBusy)
    {
        ...
        // Keep host1x up if vi/isp are busy.
        if (isNoLongerBusy &&
            (((g_channelState.Channels[NVRM_CHANNEL_VI].HwState)->refcount ) ||
             ((g_channelState.Channels[NVRM_CHANNEL_ISP].HwState)->refcount) ))
            isNoLongerBusy = NV_FALSE;
        ...
        else
        {
            ...
            // Update 2D/EPP speed, provided 2D is still enabled
            if( s_Acm.Entries[NVRM_AUTO_CLOCK_MODULE_2D].bClockEnabled )
            {
                NvRmChPrivConfigure2dSpeed(hRm);
            }
        }
    }
...
NvRmPrivAcmInit( NvRmDeviceHandle hRm )
{
    ...
    if (!s_Acm.Thread)
    {
        ...
        entry = &s_Acm.Entries[NVRM_AUTO_CLOCK_MODULE_2D];
        entry->ModuleID = NvRmModuleID_2D;
        entry->SubModules[0] = NvRmModuleID_Epp;
        entry->ModuleFunc = NvRmPriv2dModuleEnable;

        entry = &s_Acm.Entries[NVRM_AUTO_CLOCK_MODULE_3D];
        entry->ModuleID = NvRmModuleID_3D;
        entry->ModuleFunc = NvRmPriv3dModuleEnable;

        entry = &s_Acm.Entries[NVRM_AUTO_CLOCK_MODULE_MPE];
        entry->ModuleID = NvRmModuleID_Mpe;
    ...
    NvRmModuleID ModuleID, NvU32 SyncPointID, NvU32 SyncPointValue )
{
    ...
    else
    {
        ...
        if( !entry->bClockEnabled )
        {
            ...
            if( mod == NvRmModuleID_3D )
            {
                /* need to bump the memory system frequency for perf and
                 * indirect request for core voltage increase; unlike 2d,
                 * bump is persistent until 3d is disabled
                 */
                NV_CHECK_ERROR_CLEANUP(
                    NvRmPowerBusyHint( hRm, NvRmDfsClockId_Emc,
                        entry->PowerClientID, NV_WAIT_INFINITE,
                        NvRmFreqMaximum )
                );
            }
    ...
    if( NullKickoff )
    {
        ...
        while( used-- )
        {
            ...
            reg = NV_DRF_NUM( HOST1X_SYNC, SYNCPT_CPU_INCR,
                SYNCPT_CPU_INCR_VECTOR, (1 << SyncPointID) );
...
NvError
NvRmChannelRead3DRegister(NvRmChannelHandle hChannel,
    NvU32 Offset,
    NvU32* Value)
{
    return NvError_NotSupported;
    ...
    NvRmContextHandle *phContext )
{
    ...
    if( Module == NvRmModuleID_3D )
        ret = NvRmContext3DAlloc( hDevice, phContext );
    else if( Module == NvRmModuleID_Mpe )
        ret = NvRmContextMpeAlloc( hDevice, phContext );
    else
        return NvError_NotImplemented;

    if( ret == NvSuccess && *phContext == NVRM_FAKE_POWERDOWN_CONTEXT )
    {
        // There's pretty much no chance malloc would return -1, but we'll
        // handle it just in case: simply call this function again and then
        // free the first context we got.  If we get an error, touch luck.
        NvRmContextHandle c2;
        ret = NvRmContextAlloc( hDevice, Module, &c2 );
        NV_ASSERT(c2 != NVRM_FAKE_POWERDOWN_CONTEXT );
        NvRmContextFree( *phContext );
        *phContext = c2;
    }
...
NvRmContextFree(NvRmContextHandle hContext)
{
    ...
    // Acm thread likes to poke contexts.  Respect its mutex.
    NvOsMutexLock(s_Acm.Mutex);

    ch = hContext->hChannel;
    // Don't save and restore this context anymore.
    if( ch )
        NVRM_CHANNEL_CONTEXT_RESET( ch, hContext );

    // This needs access to gs_IsrHwContextMutex from ap15rm_hwcontext.c.
    NvRmPrivContextFree( hContext );
...
NvRmPrivResetSyncpt( NvRmDeviceHandle hDevice, NvU32 id )
{
    ...
    if( id == NVRM_SYNCPOINT_3D )
    {
        HOST1X_SYNC_REGW( HOST1X_SYNC_SYNCPT_BASE_0_0 +
            (NVRM_WAITBASE_3D * 4), 0 );
    }
}
...
static void
NvRmPrivResetWaitBase( NvRmDeviceHandle hDevice, NvU32 id )
{
    HOST1X_SYNC_REGW( HOST1X_SYNC_SYNCPT_BASE_0_0 + (id * 4), 0 );
}

static void
NvRmPrivResetModuleWaitBase( NvRmDeviceHandle hDevice )
{
    NvU32 i;

    for( i = 0; i < NV_HOST1X_SYNCPT_NB_BASES; i++ )
    {
        if( (1UL << i) & NVRM_WAITBASE_RESERVED )
        {
            NvRmPrivResetWaitBase( hDevice, i );
        }
    }
}
    ...
    NvU32 Index )
{
    ...
    switch( mod ) {
    case NvRmModuleID_2D:
        switch( hChannel->HwState->num ) {
        case NVRM_CHANNEL_3D:
            return NVRM_WAITBASE_2D_0;
        case NVRM_CHANNEL_2D:
            return NVRM_WAITBASE_2D_1;
        default:
            NV_ASSERT( !"bad 2d wait base channel" );
            return 0;
        }
    case NvRmModuleID_3D:
        NV_ASSERT( Index == 0 );
        return NVRM_WAITBASE_3D;
    case NvRmModuleID_Mpe:
        NV_ASSERT( Index == 0 );
        return NVRM_WAITBASE_MPE;
    default:
        return 0;
    }
}
    ...
    NvU32 *pSyncPointID )
{
    ...
    switch( mod ) {
    case NvRmModuleID_2D:
        // 2D can be either on NVRM_CHANNEL_3D, if it is mixed with 3d.  Or it
        // can be on NVRM_CHANNEL_2D, if it is not mixed with 3d.
        if (hChannel->HwState->num == NVRM_CHANNEL_3D)
        {
            if( SyncPointIndex )
            {
                *pSyncPointID = NVRM_SYNCPOINT_2D_CHANNEL1_1;
            }
            else
            {
                *pSyncPointID = NVRM_SYNCPOINT_2D_CHANNEL1_0;
            }
        }
        else if (hChannel->HwState->num == NVRM_CHANNEL_2D)
        {
            if( SyncPointIndex )
            {
                *pSyncPointID = NVRM_SYNCPOINT_2D_CHANNEL2_1;
            }
            else
            {
                *pSyncPointID = NVRM_SYNCPOINT_2D_CHANNEL2_0;
            }
        }
        else
        {
            NV_ASSERT(!"2D is not supposed to be running on this channel");
        }
        break;
    case NvRmModuleID_3D:
        *pSyncPointID = NVRM_SYNCPOINT_3D;
        break;

    case NvRmModuleID_Vi:
        if (SyncPointIndex <=
            (NVRM_SYNCPOINT_VI_ISP_4 - NVRM_SYNCPOINT_CSI_VI_0))
        {
            *pSyncPointID = NVRM_SYNCPOINT_CSI_VI_0 + SyncPointIndex;
        }
        break;
    ...
    case NvRmModuleID_Mpe:
        if (SyncPointIndex == 0)
        {
            *pSyncPointID = NVRM_SYNCPOINT_MPE;
        }
        if (SyncPointIndex == 1)
        {
            *pSyncPointID = NVRM_SYNCPOINT_MPE_EBM_EOF;
        }
        else if (SyncPointIndex == 2)
        {
            *pSyncPointID = NVRM_SYNCPOINT_MPE_WR_SAFE;
        }
        break;
    ...
    NvU32 Index)
{
    ...
    case NvRmModuleID_Vi:
        NV_ASSERT(Index == 0);
        return NVRM_MODULEMUTEX_VI;

    case NvRmModuleID_2D:
        NV_ASSERT(Index == 0 || Index == 1 || Index == 2 || Index == 3);
        switch (Index)
        {
            case 0:
                return NVRM_MODULEMUTEX_2D_FULL;
            case 1:
                return NVRM_MODULEMUTEX_2D_SIMPLE;
            case 2:
                return NVRM_MODULEMUTEX_2D_SB_A;
            case 3:
                return NVRM_MODULEMUTEX_2D_SB_B;
            default:
                NV_ASSERT(NV_FALSE);
                return 0;
        }
    ...
    const NvU32 *offsets, NvU32 *values)
{
    ...
    switch( mod ) {
    case NvRmModuleID_2D:
        ModuleId = NV_HOST1X_MODULEID_GR2D;
        break;
    case NvRmModuleID_3D:
        ModuleId = NV_HOST1X_MODULEID_GR3D;
        break;
    case NvRmModuleID_Display:
    ...
    // for 2d and 3d we need to use the indirect read path, but 2d isn't even
    // allowed to call this functions (see assert above)
...
{
    // 1. Check if VI channel is active
    // 2. Check if any pending sync point semaphore
    // 3. Save states.  Sync points and mlocks.
    // 4. Disable Graphics Host clock
    ...
    NV_ASSERT(g_channelState.Channels[NVRM_CHANNEL_VI].HwState->refcount == 0);
    NV_ASSERT(g_channelState.Channels[NVRM_CHANNEL_ISP].HwState->refcount == 0);
    ...
    /* 2d will have to be re-initialized */
    g_b2dInitialized = NV_FALSE;
...
NvRmChPrivRestoreSyncPoints( NvRmDeviceHandle hDevice )
{
    ...
    NvU32               ChannelClassReg;
    NvU32               ClassId;
    const NvU32         NUM_RESTORE_CLASSID_CMDS = 1;
    NvRmCommandBuffer   buf;
    NvRmChannelHandle   hChannel;
    NvU32               TimeOutInMicroSec = 1000000;
    ...
    if ( g_hRestoreClassIdCommands == NULL)
    {
        NV_CHECK_ERROR_CLEANUP( NvRmMemHandleCreate( hDevice,
            &g_hRestoreClassIdCommands,
            (NV_HOST1X_CHANNELS * NUM_RESTORE_CLASSID_CMDS * 4) ) );
        NV_CHECK_ERROR_CLEANUP( NvRmMemAllocTagged( g_hRestoreClassIdCommands,
            NULL, 0, 32, NvOsMemAttribute_Uncached, NVRM_MEM_TAG_RM_MISC ));
    }

    for (idx = 0; idx < g_channelState.NumChannels; idx++)
    {
        hChannel = &g_channelState.Channels[idx];
        if ( (hChannel->HwState->refcount != 0) &&
              (s_SyncPointStates.ClassId[idx] != 0) )
        {
            buf.hMem = g_hRestoreClassIdCommands;
            buf.Words = NUM_RESTORE_CLASSID_CMDS;
            buf.Offset = NUM_RESTORE_CLASSID_CMDS * idx * 4;

            // Set Class id.
            reg = NVRM_CH_OPCODE_SET_CLASS(s_SyncPointStates.ClassId[idx],0,0);
            NvRmMemWr32( g_hRestoreClassIdCommands, buf.Offset, reg );

            NvRmPrivIncrSyncPointShadow( hDevice,
                NVRM_SYNCPOINT_2D_TINYBLT_RESTORE_CLASS_ID, 0, NULL, &val );
            // SubmitCommandBufs uses 'buf' to get Offset, hMem and Words.
            // So, it can be from stack.
            NvOsMutexLock(hChannel->HwState->submit_mutex);
            NvRmPrivChannelSubmitCommandBufs( hChannel, &buf, 1,
                NULL, NULL, 0,
                NVRM_SYNCPOINT_2D_TINYBLT_RESTORE_CLASS_ID, val, NV_FALSE );
            NvOsMutexUnlock(hChannel->HwState->submit_mutex);
            /*
             * Need to ensure that Class id restore operation is complete
             * before exiting this method. Host clock is shut off after exiting
             * this method. If this operation is not complete and if suspend
             * is called immediately, then the class id read during save
             * operation can be wrong.
             * Ideally, It shoud be done in no time. But it waits for 1sec max
            ...
            do
            {
                ...
                NvOsWaitUS(1);
                TimeOutInMicroSec--;
            } while ( (ClassId != s_SyncPointStates.ClassId[idx]) && TimeOutInMicroSec );
            NV_ASSERT(TimeOutInMicroSec);
        }
    }
    return;
fail:
...
static void
NvRmChPrivConfigure2dSpeed(NvRmDeviceHandle hDevice)
{
    NvRmFreqKHz TargetKHz = NvRmFreqMaximum;    // maximum at RM discretion

    NV_ASSERT_SUCCESS(NvRmPowerModuleClockConfig(
        hDevice, NvRmModuleID_2D, 0,
        NvRmFreqUnspecified, NvRmFreqUnspecified, &TargetKHz, 1, NULL, 0));
    NV_ASSERT_SUCCESS(NvRmPowerModuleClockConfig(
        hDevice, NvRmModuleID_Epp, 0,
        NvRmFreqUnspecified, NvRmFreqUnspecified, &TargetKHz, 1, NULL, 0));
}

// AVP never use indirect access.
#if NV_IS_AVP
#  define USE_IND_ACCESS 0
#else
#  define USE_IND_ACCESS 1
#endif
    ...
    const NvU32 *offsets, NvU32 *values )
{
    ...
    /* only 3D, display, isp, vi and encoder are allowed to read registers */
    NV_ASSERT( mod == NvRmModuleID_Display  ||
        mod == NvRmModuleID_Isp ||
        mod == NvRmModuleID_Mpe ||
        mod == NvRmModuleID_3D ||
        mod == NvRmModuleID_Vi ||
    ...
    if (mod == NvRmModuleID_3D ||
        mod == NvRmModuleID_2D)
    {
        NvRmChModuleRegRd(device, id, num, offsets, values);
    }
    else
#endif
    {
    ...
    const NvU32 *offsets, const NvU32 *values )
{
    ...
    if (mod == NvRmModuleID_3D ||
        mod == NvRmModuleID_2D)
    {
        NvRmChModuleRegWr( device, id, num, offsets, values );
    }
    else
#endif
    {
