...
#include "ap20rm_hwcontext_reginfo.h"
...
typedef struct NvRmContextRec {
    ...
    // The list of registers to store and restore.
    const NvRm3DContext *ContextRegisters;
    const NvRmMpeContext *ContextRegistersMpe;
...
/*
 * Allocates a hardware context for the 3D unit.  The first call to it also
 * allocates the static block of Host Read commands.
 */
NvError
NvRmContext3DAlloc( NvRmDeviceHandle hDevice,
    NvRmContextHandle *phContext );

/*
 * Allocates a hardware context for the MPE unit.  The first call to it also
 * allocates the static block of Host Read commands.
 */
NvError
NvRmContextMpeAlloc( NvRmDeviceHandle hDevice,
    NvRmContextHandle *phContext );

/*
 * Frees memory associated with the 3D context.  Freeing the last context
 * should also free Host Read commands block, but this is not done yet.  It's
 * a TODO: item.
 */
void
NvRmContext3DFree( NvRmContextHandle hContext );

/*
 * Frees memory associated with the MPE context.  Freeing the last context
 * should also free Host Read commands block, but this is not done yet.  It's
 * a TODO: item.
 */
void
NvRmContextMpeFree( NvRmContextHandle hContext );
