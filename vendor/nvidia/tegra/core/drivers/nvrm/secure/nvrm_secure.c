...
static ChipId NvRmPrivSecureGetChipId( void )
{
    ...
    rval1 = NvRmPrivSecureReadIntFromFile("/sys/module/fuse/parameters/tegra_chip_id",  &chipId.Id);
    rval2 = NvRmPrivSecureReadIntFromFile("/sys/module/fuse/parameters/tegra_chip_rev", &chipId.Revision);
...
NvError NvRmCheckValidSecureState( NvRmDeviceHandle hDevice )
{
    ...
    if (NvRmPrivSecureReadIntFromFile("/sys/firmware/fuse/odm_production_mode", &OdmProdMode) != NvSuccess)
    {
