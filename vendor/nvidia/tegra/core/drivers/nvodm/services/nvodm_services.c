...
#ifndef SET_KERNEL_PINMUX
            // Sets  pinmux and removes the tristate on the pin & enable clock.
            NvRmExternalClockConfig(hRmDev,
                    NvOdmIoModule_ExternalClock, Instance, pOdmConfigs[Instance], EnableTristate);
#endif
            result = NV_TRUE;
        }
        else
...
NvOdmServicesOwrHandle NvOdmOwrOpen(NvU32 instance)
{
    ...
    NvOdmQueryPinMux(NvOdmIoModule_OneWire, &pOdmConfigs, &NumOdmConfigs);
    if (instance>=NumOdmConfigs || !pOdmConfigs[instance] ||
        (pOdmConfigs[instance]==NvOdmOwrPinMap_Multiplexed))
        return NULL;
#endif
...
NvBool NvOdmServicesFuseGet(NvOdmServiceFuseType FuseType, NvU32 *Value)
{
    ...
    if (Value)
    {
        switch (FuseType)
        {
            case NvOdmServiceFuseType_SKU:
            {
                NvDdkFuseGet(NvDdkFuseDataType_Sku, Value, &size);
                return NV_TRUE;
            }
            break;
            case NvOdmServiceFuseType_PackageInfo:
            {
                NvDdkFuseGet(NvDdkFuseDataType_PackageInfo, Value, &size);
                return NV_TRUE;
            }
            break;
            default:
