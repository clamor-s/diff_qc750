...
static void SPI1MuxConfigure(NvU32 SpiPinMap)
{
    ...
    switch (SpiPinMap)
    {
        case NvOdmSpiPinMap_Config1:
            TRISTATE_SLINK(ULPI_STP);
            TRISTATE_SLINK(ULPI_NXT);
            TRISTATE_SLINK(ULPI_DIR);
            TRISTATE_SLINK(ULPI_CLK);
            TRISTATE_SLINK(SPI1_CS0_N);
            TRISTATE_SLINK(SPI1_SCK);
            TRISTATE_SLINK(SPI1_MISO);
            TRISTATE_SLINK(SPI1_MOSI);
            PINMUX_SLINK(ULPI_STP,1);
            PINMUX_SLINK(ULPI_NXT,1);
            PINMUX_SLINK(ULPI_DIR,1);
            PINMUX_SLINK(ULPI_CLK,1);
            PINMUX_SLINK(SPI1_CS0_N,1);
            PINMUX_SLINK(SPI1_SCK,1);
            PINMUX_SLINK(SPI1_MISO,1);
            PINMUX_SLINK(SPI1_MOSI,1);
            break;
