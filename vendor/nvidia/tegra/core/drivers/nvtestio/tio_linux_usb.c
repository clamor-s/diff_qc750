...
static NvError OpenUsb(int *fd)
{
    ...
    usbh->timeout = 500;
    *fd = (int)usbh;
#if 0 // in RCM mode, following needs to be done
    e = NvUsbDeviceReadDevId(usbh, &devId);
    if (e == NvSuccess)
    {
        devId = devId & 0x00FF;
        NvOsDebugPrintf("the device ID is 0x%x\n", devId);
        // read UID
        e = NvUsbDeviceRead(usbh, &uid, sizeof(uid), &len);
        if (e == NvSuccess)
        {
            NvOsDebugPrintf("the UID: 0x%x\n", uid);
        }
    }
#endif
    return e;
                                  ...
                                  NvTioStreamHandle stream )
{
    ...
    if (gs_IsUsbOpen)
    {
        NvOsDebugPrintf("NvError_AlreadyAllocated\n");
...
{
    NvError e = NvSuccess;
    //NvUsbDevice *hDev = (NvUsbDevice *)tioDesc->stream->f.fd;
