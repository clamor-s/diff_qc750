...
// number of cycles since the last arbitration bank winner was the requested bank
// has reached the threshold.
// The non same-page bank counter threshold is used to block non same-page requests
// until the number of cycles since the last arbitration bank winner was the requested
// bank and it changed the same page, has reached the threshold.
...
// the requests sent to the external memory controller.
...
// - a high-priority bit
// - a time-out priority bit
// - a bandwidth share bit
// - two fixed priority bits
...
//   Value of 0 disables.
//   Value of 4 approximates (in certain situations) the behavior of earlier 
...
//   csw_ispw             46       isp
//   csw_mpecswr          48       mpec
//   csw_ppcsahbdmaw      49       ppcs
//   csw_ppcsahbslvw      50       ppcs
//   csw_ppcsarm7w        51       ppcs
//   csw_vdebseaw         52       vde
//   csw_vdebsevw         53       vde
//   csw_vdembew          54       vde
//   csw_vdetpmw          55       vde
...
// This register controls the behavior of the client data paths and
// the memory return data paths.
// When 'SINGLE', at most one (return) data path is active at any time.
// When 'MULTI', two or more (return) data paths can be active at the
// same time.
//
...
// Time-out clock counter scale factors. Reset value is 0.
...
// Out of bound address from an AXI client to memory status Register.
// If a out of bound address from AXI client to memory is detected, this register records information
// about this access: read/write indicator, AXI request ID, AXI length, AXI size, AXI burst type,
// AXI cache bits, AXI protection bits, AXI lock bits. The values remain unchanged until the interrupt is cleared.
//
// The MC AXI request ID is not the same as the mpcore AXI request ID.
// The ID seen by MC is generated by mselect and defined as follows:
//     5  reserved, always 0.
//     4  reserved, always 0.
//   3:2  identifier added by mselect for read requests ordering.
//   1:0  copy of mpcore AXI request ID [3:2].
//
// The global memory super-client IDs are defined as follows:
//
//   SUPER-CLIENT         ID
//   ------------         --
//   cmc                  0       
...
// DECERR Address from AXI client
// If a out of bound address from AXI Clients to memory is detected, this register records the request AXI byte address.
...
// - Poll the <CLIENT>_OUTREQCNT register till zero.
...
// CMC Outstanding Request Register. Read-only.
...
// Fixed-priority Register for cmc clients. Reset value is lowest.
...
// Fixed-priority Register for dcb clients. Reset value is lowest.
...
// Fixed-priority Register for mpcore clients. Reset value is lowest.
...
// Fixed-priority Register for mpea clients. Reset value is lowest.
...
// Fixed-priority Register for mpeb clients. Reset value is lowest.
...
// Fixed-priority Register for mpec clients. Reset value is lowest.
...
// Fixed-priority Register for ppcs clients. Reset value is lowest.
...
// Fixed-priority Register for vde clients. Reset value is lowest.
...
// Request time-out Register for cmc clients. Reset value is 0. 
...
// client.                                                          
// The client fifo depth is NV_<CLIENT>2MC_RFIFO_MAXCREDITS         
...
// Request time-out Register for dcb clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Request time-out Register for mpcore clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Request time-out Register for mpea clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Request time-out Register for mpeb clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Request time-out Register for mpec clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Request time-out Register for nv clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Request time-out Register for ppcs clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Request time-out Register for vde clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Request time-out Register for vi clients. Reset value is 0. 
// If not zero, the client request time-out counter is reloaded     
// with this value when it reaches zero. It is decremented when     
// there is a pending request and the time-out clock counter for    
// the request destination (internal/external memory) reaches zero. 
...
// Read Coalescing time-out register for cmc read clients. Reset value is 0 (disabled).
...
// Read Coalescing time-out register for dc read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for dcb read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for epp read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for g2 read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for hc read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for mpcore read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for mpea read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for mpeb read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for mpec read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for nv read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for ppcs read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for vde read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Read Coalescing time-out register for vi read clients. Reset value is 0 (disabled).
// Read coalescing happens inside the memory controller, right before arbitration.
// Coalescing means two (NV_MC_MW/2)-bit requests are grouped together in one NV_MC_WM-bit request.
// The register value indicates how many cycles a first write request is going to wait
// for a subsequent one for possible coalescing. The coalescing can only happen
...
// Bandwidth share register for CMC clients. Reset value is 0.
...
// A value of zero disables the bandwidth share for all CMC clients, and reset
...
// Bandwidth share register for DISPLAY clients. Reset value is 0.
// The values reflect the bandwidth share for EMEM.
// INC_VAL specifies the number of bytes the Bandwidth credit has to be incremented.
// A value of zero disables the bandwidth share for all DISPLAY clients, and reset
// the accumulated credit.
// HIGH_TH specifies the high threshold[11:4].
// If accumulated credit >= high threshold, BW bit is set.
// BW bit is reset when accumulated credit is zero.
...
// Bandwidth share register for DISPLAYB clients. Reset value is 0.
// The values reflect the bandwidth share for EMEM.
// INC_VAL specifies the number of bytes the Bandwidth credit has to be incremented.
// A value of zero disables the bandwidth share for all DISPLAYB clients, and reset
// the accumulated credit.
// HIGH_TH specifies the high threshold[11:4].
// If accumulated credit >= high threshold, BW bit is set.
// BW bit is reset when accumulated credit is zero.
...
// Bandwidth share register for FDC clients. Reset value is 0.
// The values reflect the bandwidth share for EMEM.
// INC_VAL specifies the number of bytes the Bandwidth credit has to be incremented.
// A value of zero disables the bandwidth share for all FDC clients, and reset
// the accumulated credit.
// HIGH_TH specifies the high threshold[11:4].
// If accumulated credit >= high threshold, BW bit is set.
// BW bit is reset when accumulated credit is zero.
...
// Bandwidth share register for GR2D clients. Reset value is 0.
// The values reflect the bandwidth share for EMEM.
// INC_VAL specifies the number of bytes the Bandwidth credit has to be incremented.
// A value of zero disables the bandwidth share for all GR2D clients, and reset
// the accumulated credit.
// HIGH_TH specifies the high threshold[11:4].
// If accumulated credit >= high threshold, BW bit is set.
// BW bit is reset when accumulated credit is zero.
...
// Bandwidth share register for HOST1X clients. Reset value is 0.
// The values reflect the bandwidth share for EMEM.
// INC_VAL specifies the number of bytes the Bandwidth credit has to be incremented.
// A value of zero disables the bandwidth share for all HOST1X clients, and reset
// the accumulated credit.
// HIGH_TH specifies the high threshold[11:4].
// If accumulated credit >= high threshold, BW bit is set.
// BW bit is reset when accumulated credit is zero.
...
// Bandwidth share register for IDX clients. Reset value is 0.
// The values reflect the bandwidth share for EMEM.
// INC_VAL specifies the number of bytes the Bandwidth credit has to be incremented.
// A value of zero disables the bandwidth share for all IDX clients, and reset
// the accumulated credit.
// HIGH_TH specifies the high threshold[11:4].
// If accumulated credit >= high threshold, BW bit is set.
// BW bit is reset when accumulated credit is zero.
...
// Bandwidth share register for TEX clients. Reset value is 0.
// The values reflect the bandwidth share for EMEM.
// INC_VAL specifies the number of bytes the Bandwidth credit has to be incremented.
// A value of zero disables the bandwidth share for all TEX clients, and reset
// the accumulated credit.
// HIGH_TH specifies the high threshold[11:4].
// If accumulated credit >= high threshold, BW bit is set.
// BW bit is reset when accumulated credit is zero.
