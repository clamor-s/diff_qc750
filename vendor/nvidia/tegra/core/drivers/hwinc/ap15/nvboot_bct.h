 ...
 *   from NVIDIA. In this mode, fuses can still be programmed via recovery mode.
 *   BCTs and BLs are signed with a key of all 0's, but not encrypted.
 ...
 *   all BCTs and BLs must be signed and encrypted with the secure boot key (SBK).
 * 
 ...
 *
 * To be used by the BR, the BCT's CryptoHash must match the hash value computed
 * while reading the BCT from the secondary boot device.
 *
 * For secondary boot devices that do not naturally divide storage into pages and
 * blocks, suitable values have been chosen to provide a consistent model for BCT
 * and BL loading. For eMMC devices, the page size is fixed at 512 bytes and the
...
typedef struct NvBootConfigTableRec
{
    ...
    ///    to read this value at runtime and use it for various useful features. 
    ///    For example: use of single BSP image that supports multiple product SKUs.
    NvU8                CustomerData[NVBOOT_BCT_CUSTOMER_DATA_SIZE];

    /// Specifies a reserved area at the end of the BCT that must be filled with
    /// the padding pattern.
