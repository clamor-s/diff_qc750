 ...
 *    with new parts and with those for which security is an issue.  In the
 *    OdmSecure mode, the messages must be signed with the SBK.
 ...
 *     QueryBootDataVersion: Returns the 32-bit BR data structure version number
 *     ProgramFuses: Programs the customer-programmable fuses with the
 *         supplied data.  Requires that the external programming voltage has
 *         been applied before sending the ProgramFuses message.  Upon
 *         receiving the reply from RCM, the programming voltage must be
 *         removed.  Note that the new fuse values will not affect BR operation
 ...
 *     VerifyFuses: Compares the customer-programmable fuses with the
 *         supplied data.
 ...
 * for a DownloadExecute command follows the NvBootRcmMsg structure.  Any
 * message less than the minimum message size must be padded to a multiple of
 * 16 bytes and to be at least the minimum size.  Padding bytes shall be
 ...
 * 4. RcmVersion must match the BR's RCM protocol version.
 ...
 *    b. ProgramFuses and VerifyFuses commands:
 *         i. The operating mode must be NvProduction.
 *        ii. PayloadLength must be 0.
 ...
 */
#define NVBOOT_RCM_MAX_MSG_LENGTH (96*1024 + sizeof(NvBootRcmMsg))

/**
 * Defines the starting address for downloaded applets.
 */
#define NVBOOT_RCM_DLOAD_BASE_ADDRESS 0x40008000 /* 2nd bank of iRAM */
 ...
 * TODO: Resolve the implementation choices.
 */
typedef enum
{
    ...
    /// Specifies that the fuse verification operation failed due to a mismatch.
...
typedef struct NvBootRcmFuseDataRec
{
    ...
    NvU8                 SecureBootKey[NVBOOT_SECURE_BOOT_KEY_BYTES];
...
typedef struct NvBootRcmMsgRec
{
    ...
    NvU8            Padding[NVBOOT_RCM_MSG_PADDING_LENGTH];
                               /* Pushes size of encrypted/hashed header
                                * to next multiple of AES block size.
                                */
} NvBootRcmMsg;
