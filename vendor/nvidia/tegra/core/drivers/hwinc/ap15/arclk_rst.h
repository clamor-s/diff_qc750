...
// modules that are needed for system boot-up.  At POR, the appropriate bits in RST_DEVICES_L/H 
...
// Similarly for clocks, the appropriate bits in CLK_OUT_ENB_L/H rgisters are set automatically by
// hardware.  A "1" in the bit postion signify that block will have clock running during and after
...
// must be implementd by software precisely to ensure correct operation of the hardware.
//
// ***** Precaution *****:
// -----------------------
//      (A)  All modules support changing the clock divider ratio without disabling the clock.
//      (B)  All modules' clock switching is glitch free except "audio_sync_clk" and "mpe_audio_clk".
//      (C)  For "audio_sync_clk" and "mpe_audio_clk", the clock source select needs to be setup ahead of
//           time before changing the device clock source to use that audio clock or to enable the device 
...
//      (E)  Before stopping clock (via CLK_OUT_ENB_L/H registers) and/or asserting reset (via RST_DEVICES_L/H
...
//           ensure a module is not active, (a) disable the module by programming it's disable bit if not
//           already done so, and (b) wait until the module is not active by checking for it's busy bit, done
...
// --------------------------------------------------------------------------------------------------
//   (1)  Make sure the device's reset is asserted (via RST_DEVICES_L/H registers).
//   (2)  Enable clock to the device (via CLK_OUT_ENB_L/H registers).
...
//   (6)  Wait 2us to make sure clock source/device logic is stablize.
//   (7)  Deassert device's reset (via RST_DEVICES_L/H registers).
//
...
//             (2)  Assert device's reset (via RST_DEVICES_L/H registers).
//             (3)  Make sure clock to the device is enabled (via CLK_OUT_ENB_L/H registers).
...
//             (8)  Wait 2us to make sure clock source/device logic is stablize.
//             (7)  Deassert device's reset (via RST_DEVICES_L/H registers).
//   (B)  Method 2 -- (not using reset).
//             (1)  Depends on the max rated frequency of the device, and the current and target clock source/divider value,
...
//             (2)  Wait 2us to make sure device logic is stablize.
//
// To reset a device (without need to change clock) after boot-up:
// ---------------------------------------------------------------
//   (1)  Make sure the device's reset is asserted (via RST_DEVICES_L/H registers).
//   (2)  Wait 2us to make sure device logic is stablize.
//   (3)  Deassert device's reset (via RST_DEVICES_L/H registers).
//
...
//      (C)  PLL clocks.
//           (1)  "PLLC"(pu) which is general purpose and it's output is called "pllC_out0".
//           (2)  "PLLM"(emory) which is general purpose and it's output is called "pllM_out0".
//           (3)  "PLLP"(eripheral) which is fixed at 432MHz and it's output is called "pllP_out0".
//           (4)  "PLLA"(udio) which is cascaded from PLLP and is use for audio purposes.
//           (5)  "PLLU"(sb) which is fixed at 12MHz.
//           (6)  "PLLD"(si) which can go up-to 1GHz and it's output (after a fix /2) is called "pllD_out0".
...
//
//                **Note:  With the exception of PLLA, the other 5 PLLs are all using "osc" as reference clock.
//
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//  Each of the normal states can be selected by SW from 8 different clock sources.  Futhermore, if
...
//  from a PLL, SW needs to gurantee that the PLL clock is running and stable before changing clock
//  sources.
//
...
//            PLLs can be turned off to save power and super clock divider can futher divide down the
...
//  in AP15 so that it won't get into a FIFO overrun/underrun problem in either of the audio
...
//  VCLK/HCLK/PCLK.
//
//  SCLK is the main system clock of AP15 which can run up-to 150MHz.
...
//  VCLK is the video decoder clock which can run at n/16 of SCLK (n = 1 to 16).
///////////////////////////////////////////////////////////////////////////////////////////////////
...
// Oscillator control.
//
// Since for AP15, "osc" can have any of the four frequency (12MHz, 13MHz, 19.2MHz, 26MHz) coming
// in, the OSC_FREQ field provides a way for SW to tell HW what frequency is the incoming clock
// runs at.  This information is used by hardware to auto setup the parameters to PLLP and its
...
// at.  To determine the "osc" freuqency, user only needs to program the number of 32KHz 
...
//
//  As mentioned earlier, we have 6 PLLs controllable by clock control.  Please consult the PLL
//  spec. for electrical requirement.
//  (1) "PLLC" is of type CLKPLL700_LP. 
//  (2) "PLLM/PLLP/PLLA" are of type CLKPLL500_LP_B. 
//  (3) "PLLD/PLLU" are of type CLKPLL1G_MIPI. 
//
//  In general, there are 3 requirments for each PLL that SW needs to be comply with.
...
//
///////////////////////////////////////////////////////////////////////////////////////////////////
//                              CLKPLL500_LP_B          CLKPLL700_LP            CLKPLL1G_MIPI
// -----------------------------------------------------------------------------------------------
// Input freq. (REF)            2-31MHz                 2-31MHz                 2-40MHz
// Comparison freq. (CF)        1-6MHz                  1-6MHz                  1-6MHz
// VCO freq. (VCO)              20-1000MHz              20-1400MHz              40-1000MHz
//
//                      Charge pump current (CPCON) for CLKPLL500_LP_B/CLKPLL700_LP
//                      ===========================================================
...
//              <200                            x                       0.5uA (0001)
//
//                              Charge pump current (CPCON) for CLKPLL1G_MIPI
//                              =============================================
//              CPCON                           Description     
//              ---------------------------------------------------------------------------
//              0000                            Charge Pump 0uA.        
//              0001                            Charge Pump 1uA.        
//              0010                            Charge Pump 2uA, shen N-divider=50.     
//              0011                            Charge Pump 3uA.        
//              0100                            Charge Pump 4uA, shen N-divider=300.    
//                .                                     .
//              1000                            Charge Pump 8uA, shen N-divider=600.    
//                .                                     .
//              1100                            Charge Pump 12uA, shen N-divider=1000.  
//                .                                     .
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//  PLLU configuration information (reference clock is osc/clk_m and PLLU-FO is fixed at 12MHz).
//
//  reference frequency         13.0MHz         19.2MHz         12.0MHz         26.0MHz    
//  ---------------------------------------------------------------------------------------
//      DIVN                    384 (180h)       80 (050h)      384 (180h)      384 (180h)
//      DIVM                     13 ( 0dh)        4 ( 04h)       12 ( 0ch)       26 ( 1ah)
//      DIVP                     32 (  5h)       32 (  5h)       32 (  5h)       32 (  5h)
///////////////////////////////////////////////////////////////////////////////////////////////////
//  PLLD configuration information (reference clock is osc/clk_m and PLLU-FO is max of 1GHz).
//
//  reference frequency         13.0MHz         19.2MHz         12.0MHz         26.0MHz    
//  ---------------------------------------------------------------------------------------
...
//           themself and not by the CLK_SOURCE_<mod> registers.
