...
// This UART is based on the 16550 industry standard Universal Asynchronous Receiver/Transmitter (UART),
// with enhancements to support Autobaud detection and End-of-Received Data timeout detection.
// The UART supports a device clock of up to 72Mhz, for a maximum baud rate of 4.5Mbits/second.
//
...
// The IER and DLH registers occupy the same address space, selected by the LSR.DLAB bit
// The Interrupt Enable Register     (IER) is selected if the LSR.DLAB bit is clear
// The Divisor Latch MSByte register (DLM) is selected if the LSR.DLAB bit is set
// The DLM register holds the upper 8 bits of the 16-bit Baud Divisor (16x)
// UART  Interrupt Enable por=0x00000000
//
// RX_TIMEOUT occurs when data has been sitting in the Rx FIFO for more than 4 character times without being read
// because there is not enough data to reach the trigger level.  Interrupt needed to handle this case so that
// all data is received in a timely manner.   Note that this normally occurs at the end of an incoming data stream.
//
// EORD (End of Receive Data) Interrupt occurs when the receiver detects that data stops coming in for more than 4
// character times.   Useful for determining that the sending device has completed sending all it's data.
// EORD timeout will not occur if the receiving data stream is stopped because of hardware handshaking.
//
...
// The FCR and IIR registers occupy the same address space
// THe Fifo Control Register (FCR) is Write-Only 
// The Interrupt Identification Register (IIR) is Read-Only
// UART  FIFO Control Register por=0x00000000
// The DMA can run in one of two modes:
// If Mode0 is selected (NO_CHANGE) the Rx DMA request will go active whenever the Rx FIFO is not empty.  Only single
// byte transactions can be performed in this mode.  If the FIFO is not enabled, Mode0 MUST be used.
// If Mode1 is selected (   CHANGE) the Rx DMA request will go active whenever the Rx FIFO trigger level is reached.
// For best performance, always enable the FIFO and select DMA Mode1 
// UART  Interrupt ID Register por=0x00000001
// The Interrupt ID field indicates the current higest priority interrupt.
// If more than one interrupt is pending the one with the highest priority will be shown.
...
// UART Line Control Register por=0x00000000
...
// UART (Modem Control Register) por=0x00000000
// The RTS and CTS pins are used for hardware handshaking with and external serial device
...
// If RTS_EN is set, the RTS pin will automatically remove the Request-To-Send when the FIFO reaches the trigger
// level.
...
// UART  Line Status Register por=0x00000060
...
// UART  Modem Status Register por=0x00000000
...
// UART  Scratch Pad Register por=0x00000000
...
// UART  IrDA Pulse Coding CSR Register por=0x00000000
// bits [7:6] control the InfraRed (SIR) encoding.  
// If enabled each zero bit sent will be transmitted as a short IR pulse.  No pulse is sent during idle or '1' data bits.
// The IR pulse can be either 3/16 or 4/16 of a normal bit time.
...
// UART Auto Sense Baud Register por=0x00000000
// The UART has auto-Baud sensing logic which can measure the width of the start bit of incoming data to determine baud rate.
// For this to work the incoming character must have it's LSB set.   A <cr> works well (0x0d).
// The logic will use the UART device clock to count how wide the start pulse is and the BUSY bit will be set when the
// sensing is complete.  The value in {RX_RATE_SENSE_H,RX_RATE_SENSE_L} should be divided by 16 with Round-to-Nearest,
// and the resulting value loaded into the DLM,DLL register pair to set the Baud Clock to 16X the Baud Rate.
