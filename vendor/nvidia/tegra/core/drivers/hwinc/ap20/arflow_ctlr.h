...
// We use macros to insure correct replication of registers when needed
// CPU0 registers are at same offset as old CPU registers for backwards binary compatibility, but name changed
// Comments cannot be included in the macro, so they are placed in a common comment before the replicated registers
//
// The EVENTS register is replicated three times, once for each of COP, CPU0 and CPU1
// The fields and enums have the following interpretation
// MODE define how the flow controller logic operates, the reset value is 0x0, i.e. do nothing
// Enumerated values for the field MODE are defined below, please don't use the deprecated values in new code
...
// Deprecated but still defined 
//                 FLOW_MODE_STOP                      = 2,    // DO NOT USE. LEGACY NAME WILL BE REMOVED 
//                 FLOW_MODE_STOP_AND_INT              = 3,    // DO NOT USE. LEGACY NAME WILL BE REMOVED 
//                 FLOW_MODE_STOP_UNTIL_INT            = 4,    // DO NOT USE. LEGACY NAME WILL BE REMOVED 
//                 FLOW_MODE_STOP_UNTIL_INT_AND_INT    = 5,    // DO NOT USE. LEGACY NAME WILL BE REMOVED 
//                 FLOW_MODE_STOP_OR_INT               = 6     // DO NOT USE. LEGACY NAME WILL BE REMOVED 
...
// - counted events, they decrement the field called ZERO, the flow controller resumes when this counter reaches 0
...
//  uSEC            : Resume on Nth SEC clock ticks
//  MSEC            : Resume on Nth mSEC clock ticks
...
//  ZERO            : Initialized then decremented     Note: If more than one event is enabled, the event counter
...
// The CPU_CSR registers are replicated for CPU0 and CPU1.  They define additional characteristics of 
// the flow controller linked to CPU cores, especially the interaction of the flow controller with the
// power managemenet functions.  The LP1 state is normally entered and exited via side effects of the 
