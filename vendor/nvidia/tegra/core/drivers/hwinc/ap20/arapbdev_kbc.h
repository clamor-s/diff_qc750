...
// MAX_ENT should not be greater than MAX_COLS 
#define APBDEV_KBC_MAX_ENT      8
#define APBDEV_KBC_REG_WIDTH_BYTES      4
#define APBDEV_KBC_FIFO_DEPTH   10
/////////////////////////////////////////////////////////////////////////////////////////////
//
// A keyboard controller for keypad matrix completely in SW can be quite an overhead for it. 
// The three main operations which are done by KBC are:
// A) Keypad scan
// B) Debounce  
// C) Control SM
//
...
// -----------------------
//
// Keyboard controller is enabled when the enable bit is set & some pins are configured 
// for interfacing with keypad matrix. The pins which are not interfaced to keypad matrix can be used as GPIO pins. 
// To configure KBC, the following registers should be programmed APBDEV_KBC_ROW_CFG, APBDEV_KBC_INT, 
// APBDEV_KBC_COL_CFG and APBDEV_KBC_CONTROL. KBC support a keypad of maximum size 16 X 8 (row X column). 
// All the pins interfacing with keypad use debounce logic to detect a valid key press (filter out glitches). 
//
// To avoid an overflow in case of slow reads the first level warning indicator can be programmed 
// In FIFO_TH_CNT field of the APBDEV_KBC_CONTROL register
//
// In case of overflow, SW will have two options of treating the new entries. 
// Either drop them or overwrite them. This bit is FIFO_MODE in APBDEV_KBC_CONTROL register
//
/////////////////////////////////////////////////////////////////////////////////////////////
// KBC control register. This register is the main control register. It should be configured 
...
// KBC interrupt status & clear register.
...
// First row configuration register. This register setting will take precedence in case 
// same GPIO pin is configured in column register also. It does the mapping between GPIO pins &
// rows of keypad matrix. It configures for GPIO pin# 0 to 5. 
...
// Second row configuration register. This register setting will take precedence in case 
// same GPIO pin is configured in column register also. It does the mapping between GPIO pins &
// rows of keypad matrix. It configures for GPIO pin# 6 to 11. 
...
// Third row configuration register. This register setting will take precedence in case 
// same GPIO pin is configured in column register also. It does the mapping between GPIO pins &
// rows of keypad matrix. It configures for GPIO pin# 12 to 17. 
...
// Fourth row configuration register. This register setting will take precedence in case 
// same GPIO pin is configured in column register also. It does the mapping between GPIO pins &
// rows of keypad matrix. It configures for GPIO pin# 18 to 23. 
...
// First column configuration register. It does the mapping between GPIO pins &
// columns of keypad matrix. It configures for GPIO pin# 0 to 7. 
...
// Second column configuration register. It does the mapping between GPIO pins &
// columns of keypad matrix. It configures for GPIO pin# 8 to 15. 
...
// Third column configuration register. It does the mapping between GPIO pins &
// columns of keypad matrix. It configures for GPIO pin# 16 to 23. 
...
// In CP state, if the corresponding counter crosses this value KBC goes back to WuKP (Wake-up on Key-press
// state). 
...
// This regsiter holds the initial delay for scan when KBC goes from WuKP mode to CP mode. 
...
// This regsiter holds the repeat scan delay. This is the delay between two successive scans in CP mode. 
...
// First key-press entries register. This register holds the key numbers of the keys which have been pressed. 
// It holds the first four entries
// NOTE: Both the entry registers should be read consecutively. A read of first register changes the 
// FIFO read pointer
...
// Second key-press entries register. This register holds the key numbers of the keys which have been pressed. 
