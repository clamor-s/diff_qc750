...
//  EXCEPTION VECTOR PROCESSOR
//
// Consists of the following sets of registers 
// 1) Exception Vectors (read only, each processor sees it's own set)
// 2) Status Registers  (read only, each processor sees it's own set)
// 3) Priority Vectors (NUM/Vectors pairs) (read only, each processor sees it's own set)
// 4) CPU specific registers for 1,2,3 above (read/write)
// 5) COP specific registers for 1,2,3 above (read/write)
//
// Up to 8 prioritized IRQs can be individually vectored to CPU  
// Up to 8 prioritized IRQs can be individually vectored to COP  
// Up to 4 prioritized FIQs can be individually vectored to CPU  
// Up to 4 prioritized FIQs can be individually vectored to COP  
//
// Prioritized Interrupts will vector directly into individual handlers specified by the 
//    Priority IRQ/FIQ Pointer register associated with that interrupt.
//
// Non-prioritized IRQs or FIQs automatically use the EVP vectors specific to each processor.
// Non-prioritized IRQs or FIQs have an FFS status register which can be used as an
//    index into a jump table for quick access to individual handlers from within the
//    generic handler.
//
// Either processor can configure the vectors for either processor.
// Exception Vector Pointers (Specific to the processor executing the read).
// These alias to CPU_*_VECTOR or COP_*_VECTOR as appropriate.
...
// Status Registers --  Read-only, each processor will read the value associated with it 
// FFS returns the index of the rightmost (lowest index) active interrupt
// PRIority status registers return the index of the highest priority interrupt
...
// Priority IRQ Vector (Request/Vector pointer) pairs 
// Read-Only   Each processor will read the value associated with it 
// Eight sets total 
// Entry 0 is the highest priority 
//
// Interrupts are numbered based on interrupt controller and bit position within controller
//   PRI_ICTLR[ 0] is interrupt number  0 (0x00)
//   PRI_ICTLR[31] is interrupt number 31 (0x1f)
//   SEC_ICTLR[ 0] is interrupt number 32 (0x20)
//   SEC_ICTLR[31] is interrupt number 63 (0x3f)
//   TRI_ICTLR[ 0] is interrupt number 64 (0x40)
//   TRI_ICTLR[31] is interrupt number 95 (0x5f)
//
// Entries with NUM set to 0x80 will be skipped (Entry considered undefined)
...
// Priority FIQ Vector (Request/Vector pointer) pairs 
// Read-Only   Each processor will read the value associated with it 
// Four sets total 
// Entry 0 is the highest priority 
//
...
// for CPU vectors
// Exception Vector Pointers for CPU 
// Program the pointers for the non-prioritized CPU Exception handlers.
// The reset value for these registers depends on various straps and fuses; they initially point
// to IROM (NV_ADDRESS_MAP_IROM_BASE) or NOR (NV_ADDRESS_MAP_NOR_BASE).
...
// CPU Priority IRQ Vector (Request/Vector pointer) pairs 
// Either processor can configure this register 
// Eight sets total 
// Entry 0 is the highest priority 
//
// Interrupts are numbered based on interrupt controller and bit position within controller
...
// CPU Priority FIQ Vector (Request/Vector pointer) pairs 
// Either processor can configure this register 
// Four sets total 
// Entry 0 is the highest priority 
//
...
// for COP vectors
// Exception Vector Pointers for COP 
// Program the pointers for the non-prioritized COP Exception handlers
...
// COP Status Registers --  Read-only 
// FFS returns the index of the rightmost (lowest index) active interrupt
// PRIority status registers return the index of the highest priority interrupt
...
// COP Priority IRQ Vector (Request/Vector pointer) pairs 
// Either processor can configure this register 
// Eight sets total 
// Entry 0 is the highest priority 
//
...
// COP Priority FIQ Vector (Request/Vector pointer) pairs 
// Either processor can configure this register 
// Four sets total 
// Entry 0 is the highest priority 
//
