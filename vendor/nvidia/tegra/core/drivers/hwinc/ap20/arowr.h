...
//////////////////////////////////////////////////////////////////////////////////////////
//                         OWR (ONE WIRE CONTROLLER) 
//DS2502 battery device used for AP20 
//The 1-Wire interface provides the communication line to a 1-Kbit Add-Only Memory (DS2502). 
//The required protocol for accessing the DS2502 is defined by Dallas Semiconductor. 
//The DS2502 holds battery characteristics information. 
//The interface sends or receives one bit or byte at a time.
...
//Battery Devices with EPROM only are supported, devices with NV_RAMS are not supported
//
//PROGRAMING GUIDELINES:
//Controller supports two modes
// 1.Byte transfer mode
// 2.Bit  transfer mode
//
//If the interface sends or receives byte at a time, 
//All the Timing registers, command register and required fields in control registers should be programed and then
//Go bit in ctl reg should be program.
//Rom cmd data is stored in rom registers
//eprom data/status is stored in fifos
//We also have interrupts bits to know the status of design, 
//Interrupts can be enabled or disabled, 
//
//If Interface sends or receives one bit at a time
//This mode is enabled by enabling DATA_TRANSFER_MODE in control register
//All the timing registers should be programed 
//SW writes WR0_BIT or WR1_BIT OR RD_BIT to enable the write one time slot or write zero time slot or read time slot,
//wr0_busy, wr1_busy and rd_busy to indicate the data transfer of the bit.
//to read the sampled data, the sampled data is valid only after rd_busy goes low.
//and an interrupt bit to indicate bit transfer has done and ready to accept the next bit
//In one bit transfer mode, crc calculations to detect err and when to send write slots and read slots is decided by software only
//one bit transfer mode supports only read cmds.  
///////////////////////////////////////////////////////////////////////////////////////////
// OWR Registers
...
// supports upto 256k eprom size
// MEM_ADDR and MEMORY_BYTES_TRANSFER/STATUS_BYTES_TRANSFER should always be in sync
// like if eprom end offset address is 7f, and we want to read data from 4th location address of eprom
// Programing should be in the following way
// MEM_ADDR = 0x4;
// MEMORY_BYTES_TRANSFER = 0x7B; (0x4 - 0x7f)
// similarly for status bytes
...
// Controls the one wire dq output(Pullup or pullDown) to write and read time slots
...
// Controls the one wire dq output(Pullup or pullDown) to reset initialization time slots
// timings are in micro seconds
// Note : <= means less than or equal to
...
// Controls the 12v programing pulse used to write data to eprom
// timings are in micro seconds
...
// Read ROM Cmd Reads 8-bit family code, 48 bit serial Num and 8 bit crc,
// Total of 64 bits, Register can have a max of 32 bits, 
// Read Rom Cmd data is split into 2 registers 1. READ_ROM0 2. READ_ROM1
...
// This register stores the masks for the interrupts. If a bit is set 1 and the corresponding
// interrupt condition is satisfied, interrupt line to system interrupt controller is asserted.
...
// OWR INTR Source Register 
// This is a read-only register which returns the AND of Interrupt Source and Interrupt Mask
// registers.
...
// A write to this register causes the bits in status register to be set 1 
