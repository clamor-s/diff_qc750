...
static NvU32 GetPartEntriesCount()
{
    NvError e=NvSuccess;
    NvU32 count = 0;
...
NvError FillGPT1Header(NvAbootHandle hAboot, NvU32 SectorSize)
{
    NvError e = NvSuccess;
    NvU32      PartitionIdGp1;
    NvU32      PartitionId;
    ...
    NV_CHECK_ERROR_CLEANUP(NvPartMgrGetNameById(PartitionIdGp1 , PartName));
    NV_CHECK_ERROR_CLEANUP(NvAbootGetPartitionParameters(hAboot,
            PartName, &PartitionStart, &PartitionSize,
            &TempSectorSize));

    //location of this header copy
    g_GptH1.my_lba = PartitionStart * (SectorSize /GPT_BLOCK_SIZE) + 1;
    //Partition entries starting LBA (always 2 in primary copy)
    g_GptH1.partition_entry_lba = PartitionStart * (SectorSize /GPT_BLOCK_SIZE) + 2;

    if((PartitionIdGpt2 = GetPartitionByType(NvPartMgrPartitionType_GPT)) == 0)
        return NvError_BadParameter;
    NV_CHECK_ERROR_CLEANUP(NvPartMgrGetNameById(PartitionIdGpt2 , PartName));
    NV_CHECK_ERROR_CLEANUP(NvAbootGetPartitionParameters(hAboot,
             PartName, &PartitionStart, &PartitionSize,
            &TempSectorSize));

    //backup LBA location of other header copy. End of device -1 sector
    g_GptH1.alternate_lba= (PartitionStart + PartitionSize) * (SectorSize /GPT_BLOCK_SIZE) - 1;
    ...
    NvOsMemcpy((NvU8*)&g_GptH1.disk_guid, pUuid, sizeof(pUuid));
...
NvError FillGPT2Header(NvAbootHandle hAboot, NvU32 SectorSize)
{
    NvError e = NvSuccess;
    NvU32      PartitionIdGp1;
    ...
    g_GptH2.alternate_lba =  PartitionStart * (SectorSize /GPT_BLOCK_SIZE) + 1;
    ...
    g_GptH2.my_lba= (PartitionStart + PartitionSize) * (SectorSize /GPT_BLOCK_SIZE) - 1;
    ...
    NvOsMemcpy((NvU8*)&g_GptH2.disk_guid, (NvU8*)&g_GptH1.disk_guid, sizeof(pUuid));
    //partition count
...
NvError FillGptPartitionDetails(NvAbootHandle hAboot, NvU32 SectorSize)
{
    NvError e = NvSuccess;
    NvU32   PartitionId;
    char PartName[NVPARTMGR_PARTITION_NAME_LENGTH];
...
static NvError WriteGptPartitiontable(NvAbootHandle hAboot, NvU32 SectorSize)
{
    NvError e = NvSuccess;
    NvU32 offset = MBR_PART_RECORD_OFFSET;
    NvU8 *buff = NULL;
    NvU32 size = 0, GptPartitionId = 0;
    NvU32 SectorToWrite = 0;
    NvU64 LogicalStartSector;
    NvPartInfo PartitionGpt;
    ...
    NV_CHECK_ERROR_CLEANUP(
        NvDdkBlockDevMgrDeviceOpen(hAboot->BlockDevId,
            hAboot->BlockDevInstance, GptPartitionId, &hDev)
    );
...
NvError NvAbootRawWriteEBRPartition(
        NvAbootHandle hAboot,
        NvU32 DeviceId,
...
static NvBool NvIsGPTAvailable(NvAbootHandle hAboot)
{
    NvError    e = NvSuccess;
    ...
    if(GPTPartID != 0)
    {
        ...
        NV_CHECK_ERROR_CLEANUP(
            NvDdkBlockDevMgrDeviceOpen(hAboot->BlockDevId,
                hAboot->BlockDevInstance, GPTPartID, &hDev)
        );
...
NvError NvAbootRawWriteMBRPartitionPerDevice(
    NvAbootHandle hAboot,
    NvU32 DeviceId,
    NvU32 DeviceInstance
    )
{
    ...
    if((GPTPartitionID = GetPartitionByType(NvPartMgrPartitionType_GPT)) != 0)
    {
        if(NvIsGPTAvailable(hAboot) == NV_TRUE)
        {
            e = NvSuccess;
            goto end;
        }
        if (NvAbootGetPartitionParametersbyId(hAboot,
            ...
            WriteGptPartitiontable( hAboot, SectorSize);
    }
    else
    {
