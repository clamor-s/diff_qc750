...
{
    NvU32 RegValue = NV_READ32( NV_ADDRESS_MAP_FUSE_BASE +  FUSE_SECURITY_MODE_0 );
    if ( RegValue ) 
        return NV_TRUE;
    else
        return NV_FALSE;
...
{
    NvU32 RegValue = NV_READ32 (NV_ADDRESS_MAP_FUSE_BASE +  FUSE_PRODUCTION_MODE_0 );
    if ( RegValue )
        return NV_TRUE;
    else
        return NV_FALSE;
...
void
NvBootFuseGetOpMode(NvBootFuseOperatingMode *pMode, NvBool isSbkBurned) 
{    
    ...
    {
        if ( NvBootFuseIsOdmProductionModeFuseSet() ) 
        {
            if ( isSbkBurned ) 
            {
                *pMode = NvBootFuseOperatingMode_OdmProductionSecure;
            } else 
            {
        ...
        {
            if ( NvBootFuseIsNvProductionModeFuseSet() ) 
            {
