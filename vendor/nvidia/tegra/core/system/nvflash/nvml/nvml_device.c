...
void NvMlBctDeviceHandleDeInit(void)
{
    if (BctDeviceHandlePtr)
...
NvBootError LaunchDevRead(NvMlReaderState *State)
{
    NvBootError  e;
    NvU8        *Dst;
    NvU32        PagesPerBlock;
...
NvBootError NvMlBootGetBct(NvU8 **bct, NvBootFuseOperatingMode Mode)
{
    NvMlContext     Context;
    NvBootError e = NvBootError_Success;
    NvBootAesKey key;
    ...
    if (Mode == NvBootFuseOperatingMode_OdmProductionSecure)
    {
