...
/**
 * NvMlUtilsConvert3pToBootFuseDevice(): converts 3pdevice type to boot fuse device type
 * @param[in] DevNv3p, Nv3pDevice typr, for which conversion is required
 * @param[out] pDevNvBootFuse, pointer to NvBootFuseBootDevice,
...
/**
 * NvMlUtilsConvertBootFuseDeviceTo3p(): converts boot fuse device type to 3pdevice type
...
/**
 * NvMlUtilsLockAesKeySchedule(): disables reads to the key schedule for
 * each AES engine.
 * @retval None
 * @retval None
 */
...
/**
 * NvMlUtilsLockSeAesKeySchedule(): disables reads to the key schedule for
 * AES engine in SE
 *
 * @parms None
 * @retval None
 */
void NvMlUtilsLockSeAesKeySchedule(void);
#endif
...
/**
 * NvMlUtilsGetDataOffset():returns the offset of data on which
