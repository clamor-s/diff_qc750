...
#define NVFLASH_BL_ENTRY 0x108000
#define NVFLASH_BL_ADDRESS 0x108000
...
void NvFlashUtilAp20DumpBit(NvU8 * BitBuffer, const char *dumpOption);
static NvU32 NvFlashUtilAp20GetBctSize(void);
static NvBool NvFlashUtilAp20MungeBct(const char * bct_file, const char * tmp_file);
NvU32 NvFlashUtilAp20GetBlEntry(void);
NvU32 NvFlashUtilAp20GetBlAddress(void);
...
NvAuPrintf("  %s:%s0x%04x (%04d)\n", #var, spaces,\
            PTR_TO_ADDR(&((t).var)) - PTR_TO_ADDR(&(t)),\
            PTR_TO_ADDR(&((t).var)) - PTR_TO_ADDR(&(t)))
...
DumpStructureOffsets(void)
{
    ...
    /*
     * Note: The local variables Bct and Bit shadow the global pointers.
     *       They are just used for computing differences in field addresses.
     */

    NvAuPrintf("BCT size: %d\n", sizeof(NvBootConfigTable));
    NvAuPrintf("BCT Offsets:\n");
    PRINT_OFFSET(Bct, CryptoHash, "           ");
    PRINT_OFFSET(Bct, RandomAesBlock, "       ");
    PRINT_OFFSET(Bct, BootDataVersion, "      ");
    PRINT_OFFSET(Bct, BlockSizeLog2, "        ");
    PRINT_OFFSET(Bct, PageSizeLog2, "         ");
    PRINT_OFFSET(Bct, PartitionSize, "        ");
    PRINT_OFFSET(Bct, NumParamSets, "         ");
    PRINT_OFFSET(Bct, DevType, "              ");
    PRINT_OFFSET(Bct, DevParams, "            ");
    PRINT_OFFSET(Bct, NumSdramSets, "         ");
    PRINT_OFFSET(Bct, SdramParams, "          ");
    PRINT_OFFSET(Bct, BadBlockTable, "        ");
    PRINT_OFFSET(Bct, BootLoadersUsed, "      ");
    PRINT_OFFSET(Bct, BootLoader, "           ");
    PRINT_OFFSET(Bct, CustomerData, "         ");
    PRINT_OFFSET(Bct, EnableFailBack, "       ");
    PRINT_OFFSET(Bct, Reserved, "             ");

    NvAuPrintf("BIT size: %d\n", sizeof(NvBootInfoTable));
    NvAuPrintf("BIT Offsets:\n");
    PRINT_OFFSET(Bit, BootRomVersion, "       ");
    PRINT_OFFSET(Bit, DataVersion, "          ");
    PRINT_OFFSET(Bit, RcmVersion, "           ");
    PRINT_OFFSET(Bit, BootType, "             ");
    PRINT_OFFSET(Bit, PrimaryDevice, "        ");
    PRINT_OFFSET(Bit, SecondaryDevice, "      ");
    PRINT_OFFSET(Bit, OscFrequency, "         ");
    PRINT_OFFSET(Bit, DevInitialized, "       ");
    PRINT_OFFSET(Bit, SdramInitialized, "     ");
    PRINT_OFFSET(Bit, ClearedForceRecovery, " ");
    PRINT_OFFSET(Bit, ClearedFailBack, "      ");
    PRINT_OFFSET(Bit, InvokedFailBack, "      ");
    PRINT_OFFSET(Bit, BctValid, "             ");
    PRINT_OFFSET(Bit, BctStatus, "            ");
    PRINT_OFFSET(Bit, BctLastJournalRead, "   ");
    PRINT_OFFSET(Bit, BctBlock, "             ");
    PRINT_OFFSET(Bit, BctPage, "              ");
    PRINT_OFFSET(Bit, BctSize, "              ");
    PRINT_OFFSET(Bit, BctPtr, "               ");
    PRINT_OFFSET(Bit, BlState, "              ");
    PRINT_OFFSET(Bit, SecondaryDevStatus, "   ");
    PRINT_OFFSET(Bit, SafeStartAddr, "        ");
}
...
void
NvFlashUtilAp20DumpBit(NvU8 * BitBuffer, const char *dumpOption)
{
...
NvBool
NvFlashUtilAp20MungeBct( const char *bct_file, const char *tmp_file )
{
    NvError e;
...
NvU32
NvFlashUtilAp20GetBctSize(void)
{
    return sizeof(NvBootConfigTable);
}

NvU32
NvFlashUtilAp20GetBlEntry(void)
{
    return NVFLASH_BL_ENTRY;
}

NvU32
NvFlashUtilAp20GetBlAddress(void)
{
    return NVFLASH_BL_ADDRESS;
}

NvBool NvFlashUtilGetAp20Hal(NvFlashUtilHal *pHal)
{
    if(pHal->devId == 0x20)
    {
        pHal->MungeBct = NvFlashUtilAp20MungeBct;
        pHal->GetBctSize = NvFlashUtilAp20GetBctSize;
        pHal->GetBlEntry= NvFlashUtilAp20GetBlEntry;
        pHal->GetBlAddress= NvFlashUtilAp20GetBlAddress;
        pHal->GetDumpBit = NvFlashUtilAp20DumpBit;
        return NV_TRUE;
    }
    return NV_FALSE;
}
