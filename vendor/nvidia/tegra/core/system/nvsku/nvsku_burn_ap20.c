        ...
        NvU32 BctPartId, NvU8 *NvInternalBlobBuffer, NvU32 NvInternalBlobBufferSize)
{
    ...
    if (e != NvError_InvalidState)
    {
        ...
        NV_CHECK_ERROR_CLEANUP(
            NvBctGetData(hBct, NvBctDataType_HashedPartition_CryptoHash,
            &Size, &Instance, NULL)
        );
        ...
        for (i = 0; i< Instance; i++)
        {
            ...
            NV_CHECK_ERROR_CLEANUP(
                NvBctSetData((NvBctHandle)BctBuffer, NvBctDataType_HashedPartition_PartId,
                &Size, &i, &PartId)
            );

            NV_CHECK_ERROR_CLEANUP(
                NvBctSetData((NvBctHandle)BctBuffer, NvBctDataType_HashedPartition_CryptoHash,
                &Size, &i, Hash)
            );
