    ...
    NvCryptoCipherAlgoAesKeyType KeyType)
{
    ...
    {
        /* verify signature */
        NvOsMemcpy(Hash, Buffer, HashSize);
        e = hHash->VerifyHash( hHash, Buffer, &isValidHash);
        if (!isValidHash)
        {
    ...
    {
        NV_CHECK_ERROR_CLEANUP(
            hHash->QueryHash( hHash, &HashSize, Buffer )
            );
    }
...
NvBuBctCryptoOps(
    NvBctHandle hBct,
    NvBlOperatingMode OperatingMode,
    NvU32 *Size,
    void *Buffer,
    NvBuBlCryptoOp CryptoOp)
{
    ...
    switch (CryptoOp)
    {
        case NvBuBlCryptoOp_EncryptAndSign:
            ...
            if( IsEncrypt )
            {
                NV_CHECK_ERROR_CLEANUP( BctEncryptDecrypt((pBuffer +
                           HashDataOffset), HashDataLength, NV_TRUE, KeyType)
                    );
            }
        ...
        case NvBuBlCryptoOp_VerifyAndDecrypt:
            ...
            {
                NV_CHECK_ERROR_CLEANUP( BctEncryptDecrypt((pBuffer +
                           HashDataOffset), HashDataLength, NV_FALSE, KeyType)
                    );
            }
            break;
...
NvError
NvBuBctCreateBadBlockTable( NvBctHandle hBct,
                            NvU32 PartitionId)
{
    NvError e = NvSuccess;
    NvU32 ChipId = NvBitPrivGetChipId();
    if (ChipId == 0x20)
        e = NvBuPrivCreateBadBlockTableAp20(hBct, PartitionId);
    else if (ChipId == 0x30)
        e = NvBuPrivCreateBadBlockTableT30(hBct, PartitionId);
    else
               ...
               NvU32 PartitionId,NvU8 *buffer)
{
    ...
    OperatingMode = NvFuseGetOperatingMode();

    if (OperatingMode == NvBlOperatingMode_Undefined)
