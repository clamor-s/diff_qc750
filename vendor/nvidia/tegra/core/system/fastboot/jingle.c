...
void FastbootCloseWave(FastbootWaveHandle pWave)
{
    ...
    if (pWave->DapIndex)
        NV_ASSERT_SUCCESS(
            NvRmSetOdmModuleTristate(pWave->hRm, NvOdmIoModule_Dap,
                pWave->DapIndex-1, NV_TRUE)
        );
#endif
    ...
    FastbootWaveHandle *hWav)
{
    ...
    NV_CHECK_ERROR_CLEANUP(
        NvRmMemHandleCreate(pWave->hRm,
            &pWave->hBuffMem, BUFFER_SIZE*NUM_BUFFERS)
    );
    NV_CHECK_ERROR_CLEANUP(
        NvRmMemAllocTagged(pWave->hBuffMem, NULL, 0, 32,
            NvOsMemAttribute_Uncached, NVRM_MEM_TAG_SYSTEM_MISC)
    );
