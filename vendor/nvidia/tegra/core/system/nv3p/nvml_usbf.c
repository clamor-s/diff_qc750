...
static NvBootError NVML_FUN_SOC(NvMlPrivStartEnumeration, (NvU8 *pXferBuffer));
static void NVML_FUN_SOC(NvMlUsbfDetectChargerByLineState, (void));
static void NVML_FUN_SOC(NvMlUsbfDetectChargerByReset,(void));
static NvBootError
NVML_FUN_SOC(
    NvMlPrivTransmit,
    (NvU8 *pDataBuf,
...
static void NVML_FUN_SOC(NvMlPrivTransferCancel,(NvBootUsbfEndpoint EndPoint));
static void
NVML_FUN_SOC(
...
NvBootError NVML_FUN_SOC(NvMlPrivStartEnumeration, (NvU8 *pXferBuffer))
{
    NvBootError BootError = NvBootError_Success;
    NvU32 EpSetupStatus = 0;
    NvU32 UsbfEvents = 0;
    ...
    // Look for the cable connection status once before initializing the H/W
    if (!NVML_FUN_SOC(NvMlPrivUsbfIsCableConnected,(s_pUsbfCtxt)))
    {
        return NvBootError_CableNotConnected;
    }
    ...
    BootError = NVML_FUN_SOC(NvMlPrivHwIntializeController,(s_pUsbfCtxt));
    ...
    while(!s_pUsbfCtxt->EnumerationDone)
    {
        ...
        if (!NVML_FUN_SOC(NvMlPrivUsbfIsCableConnected,(s_pUsbfCtxt)))
        {
            // bail out if cable is removed. No point in doing enumeration
            return NvBootError_CableNotConnected;
        }
    }
    ...
    NvU32 *pBytesTxfred))
{
    ...
    BootError = NVML_FUN_SOC(NvMlPrivHwTxfrStart,(s_pUsbfCtxt,
                                      USB_EP_BULK_IN,
                                      pDataBuf,
                                      TxfrSizeBytes,
                                      NV_TRUE));
...
void NVML_FUN_SOC(NvMlPrivTransferCancel,(NvBootUsbfEndpoint EndPoint))
{
    // Validating the parameters
    NV_ASSERT(s_pUsbfCtxt);
    NV_ASSERT(s_pUsbfCtxt->UsbControllerEnabled);
    ...
    (NvBootUsbfContext *pUsbFuncCtxt))
{
    ...
    // DP/DM Lines not pulled on the device side yet.
    // Detect Charger type by line state
    if(s_ChargingMode)
        NVML_FUN_SOC(NvMlUsbfDetectChargerByLineState,());
    ...
    // Perform Charger Detection through reset
    if(s_ChargingMode)
        NVML_FUN_SOC(NvMlUsbfDetectChargerByReset,());
...
NvError
NVML_FUN_SOC(Nv3pTransportReopen,( Nv3pTransportHandle *hTrans, NvU32 instance ))
{
    Nv3pTransport *trans;
...
void NVML_FUN_SOC(NvMlUsbfDetectChargerByLineState, (void))
{
    NvU32 LineState;
    NvU32 RegVal;
    s_ChargerType = NvOdmUsbChargerType_Dummy;
    // USB battery charging specification 1.1.
#define TDPSRC_ON_MS 40
#define TDPSRC_CON_MS 40

    // To get correct line status, delay 20ms after USBMODE is set to
    // DEVICE_MODE. In the same way as NvRmUsbDetectChargerState().
    NvOsSleepMS(20);

    // Look for non-complinent USB chargers first.
    LineState = USB_REG_READ_VAL(PORTSC1, LS);

    switch (LineState)
    {
        case 0:
            NvOsDebugPrintf("Found SE0.\n");
            s_ChargerType = NvOdmUsbChargerType_SE0;
            break; // Go on to the USB compliant charger detection.
        case 3:
            NvOsDebugPrintf("Found SE1.\n");
            s_ChargerType = NvOdmUsbChargerType_SE1;
            break;
        case 1:
            NvOsDebugPrintf("Found DP pulled up.\n");
            s_ChargerType = NvOdmUsbChargerType_SJ;
            return;
        case 2:
            NvOsDebugPrintf("Found DM pulled up.\n");
            s_ChargerType = NvOdmUsbChargerType_SK;
            return;
        default:
            NV_ASSERT(!"Unknown line state.");
    }

    // Enable USB compliant charger detection.
    ...
    if (USBIF_DRF_VAL(USB_PHY_VBUS_WAKEUP_ID, VDAT_DET_STS, RegVal))
    {
        NvOsDebugPrintf("  Detected VDP_SRC loopback.\n");
    }
    else
    {
        NvOsDebugPrintf("Non-compliant charger\n");
    }

    // Disable USB compliant charger detection.
    RegVal = UTMIP_REG_RD(NV_ADDRESS_MAP_USB_BASE, BAT_CHRG_CFG0);
    RegVal = NV_FLD_SET_DRF_NUM(USB1_UTMIP, BAT_CHRG_CFG0,
...
void
NVML_FUN_SOC(NvMlUsbfDetectChargerByReset,(void))
{
    NvU32 ResetReceived;
    // Heuristic wait.
    NvOsSleepMS(300);
    // This test is only for SE0.
    if (s_ChargerType != NvOdmUsbChargerType_SE0) return;
    // Otherwise decide charger type.
    ResetReceived = USB_REG_READ_VAL(USBSTS, URI);
    if (ResetReceived)
    {
        NvOsDebugPrintf("  RESET received.\n");
        s_ChargerType = NvOdmUsbChargerType_UsbHost;
    }
    else
    {
        NvOsDebugPrintf("  RESET not received.\n");
    }
}
...
NVML_FUN_SOC(Nv3pTransportClose,(Nv3pTransportHandle hTrans))
{
    ...
    /* wait for host to drop connection */
    NvOsSleepMS(200);
}
