...
#define LENS_HORIZONTAL_VIEW_ANGLE (47.0f)  //These parameters are set to some value in order to pass CTS
#define LENS_VERTICAL_VIEW_ANGLE   (36.0f)  //We'll have to replace them with appropriate values if we get these sensor specs from some source.
#define LENS_FOCAL_LENGTH          (2.65f)
//aperture 2.8f
//d:57f

static NvOdmImagerCapabilities g_SensorYuvCaps =
{
    "gc0308",  // string identifier, used for debugging
 ...
 */
static NvBool SensorYuv_gc0308_Open(NvOdmImagerHandle hImager);
static void SensorYuv_gc0308_Close(NvOdmImagerHandle hImager);

static void
SensorYuv_gc0308_GetCapabilities(
    NvOdmImagerHandle hImager,
    NvOdmImagerCapabilities *pCapabilities);

static void
SensorYuv_gc0308_ListModes(
    NvOdmImagerHandle hImager,
    NvOdmImagerSensorMode *pModes,
    NvS32 *pNumberOfModes);

static NvBool
SensorYuv_gc0308_SetMode(
    NvOdmImagerHandle hImager,
...
static NvBool
SensorYuv_gc0308_SetPowerLevel(
    NvOdmImagerHandle hImager,
    NvOdmImagerPowerLevel PowerLevel);

static NvBool
SensorYuv_gc0308_SetParameter(
    NvOdmImagerHandle hImager,
...
static void
SensorYuv_gc0308_GetPowerLevel(
    NvOdmImagerHandle hImager,
    NvOdmImagerPowerLevel *pPowerLevel);


/**
 * SensorYuv_gc0308_Open. Phase 1.
...
/**
 * SensorYuv_gc0308_Close. Phase 1.
...
/**
 * SensorYuv_gc0308_GetCapabilities. Phase 1.
...
/**
 * SensorYuv_gc0308_ListModes. Phase 1.
...
/**
 * SensorYuv_gc0308_SetMode. Phase 1.
    ...
    SetModeParameters *pResult)
{
    ...
    ret = ioctl(pContext->camera_fd, GC0308_IOCTL_SET_MODE, &mode);
    if (ret < 0) {
...
/**
 * SensorYuv_gc0308_SetPowerLevel. Phase 1
    ...
    NvOdmImagerPowerLevel PowerLevel)
{
    ...
    NvOsDebugPrintf("SensorYuv_gc0308_SetPowerLevel %d\n", PowerLevel);
    ...
    switch(PowerLevel)
    {
        case NvOdmImagerPowerLevel_On:
#ifdef O_CLOEXEC
            pContext->camera_fd = open("/dev/gc0308", O_RDWR|O_CLOEXEC);
#else
            pContext->camera_fd = open("/dev/gc0308", O_RDWR);
#endif
            if (pContext->camera_fd < 0) {
                ...
                NvOsDebugPrintf("gc0308 Camera fd open as: %d\n", pContext->camera_fd);
                Status = NV_TRUE;
            }
            if (!Status)
            {
...
/**
 * SensorYuv_gc0308_SetParameter. Phase 2.
    ...
    const void *pValue)
{
    ...
    switch(Param)
    {
        case NvOdmImagerParameter_SensorInputClock:
            ...
            InClock = ((NvOdmImagerClockProfile*)pValue)->ExternalClockKHz;
...
static int
SensorYuv_gc0308_GetStatus(SensorYuvContext *pContext, NvOdmImagerDeviceStatus *pStatus)
{
#if (BUILD_FOR_AOS == 0)
   ...
   // ret = ioctl(pContext->camera_fd, gc0308_IOCTL_GET_STATUS, &dev_status);
    //if (ret)
        //return ret;
...
/**
 * SensorYuv_gc0308_GetParameter. Phase 1.
    ...
    switch(Param)
    {
        case NvOdmImagerParameter_DeviceStatus:
                ...
                sizeof(NvOdmImagerDeviceStatus));
            {
                ...
                ret = SensorYuv_gc0308_GetStatus(pContext, pStatus);
                if (ret)
                    return NV_FALSE;
            }
            break;
...
/**
 * SensorYuv_gc0308_GetPowerLevel. Phase 2.
...
/**
 * SensorYuv_gc0308_GetHal. Phase 1.
 * return the hal functions associated with sensor yuv
 */
NvBool SensorYuv_Gc0308_GetHal(NvOdmImagerHandle hImager)
{
    if (!hImager || !hImager->pSensor)
        return NV_FALSE;

    hImager->pSensor->pfnOpen = SensorYuv_gc0308_Open;
    hImager->pSensor->pfnClose = SensorYuv_gc0308_Close;
    hImager->pSensor->pfnGetCapabilities = SensorYuv_gc0308_GetCapabilities;
    hImager->pSensor->pfnListModes = SensorYuv_gc0308_ListModes;
    hImager->pSensor->pfnSetMode = SensorYuv_gc0308_SetMode;
    hImager->pSensor->pfnSetPowerLevel = SensorYuv_gc0308_SetPowerLevel;
    hImager->pSensor->pfnGetPowerLevel = SensorYuv_gc0308_GetPowerLevel;
    hImager->pSensor->pfnSetParameter = SensorYuv_gc0308_SetParameter;
    hImager->pSensor->pfnGetParameter = SensorYuv_gc0308_GetParameter;
