...
{
    "hm2057",  // string identifier, used for debugging
    ...
    { {48000, 130.f/48.f}, {0,0} }, // preferred clock profile
 ...
 */
static NvBool SensorYuv_hm2057_Open(NvOdmImagerHandle hImager);
static void SensorYuv_hm2057_Close(NvOdmImagerHandle hImager);

static void
SensorYuv_hm2057_GetCapabilities(
    NvOdmImagerHandle hImager,
    NvOdmImagerCapabilities *pCapabilities);

static void
SensorYuv_hm2057_ListModes(
    NvOdmImagerHandle hImager,
    NvOdmImagerSensorMode *pModes,
    NvS32 *pNumberOfModes);

static NvBool
SensorYuv_hm2057_SetMode(
    NvOdmImagerHandle hImager,
...
static NvBool
SensorYuv_hm2057_SetPowerLevel(
    NvOdmImagerHandle hImager,
    NvOdmImagerPowerLevel PowerLevel);

static NvBool
SensorYuv_hm2057_SetParameter(
    NvOdmImagerHandle hImager,
...
static void
SensorYuv_hm2057_GetPowerLevel(
    NvOdmImagerHandle hImager,
    NvOdmImagerPowerLevel *pPowerLevel);


/**
 * SensorYuv_hm2057_Open. Phase 1.
...
/**
 * SensorYuv_hm2057_Close. Phase 1.
...
/**
 * SensorYuv_hm2057_GetCapabilities. Phase 1.
...
/**
 * SensorYuv_hm2057_ListModes. Phase 1.
...
/**
 * SensorYuv_hm2057_SetMode. Phase 1.
    ...
    SetModeParameters *pResult)
{
    ...
    ret = ioctl(pContext->camera_fd, HM2057_IOCTL_SET_MODE, &mode);
    if (ret < 0) {
...
/**
 * SensorYuv_hm2057_SetPowerLevel. Phase 1
    ...
    NvOdmImagerPowerLevel PowerLevel)
{
    ...
    NvOsDebugPrintf("SensorYuv_hm2057_SetPowerLevel %d\n", PowerLevel);
...
#ifdef O_CLOEXEC
            pContext->camera_fd = open("/dev/hm2057", O_RDWR|O_CLOEXEC);
#else
            pContext->camera_fd = open("/dev/hm2057", O_RDWR);
#endif
...
/**
 * SensorYuv_hm2057_SetParameter. Phase 2.
...
static int
SensorYuv_hm2057_GetStatus(SensorYuvContext *pContext, NvOdmImagerDeviceStatus *pStatus)
{
#if (BUILD_FOR_AOS == 0)
   ...
   // ret = ioctl(pContext->camera_fd, hm2057_IOCTL_GET_STATUS, &dev_status);
    //if (ret)
        //return ret;
...
/**
 * SensorYuv_hm2057_GetParameter. Phase 1.
    ...
    switch(Param)
    {
        case NvOdmImagerParameter_DeviceStatus:
                ...
                sizeof(NvOdmImagerDeviceStatus));
            {
                ...
                ret = SensorYuv_hm2057_GetStatus(pContext, pStatus);
                if (ret)
                    return NV_FALSE;
            }
            break;
...
/**
 * SensorYuv_hm2057_GetPowerLevel. Phase 2.
...
/**
 * SensorYuv_hm2057_GetHal. Phase 1.
...
NvBool SensorYuv_Hm2057_GetHal(NvOdmImagerHandle hImager)
{
    ...
    hImager->pSensor->pfnOpen = SensorYuv_hm2057_Open;
    hImager->pSensor->pfnClose = SensorYuv_hm2057_Close;
    hImager->pSensor->pfnGetCapabilities = SensorYuv_hm2057_GetCapabilities;
    hImager->pSensor->pfnListModes = SensorYuv_hm2057_ListModes;
    hImager->pSensor->pfnSetMode = SensorYuv_hm2057_SetMode;
    hImager->pSensor->pfnSetPowerLevel = SensorYuv_hm2057_SetPowerLevel;
    hImager->pSensor->pfnGetPowerLevel = SensorYuv_hm2057_GetPowerLevel;
    hImager->pSensor->pfnSetParameter = SensorYuv_hm2057_SetParameter;
    hImager->pSensor->pfnGetParameter = SensorYuv_hm2057_GetParameter;
