...
static SensorSetModeSequence g_SensorBayerSetModeSequenceList[] =
{
    ...
    {{{3264, 2448}, {0, 0}, 15,   1.0,  16.0, NvOdmImagerNoCropMode, {{0, 0, 0, 0}, {0.f, 0.f}}, NvOdmImagerModeType_Normal}, NULL, &ModeDependentSettings_3264x2448},
    // for 1080P encoding, aspect ratio of this mode = 16:9, no scale desired during encoding, so ar=1.0
    {{{2880, 1620}, {0, 0}, 24,   1.0,  16.0, NvOdmImagerPartialCropMode, {{0, 0, 0, 0}, {0.f, 0.f}}, NvOdmImagerModeType_Normal}, NULL, &ModeDependentSettings_2880x1620},
    // for 1080P encoding, aspect ratio of this mode = 16:9, no scale desired during encoding, so ar=1.0
    {{{1920, 1080}, {0, 0}, 30,   1.0,  16.0, NvOdmImagerPartialCropMode, {{0, 0, 0, 0}, {0.f, 0.f}}, NvOdmImagerModeType_Normal}, NULL, &ModeDependentSettings_1920x1080},
    // for preview @ 30fps), aspect ratio of this mode = 4:3, no scale desired during encoding, so ar=1.0
    {{{1632, 1224},  {0, 0}, 30,   1.0,  16.0, NvOdmImagerNoCropMode, {{0, 0, 0, 0}, {0.f, 0.f}}, NvOdmImagerModeType_Normal}, NULL, &ModeDependentSettings_1632x1224},
    // for preview @ 30fps), aspect ratio of this mode = 4:3, no scale desired during encoding, so ar=1.0
    {{{800,  600},  {0, 0}, 120,   1.0, 16.0, NvOdmImagerPartialCropMode, {{0, 0, 0, 0}, {0.f, 0.f}}, NvOdmImagerModeType_Normal}, NULL, &ModeDependentSettings_800x600},
};
    ...
    const NvOdmImagerClockProfile *pClockProfile)
{
    ...
    if (pClockProfile->ExternalClockKHz <
        g_SensorBayerCaps[pContext->ActiveGUIDIndex].Caps.InitialSensorClockRateKHz)
        return NV_FALSE;

    pContext->SensorInputClockkHz = pClockProfile->ExternalClockKHz;
    NV_DEBUG_PRINTF(("[%s] SensorInputClockkHz = %d \n", __func__,
                            pContext->SensorInputClockkHz));

    pContext->SensorClockRatio = pClockProfile->ClockMultiplier;
    NV_DEBUG_PRINTF(("[%s] SensorClockRatio = %f \n", __func__,
                            pContext->SensorClockRatio));
    ...
    if (pContext->SensorInitialized)
    {
        ...
        pContext->Exposure = (((NvF32)pContext->CoarseTime-DIFF_INTEGRATION_TIME_OF_MODE)
                            * (NvF32)pContext->LineLength) / (NvF32)pContext->VtPixClkFreqHz;
        pContext->MaxExposure = (((NvF32)SENSOR_BAYER_DEFAULT_MAX_EXPOSURE_COARSE-DIFF_INTEGRATION_TIME_OF_MODE)
                            * (NvF32)pContext->LineLength ) / (NvF32)pContext->VtPixClkFreqHz;
        pContext->MinExposure = (((NvF32)SENSOR_BAYER_DEFAULT_MIN_EXPOSURE_COARSE-DIFF_INTEGRATION_TIME_OF_MODE)
                            * (NvF32)pContext->LineLength ) / (NvF32)pContext->VtPixClkFreqHz;
    ...
    const void *pValue)
{
    ...
    switch(Param)
    {
        ...
        case NvOdmImagerParameter_SensorInputClock:
            CHECK_PARAM_SIZE_RETURN_MISMATCH(SizeOfValue, sizeof(NvOdmImagerClockProfile));
            Status = SensorBayer_SetInputClock(pContext, ((NvOdmImagerClockProfile*)pValue));
            break;
