...
static NvU32 TPS6586xPmuVoltageSetSM2(const NvU32 millivolts)
{
    if (millivolts < 3000)
        ...
        return MIN((millivolts - 3000) / 50, 0x1f);
}
...
{
    VOLTAGE_SEARCH(MIN(1225, millivolts), VLDO2);
}
    ...
    NvU32* pSettleMicroSeconds)
{
    ...
    else
    {
        ...
        if (NV_FALSE == status)
            NVODMPMU_PRINTF(("TPS:Reading from PMU I2C fails 1... supplyaddress: %d\n", pSupplyInfo->supplyRegInfo.addr));
        ...
        if (status)
        {
                ...
                (vddRail == TPS6586xPmuSupply_LDO2))
            {
                status = Tps6586xI2cRead8(hDevice, pSupplyInfo->supplyRegInfo.addr, &data);
                if (NV_FALSE == status)
                NVODMPMU_PRINTF(("TPS:Reading from PMU I2C fails 2... supplyaddress: %d\n", pSupplyInfo->supplyRegInfo.addr));
    ...
    if (MilliVolts == ODM_VOLTAGE_OFF)
    {
        ...
        if ((((NvOdmPmuDeviceTPS *)(hDevice->pPrivate))->supplyRefCntTable[vddRail] == 1) || (vddRail == TPS6586xPmuSupply_SoC))
        {
            /* Disable */
...
/* Program the Crystall oscillator Load */
static NvBool
Tps6586xProgramCrystalLoad(NvOdmPmuDeviceHandle hDevice, NvU8 ClSelValue)
{
    NvU32 data = 0;

    if (!Tps6586xI2cRead8(hDevice, TPS6586x_RC0_RTC_CTRL, &data))
    {
        return NV_FALSE;
    }
    ...
    if (!Tps6586xI2cWrite8(hDevice, TPS6586x_RC0_RTC_CTRL, data))
    {
        return NV_FALSE;
    }
    return NV_TRUE;
...
#ifdef NV_SKU_SUPPORT
    {
        ...
        if ( (NvSuccess == NvBootGetSkuNumber(&MachineNumber)))
        {
            switch (MachineNumber)
            {
                ...
                case TEGRA_P852_SKU1_B00:
                case TEGRA_P852_SKU1_C0x:
                    {
                        NvU8 ClSelValue = 0;
                        // Program the right value for CL_SEL
                        if (!Tps6586xProgramCrystalLoad(hDevice, ClSelValue))
                        {
                            return NV_FALSE;
                        }
                    }
    ...
    NvU8 *pStatus)
{
    ...
    if (batteryInst == NvOdmPmuBatteryInst_Main)
    {
        ...
        {
            // The logic looks a bit odd though.
            status = NVODM_BATTERY_STATUS_CHARGING;
        }
...
void Tps6586xPowerOff( NvOdmPmuDeviceHandle  hDevice)
{
    NvU32 ReadBuffer = 0;

    // Read current value of SUPPLYENE(R 0x14) register
    Tps6586xI2cRead8(hDevice, TPS6586x_R14_SUPPLYENE, &ReadBuffer);
    // Set SLEEP MODE bit i.e. B3
    ReadBuffer = (ReadBuffer | 0x8);
    // Good bye! this is the very last command
    // All power supplies on the board will be switched off after this!
    Tps6586xI2cWrite8(hDevice, TPS6586x_R14_SUPPLYENE, ReadBuffer);

    // We should never reach to this point. Some thing wrong has happened
    // Display error message and halt here forever
    NV_ASSERT(0);
    NVODMPMU_PRINTF(("Error in powering off the board!...\n"));
    while(1);
}
