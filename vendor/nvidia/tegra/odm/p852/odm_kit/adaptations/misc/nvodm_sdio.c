...
//=========================================================================
// Programming the Secondary PMU MIC2826 to enable/disable the WiFi Related  Power Rails.
// The sequence of Programming for Power ON is as follows
//  step   device address     direction   register address     register data
//  ====    ===========        ======      ============        =========
//   1       7’h5A              write        03h                  45h
//   2       7’h5A              write        04h                  D1h
//   3       7’h5A              write        05h                  85h
//   4       7’h5A              write        00h                  11h
//   5       7’h5A              write        00h                  1Dh
//   6       7’h5A              write        00h                  1Fh
// The sequence of Programming for Power OFF is as follows
//
//=========================================================================
static NvError PowerOnOffVCoreWF(NvBool IsEnable)
{
...
#define I2C_TRANSACTION_TIMEOUT_MS  1000
    ...
    NvU32 NumOfRegUpdates;
    // Power Enable Values in Sequence
    NvOdmPmuSettings PmuPowerOnConfig[]  = {
        {0x3, 0x45},
        {0x4, 0xD1},
        {0x5, 0x85},
        {0x0, 0x11},
     ...
     // Power Disable Values in Sequence // TODO: FIXME: Program Correct Values to Power down the rails.
    NvOdmPmuSettings PmuPowerOffConfig[]  = {
        {0x3, 0x45},
        {0x4, 0xD1},
        {0x5, 0x85},
        {0x0, 0x11},
    ...
    if(IsEnable)
    {
        NumOfRegUpdates =  NV_ARRAY_SIZE(PmuPowerOnConfig);
    }
    else // Disabling Power Rails
    {
        // TODO: FIXME: Program Correct Values to Power down the rails.
        NumOfRegUpdates = 0; //NV_ARRAY_SIZE(PmuPowerOffConfig);
    }

    hOdmI2c = NvOdmI2cOpen(NvOdmIoModule_I2c_Pmu, 0); //instance=0
    NV_ASSERT(hOdmI2c);

    for (i = 0; i < NumOfRegUpdates; i++)
    {
         if(IsEnable)
         {
            // Enabling Power Rails
            DataBuffer[0] = PmuPowerOnConfig[i].PmuRegAddr;
            DataBuffer[1] = PmuPowerOnConfig[i].PmuRegData;
         }
         else
         {
            // Disabling Power Rails
            DataBuffer[0] = PmuPowerOffConfig[i].PmuRegAddr;
            DataBuffer[1] = PmuPowerOffConfig[i].PmuRegData;
         }
