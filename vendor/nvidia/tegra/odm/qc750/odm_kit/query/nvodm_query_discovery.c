    ...
    NvOdmBoardInfo *pBoardInfo)
{
	...
	if(s_ReadBoardInfoDone==NV_FALSE){
		s_ReadBoardInfoDone =NV_TRUE;
		s_BoardModuleTable[0].BoardID = BOARD_E1565;
		s_BoardModuleTable[0].SKU = 0;
		s_BoardModuleTable[0].Fab = BOARD_FAB_A01;
		s_BoardModuleTable[0].Revision = 0;
		s_BoardModuleTable[0].MinorRevision = 0;
		NumBoards = 1;

		NvOdmOsPrintf("BoardInfo: 0x%04x:0x%04x:0x%02x:0x%02x:0x%02x\n",
	      	s_BoardModuleTable[0].BoardID, s_BoardModuleTable[0].SKU,
	      	s_BoardModuleTable[0].Fab, s_BoardModuleTable[0].Revision, s_BoardModuleTable[0].MinorRevision);

	}

	if (s_BoardModuleTable[0].BoardID == BOARD_E1565)
	{
		// Match found
		pBoardInfo->BoardID  = s_BoardModuleTable[0].BoardID;
		pBoardInfo->SKU      = s_BoardModuleTable[0].SKU;
		pBoardInfo->Fab      = s_BoardModuleTable[0].Fab;
		pBoardInfo->Revision = s_BoardModuleTable[0].Revision;
		pBoardInfo->MinorRevision = s_BoardModuleTable[0].MinorRevision;
		return NV_TRUE;
	}
    return NV_FALSE;
}
