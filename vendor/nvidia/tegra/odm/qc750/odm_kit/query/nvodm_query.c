...
static const NvU8
s_NvOdmQueryPlatformSetting[] = {'q','c','7','5','0',0};
...
#endif
//for get hardware version from gpio
void NvAppLetDevieShutDown(void){
	NvOdmGpioPinHandle hOdmPins;
	NvU32 pinState =1;
	
	NvU32 Port =(NvU32)('k'-'a') ;
	NvU32 Pin =(NvU32)(1);
	
	NvOdmServicesGpioHandle hOdmGpio = NvOdmGpioOpen();
    	if (!hOdmGpio)
        goto fail;
        hOdmPins = NvOdmGpioAcquirePinHandle(hOdmGpio,Port,Pin);
	 if (!hOdmPins){
	 	NvOsDebugPrintf("NvAppLetDevieShutDown: request gpioK1 failed!<========###=======>\n");
	        goto request_gpio_fail;
	  }
	 NvOdmGpioConfig(hOdmGpio, hOdmPins, NvOdmGpioPinMode_Output);
	 NvOdmGpioSetState(hOdmGpio, hOdmPins, pinState);
	 NvOdmGpioReleasePinHandle(hOdmGpio, hOdmPins);
...
NvU32
NvOdmQueryGetHardwareVersion(void)
{

    NvU32 hardwareVersion = 0 ;
    NvU32 pinState;
    NvOdmGpioPinHandle hOdmPins;
    int i;
    KhOdmGpioPinInfo hardware_gpios[] = {
	{
		.Port = (NvU32)('h'-'a') ,
		.Pin =(NvU32)(4),
		.MultBase= 8,
...
#define KH_ARRAY_SIZE(x) (sizeof(x) / sizeof((x)[0]))
   for(i=0;i<KH_ARRAY_SIZE(hardware_gpios);i++){
	    hOdmPins = NvOdmGpioAcquirePinHandle(hOdmGpio,hardware_gpios[i].Port,hardware_gpios[i].Pin);
	    if (!hOdmPins)
	        goto request_gpio_fail;
	    NvOdmGpioConfig(hOdmGpio, hOdmPins, NvOdmGpioPinMode_InputData);
	    NvOdmGpioGetState(hOdmGpio, hOdmPins, &pinState);
	    
	    hardwareVersion += pinState*hardware_gpios[i].MultBase;
	    NvOdmGpioReleasePinHandle(hOdmGpio, hOdmPins);
   }
   #undef KH_ARRAY_SIZE
...
NvOdmGpioClose(hOdmGpio);
      ...
      hardwareVersion = 15;//emp for test
...
NvU32
NvOdmQueryGetModule3gConfig(void)
{
    NvU32 pinState=0;
    NvOdmGpioPinHandle hOdmPins;
    ...
    if (!hOdmGpio)
    {
        NvOsDebugPrintf("NvOdmQueryGetModule3gConfig: GpioHandle failed!<========###=======>\n");
        goto fail;
    }
    hOdmPins = NvOdmGpioAcquirePinHandle(hOdmGpio,(NvU32)('h'-'a'),(NvU32)7);
    if (!hOdmPins)
    {
        NvOsDebugPrintf("NvOdmQueryGetModule3gConfig: request gpioH7 failed!<========###=======>\n");
	    goto request_gpio_fail;
    }
	NvOdmGpioConfig(hOdmGpio, hOdmPins, NvOdmGpioPinMode_InputData);
	NvOdmGpioGetState(hOdmGpio, hOdmPins, &pinState);
