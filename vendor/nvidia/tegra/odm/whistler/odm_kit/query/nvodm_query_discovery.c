...
NvIsFilteredPeripheral(const NvOdmPeripheralConnectivity* pConnectivity)
{
    ...
    {
        if (NV_DRF_VAL(TEGRA_DEVKIT, BCT_CUSTOPT, RIL, CustOpt) == TEGRA_DEVKIT_BCT_CUSTOPT_0_RIL_EMP_RAINBOW)
        {
            return NV_FALSE; // Customer option says EMP RAINBOW supported - don't filter it
        }
        else
        {
            return NV_TRUE; // Customer option does not say EMP RAINBOW supported - filter it
        }
    }
	
	if (pConnectivity->Guid == NV_ODM_GUID('e','m','p',' ','M','5','7','0'))
    {
        if (NV_DRF_VAL(TEGRA_DEVKIT, BCT_CUSTOPT, RIL, CustOpt) == TEGRA_DEVKIT_BCT_CUSTOPT_0_RIL_EMP_RAINBOW_ULPI)
        {
            return NV_FALSE; // Customer option says EMP RAINBOW ULPI supported - don't filter it
        }
        else
        {
            return NV_TRUE; // Customer option does not say EMP RAINBOW ULPI supported - filter it
        }
    }
    ...
    switch (mType)
    {
        case NvOdmBoardModuleType_ProcessorBoard:
            if (DataLen != sizeof(NvOdmBoardInfo))
                return NV_FALSE;
            return NvOdmPeripheralGetBoardInfo(0xB08, BoardModuleData);
