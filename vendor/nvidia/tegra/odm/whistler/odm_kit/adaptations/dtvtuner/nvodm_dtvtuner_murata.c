...
#define LD0_CTRL18 0x72
#define LD0_CTRL20 0x74
#define LD0_CTRL18_18V 0x1F
#define LD0_CTRL20_18V 0x2E
#define I2CADDR_PMU 0x78
        ...
        NvOdmDtvtunerPowerLevel PowerLevel)
{
    ...
    switch (PowerLevel) {
        case NvOdmDtvtunerPowerLevel_On:
            for( i = 0; i < pHW->pPeripheralConnectivity->NumAddress; i++)
            {
                switch(pHW->pPeripheralConnectivity->AddressList[i].Interface)
                {
                    ...
                    case NvOdmIoModule_Gpio:
                        ...
                        // Allocate pin if not done already
                        if (NULL == pHW->pGpioPins[i])
                            pHW->pGpioPins[i] = NvOdmGpioAcquirePinHandle(pHW->hGpio,
                                pHW->pPeripheralConnectivity->AddressList[i].Instance,
                                pHW->pPeripheralConnectivity->AddressList[i].Address);

                        NvOdmGpioConfig(pHW->hGpio, pHW->pGpioPins[i], NvOdmGpioPinMode_Output);
                        NvOdmGpioSetState(pHW->hGpio, pHW->pGpioPins[i], NvOdmGpioPinActiveState_Low);
                        break;
            ...
            if(!pHW->hI2C)
                pHW->hI2C = NvOdmI2cOpen(NvOdmIoModule_I2c, 2);
            if (!pHW->hI2C) {
          ...
          NvU32 channel)
{
    ...
    NvOdmTVI2cWrite8(pHW->hI2CPMU, I2CADDR_PMU, LD0_CTRL18, LD0_CTRL18_18V);
    NvOdmTVI2cWrite8(pHW->hI2CPMU, I2CADDR_PMU, LD0_CTRL20, LD0_CTRL20_18V);
