...
NvOdmDtvtunerHandle NvOdmDtvtunerOpen(void)
{
    ...
    // Open GPIO and allocate pins table
    pDC->HWctx.hGpio = NvOdmGpioOpen();
    if (!pDC->HWctx.hGpio)
    {
        errorCode = NV_TRUE;
        goto EXIT;
    ...
    pDC->HWctx.pGpioPins = NvOdmOsAlloc(pDC->HWctx.pPeripheralConnectivity->NumAddress * sizeof(NvOdmGpioPinHandle));
    if (!pDC->HWctx.pGpioPins)
    {
        errorCode = NV_TRUE;
        goto EXIT;
    }

    NvOdmOsMemset(pDC->HWctx.pGpioPins, 0, pDC->HWctx.pPeripheralConnectivity->NumAddress * sizeof(NvOdmGpioPinHandle));

EXIT:
    if (errorCode)
    {
        if (pDC) 
            NvOdmDtvtunerClose(&pDC);
        return NULL;
    }
    else
        return pDC;
...
void NvOdmDtvtunerClose(NvOdmDtvtunerHandle *phDtvtuner)
{
    ...
    if (pDC)
    {
        if (pDC->HWctx.pGpioPins)
        {
            ...
            // Close GPIO pins if any
            for (i = 0; i < pDC->HWctx.pPeripheralConnectivity->NumAddress; i++)
            {
                if (pDC->HWctx.pGpioPins[i])
                {
                    NvOdmGpioReleasePinHandle(pDC->HWctx.hGpio, pDC->HWctx.pGpioPins[i]);
                }
            }
            NvOdmOsFree(pDC->HWctx.pGpioPins);
        }
        NvOdmGpioClose(pDC->HWctx.hGpio);
