...
OpenSpiHandle(const NvOdmIoAddress *pAddr)
{
    ...
    NvU32 NumPinConfigs;
    const NvU32 *pPinConfigs;

    NvOdmQueryPinMux(pAddr->Interface, &pPinConfigs, &NumPinConfigs);
    NV_ASSERT((pAddr->Instance<NumPinConfigs) &&
              (pPinConfigs[pAddr->Instance]!=0));

    if (pPinConfigs[pAddr->Instance]==NvOdmSpiPinMap_Multiplexed)
    {
        NV_ASSERT(ADAPTATION_SUPPORTS_MULTIPLEXED_SPI &&
                  "If multiplexing is used, you must define the correct "
                  "pin mux configuration inside the display adaptation");

        hSpi = NvOdmSpiPinMuxOpen(pAddr->Interface, pAddr->Instance,
                                  ADAPTATION_SUPPORTS_MULTIPLEXED_SPI);
    }
    else
    {
...
PanelResume(const AmoLedState *AmoLedInstance)
{
    ...
    SPI(hSpi, pAddr->Address, 0x1F, 0x10);  // sample on rising edge to prevent flickering
                           ...
                           NvU32 *pVal)
{
    ...
    case NvOdmDispParameter_Type:
        *pVal = (NvU32)NvOdmDispDeviceType_TFT;
        break;
    case NvOdmDispParameter_Usage:
        *pVal = (NvU32)NvOdmDispDeviceUsage_Unassigned;
        break;
    case NvOdmDispParameter_MaxHorizontalResolution:
        *pVal = AmoLedAmaF002Modes[0].width;
        break;
    case NvOdmDispParameter_MaxVerticalResolution:
        *pVal = AmoLedAmaF002Modes[0].height;
        break;
    case NvOdmDispParameter_BaseColorSize:
        *pVal = (NvU32)NvOdmDispBaseColorSize_888;
        break;
    case NvOdmDispParameter_DataAlignment:
        *pVal = (NvU32)NvOdmDispDataAlignment_MSB;
        break;
    case NvOdmDispParameter_PinMap:
        *pVal = (NvU32)NvOdmDispPinMap_Single_Rgb24_Spi5;
        break;
    case NvOdmDispParameter_PwmOutputPin:
        *pVal = (NvU32)NvOdmDispPwmOutputPin_Unspecified;
        break;
    default:
        return NV_FALSE;
    }
