...
tpowvga_resume( NvOdmDispDeviceHandle hDevice )
{
    ...
    NvOdmQueryPinMux(NvOdmIoModule_Spi, &pSpiConfigs, &NumSpiConfigs);

    if (s_spi_instance >= NumSpiConfigs)
        return;

    if (pSpiConfigs[s_spi_instance]==NvOdmSpiPinMap_Multiplexed)
        hSpi = NvOdmSpiPinMuxOpen(NvOdmIoModule_Spi, s_spi_instance, NvOdmSpiPinMap_Config3);
    else
