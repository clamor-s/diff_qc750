 ...
 * license agreement from NVIDIA CORPORATION strictly prohibited.
 */
...
{
    {NvOdmQuerySpiSignalMode_0, NV_TRUE, NV_FALSE, NV_FALSE, 0, 0}    // Spi1_Devices_0 (chip sel 0)
};
...
static const NvOdmQueryDapPortConnection s_NvOdmQueryDapPortConnectionTable[] =
{
    ...
    // Bluetooth to Codec
    { NvOdmDapConnectionIndex_BlueTooth_Codec, 4,
    { {NvOdmDapPort_Dap4, NvOdmDapPort_I2s2, NV_TRUE},
    ...
    NvU32 ChipSelect)
{
    if ((OdmIoModule == NvOdmIoModule_Spi) ||
        (OdmIoModule == NvOdmIoModule_Sflash))
...
const NvOdmSocPowerStateInfo* NvOdmQueryLowestSocPowerState(void)
{
    ...
    if (pPowerStateInfo == NULL)
    {
...
#if 1   // force lowest power state to LP1 untill LP0 issues are fixed
        ...
        PowerStateInfo.LowestPowerState = ((LPStateSelection == TEGRA_DEVKIT_BCT_CUSTOPT_0_LPSTATE_LP1)?
                                            NvOdmSocPowerState_DeepSleep : NvOdmSocPowerState_Suspend);
#else
