...
static NvError NvEcGetFirmwareVersion(NvOdmBatteryDevice *pBattContext,
                                      NvS32 *MajorVersion,
                                      NvS32 *MinorVersion);
#endif
 ...
 */
static NvError NvEcGetFirmwareVersion(NvOdmBatteryDevice *pBattContext,
                                      NvS32 *MajorVersion, NvS32 *MinorVersion)
{
    ...
    NvOsMemcpy(&FirmwareVersion, EcResponse.Payload, EcResponse.NumPayloadBytes);
                              ...
                              NvOdmOsSemaphoreHandle *hOdmSemaphore)
{
    ...
    NvStatus = NvEcGetFirmwareVersion(pBattContext, &MajorVersion, &MinorVersion);
    if (NvStatus != NvError_Success)
    {
        goto Cleanup;
    }
    ...
    if (hOdmSemaphore != NULL && *hOdmSemaphore != NULL)
    {
        ...
        if (pBattContext->ECVersion >= NVODM_BATTERY_EC_FIRMWARE_VER_R04)
        {
           ...
           /* Configure the Batter present events */
            EcRequest.PacketType = NvEcPacketType_Request;
            EcRequest.RequestType = NvEcRequestResponseType_Battery;
            EcRequest.RequestSubtype = (NvEcRequestResponseSubtype)
           ...
           /* Configure the AC present event */
            EcRequest.PacketType = NvEcPacketType_Request;
       ...
       NvOdmBatteryAcLineStatus *pStatus)
{
    ...
    *pStatus = NvOdmBatteryAcLine_Online;
    ...
    /* For R01 EC Firware tell AC is online as it has not support for this */
       ...
       NvU8 *pStatus)
{
    ...
    *pStatus = 0;
    ...
    /* For R01 firmware Battery support is not present is tell Battery is not present */
    if (pBattContext->ECVersion == NVODM_BATTERY_EC_FIRMWARE_VER_R01)
    {
        *pStatus = NVODM_BATTERY_STATUS_UNKNOWN;
        return NV_TRUE;
