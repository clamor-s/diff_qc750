...
static NvBool OpenOdmServiceHandle(NvOdmPrivAudioCodecHandle hOdmAudioCodec)
{
    ...
    {
#ifndef SET_KERNEL_PINMUX
        NvOdmQueryPinMux(hAudioCodec->IoModule, &pSpiConfigs, &NumSpiConfigs);
        if (hAudioCodec->InstanceId >= NumSpiConfigs)
            return NV_FALSE;
#endif
...
#ifndef SET_KERNEL_PINMUX
        if (pSpiConfigs[hAudioCodec->InstanceId] == NvOdmSpiPinMap_Multiplexed)
        {
            hAudioCodec->hOdmSpiService =
                NvOdmSpiPinMuxOpen(hAudioCodec->IoModule,
                                   hAudioCodec->InstanceId,
                                   ((hAudioCodec->IsBoardWhistler)? NvOdmSpiPinMap_Config5 : NvOdmSpiPinMap_Config1));
        }
        else
        {
#endif
