                ...
                NvOdmPmuRailProps *pOdmRailProps, NvS32 NrOfOdmRailProps)
{
    ...
    for (i = 1; i < TPS65090PmuSupply_Num; ++i)
    {
        ...
        if (hTps65090PmuInfo->RailInfo[VddId].OdmProps.Setup_mV)
             hTps65090PmuInfo->RailInfo[VddId].Caps.requestMilliVolts =
                        hTps65090PmuInfo->RailInfo[VddId].OdmProps.Setup_mV;
        else
        {
        ...
        hTps65090PmuInfo->RailInfo[VddId].UserCount  = 0;
    ...
    if (MilliVolts == ODM_VOLTAGE_OFF)
    {
        ...
        if (pRailInfo->UserCount == 1)
        {
            NvOdmServicesPmuSetSocRailPowerState(hTps65090PmuInfo->hOdmPmuService,
                    hTps65090PmuInfo->RailInfo[VddId].SystemRailId, NV_FALSE);
            Status = pPmuProps->pOps->DisableRail(hTps65090PmuInfo, VddId, pSettleMicroSeconds);
        }
        pRailInfo->UserCount--;
        goto End;
    }
