                            ...
                            NvU32 *pI2cInstance, NvU32 *pI2cAddress)
{
    ...
    const NvOdmPeripheralConnectivity *pConnectivity =
                           NvOdmPeripheralGetGuid(MAX17048_PMUGUID);
    if (pConnectivity)
    {
...
Max17048FuelGaugeDriverOpen(NvOdmPmuDeviceHandle hDevice)
{
    ...
    if (!s_Max17048CoreDriver.OpenCount)
    {
        ...
        {
            NvOdmOsPrintf("%s: Error MAX17048 version number.\n", __func__);
            return NULL;
        }

        // Initialize with the model data
        // Unlock model access
...
NvBool
Max17048FuelGaugeDriverReadSoc(NvU32 *pData)
{
    NvU16 soc = 0;
