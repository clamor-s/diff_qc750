 ...
 */
static int mountEmulatedStorage(uid_t uid, u4 mountMode, bool isApp2Sd2Sd) {
    ...
    if (mountMode == MOUNT_EXTERNAL_MULTIUSER
        ...
        char* cource_sd1_obb =getenv("APP2SD_STORAGE_OBB");
        ...
        if(isApp2Sd2Sd){
            if(fs_is_path_exist(cource_sd1_obb) != 0){
                isApp2Sd2Sd =false;
                SLOGE("want enable the app2sd, but the dir %s is not existed",cource_sd1_obb);
            }
            else{
                SLOGD("we will move the obb to %s ",cource_sd1_obb);
            }
        }

        SLOGD("app2sd state=%d, pid=%d,uid=%d",isApp2Sd2Sd,getpid(),getuid());

        // Prepare source paths
        char source_user[PATH_MAX];
        char source_obb[PATH_MAX];
        char target_user[PATH_MAX];
        char* readl_source_obb;
        ...
        if(isApp2Sd2Sd){
            readl_source_obb =cource_sd1_obb;
        }
        else{
            readl_source_obb =source_obb;
        }

        if (fs_prepare_dir(source_user, 0000, 0, 0) == -1
                || (!isApp2Sd2Sd &&fs_prepare_dir(readl_source_obb, 0000, 0, 0) == -1)
        ...
        if (mount(readl_source_obb, target_obb, NULL, MS_BIND, NULL) == -1) {
            SLOGE("Failed to mount %s to %s: %s", readl_source_obb, target_obb, strerror(errno));
            return -1;
        }
...
static pid_t forkAndSpecializeCommon(const u4* args, bool isSystemServer)
{
    ...
    if (pid == 0) {
        ...
        char enableApp2Sd2[255];
        bool isApp2Sd2Sd =false;
        property_get("sys.en.app2sd2", enableApp2Sd2, "false");
        if(!strcmp(enableApp2Sd2, "true")){
            isApp2Sd2Sd = true;
        }
        if(isApp2Sd2Sd){
            property_get("sys.en.app2sd2.obb", enableApp2Sd2, "false");
            if(!strcmp(enableApp2Sd2, "false")){
                isApp2Sd2Sd = false;
            }
        }
        
        if (mountMode != MOUNT_EXTERNAL_NONE) {
            err = mountEmulatedStorage(uid, mountMode,isApp2Sd2Sd);
            if (err < 0) {
