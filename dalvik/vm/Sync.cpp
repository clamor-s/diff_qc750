...
void dvmThreadSleep(u8 msec, u4 nsec)
{
    ...
    if (msec == 0 && nsec == 0) {
        /* Guard against several repeated sleep(0,0) calls which can
         * inadvertently turn an application into a busy loop.
         *
         * Give immediate wakes first but if it continues, simply degrade to
         * around 0.1ms range to conserve power. The sleep(0,0) behaviour
         * seems to be unofficial: there's no maximum bound for the duration
         * of the "zero second sleep" that users would expect. Programs can
         * still request 1ns sleeps explicitly.
         *
         * If the last dvmThreadSleep(0, 0) was more than a second ago, reset
         * the delay to 1 ns.
         */
        static time_t last_zero_sleep = 0;
        static long int bump = 1;
        time_t now;

        if (bump == 1) {
            LOGW("a sleep of (0ms, 0ns) requested: forcing a longer delay");
        }

        now = time(NULL);

        if (now - last_zero_sleep > 1)
            bump = 1;
        else if (bump < 100000) {
            bump += 10;
        }
